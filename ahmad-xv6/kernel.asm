
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 90 c6 10 80       	mov    $0x8010c690,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 d4 34 10 80       	mov    $0x801034d4,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 98 87 10 	movl   $0x80108798,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80100049:	e8 08 4d 00 00       	call   80104d56 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 b0 05 11 80 a4 	movl   $0x801105a4,0x801105b0
80100055:	05 11 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 b4 05 11 80 a4 	movl   $0x801105a4,0x801105b4
8010005f:	05 11 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 d4 c6 10 80 	movl   $0x8010c6d4,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 b4 05 11 80    	mov    0x801105b4,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c a4 05 11 80 	movl   $0x801105a4,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 b4 05 11 80       	mov    0x801105b4,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 b4 05 11 80       	mov    %eax,0x801105b4

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 a4 05 11 80 	cmpl   $0x801105a4,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate a buffer.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
801000bd:	e8 b5 4c 00 00       	call   80104d77 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 b4 05 11 80       	mov    0x801105b4,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	83 c8 01             	or     $0x1,%eax
801000f6:	89 c2                	mov    %eax,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
80100104:	e8 d0 4c 00 00       	call   80104dd9 <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 a0 c6 10 	movl   $0x8010c6a0,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 80 49 00 00       	call   80104aa4 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 a4 05 11 80 	cmpl   $0x801105a4,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
  }

  // Not cached; recycle some non-busy and clean buffer.
  // "clean" because B_DIRTY and !B_BUSY means log.c
  // hasn't yet committed the changes to the buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 b0 05 11 80       	mov    0x801105b0,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
8010017c:	e8 58 4c 00 00       	call   80104dd9 <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
  }

  // Not cached; recycle some non-busy and clean buffer.
  // "clean" because B_DIRTY and !B_BUSY means log.c
  // hasn't yet committed the changes to the buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 a4 05 11 80 	cmpl   $0x801105a4,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 9f 87 10 80 	movl   $0x8010879f,(%esp)
8010019f:	e8 96 03 00 00       	call   8010053a <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 c9 25 00 00       	call   801027a1 <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 b0 87 10 80 	movl   $0x801087b0,(%esp)
801001f6:	e8 3f 03 00 00       	call   8010053a <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	83 c8 04             	or     $0x4,%eax
80100203:	89 c2                	mov    %eax,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 8c 25 00 00       	call   801027a1 <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 b7 87 10 80 	movl   $0x801087b7,(%esp)
80100230:	e8 05 03 00 00       	call   8010053a <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
8010023c:	e8 36 4b 00 00       	call   80104d77 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 b4 05 11 80    	mov    0x801105b4,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c a4 05 11 80 	movl   $0x801105a4,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 b4 05 11 80       	mov    0x801105b4,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 b4 05 11 80       	mov    %eax,0x801105b4

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	83 e0 fe             	and    $0xfffffffe,%eax
80100290:	89 c2                	mov    %eax,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 de 48 00 00       	call   80104b80 <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 a0 c6 10 80 	movl   $0x8010c6a0,(%esp)
801002a9:	e8 2b 4b 00 00       	call   80104dd9 <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	83 ec 14             	sub    $0x14,%esp
801002b6:	8b 45 08             	mov    0x8(%ebp),%eax
801002b9:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002bd:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
801002c1:	89 c2                	mov    %eax,%edx
801002c3:	ec                   	in     (%dx),%al
801002c4:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801002c7:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
801002cb:	c9                   	leave  
801002cc:	c3                   	ret    

801002cd <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002cd:	55                   	push   %ebp
801002ce:	89 e5                	mov    %esp,%ebp
801002d0:	83 ec 08             	sub    $0x8,%esp
801002d3:	8b 55 08             	mov    0x8(%ebp),%edx
801002d6:	8b 45 0c             	mov    0xc(%ebp),%eax
801002d9:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002dd:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002e0:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002e4:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002e8:	ee                   	out    %al,(%dx)
}
801002e9:	c9                   	leave  
801002ea:	c3                   	ret    

801002eb <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002eb:	55                   	push   %ebp
801002ec:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002ee:	fa                   	cli    
}
801002ef:	5d                   	pop    %ebp
801002f0:	c3                   	ret    

801002f1 <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002f1:	55                   	push   %ebp
801002f2:	89 e5                	mov    %esp,%ebp
801002f4:	56                   	push   %esi
801002f5:	53                   	push   %ebx
801002f6:	83 ec 30             	sub    $0x30,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
801002f9:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801002fd:	74 1c                	je     8010031b <printint+0x2a>
801002ff:	8b 45 08             	mov    0x8(%ebp),%eax
80100302:	c1 e8 1f             	shr    $0x1f,%eax
80100305:	0f b6 c0             	movzbl %al,%eax
80100308:	89 45 10             	mov    %eax,0x10(%ebp)
8010030b:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010030f:	74 0a                	je     8010031b <printint+0x2a>
    x = -xx;
80100311:	8b 45 08             	mov    0x8(%ebp),%eax
80100314:	f7 d8                	neg    %eax
80100316:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100319:	eb 06                	jmp    80100321 <printint+0x30>
  else
    x = xx;
8010031b:	8b 45 08             	mov    0x8(%ebp),%eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100321:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100328:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010032b:	8d 41 01             	lea    0x1(%ecx),%eax
8010032e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100331:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80100334:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100337:	ba 00 00 00 00       	mov    $0x0,%edx
8010033c:	f7 f3                	div    %ebx
8010033e:	89 d0                	mov    %edx,%eax
80100340:	0f b6 80 04 90 10 80 	movzbl -0x7fef6ffc(%eax),%eax
80100347:	88 44 0d e0          	mov    %al,-0x20(%ebp,%ecx,1)
  }while((x /= base) != 0);
8010034b:	8b 75 0c             	mov    0xc(%ebp),%esi
8010034e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100351:	ba 00 00 00 00       	mov    $0x0,%edx
80100356:	f7 f6                	div    %esi
80100358:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010035b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010035f:	75 c7                	jne    80100328 <printint+0x37>

  if(sign)
80100361:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100365:	74 10                	je     80100377 <printint+0x86>
    buf[i++] = '-';
80100367:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010036a:	8d 50 01             	lea    0x1(%eax),%edx
8010036d:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100370:	c6 44 05 e0 2d       	movb   $0x2d,-0x20(%ebp,%eax,1)

  while(--i >= 0)
80100375:	eb 18                	jmp    8010038f <printint+0x9e>
80100377:	eb 16                	jmp    8010038f <printint+0x9e>
    consputc(buf[i]);
80100379:	8d 55 e0             	lea    -0x20(%ebp),%edx
8010037c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010037f:	01 d0                	add    %edx,%eax
80100381:	0f b6 00             	movzbl (%eax),%eax
80100384:	0f be c0             	movsbl %al,%eax
80100387:	89 04 24             	mov    %eax,(%esp)
8010038a:	e8 c1 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
8010038f:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100393:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100397:	79 e0                	jns    80100379 <printint+0x88>
    consputc(buf[i]);
}
80100399:	83 c4 30             	add    $0x30,%esp
8010039c:	5b                   	pop    %ebx
8010039d:	5e                   	pop    %esi
8010039e:	5d                   	pop    %ebp
8010039f:	c3                   	ret    

801003a0 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a0:	55                   	push   %ebp
801003a1:	89 e5                	mov    %esp,%ebp
801003a3:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a6:	a1 34 b6 10 80       	mov    0x8010b634,%eax
801003ab:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003ae:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b2:	74 0c                	je     801003c0 <cprintf+0x20>
    acquire(&cons.lock);
801003b4:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801003bb:	e8 b7 49 00 00       	call   80104d77 <acquire>

  if (fmt == 0)
801003c0:	8b 45 08             	mov    0x8(%ebp),%eax
801003c3:	85 c0                	test   %eax,%eax
801003c5:	75 0c                	jne    801003d3 <cprintf+0x33>
    panic("null fmt");
801003c7:	c7 04 24 be 87 10 80 	movl   $0x801087be,(%esp)
801003ce:	e8 67 01 00 00       	call   8010053a <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d3:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003d9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e0:	e9 21 01 00 00       	jmp    80100506 <cprintf+0x166>
    if(c != '%'){
801003e5:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003e9:	74 10                	je     801003fb <cprintf+0x5b>
      consputc(c);
801003eb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ee:	89 04 24             	mov    %eax,(%esp)
801003f1:	e8 5a 03 00 00       	call   80100750 <consputc>
      continue;
801003f6:	e9 07 01 00 00       	jmp    80100502 <cprintf+0x162>
    }
    c = fmt[++i] & 0xff;
801003fb:	8b 55 08             	mov    0x8(%ebp),%edx
801003fe:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100402:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100405:	01 d0                	add    %edx,%eax
80100407:	0f b6 00             	movzbl (%eax),%eax
8010040a:	0f be c0             	movsbl %al,%eax
8010040d:	25 ff 00 00 00       	and    $0xff,%eax
80100412:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100415:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100419:	75 05                	jne    80100420 <cprintf+0x80>
      break;
8010041b:	e9 06 01 00 00       	jmp    80100526 <cprintf+0x186>
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4f                	je     80100477 <cprintf+0xd7>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0xa0>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13c>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xaf>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x14a>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 57                	je     8010049c <cprintf+0xfc>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2d                	je     80100477 <cprintf+0xd7>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x14a>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8d 50 04             	lea    0x4(%eax),%edx
80100455:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100458:	8b 00                	mov    (%eax),%eax
8010045a:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80100461:	00 
80100462:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100469:	00 
8010046a:	89 04 24             	mov    %eax,(%esp)
8010046d:	e8 7f fe ff ff       	call   801002f1 <printint>
      break;
80100472:	e9 8b 00 00 00       	jmp    80100502 <cprintf+0x162>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100477:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010047a:	8d 50 04             	lea    0x4(%eax),%edx
8010047d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100480:	8b 00                	mov    (%eax),%eax
80100482:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100489:	00 
8010048a:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
80100491:	00 
80100492:	89 04 24             	mov    %eax,(%esp)
80100495:	e8 57 fe ff ff       	call   801002f1 <printint>
      break;
8010049a:	eb 66                	jmp    80100502 <cprintf+0x162>
    case 's':
      if((s = (char*)*argp++) == 0)
8010049c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049f:	8d 50 04             	lea    0x4(%eax),%edx
801004a2:	89 55 f0             	mov    %edx,-0x10(%ebp)
801004a5:	8b 00                	mov    (%eax),%eax
801004a7:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004aa:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004ae:	75 09                	jne    801004b9 <cprintf+0x119>
        s = "(null)";
801004b0:	c7 45 ec c7 87 10 80 	movl   $0x801087c7,-0x14(%ebp)
      for(; *s; s++)
801004b7:	eb 17                	jmp    801004d0 <cprintf+0x130>
801004b9:	eb 15                	jmp    801004d0 <cprintf+0x130>
        consputc(*s);
801004bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004be:	0f b6 00             	movzbl (%eax),%eax
801004c1:	0f be c0             	movsbl %al,%eax
801004c4:	89 04 24             	mov    %eax,(%esp)
801004c7:	e8 84 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004cc:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 e1                	jne    801004bb <cprintf+0x11b>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x162>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x162>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 bf fe ff ff    	jne    801003e5 <cprintf+0x45>
      consputc(c);
      break;
    }
  }

  if(locking)
80100526:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052a:	74 0c                	je     80100538 <cprintf+0x198>
    release(&cons.lock);
8010052c:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80100533:	e8 a1 48 00 00       	call   80104dd9 <release>
}
80100538:	c9                   	leave  
80100539:	c3                   	ret    

8010053a <panic>:

void
panic(char *s)
{
8010053a:	55                   	push   %ebp
8010053b:	89 e5                	mov    %esp,%ebp
8010053d:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100540:	e8 a6 fd ff ff       	call   801002eb <cli>
  cons.locking = 0;
80100545:	c7 05 34 b6 10 80 00 	movl   $0x0,0x8010b634
8010054c:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
8010054f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100555:	0f b6 00             	movzbl (%eax),%eax
80100558:	0f b6 c0             	movzbl %al,%eax
8010055b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010055f:	c7 04 24 ce 87 10 80 	movl   $0x801087ce,(%esp)
80100566:	e8 35 fe ff ff       	call   801003a0 <cprintf>
  cprintf(s);
8010056b:	8b 45 08             	mov    0x8(%ebp),%eax
8010056e:	89 04 24             	mov    %eax,(%esp)
80100571:	e8 2a fe ff ff       	call   801003a0 <cprintf>
  cprintf("\n");
80100576:	c7 04 24 dd 87 10 80 	movl   $0x801087dd,(%esp)
8010057d:	e8 1e fe ff ff       	call   801003a0 <cprintf>
  getcallerpcs(&s, pcs);
80100582:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100585:	89 44 24 04          	mov    %eax,0x4(%esp)
80100589:	8d 45 08             	lea    0x8(%ebp),%eax
8010058c:	89 04 24             	mov    %eax,(%esp)
8010058f:	e8 94 48 00 00       	call   80104e28 <getcallerpcs>
  for(i=0; i<10; i++)
80100594:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059b:	eb 1b                	jmp    801005b8 <panic+0x7e>
    cprintf(" %p", pcs[i]);
8010059d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a0:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801005a8:	c7 04 24 df 87 10 80 	movl   $0x801087df,(%esp)
801005af:	e8 ec fd ff ff       	call   801003a0 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b4:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005b8:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bc:	7e df                	jle    8010059d <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005be:	c7 05 e0 b5 10 80 01 	movl   $0x1,0x8010b5e0
801005c5:	00 00 00 
  for(;;)
    ;
801005c8:	eb fe                	jmp    801005c8 <panic+0x8e>

801005ca <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005ca:	55                   	push   %ebp
801005cb:	89 e5                	mov    %esp,%ebp
801005cd:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d0:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005d7:	00 
801005d8:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005df:	e8 e9 fc ff ff       	call   801002cd <outb>
  pos = inb(CRTPORT+1) << 8;
801005e4:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005eb:	e8 c0 fc ff ff       	call   801002b0 <inb>
801005f0:	0f b6 c0             	movzbl %al,%eax
801005f3:	c1 e0 08             	shl    $0x8,%eax
801005f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005f9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100600:	00 
80100601:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100608:	e8 c0 fc ff ff       	call   801002cd <outb>
  pos |= inb(CRTPORT+1);
8010060d:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100614:	e8 97 fc ff ff       	call   801002b0 <inb>
80100619:	0f b6 c0             	movzbl %al,%eax
8010061c:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
8010061f:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100623:	75 30                	jne    80100655 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100625:	8b 4d f4             	mov    -0xc(%ebp),%ecx
80100628:	ba 67 66 66 66       	mov    $0x66666667,%edx
8010062d:	89 c8                	mov    %ecx,%eax
8010062f:	f7 ea                	imul   %edx
80100631:	c1 fa 05             	sar    $0x5,%edx
80100634:	89 c8                	mov    %ecx,%eax
80100636:	c1 f8 1f             	sar    $0x1f,%eax
80100639:	29 c2                	sub    %eax,%edx
8010063b:	89 d0                	mov    %edx,%eax
8010063d:	c1 e0 02             	shl    $0x2,%eax
80100640:	01 d0                	add    %edx,%eax
80100642:	c1 e0 04             	shl    $0x4,%eax
80100645:	29 c1                	sub    %eax,%ecx
80100647:	89 ca                	mov    %ecx,%edx
80100649:	b8 50 00 00 00       	mov    $0x50,%eax
8010064e:	29 d0                	sub    %edx,%eax
80100650:	01 45 f4             	add    %eax,-0xc(%ebp)
80100653:	eb 35                	jmp    8010068a <cgaputc+0xc0>
  else if(c == BACKSPACE){
80100655:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065c:	75 0c                	jne    8010066a <cgaputc+0xa0>
    if(pos > 0) --pos;
8010065e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100662:	7e 26                	jle    8010068a <cgaputc+0xc0>
80100664:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100668:	eb 20                	jmp    8010068a <cgaputc+0xc0>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066a:	8b 0d 00 90 10 80    	mov    0x80109000,%ecx
80100670:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100673:	8d 50 01             	lea    0x1(%eax),%edx
80100676:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100679:	01 c0                	add    %eax,%eax
8010067b:	8d 14 01             	lea    (%ecx,%eax,1),%edx
8010067e:	8b 45 08             	mov    0x8(%ebp),%eax
80100681:	0f b6 c0             	movzbl %al,%eax
80100684:	80 cc 07             	or     $0x7,%ah
80100687:	66 89 02             	mov    %ax,(%edx)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x11c>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 90 10 80       	mov    0x80109000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 90 10 80       	mov    0x80109000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 e3 49 00 00       	call   8010509a <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	8d 14 00             	lea    (%eax,%eax,1),%edx
801006c6:	a1 00 90 10 80       	mov    0x80109000,%eax
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 c8                	add    %ecx,%eax
801006d2:	89 54 24 08          	mov    %edx,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 04 24             	mov    %eax,(%esp)
801006e1:	e8 e5 48 00 00       	call   80104fcb <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 d3 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 ba fb ff ff       	call   801002cd <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 a6 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 90 fb ff ff       	call   801002cd <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 e0 b5 10 80       	mov    0x8010b5e0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 87 fb ff ff       	call   801002eb <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 5f 66 00 00       	call   80106dda <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 53 66 00 00       	call   80106dda <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 47 66 00 00       	call   80106dda <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 3a 66 00 00       	call   80106dda <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 1f fe ff ff       	call   801005ca <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
801007ba:	e8 b8 45 00 00       	call   80104d77 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 37 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 64                	je     8010083a <consoleintr+0x8d>
801007d6:	e9 91 00 00 00       	jmp    8010086c <consoleintr+0xbf>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 55                	je     8010083a <consoleintr+0x8d>
801007e5:	e9 82 00 00 00       	jmp    8010086c <consoleintr+0xbf>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 37 44 00 00       	call   80104c26 <procdump>
      break;
801007ef:	e9 07 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 7c 08 11 80       	mov    0x8011087c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 7c 08 11 80       	mov    %eax,0x8011087c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 7c 08 11 80    	mov    0x8011087c,%edx
80100816:	a1 78 08 11 80       	mov    0x80110878,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	74 16                	je     80100835 <consoleintr+0x88>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
8010081f:	a1 7c 08 11 80       	mov    0x8011087c,%eax
80100824:	83 e8 01             	sub    $0x1,%eax
80100827:	83 e0 7f             	and    $0x7f,%eax
8010082a:	0f b6 80 f4 07 11 80 	movzbl -0x7feef80c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100831:	3c 0a                	cmp    $0xa,%al
80100833:	75 bf                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100835:	e9 c1 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083a:	8b 15 7c 08 11 80    	mov    0x8011087c,%edx
80100840:	a1 78 08 11 80       	mov    0x80110878,%eax
80100845:	39 c2                	cmp    %eax,%edx
80100847:	74 1e                	je     80100867 <consoleintr+0xba>
        input.e--;
80100849:	a1 7c 08 11 80       	mov    0x8011087c,%eax
8010084e:	83 e8 01             	sub    $0x1,%eax
80100851:	a3 7c 08 11 80       	mov    %eax,0x8011087c
        consputc(BACKSPACE);
80100856:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
8010085d:	e8 ee fe ff ff       	call   80100750 <consputc>
      }
      break;
80100862:	e9 94 00 00 00       	jmp    801008fb <consoleintr+0x14e>
80100867:	e9 8f 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100870:	0f 84 84 00 00 00    	je     801008fa <consoleintr+0x14d>
80100876:	8b 15 7c 08 11 80    	mov    0x8011087c,%edx
8010087c:	a1 74 08 11 80       	mov    0x80110874,%eax
80100881:	29 c2                	sub    %eax,%edx
80100883:	89 d0                	mov    %edx,%eax
80100885:	83 f8 7f             	cmp    $0x7f,%eax
80100888:	77 70                	ja     801008fa <consoleintr+0x14d>
        c = (c == '\r') ? '\n' : c;
8010088a:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
8010088e:	74 05                	je     80100895 <consoleintr+0xe8>
80100890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100893:	eb 05                	jmp    8010089a <consoleintr+0xed>
80100895:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089a:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
8010089d:	a1 7c 08 11 80       	mov    0x8011087c,%eax
801008a2:	8d 50 01             	lea    0x1(%eax),%edx
801008a5:	89 15 7c 08 11 80    	mov    %edx,0x8011087c
801008ab:	83 e0 7f             	and    $0x7f,%eax
801008ae:	89 c2                	mov    %eax,%edx
801008b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008b3:	88 82 f4 07 11 80    	mov    %al,-0x7feef80c(%edx)
        consputc(c);
801008b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008bc:	89 04 24             	mov    %eax,(%esp)
801008bf:	e8 8c fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c4:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008c8:	74 18                	je     801008e2 <consoleintr+0x135>
801008ca:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008ce:	74 12                	je     801008e2 <consoleintr+0x135>
801008d0:	a1 7c 08 11 80       	mov    0x8011087c,%eax
801008d5:	8b 15 74 08 11 80    	mov    0x80110874,%edx
801008db:	83 ea 80             	sub    $0xffffff80,%edx
801008de:	39 d0                	cmp    %edx,%eax
801008e0:	75 18                	jne    801008fa <consoleintr+0x14d>
          input.w = input.e;
801008e2:	a1 7c 08 11 80       	mov    0x8011087c,%eax
801008e7:	a3 78 08 11 80       	mov    %eax,0x80110878
          wakeup(&input.r);
801008ec:	c7 04 24 74 08 11 80 	movl   $0x80110874,(%esp)
801008f3:	e8 88 42 00 00       	call   80104b80 <wakeup>
        }
      }
      break;
801008f8:	eb 00                	jmp    801008fa <consoleintr+0x14d>
801008fa:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
801008fb:	8b 45 08             	mov    0x8(%ebp),%eax
801008fe:	ff d0                	call   *%eax
80100900:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100903:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100907:	0f 89 b7 fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
8010090d:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
80100914:	e8 c0 44 00 00       	call   80104dd9 <release>
}
80100919:	c9                   	leave  
8010091a:	c3                   	ret    

8010091b <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
8010091b:	55                   	push   %ebp
8010091c:	89 e5                	mov    %esp,%ebp
8010091e:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
80100921:	8b 45 08             	mov    0x8(%ebp),%eax
80100924:	89 04 24             	mov    %eax,(%esp)
80100927:	e8 7d 10 00 00       	call   801019a9 <iunlock>
  target = n;
8010092c:	8b 45 10             	mov    0x10(%ebp),%eax
8010092f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
80100932:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
80100939:	e8 39 44 00 00       	call   80104d77 <acquire>
  while(n > 0){
8010093e:	e9 aa 00 00 00       	jmp    801009ed <consoleread+0xd2>
    while(input.r == input.w){
80100943:	eb 42                	jmp    80100987 <consoleread+0x6c>
      if(proc->killed){
80100945:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010094b:	8b 40 24             	mov    0x24(%eax),%eax
8010094e:	85 c0                	test   %eax,%eax
80100950:	74 21                	je     80100973 <consoleread+0x58>
        release(&input.lock);
80100952:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
80100959:	e8 7b 44 00 00       	call   80104dd9 <release>
        ilock(ip);
8010095e:	8b 45 08             	mov    0x8(%ebp),%eax
80100961:	89 04 24             	mov    %eax,(%esp)
80100964:	e8 f2 0e 00 00       	call   8010185b <ilock>
        return -1;
80100969:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010096e:	e9 a5 00 00 00       	jmp    80100a18 <consoleread+0xfd>
      }
      sleep(&input.r, &input.lock);
80100973:	c7 44 24 04 c0 07 11 	movl   $0x801107c0,0x4(%esp)
8010097a:	80 
8010097b:	c7 04 24 74 08 11 80 	movl   $0x80110874,(%esp)
80100982:	e8 1d 41 00 00       	call   80104aa4 <sleep>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100987:	8b 15 74 08 11 80    	mov    0x80110874,%edx
8010098d:	a1 78 08 11 80       	mov    0x80110878,%eax
80100992:	39 c2                	cmp    %eax,%edx
80100994:	74 af                	je     80100945 <consoleread+0x2a>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
80100996:	a1 74 08 11 80       	mov    0x80110874,%eax
8010099b:	8d 50 01             	lea    0x1(%eax),%edx
8010099e:	89 15 74 08 11 80    	mov    %edx,0x80110874
801009a4:	83 e0 7f             	and    $0x7f,%eax
801009a7:	0f b6 80 f4 07 11 80 	movzbl -0x7feef80c(%eax),%eax
801009ae:	0f be c0             	movsbl %al,%eax
801009b1:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(c == C('D')){  // EOF
801009b4:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009b8:	75 19                	jne    801009d3 <consoleread+0xb8>
      if(n < target){
801009ba:	8b 45 10             	mov    0x10(%ebp),%eax
801009bd:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009c0:	73 0f                	jae    801009d1 <consoleread+0xb6>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009c2:	a1 74 08 11 80       	mov    0x80110874,%eax
801009c7:	83 e8 01             	sub    $0x1,%eax
801009ca:	a3 74 08 11 80       	mov    %eax,0x80110874
      }
      break;
801009cf:	eb 26                	jmp    801009f7 <consoleread+0xdc>
801009d1:	eb 24                	jmp    801009f7 <consoleread+0xdc>
    }
    *dst++ = c;
801009d3:	8b 45 0c             	mov    0xc(%ebp),%eax
801009d6:	8d 50 01             	lea    0x1(%eax),%edx
801009d9:	89 55 0c             	mov    %edx,0xc(%ebp)
801009dc:	8b 55 f0             	mov    -0x10(%ebp),%edx
801009df:	88 10                	mov    %dl,(%eax)
    --n;
801009e1:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009e5:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009e9:	75 02                	jne    801009ed <consoleread+0xd2>
      break;
801009eb:	eb 0a                	jmp    801009f7 <consoleread+0xdc>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009ed:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f1:	0f 8f 4c ff ff ff    	jg     80100943 <consoleread+0x28>
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
  }
  release(&input.lock);
801009f7:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
801009fe:	e8 d6 43 00 00       	call   80104dd9 <release>
  ilock(ip);
80100a03:	8b 45 08             	mov    0x8(%ebp),%eax
80100a06:	89 04 24             	mov    %eax,(%esp)
80100a09:	e8 4d 0e 00 00       	call   8010185b <ilock>

  return target - n;
80100a0e:	8b 45 10             	mov    0x10(%ebp),%eax
80100a11:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a14:	29 c2                	sub    %eax,%edx
80100a16:	89 d0                	mov    %edx,%eax
}
80100a18:	c9                   	leave  
80100a19:	c3                   	ret    

80100a1a <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a1a:	55                   	push   %ebp
80100a1b:	89 e5                	mov    %esp,%ebp
80100a1d:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a20:	8b 45 08             	mov    0x8(%ebp),%eax
80100a23:	89 04 24             	mov    %eax,(%esp)
80100a26:	e8 7e 0f 00 00       	call   801019a9 <iunlock>
  acquire(&cons.lock);
80100a2b:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80100a32:	e8 40 43 00 00       	call   80104d77 <acquire>
  for(i = 0; i < n; i++)
80100a37:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a3e:	eb 1d                	jmp    80100a5d <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a40:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a43:	8b 45 0c             	mov    0xc(%ebp),%eax
80100a46:	01 d0                	add    %edx,%eax
80100a48:	0f b6 00             	movzbl (%eax),%eax
80100a4b:	0f be c0             	movsbl %al,%eax
80100a4e:	0f b6 c0             	movzbl %al,%eax
80100a51:	89 04 24             	mov    %eax,(%esp)
80100a54:	e8 f7 fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a60:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a63:	7c db                	jl     80100a40 <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a65:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80100a6c:	e8 68 43 00 00       	call   80104dd9 <release>
  ilock(ip);
80100a71:	8b 45 08             	mov    0x8(%ebp),%eax
80100a74:	89 04 24             	mov    %eax,(%esp)
80100a77:	e8 df 0d 00 00       	call   8010185b <ilock>

  return n;
80100a7c:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a7f:	c9                   	leave  
80100a80:	c3                   	ret    

80100a81 <consoleinit>:

void
consoleinit(void)
{
80100a81:	55                   	push   %ebp
80100a82:	89 e5                	mov    %esp,%ebp
80100a84:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a87:	c7 44 24 04 e3 87 10 	movl   $0x801087e3,0x4(%esp)
80100a8e:	80 
80100a8f:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80100a96:	e8 bb 42 00 00       	call   80104d56 <initlock>
  initlock(&input.lock, "input");
80100a9b:	c7 44 24 04 eb 87 10 	movl   $0x801087eb,0x4(%esp)
80100aa2:	80 
80100aa3:	c7 04 24 c0 07 11 80 	movl   $0x801107c0,(%esp)
80100aaa:	e8 a7 42 00 00       	call   80104d56 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100aaf:	c7 05 2c 12 11 80 1a 	movl   $0x80100a1a,0x8011122c
80100ab6:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ab9:	c7 05 28 12 11 80 1b 	movl   $0x8010091b,0x80111228
80100ac0:	09 10 80 
  cons.locking = 1;
80100ac3:	c7 05 34 b6 10 80 01 	movl   $0x1,0x8010b634
80100aca:	00 00 00 

  picenable(IRQ_KBD);
80100acd:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ad4:	e8 98 30 00 00       	call   80103b71 <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ad9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100ae0:	00 
80100ae1:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae8:	e8 70 1e 00 00       	call   8010295d <ioapicenable>
}
80100aed:	c9                   	leave  
80100aee:	c3                   	ret    

80100aef <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100aef:	55                   	push   %ebp
80100af0:	89 e5                	mov    %esp,%ebp
80100af2:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  begin_op();
80100af8:	e8 e8 26 00 00       	call   801031e5 <begin_op>
  if((ip = namei(path)) == 0){
80100afd:	8b 45 08             	mov    0x8(%ebp),%eax
80100b00:	89 04 24             	mov    %eax,(%esp)
80100b03:	e8 fe 18 00 00       	call   80102406 <namei>
80100b08:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b0b:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b0f:	75 0f                	jne    80100b20 <exec+0x31>
    end_op();
80100b11:	e8 53 27 00 00       	call   80103269 <end_op>
    return -1;
80100b16:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1b:	e9 e8 03 00 00       	jmp    80100f08 <exec+0x419>
  }
  ilock(ip);
80100b20:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b23:	89 04 24             	mov    %eax,(%esp)
80100b26:	e8 30 0d 00 00       	call   8010185b <ilock>
  pgdir = 0;
80100b2b:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b32:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b39:	00 
80100b3a:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b41:	00 
80100b42:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4c:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b4f:	89 04 24             	mov    %eax,(%esp)
80100b52:	e8 11 12 00 00       	call   80101d68 <readi>
80100b57:	83 f8 33             	cmp    $0x33,%eax
80100b5a:	77 05                	ja     80100b61 <exec+0x72>
    goto bad;
80100b5c:	e9 7b 03 00 00       	jmp    80100edc <exec+0x3ed>
  if(elf.magic != ELF_MAGIC)
80100b61:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b67:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6c:	74 05                	je     80100b73 <exec+0x84>
    goto bad;
80100b6e:	e9 69 03 00 00       	jmp    80100edc <exec+0x3ed>

  if((pgdir = setupkvm()) == 0)
80100b73:	e8 b3 73 00 00       	call   80107f2b <setupkvm>
80100b78:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b7b:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b7f:	75 05                	jne    80100b86 <exec+0x97>
    goto bad;
80100b81:	e9 56 03 00 00       	jmp    80100edc <exec+0x3ed>

  // Load program into memory.
  sz = 0;
80100b86:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b8d:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b94:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100b9a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100b9d:	e9 cb 00 00 00       	jmp    80100c6d <exec+0x17e>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100ba5:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bac:	00 
80100bad:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb1:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bb7:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bbb:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bbe:	89 04 24             	mov    %eax,(%esp)
80100bc1:	e8 a2 11 00 00       	call   80101d68 <readi>
80100bc6:	83 f8 20             	cmp    $0x20,%eax
80100bc9:	74 05                	je     80100bd0 <exec+0xe1>
      goto bad;
80100bcb:	e9 0c 03 00 00       	jmp    80100edc <exec+0x3ed>
    if(ph.type != ELF_PROG_LOAD)
80100bd0:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bd6:	83 f8 01             	cmp    $0x1,%eax
80100bd9:	74 05                	je     80100be0 <exec+0xf1>
      continue;
80100bdb:	e9 80 00 00 00       	jmp    80100c60 <exec+0x171>
    if(ph.memsz < ph.filesz)
80100be0:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be6:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bec:	39 c2                	cmp    %eax,%edx
80100bee:	73 05                	jae    80100bf5 <exec+0x106>
      goto bad;
80100bf0:	e9 e7 02 00 00       	jmp    80100edc <exec+0x3ed>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 e0 76 00 00       	call   801082f9 <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	75 05                	jne    80100c27 <exec+0x138>
      goto bad;
80100c22:	e9 b5 02 00 00       	jmp    80100edc <exec+0x3ed>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c27:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2d:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c33:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c39:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3d:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c41:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c44:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4c:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4f:	89 04 24             	mov    %eax,(%esp)
80100c52:	e8 b7 75 00 00       	call   8010820e <loaduvm>
80100c57:	85 c0                	test   %eax,%eax
80100c59:	79 05                	jns    80100c60 <exec+0x171>
      goto bad;
80100c5b:	e9 7c 02 00 00       	jmp    80100edc <exec+0x3ed>
  if((pgdir = setupkvm()) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c60:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c64:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c67:	83 c0 20             	add    $0x20,%eax
80100c6a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6d:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c74:	0f b7 c0             	movzwl %ax,%eax
80100c77:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7a:	0f 8f 22 ff ff ff    	jg     80100ba2 <exec+0xb3>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c80:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c83:	89 04 24             	mov    %eax,(%esp)
80100c86:	e8 54 0e 00 00       	call   80101adf <iunlockput>
  end_op();
80100c8b:	e8 d9 25 00 00       	call   80103269 <end_op>
  ip = 0;
80100c90:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c97:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c9a:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca4:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca7:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100caa:	05 00 20 00 00       	add    $0x2000,%eax
80100caf:	89 44 24 08          	mov    %eax,0x8(%esp)
80100cb3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cba:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cbd:	89 04 24             	mov    %eax,(%esp)
80100cc0:	e8 34 76 00 00       	call   801082f9 <allocuvm>
80100cc5:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc8:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100ccc:	75 05                	jne    80100cd3 <exec+0x1e4>
    goto bad;
80100cce:	e9 09 02 00 00       	jmp    80100edc <exec+0x3ed>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cd3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd6:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cdb:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cdf:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ce2:	89 04 24             	mov    %eax,(%esp)
80100ce5:	e8 3f 78 00 00       	call   80108529 <clearpteu>
  sp = sz;
80100cea:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ced:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100cf0:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf7:	e9 9a 00 00 00       	jmp    80100d96 <exec+0x2a7>
    if(argc >= MAXARG)
80100cfc:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100d00:	76 05                	jbe    80100d07 <exec+0x218>
      goto bad;
80100d02:	e9 d5 01 00 00       	jmp    80100edc <exec+0x3ed>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d07:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d0a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d11:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d14:	01 d0                	add    %edx,%eax
80100d16:	8b 00                	mov    (%eax),%eax
80100d18:	89 04 24             	mov    %eax,(%esp)
80100d1b:	e8 15 45 00 00       	call   80105235 <strlen>
80100d20:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100d23:	29 c2                	sub    %eax,%edx
80100d25:	89 d0                	mov    %edx,%eax
80100d27:	83 e8 01             	sub    $0x1,%eax
80100d2a:	83 e0 fc             	and    $0xfffffffc,%eax
80100d2d:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d30:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d33:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d3a:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d3d:	01 d0                	add    %edx,%eax
80100d3f:	8b 00                	mov    (%eax),%eax
80100d41:	89 04 24             	mov    %eax,(%esp)
80100d44:	e8 ec 44 00 00       	call   80105235 <strlen>
80100d49:	83 c0 01             	add    $0x1,%eax
80100d4c:	89 c2                	mov    %eax,%edx
80100d4e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d51:	8d 0c 85 00 00 00 00 	lea    0x0(,%eax,4),%ecx
80100d58:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d5b:	01 c8                	add    %ecx,%eax
80100d5d:	8b 00                	mov    (%eax),%eax
80100d5f:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d63:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d67:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d6e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d71:	89 04 24             	mov    %eax,(%esp)
80100d74:	e8 75 79 00 00       	call   801086ee <copyout>
80100d79:	85 c0                	test   %eax,%eax
80100d7b:	79 05                	jns    80100d82 <exec+0x293>
      goto bad;
80100d7d:	e9 5a 01 00 00       	jmp    80100edc <exec+0x3ed>
    ustack[3+argc] = sp;
80100d82:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d85:	8d 50 03             	lea    0x3(%eax),%edx
80100d88:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d8b:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d92:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d96:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d99:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100da0:	8b 45 0c             	mov    0xc(%ebp),%eax
80100da3:	01 d0                	add    %edx,%eax
80100da5:	8b 00                	mov    (%eax),%eax
80100da7:	85 c0                	test   %eax,%eax
80100da9:	0f 85 4d ff ff ff    	jne    80100cfc <exec+0x20d>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 03             	add    $0x3,%eax
80100db5:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100dbc:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100dc0:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100dc7:	ff ff ff 
  ustack[1] = argc;
80100dca:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dcd:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 01             	add    $0x1,%eax
80100dd9:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100de0:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100de3:	29 d0                	sub    %edx,%eax
80100de5:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100deb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dee:	83 c0 04             	add    $0x4,%eax
80100df1:	c1 e0 02             	shl    $0x2,%eax
80100df4:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100df7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dfa:	83 c0 04             	add    $0x4,%eax
80100dfd:	c1 e0 02             	shl    $0x2,%eax
80100e00:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100e04:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100e0a:	89 44 24 08          	mov    %eax,0x8(%esp)
80100e0e:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100e11:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e15:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100e18:	89 04 24             	mov    %eax,(%esp)
80100e1b:	e8 ce 78 00 00       	call   801086ee <copyout>
80100e20:	85 c0                	test   %eax,%eax
80100e22:	79 05                	jns    80100e29 <exec+0x33a>
    goto bad;
80100e24:	e9 b3 00 00 00       	jmp    80100edc <exec+0x3ed>

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e29:	8b 45 08             	mov    0x8(%ebp),%eax
80100e2c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e32:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e35:	eb 17                	jmp    80100e4e <exec+0x35f>
    if(*s == '/')
80100e37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e3a:	0f b6 00             	movzbl (%eax),%eax
80100e3d:	3c 2f                	cmp    $0x2f,%al
80100e3f:	75 09                	jne    80100e4a <exec+0x35b>
      last = s+1;
80100e41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e44:	83 c0 01             	add    $0x1,%eax
80100e47:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e4a:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e4e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e51:	0f b6 00             	movzbl (%eax),%eax
80100e54:	84 c0                	test   %al,%al
80100e56:	75 df                	jne    80100e37 <exec+0x348>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e58:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e5e:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e61:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e68:	00 
80100e69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e6c:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e70:	89 14 24             	mov    %edx,(%esp)
80100e73:	e8 73 43 00 00       	call   801051eb <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e78:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7e:	8b 40 04             	mov    0x4(%eax),%eax
80100e81:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e84:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8a:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e8d:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e90:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e96:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e99:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e9b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ea1:	8b 40 18             	mov    0x18(%eax),%eax
80100ea4:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100eaa:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100ead:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100eb3:	8b 40 18             	mov    0x18(%eax),%eax
80100eb6:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100eb9:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100ebc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ec2:	89 04 24             	mov    %eax,(%esp)
80100ec5:	e8 52 71 00 00       	call   8010801c <switchuvm>
  freevm(oldpgdir);
80100eca:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ecd:	89 04 24             	mov    %eax,(%esp)
80100ed0:	e8 ba 75 00 00       	call   8010848f <freevm>
  return 0;
80100ed5:	b8 00 00 00 00       	mov    $0x0,%eax
80100eda:	eb 2c                	jmp    80100f08 <exec+0x419>

 bad:
  if(pgdir)
80100edc:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100ee0:	74 0b                	je     80100eed <exec+0x3fe>
    freevm(pgdir);
80100ee2:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ee5:	89 04 24             	mov    %eax,(%esp)
80100ee8:	e8 a2 75 00 00       	call   8010848f <freevm>
  if(ip){
80100eed:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100ef1:	74 10                	je     80100f03 <exec+0x414>
    iunlockput(ip);
80100ef3:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef6:	89 04 24             	mov    %eax,(%esp)
80100ef9:	e8 e1 0b 00 00       	call   80101adf <iunlockput>
    end_op();
80100efe:	e8 66 23 00 00       	call   80103269 <end_op>
  }
  return -1;
80100f03:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100f08:	c9                   	leave  
80100f09:	c3                   	ret    

80100f0a <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f0a:	55                   	push   %ebp
80100f0b:	89 e5                	mov    %esp,%ebp
80100f0d:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f10:	c7 44 24 04 f1 87 10 	movl   $0x801087f1,0x4(%esp)
80100f17:	80 
80100f18:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100f1f:	e8 32 3e 00 00       	call   80104d56 <initlock>
}
80100f24:	c9                   	leave  
80100f25:	c3                   	ret    

80100f26 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f26:	55                   	push   %ebp
80100f27:	89 e5                	mov    %esp,%ebp
80100f29:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f2c:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100f33:	e8 3f 3e 00 00       	call   80104d77 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f38:	c7 45 f4 b4 08 11 80 	movl   $0x801108b4,-0xc(%ebp)
80100f3f:	eb 29                	jmp    80100f6a <filealloc+0x44>
    if(f->ref == 0){
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	8b 40 04             	mov    0x4(%eax),%eax
80100f47:	85 c0                	test   %eax,%eax
80100f49:	75 1b                	jne    80100f66 <filealloc+0x40>
      f->ref = 1;
80100f4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f4e:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f55:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100f5c:	e8 78 3e 00 00       	call   80104dd9 <release>
      return f;
80100f61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f64:	eb 1e                	jmp    80100f84 <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f66:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f6a:	81 7d f4 14 12 11 80 	cmpl   $0x80111214,-0xc(%ebp)
80100f71:	72 ce                	jb     80100f41 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f73:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100f7a:	e8 5a 3e 00 00       	call   80104dd9 <release>
  return 0;
80100f7f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f84:	c9                   	leave  
80100f85:	c3                   	ret    

80100f86 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f86:	55                   	push   %ebp
80100f87:	89 e5                	mov    %esp,%ebp
80100f89:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f8c:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100f93:	e8 df 3d 00 00       	call   80104d77 <acquire>
  if(f->ref < 1)
80100f98:	8b 45 08             	mov    0x8(%ebp),%eax
80100f9b:	8b 40 04             	mov    0x4(%eax),%eax
80100f9e:	85 c0                	test   %eax,%eax
80100fa0:	7f 0c                	jg     80100fae <filedup+0x28>
    panic("filedup");
80100fa2:	c7 04 24 f8 87 10 80 	movl   $0x801087f8,(%esp)
80100fa9:	e8 8c f5 ff ff       	call   8010053a <panic>
  f->ref++;
80100fae:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb1:	8b 40 04             	mov    0x4(%eax),%eax
80100fb4:	8d 50 01             	lea    0x1(%eax),%edx
80100fb7:	8b 45 08             	mov    0x8(%ebp),%eax
80100fba:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fbd:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100fc4:	e8 10 3e 00 00       	call   80104dd9 <release>
  return f;
80100fc9:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fcc:	c9                   	leave  
80100fcd:	c3                   	ret    

80100fce <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fce:	55                   	push   %ebp
80100fcf:	89 e5                	mov    %esp,%ebp
80100fd1:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fd4:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80100fdb:	e8 97 3d 00 00       	call   80104d77 <acquire>
  if(f->ref < 1)
80100fe0:	8b 45 08             	mov    0x8(%ebp),%eax
80100fe3:	8b 40 04             	mov    0x4(%eax),%eax
80100fe6:	85 c0                	test   %eax,%eax
80100fe8:	7f 0c                	jg     80100ff6 <fileclose+0x28>
    panic("fileclose");
80100fea:	c7 04 24 00 88 10 80 	movl   $0x80108800,(%esp)
80100ff1:	e8 44 f5 ff ff       	call   8010053a <panic>
  if(--f->ref > 0){
80100ff6:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff9:	8b 40 04             	mov    0x4(%eax),%eax
80100ffc:	8d 50 ff             	lea    -0x1(%eax),%edx
80100fff:	8b 45 08             	mov    0x8(%ebp),%eax
80101002:	89 50 04             	mov    %edx,0x4(%eax)
80101005:	8b 45 08             	mov    0x8(%ebp),%eax
80101008:	8b 40 04             	mov    0x4(%eax),%eax
8010100b:	85 c0                	test   %eax,%eax
8010100d:	7e 11                	jle    80101020 <fileclose+0x52>
    release(&ftable.lock);
8010100f:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80101016:	e8 be 3d 00 00       	call   80104dd9 <release>
8010101b:	e9 82 00 00 00       	jmp    801010a2 <fileclose+0xd4>
    return;
  }
  ff = *f;
80101020:	8b 45 08             	mov    0x8(%ebp),%eax
80101023:	8b 10                	mov    (%eax),%edx
80101025:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101028:	8b 50 04             	mov    0x4(%eax),%edx
8010102b:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010102e:	8b 50 08             	mov    0x8(%eax),%edx
80101031:	89 55 e8             	mov    %edx,-0x18(%ebp)
80101034:	8b 50 0c             	mov    0xc(%eax),%edx
80101037:	89 55 ec             	mov    %edx,-0x14(%ebp)
8010103a:	8b 50 10             	mov    0x10(%eax),%edx
8010103d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101040:	8b 40 14             	mov    0x14(%eax),%eax
80101043:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101050:	8b 45 08             	mov    0x8(%ebp),%eax
80101053:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
80101059:	c7 04 24 80 08 11 80 	movl   $0x80110880,(%esp)
80101060:	e8 74 3d 00 00       	call   80104dd9 <release>
  
  if(ff.type == FD_PIPE)
80101065:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101068:	83 f8 01             	cmp    $0x1,%eax
8010106b:	75 18                	jne    80101085 <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
8010106d:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101071:	0f be d0             	movsbl %al,%edx
80101074:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101077:	89 54 24 04          	mov    %edx,0x4(%esp)
8010107b:	89 04 24             	mov    %eax,(%esp)
8010107e:	e8 9e 2d 00 00       	call   80103e21 <pipeclose>
80101083:	eb 1d                	jmp    801010a2 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
80101085:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101088:	83 f8 02             	cmp    $0x2,%eax
8010108b:	75 15                	jne    801010a2 <fileclose+0xd4>
    begin_op();
8010108d:	e8 53 21 00 00       	call   801031e5 <begin_op>
    iput(ff.ip);
80101092:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101095:	89 04 24             	mov    %eax,(%esp)
80101098:	e8 71 09 00 00       	call   80101a0e <iput>
    end_op();
8010109d:	e8 c7 21 00 00       	call   80103269 <end_op>
  }
}
801010a2:	c9                   	leave  
801010a3:	c3                   	ret    

801010a4 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
801010a4:	55                   	push   %ebp
801010a5:	89 e5                	mov    %esp,%ebp
801010a7:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 00                	mov    (%eax),%eax
801010af:	83 f8 02             	cmp    $0x2,%eax
801010b2:	75 38                	jne    801010ec <filestat+0x48>
    ilock(f->ip);
801010b4:	8b 45 08             	mov    0x8(%ebp),%eax
801010b7:	8b 40 10             	mov    0x10(%eax),%eax
801010ba:	89 04 24             	mov    %eax,(%esp)
801010bd:	e8 99 07 00 00       	call   8010185b <ilock>
    stati(f->ip, st);
801010c2:	8b 45 08             	mov    0x8(%ebp),%eax
801010c5:	8b 40 10             	mov    0x10(%eax),%eax
801010c8:	8b 55 0c             	mov    0xc(%ebp),%edx
801010cb:	89 54 24 04          	mov    %edx,0x4(%esp)
801010cf:	89 04 24             	mov    %eax,(%esp)
801010d2:	e8 4c 0c 00 00       	call   80101d23 <stati>
    iunlock(f->ip);
801010d7:	8b 45 08             	mov    0x8(%ebp),%eax
801010da:	8b 40 10             	mov    0x10(%eax),%eax
801010dd:	89 04 24             	mov    %eax,(%esp)
801010e0:	e8 c4 08 00 00       	call   801019a9 <iunlock>
    return 0;
801010e5:	b8 00 00 00 00       	mov    $0x0,%eax
801010ea:	eb 05                	jmp    801010f1 <filestat+0x4d>
  }
  return -1;
801010ec:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010f1:	c9                   	leave  
801010f2:	c3                   	ret    

801010f3 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010f3:	55                   	push   %ebp
801010f4:	89 e5                	mov    %esp,%ebp
801010f6:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010f9:	8b 45 08             	mov    0x8(%ebp),%eax
801010fc:	0f b6 40 08          	movzbl 0x8(%eax),%eax
80101100:	84 c0                	test   %al,%al
80101102:	75 0a                	jne    8010110e <fileread+0x1b>
    return -1;
80101104:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101109:	e9 9f 00 00 00       	jmp    801011ad <fileread+0xba>
  if(f->type == FD_PIPE)
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 00                	mov    (%eax),%eax
80101113:	83 f8 01             	cmp    $0x1,%eax
80101116:	75 1e                	jne    80101136 <fileread+0x43>
    return piperead(f->pipe, addr, n);
80101118:	8b 45 08             	mov    0x8(%ebp),%eax
8010111b:	8b 40 0c             	mov    0xc(%eax),%eax
8010111e:	8b 55 10             	mov    0x10(%ebp),%edx
80101121:	89 54 24 08          	mov    %edx,0x8(%esp)
80101125:	8b 55 0c             	mov    0xc(%ebp),%edx
80101128:	89 54 24 04          	mov    %edx,0x4(%esp)
8010112c:	89 04 24             	mov    %eax,(%esp)
8010112f:	e8 6e 2e 00 00       	call   80103fa2 <piperead>
80101134:	eb 77                	jmp    801011ad <fileread+0xba>
  if(f->type == FD_INODE){
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 00                	mov    (%eax),%eax
8010113b:	83 f8 02             	cmp    $0x2,%eax
8010113e:	75 61                	jne    801011a1 <fileread+0xae>
    ilock(f->ip);
80101140:	8b 45 08             	mov    0x8(%ebp),%eax
80101143:	8b 40 10             	mov    0x10(%eax),%eax
80101146:	89 04 24             	mov    %eax,(%esp)
80101149:	e8 0d 07 00 00       	call   8010185b <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
8010114e:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101151:	8b 45 08             	mov    0x8(%ebp),%eax
80101154:	8b 50 14             	mov    0x14(%eax),%edx
80101157:	8b 45 08             	mov    0x8(%ebp),%eax
8010115a:	8b 40 10             	mov    0x10(%eax),%eax
8010115d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101161:	89 54 24 08          	mov    %edx,0x8(%esp)
80101165:	8b 55 0c             	mov    0xc(%ebp),%edx
80101168:	89 54 24 04          	mov    %edx,0x4(%esp)
8010116c:	89 04 24             	mov    %eax,(%esp)
8010116f:	e8 f4 0b 00 00       	call   80101d68 <readi>
80101174:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101177:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010117b:	7e 11                	jle    8010118e <fileread+0x9b>
      f->off += r;
8010117d:	8b 45 08             	mov    0x8(%ebp),%eax
80101180:	8b 50 14             	mov    0x14(%eax),%edx
80101183:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101186:	01 c2                	add    %eax,%edx
80101188:	8b 45 08             	mov    0x8(%ebp),%eax
8010118b:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
8010118e:	8b 45 08             	mov    0x8(%ebp),%eax
80101191:	8b 40 10             	mov    0x10(%eax),%eax
80101194:	89 04 24             	mov    %eax,(%esp)
80101197:	e8 0d 08 00 00       	call   801019a9 <iunlock>
    return r;
8010119c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010119f:	eb 0c                	jmp    801011ad <fileread+0xba>
  }
  panic("fileread");
801011a1:	c7 04 24 0a 88 10 80 	movl   $0x8010880a,(%esp)
801011a8:	e8 8d f3 ff ff       	call   8010053a <panic>
}
801011ad:	c9                   	leave  
801011ae:	c3                   	ret    

801011af <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011af:	55                   	push   %ebp
801011b0:	89 e5                	mov    %esp,%ebp
801011b2:	53                   	push   %ebx
801011b3:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011b6:	8b 45 08             	mov    0x8(%ebp),%eax
801011b9:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011bd:	84 c0                	test   %al,%al
801011bf:	75 0a                	jne    801011cb <filewrite+0x1c>
    return -1;
801011c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011c6:	e9 20 01 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_PIPE)
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 00                	mov    (%eax),%eax
801011d0:	83 f8 01             	cmp    $0x1,%eax
801011d3:	75 21                	jne    801011f6 <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011d5:	8b 45 08             	mov    0x8(%ebp),%eax
801011d8:	8b 40 0c             	mov    0xc(%eax),%eax
801011db:	8b 55 10             	mov    0x10(%ebp),%edx
801011de:	89 54 24 08          	mov    %edx,0x8(%esp)
801011e2:	8b 55 0c             	mov    0xc(%ebp),%edx
801011e5:	89 54 24 04          	mov    %edx,0x4(%esp)
801011e9:	89 04 24             	mov    %eax,(%esp)
801011ec:	e8 c2 2c 00 00       	call   80103eb3 <pipewrite>
801011f1:	e9 f5 00 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_INODE){
801011f6:	8b 45 08             	mov    0x8(%ebp),%eax
801011f9:	8b 00                	mov    (%eax),%eax
801011fb:	83 f8 02             	cmp    $0x2,%eax
801011fe:	0f 85 db 00 00 00    	jne    801012df <filewrite+0x130>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
80101204:	c7 45 ec 00 1a 00 00 	movl   $0x1a00,-0x14(%ebp)
    int i = 0;
8010120b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101212:	e9 a8 00 00 00       	jmp    801012bf <filewrite+0x110>
      int n1 = n - i;
80101217:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010121a:	8b 55 10             	mov    0x10(%ebp),%edx
8010121d:	29 c2                	sub    %eax,%edx
8010121f:	89 d0                	mov    %edx,%eax
80101221:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
80101224:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101227:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010122a:	7e 06                	jle    80101232 <filewrite+0x83>
        n1 = max;
8010122c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010122f:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_op();
80101232:	e8 ae 1f 00 00       	call   801031e5 <begin_op>
      ilock(f->ip);
80101237:	8b 45 08             	mov    0x8(%ebp),%eax
8010123a:	8b 40 10             	mov    0x10(%eax),%eax
8010123d:	89 04 24             	mov    %eax,(%esp)
80101240:	e8 16 06 00 00       	call   8010185b <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
80101245:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80101248:	8b 45 08             	mov    0x8(%ebp),%eax
8010124b:	8b 50 14             	mov    0x14(%eax),%edx
8010124e:	8b 5d f4             	mov    -0xc(%ebp),%ebx
80101251:	8b 45 0c             	mov    0xc(%ebp),%eax
80101254:	01 c3                	add    %eax,%ebx
80101256:	8b 45 08             	mov    0x8(%ebp),%eax
80101259:	8b 40 10             	mov    0x10(%eax),%eax
8010125c:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101260:	89 54 24 08          	mov    %edx,0x8(%esp)
80101264:	89 5c 24 04          	mov    %ebx,0x4(%esp)
80101268:	89 04 24             	mov    %eax,(%esp)
8010126b:	e8 5c 0c 00 00       	call   80101ecc <writei>
80101270:	89 45 e8             	mov    %eax,-0x18(%ebp)
80101273:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101277:	7e 11                	jle    8010128a <filewrite+0xdb>
        f->off += r;
80101279:	8b 45 08             	mov    0x8(%ebp),%eax
8010127c:	8b 50 14             	mov    0x14(%eax),%edx
8010127f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101282:	01 c2                	add    %eax,%edx
80101284:	8b 45 08             	mov    0x8(%ebp),%eax
80101287:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
8010128a:	8b 45 08             	mov    0x8(%ebp),%eax
8010128d:	8b 40 10             	mov    0x10(%eax),%eax
80101290:	89 04 24             	mov    %eax,(%esp)
80101293:	e8 11 07 00 00       	call   801019a9 <iunlock>
      end_op();
80101298:	e8 cc 1f 00 00       	call   80103269 <end_op>

      if(r < 0)
8010129d:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801012a1:	79 02                	jns    801012a5 <filewrite+0xf6>
        break;
801012a3:	eb 26                	jmp    801012cb <filewrite+0x11c>
      if(r != n1)
801012a5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012a8:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012ab:	74 0c                	je     801012b9 <filewrite+0x10a>
        panic("short filewrite");
801012ad:	c7 04 24 13 88 10 80 	movl   $0x80108813,(%esp)
801012b4:	e8 81 f2 ff ff       	call   8010053a <panic>
      i += r;
801012b9:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012bc:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c2:	3b 45 10             	cmp    0x10(%ebp),%eax
801012c5:	0f 8c 4c ff ff ff    	jl     80101217 <filewrite+0x68>
        break;
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012ce:	3b 45 10             	cmp    0x10(%ebp),%eax
801012d1:	75 05                	jne    801012d8 <filewrite+0x129>
801012d3:	8b 45 10             	mov    0x10(%ebp),%eax
801012d6:	eb 05                	jmp    801012dd <filewrite+0x12e>
801012d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012dd:	eb 0c                	jmp    801012eb <filewrite+0x13c>
  }
  panic("filewrite");
801012df:	c7 04 24 23 88 10 80 	movl   $0x80108823,(%esp)
801012e6:	e8 4f f2 ff ff       	call   8010053a <panic>
}
801012eb:	83 c4 24             	add    $0x24,%esp
801012ee:	5b                   	pop    %ebx
801012ef:	5d                   	pop    %ebp
801012f0:	c3                   	ret    

801012f1 <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012f1:	55                   	push   %ebp
801012f2:	89 e5                	mov    %esp,%ebp
801012f4:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f7:	8b 45 08             	mov    0x8(%ebp),%eax
801012fa:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80101301:	00 
80101302:	89 04 24             	mov    %eax,(%esp)
80101305:	e8 9c ee ff ff       	call   801001a6 <bread>
8010130a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
8010130d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101310:	83 c0 18             	add    $0x18,%eax
80101313:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010131a:	00 
8010131b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101322:	89 04 24             	mov    %eax,(%esp)
80101325:	e8 70 3d 00 00       	call   8010509a <memmove>
  brelse(bp);
8010132a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010132d:	89 04 24             	mov    %eax,(%esp)
80101330:	e8 e2 ee ff ff       	call   80100217 <brelse>
}
80101335:	c9                   	leave  
80101336:	c3                   	ret    

80101337 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101337:	55                   	push   %ebp
80101338:	89 e5                	mov    %esp,%ebp
8010133a:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
8010133d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101340:	8b 45 08             	mov    0x8(%ebp),%eax
80101343:	89 54 24 04          	mov    %edx,0x4(%esp)
80101347:	89 04 24             	mov    %eax,(%esp)
8010134a:	e8 57 ee ff ff       	call   801001a6 <bread>
8010134f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
80101352:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101355:	83 c0 18             	add    $0x18,%eax
80101358:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135f:	00 
80101360:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101367:	00 
80101368:	89 04 24             	mov    %eax,(%esp)
8010136b:	e8 5b 3c 00 00       	call   80104fcb <memset>
  log_write(bp);
80101370:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101373:	89 04 24             	mov    %eax,(%esp)
80101376:	e8 75 20 00 00       	call   801033f0 <log_write>
  brelse(bp);
8010137b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010137e:	89 04 24             	mov    %eax,(%esp)
80101381:	e8 91 ee ff ff       	call   80100217 <brelse>
}
80101386:	c9                   	leave  
80101387:	c3                   	ret    

80101388 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101388:	55                   	push   %ebp
80101389:	89 e5                	mov    %esp,%ebp
8010138b:	83 ec 38             	sub    $0x38,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138e:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101395:	8b 45 08             	mov    0x8(%ebp),%eax
80101398:	8d 55 d8             	lea    -0x28(%ebp),%edx
8010139b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139f:	89 04 24             	mov    %eax,(%esp)
801013a2:	e8 4a ff ff ff       	call   801012f1 <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013ae:	e9 07 01 00 00       	jmp    801014ba <balloc+0x132>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b6:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013bc:	85 c0                	test   %eax,%eax
801013be:	0f 48 c2             	cmovs  %edx,%eax
801013c1:	c1 f8 0c             	sar    $0xc,%eax
801013c4:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c7:	c1 ea 03             	shr    $0x3,%edx
801013ca:	01 d0                	add    %edx,%eax
801013cc:	83 c0 03             	add    $0x3,%eax
801013cf:	89 44 24 04          	mov    %eax,0x4(%esp)
801013d3:	8b 45 08             	mov    0x8(%ebp),%eax
801013d6:	89 04 24             	mov    %eax,(%esp)
801013d9:	e8 c8 ed ff ff       	call   801001a6 <bread>
801013de:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013e1:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e8:	e9 9d 00 00 00       	jmp    8010148a <balloc+0x102>
      m = 1 << (bi % 8);
801013ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013f0:	99                   	cltd   
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 c1                	mov    %eax,%ecx
80101402:	d3 e2                	shl    %cl,%edx
80101404:	89 d0                	mov    %edx,%eax
80101406:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101409:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140c:	8d 50 07             	lea    0x7(%eax),%edx
8010140f:	85 c0                	test   %eax,%eax
80101411:	0f 48 c2             	cmovs  %edx,%eax
80101414:	c1 f8 03             	sar    $0x3,%eax
80101417:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141a:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010141f:	0f b6 c0             	movzbl %al,%eax
80101422:	23 45 e8             	and    -0x18(%ebp),%eax
80101425:	85 c0                	test   %eax,%eax
80101427:	75 5d                	jne    80101486 <balloc+0xfe>
        bp->data[bi/8] |= m;  // Mark block in use.
80101429:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142c:	8d 50 07             	lea    0x7(%eax),%edx
8010142f:	85 c0                	test   %eax,%eax
80101431:	0f 48 c2             	cmovs  %edx,%eax
80101434:	c1 f8 03             	sar    $0x3,%eax
80101437:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143a:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010143f:	89 d1                	mov    %edx,%ecx
80101441:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101444:	09 ca                	or     %ecx,%edx
80101446:	89 d1                	mov    %edx,%ecx
80101448:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144b:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
8010144f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101452:	89 04 24             	mov    %eax,(%esp)
80101455:	e8 96 1f 00 00       	call   801033f0 <log_write>
        brelse(bp);
8010145a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145d:	89 04 24             	mov    %eax,(%esp)
80101460:	e8 b2 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101465:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101468:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146b:	01 c2                	add    %eax,%edx
8010146d:	8b 45 08             	mov    0x8(%ebp),%eax
80101470:	89 54 24 04          	mov    %edx,0x4(%esp)
80101474:	89 04 24             	mov    %eax,(%esp)
80101477:	e8 bb fe ff ff       	call   80101337 <bzero>
        return b + bi;
8010147c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010147f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101482:	01 d0                	add    %edx,%eax
80101484:	eb 4e                	jmp    801014d4 <balloc+0x14c>

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101486:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
8010148a:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101491:	7f 15                	jg     801014a8 <balloc+0x120>
80101493:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101496:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101499:	01 d0                	add    %edx,%eax
8010149b:	89 c2                	mov    %eax,%edx
8010149d:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a0:	39 c2                	cmp    %eax,%edx
801014a2:	0f 82 45 ff ff ff    	jb     801013ed <balloc+0x65>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014a8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014ab:	89 04 24             	mov    %eax,(%esp)
801014ae:	e8 64 ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b3:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014ba:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014bd:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c0:	39 c2                	cmp    %eax,%edx
801014c2:	0f 82 eb fe ff ff    	jb     801013b3 <balloc+0x2b>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014c8:	c7 04 24 2d 88 10 80 	movl   $0x8010882d,(%esp)
801014cf:	e8 66 f0 ff ff       	call   8010053a <panic>
}
801014d4:	c9                   	leave  
801014d5:	c3                   	ret    

801014d6 <bfree>:

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014d6:	55                   	push   %ebp
801014d7:	89 e5                	mov    %esp,%ebp
801014d9:	83 ec 38             	sub    $0x38,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014dc:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014df:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e3:	8b 45 08             	mov    0x8(%ebp),%eax
801014e6:	89 04 24             	mov    %eax,(%esp)
801014e9:	e8 03 fe ff ff       	call   801012f1 <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014ee:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f1:	c1 e8 0c             	shr    $0xc,%eax
801014f4:	89 c2                	mov    %eax,%edx
801014f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014f9:	c1 e8 03             	shr    $0x3,%eax
801014fc:	01 d0                	add    %edx,%eax
801014fe:	8d 50 03             	lea    0x3(%eax),%edx
80101501:	8b 45 08             	mov    0x8(%ebp),%eax
80101504:	89 54 24 04          	mov    %edx,0x4(%esp)
80101508:	89 04 24             	mov    %eax,(%esp)
8010150b:	e8 96 ec ff ff       	call   801001a6 <bread>
80101510:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101513:	8b 45 0c             	mov    0xc(%ebp),%eax
80101516:	25 ff 0f 00 00       	and    $0xfff,%eax
8010151b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
8010151e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101521:	99                   	cltd   
80101522:	c1 ea 1d             	shr    $0x1d,%edx
80101525:	01 d0                	add    %edx,%eax
80101527:	83 e0 07             	and    $0x7,%eax
8010152a:	29 d0                	sub    %edx,%eax
8010152c:	ba 01 00 00 00       	mov    $0x1,%edx
80101531:	89 c1                	mov    %eax,%ecx
80101533:	d3 e2                	shl    %cl,%edx
80101535:	89 d0                	mov    %edx,%eax
80101537:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
8010153a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010153d:	8d 50 07             	lea    0x7(%eax),%edx
80101540:	85 c0                	test   %eax,%eax
80101542:	0f 48 c2             	cmovs  %edx,%eax
80101545:	c1 f8 03             	sar    $0x3,%eax
80101548:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010154b:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101550:	0f b6 c0             	movzbl %al,%eax
80101553:	23 45 ec             	and    -0x14(%ebp),%eax
80101556:	85 c0                	test   %eax,%eax
80101558:	75 0c                	jne    80101566 <bfree+0x90>
    panic("freeing free block");
8010155a:	c7 04 24 43 88 10 80 	movl   $0x80108843,(%esp)
80101561:	e8 d4 ef ff ff       	call   8010053a <panic>
  bp->data[bi/8] &= ~m;
80101566:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101569:	8d 50 07             	lea    0x7(%eax),%edx
8010156c:	85 c0                	test   %eax,%eax
8010156e:	0f 48 c2             	cmovs  %edx,%eax
80101571:	c1 f8 03             	sar    $0x3,%eax
80101574:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101577:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010157c:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010157f:	f7 d1                	not    %ecx
80101581:	21 ca                	and    %ecx,%edx
80101583:	89 d1                	mov    %edx,%ecx
80101585:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101588:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
8010158c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010158f:	89 04 24             	mov    %eax,(%esp)
80101592:	e8 59 1e 00 00       	call   801033f0 <log_write>
  brelse(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 75 ec ff ff       	call   80100217 <brelse>
}
801015a2:	c9                   	leave  
801015a3:	c3                   	ret    

801015a4 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015a4:	55                   	push   %ebp
801015a5:	89 e5                	mov    %esp,%ebp
801015a7:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015aa:	c7 44 24 04 56 88 10 	movl   $0x80108856,0x4(%esp)
801015b1:	80 
801015b2:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
801015b9:	e8 98 37 00 00       	call   80104d56 <initlock>
}
801015be:	c9                   	leave  
801015bf:	c3                   	ret    

801015c0 <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015c0:	55                   	push   %ebp
801015c1:	89 e5                	mov    %esp,%ebp
801015c3:	83 ec 38             	sub    $0x38,%esp
801015c6:	8b 45 0c             	mov    0xc(%ebp),%eax
801015c9:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015cd:	8b 45 08             	mov    0x8(%ebp),%eax
801015d0:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015d3:	89 54 24 04          	mov    %edx,0x4(%esp)
801015d7:	89 04 24             	mov    %eax,(%esp)
801015da:	e8 12 fd ff ff       	call   801012f1 <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015df:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015e6:	e9 98 00 00 00       	jmp    80101683 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015ee:	c1 e8 03             	shr    $0x3,%eax
801015f1:	83 c0 02             	add    $0x2,%eax
801015f4:	89 44 24 04          	mov    %eax,0x4(%esp)
801015f8:	8b 45 08             	mov    0x8(%ebp),%eax
801015fb:	89 04 24             	mov    %eax,(%esp)
801015fe:	e8 a3 eb ff ff       	call   801001a6 <bread>
80101603:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101606:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101609:	8d 50 18             	lea    0x18(%eax),%edx
8010160c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010160f:	83 e0 07             	and    $0x7,%eax
80101612:	c1 e0 06             	shl    $0x6,%eax
80101615:	01 d0                	add    %edx,%eax
80101617:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
8010161a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010161d:	0f b7 00             	movzwl (%eax),%eax
80101620:	66 85 c0             	test   %ax,%ax
80101623:	75 4f                	jne    80101674 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101625:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010162c:	00 
8010162d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101634:	00 
80101635:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101638:	89 04 24             	mov    %eax,(%esp)
8010163b:	e8 8b 39 00 00       	call   80104fcb <memset>
      dip->type = type;
80101640:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101643:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101647:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
8010164a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010164d:	89 04 24             	mov    %eax,(%esp)
80101650:	e8 9b 1d 00 00       	call   801033f0 <log_write>
      brelse(bp);
80101655:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101658:	89 04 24             	mov    %eax,(%esp)
8010165b:	e8 b7 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
80101660:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101663:	89 44 24 04          	mov    %eax,0x4(%esp)
80101667:	8b 45 08             	mov    0x8(%ebp),%eax
8010166a:	89 04 24             	mov    %eax,(%esp)
8010166d:	e8 e5 00 00 00       	call   80101757 <iget>
80101672:	eb 29                	jmp    8010169d <ialloc+0xdd>
    }
    brelse(bp);
80101674:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101677:	89 04 24             	mov    %eax,(%esp)
8010167a:	e8 98 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010167f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101683:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101686:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101689:	39 c2                	cmp    %eax,%edx
8010168b:	0f 82 5a ff ff ff    	jb     801015eb <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
80101691:	c7 04 24 5d 88 10 80 	movl   $0x8010885d,(%esp)
80101698:	e8 9d ee ff ff       	call   8010053a <panic>
}
8010169d:	c9                   	leave  
8010169e:	c3                   	ret    

8010169f <iupdate>:

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
8010169f:	55                   	push   %ebp
801016a0:	89 e5                	mov    %esp,%ebp
801016a2:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016a5:	8b 45 08             	mov    0x8(%ebp),%eax
801016a8:	8b 40 04             	mov    0x4(%eax),%eax
801016ab:	c1 e8 03             	shr    $0x3,%eax
801016ae:	8d 50 02             	lea    0x2(%eax),%edx
801016b1:	8b 45 08             	mov    0x8(%ebp),%eax
801016b4:	8b 00                	mov    (%eax),%eax
801016b6:	89 54 24 04          	mov    %edx,0x4(%esp)
801016ba:	89 04 24             	mov    %eax,(%esp)
801016bd:	e8 e4 ea ff ff       	call   801001a6 <bread>
801016c2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016c8:	8d 50 18             	lea    0x18(%eax),%edx
801016cb:	8b 45 08             	mov    0x8(%ebp),%eax
801016ce:	8b 40 04             	mov    0x4(%eax),%eax
801016d1:	83 e0 07             	and    $0x7,%eax
801016d4:	c1 e0 06             	shl    $0x6,%eax
801016d7:	01 d0                	add    %edx,%eax
801016d9:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016dc:	8b 45 08             	mov    0x8(%ebp),%eax
801016df:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016e3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016e6:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
801016f7:	8b 45 08             	mov    0x8(%ebp),%eax
801016fa:	0f b7 50 14          	movzwl 0x14(%eax),%edx
801016fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101701:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101705:	8b 45 08             	mov    0x8(%ebp),%eax
80101708:	0f b7 50 16          	movzwl 0x16(%eax),%edx
8010170c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170f:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101713:	8b 45 08             	mov    0x8(%ebp),%eax
80101716:	8b 50 18             	mov    0x18(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010171f:	8b 45 08             	mov    0x8(%ebp),%eax
80101722:	8d 50 1c             	lea    0x1c(%eax),%edx
80101725:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101728:	83 c0 0c             	add    $0xc,%eax
8010172b:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101732:	00 
80101733:	89 54 24 04          	mov    %edx,0x4(%esp)
80101737:	89 04 24             	mov    %eax,(%esp)
8010173a:	e8 5b 39 00 00       	call   8010509a <memmove>
  log_write(bp);
8010173f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101742:	89 04 24             	mov    %eax,(%esp)
80101745:	e8 a6 1c 00 00       	call   801033f0 <log_write>
  brelse(bp);
8010174a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174d:	89 04 24             	mov    %eax,(%esp)
80101750:	e8 c2 ea ff ff       	call   80100217 <brelse>
}
80101755:	c9                   	leave  
80101756:	c3                   	ret    

80101757 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101757:	55                   	push   %ebp
80101758:	89 e5                	mov    %esp,%ebp
8010175a:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010175d:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101764:	e8 0e 36 00 00       	call   80104d77 <acquire>

  // Is the inode already cached?
  empty = 0;
80101769:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101770:	c7 45 f4 b4 12 11 80 	movl   $0x801112b4,-0xc(%ebp)
80101777:	eb 59                	jmp    801017d2 <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101779:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010177c:	8b 40 08             	mov    0x8(%eax),%eax
8010177f:	85 c0                	test   %eax,%eax
80101781:	7e 35                	jle    801017b8 <iget+0x61>
80101783:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101786:	8b 00                	mov    (%eax),%eax
80101788:	3b 45 08             	cmp    0x8(%ebp),%eax
8010178b:	75 2b                	jne    801017b8 <iget+0x61>
8010178d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101790:	8b 40 04             	mov    0x4(%eax),%eax
80101793:	3b 45 0c             	cmp    0xc(%ebp),%eax
80101796:	75 20                	jne    801017b8 <iget+0x61>
      ip->ref++;
80101798:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179b:	8b 40 08             	mov    0x8(%eax),%eax
8010179e:	8d 50 01             	lea    0x1(%eax),%edx
801017a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a4:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017a7:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
801017ae:	e8 26 36 00 00       	call   80104dd9 <release>
      return ip;
801017b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b6:	eb 6f                	jmp    80101827 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017b8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017bc:	75 10                	jne    801017ce <iget+0x77>
801017be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c1:	8b 40 08             	mov    0x8(%eax),%eax
801017c4:	85 c0                	test   %eax,%eax
801017c6:	75 06                	jne    801017ce <iget+0x77>
      empty = ip;
801017c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017cb:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017ce:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017d2:	81 7d f4 54 22 11 80 	cmpl   $0x80112254,-0xc(%ebp)
801017d9:	72 9e                	jb     80101779 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017db:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017df:	75 0c                	jne    801017ed <iget+0x96>
    panic("iget: no inodes");
801017e1:	c7 04 24 6f 88 10 80 	movl   $0x8010886f,(%esp)
801017e8:	e8 4d ed ff ff       	call   8010053a <panic>

  ip = empty;
801017ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017f0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
801017f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017f6:	8b 55 08             	mov    0x8(%ebp),%edx
801017f9:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
801017fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017fe:	8b 55 0c             	mov    0xc(%ebp),%edx
80101801:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101804:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101807:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010180e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101811:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101818:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
8010181f:	e8 b5 35 00 00       	call   80104dd9 <release>

  return ip;
80101824:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101827:	c9                   	leave  
80101828:	c3                   	ret    

80101829 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101829:	55                   	push   %ebp
8010182a:	89 e5                	mov    %esp,%ebp
8010182c:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010182f:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101836:	e8 3c 35 00 00       	call   80104d77 <acquire>
  ip->ref++;
8010183b:	8b 45 08             	mov    0x8(%ebp),%eax
8010183e:	8b 40 08             	mov    0x8(%eax),%eax
80101841:	8d 50 01             	lea    0x1(%eax),%edx
80101844:	8b 45 08             	mov    0x8(%ebp),%eax
80101847:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
8010184a:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101851:	e8 83 35 00 00       	call   80104dd9 <release>
  return ip;
80101856:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101859:	c9                   	leave  
8010185a:	c3                   	ret    

8010185b <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
8010185b:	55                   	push   %ebp
8010185c:	89 e5                	mov    %esp,%ebp
8010185e:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
80101861:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101865:	74 0a                	je     80101871 <ilock+0x16>
80101867:	8b 45 08             	mov    0x8(%ebp),%eax
8010186a:	8b 40 08             	mov    0x8(%eax),%eax
8010186d:	85 c0                	test   %eax,%eax
8010186f:	7f 0c                	jg     8010187d <ilock+0x22>
    panic("ilock");
80101871:	c7 04 24 7f 88 10 80 	movl   $0x8010887f,(%esp)
80101878:	e8 bd ec ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
8010187d:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101884:	e8 ee 34 00 00       	call   80104d77 <acquire>
  while(ip->flags & I_BUSY)
80101889:	eb 13                	jmp    8010189e <ilock+0x43>
    sleep(ip, &icache.lock);
8010188b:	c7 44 24 04 80 12 11 	movl   $0x80111280,0x4(%esp)
80101892:	80 
80101893:	8b 45 08             	mov    0x8(%ebp),%eax
80101896:	89 04 24             	mov    %eax,(%esp)
80101899:	e8 06 32 00 00       	call   80104aa4 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
8010189e:	8b 45 08             	mov    0x8(%ebp),%eax
801018a1:	8b 40 0c             	mov    0xc(%eax),%eax
801018a4:	83 e0 01             	and    $0x1,%eax
801018a7:	85 c0                	test   %eax,%eax
801018a9:	75 e0                	jne    8010188b <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 c8 01             	or     $0x1,%eax
801018b4:	89 c2                	mov    %eax,%edx
801018b6:	8b 45 08             	mov    0x8(%ebp),%eax
801018b9:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018bc:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
801018c3:	e8 11 35 00 00       	call   80104dd9 <release>

  if(!(ip->flags & I_VALID)){
801018c8:	8b 45 08             	mov    0x8(%ebp),%eax
801018cb:	8b 40 0c             	mov    0xc(%eax),%eax
801018ce:	83 e0 02             	and    $0x2,%eax
801018d1:	85 c0                	test   %eax,%eax
801018d3:	0f 85 ce 00 00 00    	jne    801019a7 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018d9:	8b 45 08             	mov    0x8(%ebp),%eax
801018dc:	8b 40 04             	mov    0x4(%eax),%eax
801018df:	c1 e8 03             	shr    $0x3,%eax
801018e2:	8d 50 02             	lea    0x2(%eax),%edx
801018e5:	8b 45 08             	mov    0x8(%ebp),%eax
801018e8:	8b 00                	mov    (%eax),%eax
801018ea:	89 54 24 04          	mov    %edx,0x4(%esp)
801018ee:	89 04 24             	mov    %eax,(%esp)
801018f1:	e8 b0 e8 ff ff       	call   801001a6 <bread>
801018f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
801018f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801018fc:	8d 50 18             	lea    0x18(%eax),%edx
801018ff:	8b 45 08             	mov    0x8(%ebp),%eax
80101902:	8b 40 04             	mov    0x4(%eax),%eax
80101905:	83 e0 07             	and    $0x7,%eax
80101908:	c1 e0 06             	shl    $0x6,%eax
8010190b:	01 d0                	add    %edx,%eax
8010190d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
80101910:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101913:	0f b7 10             	movzwl (%eax),%edx
80101916:	8b 45 08             	mov    0x8(%ebp),%eax
80101919:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101924:	8b 45 08             	mov    0x8(%ebp),%eax
80101927:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
8010192b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192e:	0f b7 50 04          	movzwl 0x4(%eax),%edx
80101932:	8b 45 08             	mov    0x8(%ebp),%eax
80101935:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101939:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193c:	0f b7 50 06          	movzwl 0x6(%eax),%edx
80101940:	8b 45 08             	mov    0x8(%ebp),%eax
80101943:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101947:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010194a:	8b 50 08             	mov    0x8(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101953:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101956:	8d 50 0c             	lea    0xc(%eax),%edx
80101959:	8b 45 08             	mov    0x8(%ebp),%eax
8010195c:	83 c0 1c             	add    $0x1c,%eax
8010195f:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101966:	00 
80101967:	89 54 24 04          	mov    %edx,0x4(%esp)
8010196b:	89 04 24             	mov    %eax,(%esp)
8010196e:	e8 27 37 00 00       	call   8010509a <memmove>
    brelse(bp);
80101973:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101976:	89 04 24             	mov    %eax,(%esp)
80101979:	e8 99 e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010197e:	8b 45 08             	mov    0x8(%ebp),%eax
80101981:	8b 40 0c             	mov    0xc(%eax),%eax
80101984:	83 c8 02             	or     $0x2,%eax
80101987:	89 c2                	mov    %eax,%edx
80101989:	8b 45 08             	mov    0x8(%ebp),%eax
8010198c:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010198f:	8b 45 08             	mov    0x8(%ebp),%eax
80101992:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101996:	66 85 c0             	test   %ax,%ax
80101999:	75 0c                	jne    801019a7 <ilock+0x14c>
      panic("ilock: no type");
8010199b:	c7 04 24 85 88 10 80 	movl   $0x80108885,(%esp)
801019a2:	e8 93 eb ff ff       	call   8010053a <panic>
  }
}
801019a7:	c9                   	leave  
801019a8:	c3                   	ret    

801019a9 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019a9:	55                   	push   %ebp
801019aa:	89 e5                	mov    %esp,%ebp
801019ac:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019af:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019b3:	74 17                	je     801019cc <iunlock+0x23>
801019b5:	8b 45 08             	mov    0x8(%ebp),%eax
801019b8:	8b 40 0c             	mov    0xc(%eax),%eax
801019bb:	83 e0 01             	and    $0x1,%eax
801019be:	85 c0                	test   %eax,%eax
801019c0:	74 0a                	je     801019cc <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 08             	mov    0x8(%eax),%eax
801019c8:	85 c0                	test   %eax,%eax
801019ca:	7f 0c                	jg     801019d8 <iunlock+0x2f>
    panic("iunlock");
801019cc:	c7 04 24 94 88 10 80 	movl   $0x80108894,(%esp)
801019d3:	e8 62 eb ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
801019d8:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
801019df:	e8 93 33 00 00       	call   80104d77 <acquire>
  ip->flags &= ~I_BUSY;
801019e4:	8b 45 08             	mov    0x8(%ebp),%eax
801019e7:	8b 40 0c             	mov    0xc(%eax),%eax
801019ea:	83 e0 fe             	and    $0xfffffffe,%eax
801019ed:	89 c2                	mov    %eax,%edx
801019ef:	8b 45 08             	mov    0x8(%ebp),%eax
801019f2:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
801019f5:	8b 45 08             	mov    0x8(%ebp),%eax
801019f8:	89 04 24             	mov    %eax,(%esp)
801019fb:	e8 80 31 00 00       	call   80104b80 <wakeup>
  release(&icache.lock);
80101a00:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101a07:	e8 cd 33 00 00       	call   80104dd9 <release>
}
80101a0c:	c9                   	leave  
80101a0d:	c3                   	ret    

80101a0e <iput>:
// to it, free the inode (and its content) on disk.
// All calls to iput() must be inside a transaction in
// case it has to free the inode.
void
iput(struct inode *ip)
{
80101a0e:	55                   	push   %ebp
80101a0f:	89 e5                	mov    %esp,%ebp
80101a11:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a14:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101a1b:	e8 57 33 00 00       	call   80104d77 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a20:	8b 45 08             	mov    0x8(%ebp),%eax
80101a23:	8b 40 08             	mov    0x8(%eax),%eax
80101a26:	83 f8 01             	cmp    $0x1,%eax
80101a29:	0f 85 93 00 00 00    	jne    80101ac2 <iput+0xb4>
80101a2f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a32:	8b 40 0c             	mov    0xc(%eax),%eax
80101a35:	83 e0 02             	and    $0x2,%eax
80101a38:	85 c0                	test   %eax,%eax
80101a3a:	0f 84 82 00 00 00    	je     80101ac2 <iput+0xb4>
80101a40:	8b 45 08             	mov    0x8(%ebp),%eax
80101a43:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a47:	66 85 c0             	test   %ax,%ax
80101a4a:	75 76                	jne    80101ac2 <iput+0xb4>
    // inode has no links and no other references: truncate and free.
    if(ip->flags & I_BUSY)
80101a4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a4f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a52:	83 e0 01             	and    $0x1,%eax
80101a55:	85 c0                	test   %eax,%eax
80101a57:	74 0c                	je     80101a65 <iput+0x57>
      panic("iput busy");
80101a59:	c7 04 24 9c 88 10 80 	movl   $0x8010889c,(%esp)
80101a60:	e8 d5 ea ff ff       	call   8010053a <panic>
    ip->flags |= I_BUSY;
80101a65:	8b 45 08             	mov    0x8(%ebp),%eax
80101a68:	8b 40 0c             	mov    0xc(%eax),%eax
80101a6b:	83 c8 01             	or     $0x1,%eax
80101a6e:	89 c2                	mov    %eax,%edx
80101a70:	8b 45 08             	mov    0x8(%ebp),%eax
80101a73:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a76:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101a7d:	e8 57 33 00 00       	call   80104dd9 <release>
    itrunc(ip);
80101a82:	8b 45 08             	mov    0x8(%ebp),%eax
80101a85:	89 04 24             	mov    %eax,(%esp)
80101a88:	e8 7d 01 00 00       	call   80101c0a <itrunc>
    ip->type = 0;
80101a8d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a90:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101a96:	8b 45 08             	mov    0x8(%ebp),%eax
80101a99:	89 04 24             	mov    %eax,(%esp)
80101a9c:	e8 fe fb ff ff       	call   8010169f <iupdate>
    acquire(&icache.lock);
80101aa1:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101aa8:	e8 ca 32 00 00       	call   80104d77 <acquire>
    ip->flags = 0;
80101aad:	8b 45 08             	mov    0x8(%ebp),%eax
80101ab0:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ab7:	8b 45 08             	mov    0x8(%ebp),%eax
80101aba:	89 04 24             	mov    %eax,(%esp)
80101abd:	e8 be 30 00 00       	call   80104b80 <wakeup>
  }
  ip->ref--;
80101ac2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac5:	8b 40 08             	mov    0x8(%eax),%eax
80101ac8:	8d 50 ff             	lea    -0x1(%eax),%edx
80101acb:	8b 45 08             	mov    0x8(%ebp),%eax
80101ace:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ad1:	c7 04 24 80 12 11 80 	movl   $0x80111280,(%esp)
80101ad8:	e8 fc 32 00 00       	call   80104dd9 <release>
}
80101add:	c9                   	leave  
80101ade:	c3                   	ret    

80101adf <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101adf:	55                   	push   %ebp
80101ae0:	89 e5                	mov    %esp,%ebp
80101ae2:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101ae5:	8b 45 08             	mov    0x8(%ebp),%eax
80101ae8:	89 04 24             	mov    %eax,(%esp)
80101aeb:	e8 b9 fe ff ff       	call   801019a9 <iunlock>
  iput(ip);
80101af0:	8b 45 08             	mov    0x8(%ebp),%eax
80101af3:	89 04 24             	mov    %eax,(%esp)
80101af6:	e8 13 ff ff ff       	call   80101a0e <iput>
}
80101afb:	c9                   	leave  
80101afc:	c3                   	ret    

80101afd <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101afd:	55                   	push   %ebp
80101afe:	89 e5                	mov    %esp,%ebp
80101b00:	53                   	push   %ebx
80101b01:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b04:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b08:	77 3e                	ja     80101b48 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b0a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b0d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b10:	83 c2 04             	add    $0x4,%edx
80101b13:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b17:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b1a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b1e:	75 20                	jne    80101b40 <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b20:	8b 45 08             	mov    0x8(%ebp),%eax
80101b23:	8b 00                	mov    (%eax),%eax
80101b25:	89 04 24             	mov    %eax,(%esp)
80101b28:	e8 5b f8 ff ff       	call   80101388 <balloc>
80101b2d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b30:	8b 45 08             	mov    0x8(%ebp),%eax
80101b33:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b36:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b39:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b3c:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b40:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b43:	e9 bc 00 00 00       	jmp    80101c04 <bmap+0x107>
  }
  bn -= NDIRECT;
80101b48:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b4c:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b50:	0f 87 a2 00 00 00    	ja     80101bf8 <bmap+0xfb>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b56:	8b 45 08             	mov    0x8(%ebp),%eax
80101b59:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b5c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b5f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b63:	75 19                	jne    80101b7e <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b65:	8b 45 08             	mov    0x8(%ebp),%eax
80101b68:	8b 00                	mov    (%eax),%eax
80101b6a:	89 04 24             	mov    %eax,(%esp)
80101b6d:	e8 16 f8 ff ff       	call   80101388 <balloc>
80101b72:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b75:	8b 45 08             	mov    0x8(%ebp),%eax
80101b78:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b7b:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101b81:	8b 00                	mov    (%eax),%eax
80101b83:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b86:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b8a:	89 04 24             	mov    %eax,(%esp)
80101b8d:	e8 14 e6 ff ff       	call   801001a6 <bread>
80101b92:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101b95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101b98:	83 c0 18             	add    $0x18,%eax
80101b9b:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101b9e:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ba1:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ba8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bab:	01 d0                	add    %edx,%eax
80101bad:	8b 00                	mov    (%eax),%eax
80101baf:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bb6:	75 30                	jne    80101be8 <bmap+0xeb>
      a[bn] = addr = balloc(ip->dev);
80101bb8:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bbb:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101bc2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bc5:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80101bc8:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcb:	8b 00                	mov    (%eax),%eax
80101bcd:	89 04 24             	mov    %eax,(%esp)
80101bd0:	e8 b3 f7 ff ff       	call   80101388 <balloc>
80101bd5:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bd8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdb:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be0:	89 04 24             	mov    %eax,(%esp)
80101be3:	e8 08 18 00 00       	call   801033f0 <log_write>
    }
    brelse(bp);
80101be8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101beb:	89 04 24             	mov    %eax,(%esp)
80101bee:	e8 24 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf6:	eb 0c                	jmp    80101c04 <bmap+0x107>
  }

  panic("bmap: out of range");
80101bf8:	c7 04 24 a6 88 10 80 	movl   $0x801088a6,(%esp)
80101bff:	e8 36 e9 ff ff       	call   8010053a <panic>
}
80101c04:	83 c4 24             	add    $0x24,%esp
80101c07:	5b                   	pop    %ebx
80101c08:	5d                   	pop    %ebp
80101c09:	c3                   	ret    

80101c0a <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0a:	55                   	push   %ebp
80101c0b:	89 e5                	mov    %esp,%ebp
80101c0d:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c10:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c17:	eb 44                	jmp    80101c5d <itrunc+0x53>
    if(ip->addrs[i]){
80101c19:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1c:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c1f:	83 c2 04             	add    $0x4,%edx
80101c22:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c26:	85 c0                	test   %eax,%eax
80101c28:	74 2f                	je     80101c59 <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c30:	83 c2 04             	add    $0x4,%edx
80101c33:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c37:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3a:	8b 00                	mov    (%eax),%eax
80101c3c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c40:	89 04 24             	mov    %eax,(%esp)
80101c43:	e8 8e f8 ff ff       	call   801014d6 <bfree>
      ip->addrs[i] = 0;
80101c48:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c4e:	83 c2 04             	add    $0x4,%edx
80101c51:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c58:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5d:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c61:	7e b6                	jle    80101c19 <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c63:	8b 45 08             	mov    0x8(%ebp),%eax
80101c66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c69:	85 c0                	test   %eax,%eax
80101c6b:	0f 84 9b 00 00 00    	je     80101d0c <itrunc+0x102>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c71:	8b 45 08             	mov    0x8(%ebp),%eax
80101c74:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c77:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7a:	8b 00                	mov    (%eax),%eax
80101c7c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c80:	89 04 24             	mov    %eax,(%esp)
80101c83:	e8 1e e5 ff ff       	call   801001a6 <bread>
80101c88:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8b:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c8e:	83 c0 18             	add    $0x18,%eax
80101c91:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c94:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9b:	eb 3b                	jmp    80101cd8 <itrunc+0xce>
      if(a[j])
80101c9d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca0:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ca7:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101caa:	01 d0                	add    %edx,%eax
80101cac:	8b 00                	mov    (%eax),%eax
80101cae:	85 c0                	test   %eax,%eax
80101cb0:	74 22                	je     80101cd4 <itrunc+0xca>
        bfree(ip->dev, a[j]);
80101cb2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101cbc:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101cbf:	01 d0                	add    %edx,%eax
80101cc1:	8b 10                	mov    (%eax),%edx
80101cc3:	8b 45 08             	mov    0x8(%ebp),%eax
80101cc6:	8b 00                	mov    (%eax),%eax
80101cc8:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ccc:	89 04 24             	mov    %eax,(%esp)
80101ccf:	e8 02 f8 ff ff       	call   801014d6 <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cd4:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cd8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cdb:	83 f8 7f             	cmp    $0x7f,%eax
80101cde:	76 bd                	jbe    80101c9d <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101ce0:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ce3:	89 04 24             	mov    %eax,(%esp)
80101ce6:	e8 2c e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ceb:	8b 45 08             	mov    0x8(%ebp),%eax
80101cee:	8b 50 4c             	mov    0x4c(%eax),%edx
80101cf1:	8b 45 08             	mov    0x8(%ebp),%eax
80101cf4:	8b 00                	mov    (%eax),%eax
80101cf6:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cfa:	89 04 24             	mov    %eax,(%esp)
80101cfd:	e8 d4 f7 ff ff       	call   801014d6 <bfree>
    ip->addrs[NDIRECT] = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d16:	8b 45 08             	mov    0x8(%ebp),%eax
80101d19:	89 04 24             	mov    %eax,(%esp)
80101d1c:	e8 7e f9 ff ff       	call   8010169f <iupdate>
}
80101d21:	c9                   	leave  
80101d22:	c3                   	ret    

80101d23 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d23:	55                   	push   %ebp
80101d24:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d26:	8b 45 08             	mov    0x8(%ebp),%eax
80101d29:	8b 00                	mov    (%eax),%eax
80101d2b:	89 c2                	mov    %eax,%edx
80101d2d:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d30:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d33:	8b 45 08             	mov    0x8(%ebp),%eax
80101d36:	8b 50 04             	mov    0x4(%eax),%edx
80101d39:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3c:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101d42:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d46:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d49:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d4f:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d53:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d56:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d5a:	8b 45 08             	mov    0x8(%ebp),%eax
80101d5d:	8b 50 18             	mov    0x18(%eax),%edx
80101d60:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d63:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d66:	5d                   	pop    %ebp
80101d67:	c3                   	ret    

80101d68 <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d68:	55                   	push   %ebp
80101d69:	89 e5                	mov    %esp,%ebp
80101d6b:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d6e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d71:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d75:	66 83 f8 03          	cmp    $0x3,%ax
80101d79:	75 60                	jne    80101ddb <readi+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d7b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d7e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d82:	66 85 c0             	test   %ax,%ax
80101d85:	78 20                	js     80101da7 <readi+0x3f>
80101d87:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8a:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d8e:	66 83 f8 09          	cmp    $0x9,%ax
80101d92:	7f 13                	jg     80101da7 <readi+0x3f>
80101d94:	8b 45 08             	mov    0x8(%ebp),%eax
80101d97:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d9b:	98                   	cwtl   
80101d9c:	8b 04 c5 20 12 11 80 	mov    -0x7feeede0(,%eax,8),%eax
80101da3:	85 c0                	test   %eax,%eax
80101da5:	75 0a                	jne    80101db1 <readi+0x49>
      return -1;
80101da7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101dac:	e9 19 01 00 00       	jmp    80101eca <readi+0x162>
    return devsw[ip->major].read(ip, dst, n);
80101db1:	8b 45 08             	mov    0x8(%ebp),%eax
80101db4:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101db8:	98                   	cwtl   
80101db9:	8b 04 c5 20 12 11 80 	mov    -0x7feeede0(,%eax,8),%eax
80101dc0:	8b 55 14             	mov    0x14(%ebp),%edx
80101dc3:	89 54 24 08          	mov    %edx,0x8(%esp)
80101dc7:	8b 55 0c             	mov    0xc(%ebp),%edx
80101dca:	89 54 24 04          	mov    %edx,0x4(%esp)
80101dce:	8b 55 08             	mov    0x8(%ebp),%edx
80101dd1:	89 14 24             	mov    %edx,(%esp)
80101dd4:	ff d0                	call   *%eax
80101dd6:	e9 ef 00 00 00       	jmp    80101eca <readi+0x162>
  }

  if(off > ip->size || off + n < off)
80101ddb:	8b 45 08             	mov    0x8(%ebp),%eax
80101dde:	8b 40 18             	mov    0x18(%eax),%eax
80101de1:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de4:	72 0d                	jb     80101df3 <readi+0x8b>
80101de6:	8b 45 14             	mov    0x14(%ebp),%eax
80101de9:	8b 55 10             	mov    0x10(%ebp),%edx
80101dec:	01 d0                	add    %edx,%eax
80101dee:	3b 45 10             	cmp    0x10(%ebp),%eax
80101df1:	73 0a                	jae    80101dfd <readi+0x95>
    return -1;
80101df3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101df8:	e9 cd 00 00 00       	jmp    80101eca <readi+0x162>
  if(off + n > ip->size)
80101dfd:	8b 45 14             	mov    0x14(%ebp),%eax
80101e00:	8b 55 10             	mov    0x10(%ebp),%edx
80101e03:	01 c2                	add    %eax,%edx
80101e05:	8b 45 08             	mov    0x8(%ebp),%eax
80101e08:	8b 40 18             	mov    0x18(%eax),%eax
80101e0b:	39 c2                	cmp    %eax,%edx
80101e0d:	76 0c                	jbe    80101e1b <readi+0xb3>
    n = ip->size - off;
80101e0f:	8b 45 08             	mov    0x8(%ebp),%eax
80101e12:	8b 40 18             	mov    0x18(%eax),%eax
80101e15:	2b 45 10             	sub    0x10(%ebp),%eax
80101e18:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e1b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e22:	e9 94 00 00 00       	jmp    80101ebb <readi+0x153>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e27:	8b 45 10             	mov    0x10(%ebp),%eax
80101e2a:	c1 e8 09             	shr    $0x9,%eax
80101e2d:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e31:	8b 45 08             	mov    0x8(%ebp),%eax
80101e34:	89 04 24             	mov    %eax,(%esp)
80101e37:	e8 c1 fc ff ff       	call   80101afd <bmap>
80101e3c:	8b 55 08             	mov    0x8(%ebp),%edx
80101e3f:	8b 12                	mov    (%edx),%edx
80101e41:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e45:	89 14 24             	mov    %edx,(%esp)
80101e48:	e8 59 e3 ff ff       	call   801001a6 <bread>
80101e4d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e50:	8b 45 10             	mov    0x10(%ebp),%eax
80101e53:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e58:	89 c2                	mov    %eax,%edx
80101e5a:	b8 00 02 00 00       	mov    $0x200,%eax
80101e5f:	29 d0                	sub    %edx,%eax
80101e61:	89 c2                	mov    %eax,%edx
80101e63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e66:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e69:	29 c1                	sub    %eax,%ecx
80101e6b:	89 c8                	mov    %ecx,%eax
80101e6d:	39 c2                	cmp    %eax,%edx
80101e6f:	0f 46 c2             	cmovbe %edx,%eax
80101e72:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e75:	8b 45 10             	mov    0x10(%ebp),%eax
80101e78:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7d:	8d 50 10             	lea    0x10(%eax),%edx
80101e80:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e83:	01 d0                	add    %edx,%eax
80101e85:	8d 50 08             	lea    0x8(%eax),%edx
80101e88:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e8b:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e8f:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e93:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e96:	89 04 24             	mov    %eax,(%esp)
80101e99:	e8 fc 31 00 00       	call   8010509a <memmove>
    brelse(bp);
80101e9e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ea1:	89 04 24             	mov    %eax,(%esp)
80101ea4:	e8 6e e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea9:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eac:	01 45 f4             	add    %eax,-0xc(%ebp)
80101eaf:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb2:	01 45 10             	add    %eax,0x10(%ebp)
80101eb5:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb8:	01 45 0c             	add    %eax,0xc(%ebp)
80101ebb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101ebe:	3b 45 14             	cmp    0x14(%ebp),%eax
80101ec1:	0f 82 60 ff ff ff    	jb     80101e27 <readi+0xbf>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec7:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101eca:	c9                   	leave  
80101ecb:	c3                   	ret    

80101ecc <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ecc:	55                   	push   %ebp
80101ecd:	89 e5                	mov    %esp,%ebp
80101ecf:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed5:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed9:	66 83 f8 03          	cmp    $0x3,%ax
80101edd:	75 60                	jne    80101f3f <writei+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee2:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee6:	66 85 c0             	test   %ax,%ax
80101ee9:	78 20                	js     80101f0b <writei+0x3f>
80101eeb:	8b 45 08             	mov    0x8(%ebp),%eax
80101eee:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef2:	66 83 f8 09          	cmp    $0x9,%ax
80101ef6:	7f 13                	jg     80101f0b <writei+0x3f>
80101ef8:	8b 45 08             	mov    0x8(%ebp),%eax
80101efb:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101eff:	98                   	cwtl   
80101f00:	8b 04 c5 24 12 11 80 	mov    -0x7feeeddc(,%eax,8),%eax
80101f07:	85 c0                	test   %eax,%eax
80101f09:	75 0a                	jne    80101f15 <writei+0x49>
      return -1;
80101f0b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f10:	e9 44 01 00 00       	jmp    80102059 <writei+0x18d>
    return devsw[ip->major].write(ip, src, n);
80101f15:	8b 45 08             	mov    0x8(%ebp),%eax
80101f18:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1c:	98                   	cwtl   
80101f1d:	8b 04 c5 24 12 11 80 	mov    -0x7feeeddc(,%eax,8),%eax
80101f24:	8b 55 14             	mov    0x14(%ebp),%edx
80101f27:	89 54 24 08          	mov    %edx,0x8(%esp)
80101f2b:	8b 55 0c             	mov    0xc(%ebp),%edx
80101f2e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101f32:	8b 55 08             	mov    0x8(%ebp),%edx
80101f35:	89 14 24             	mov    %edx,(%esp)
80101f38:	ff d0                	call   *%eax
80101f3a:	e9 1a 01 00 00       	jmp    80102059 <writei+0x18d>
  }

  if(off > ip->size || off + n < off)
80101f3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101f42:	8b 40 18             	mov    0x18(%eax),%eax
80101f45:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f48:	72 0d                	jb     80101f57 <writei+0x8b>
80101f4a:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4d:	8b 55 10             	mov    0x10(%ebp),%edx
80101f50:	01 d0                	add    %edx,%eax
80101f52:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f55:	73 0a                	jae    80101f61 <writei+0x95>
    return -1;
80101f57:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5c:	e9 f8 00 00 00       	jmp    80102059 <writei+0x18d>
  if(off + n > MAXFILE*BSIZE)
80101f61:	8b 45 14             	mov    0x14(%ebp),%eax
80101f64:	8b 55 10             	mov    0x10(%ebp),%edx
80101f67:	01 d0                	add    %edx,%eax
80101f69:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6e:	76 0a                	jbe    80101f7a <writei+0xae>
    return -1;
80101f70:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f75:	e9 df 00 00 00       	jmp    80102059 <writei+0x18d>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f7a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f81:	e9 9f 00 00 00       	jmp    80102025 <writei+0x159>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f86:	8b 45 10             	mov    0x10(%ebp),%eax
80101f89:	c1 e8 09             	shr    $0x9,%eax
80101f8c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f90:	8b 45 08             	mov    0x8(%ebp),%eax
80101f93:	89 04 24             	mov    %eax,(%esp)
80101f96:	e8 62 fb ff ff       	call   80101afd <bmap>
80101f9b:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9e:	8b 12                	mov    (%edx),%edx
80101fa0:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa4:	89 14 24             	mov    %edx,(%esp)
80101fa7:	e8 fa e1 ff ff       	call   801001a6 <bread>
80101fac:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101faf:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb2:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fb7:	89 c2                	mov    %eax,%edx
80101fb9:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbe:	29 d0                	sub    %edx,%eax
80101fc0:	89 c2                	mov    %eax,%edx
80101fc2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc5:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc8:	29 c1                	sub    %eax,%ecx
80101fca:	89 c8                	mov    %ecx,%eax
80101fcc:	39 c2                	cmp    %eax,%edx
80101fce:	0f 46 c2             	cmovbe %edx,%eax
80101fd1:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd4:	8b 45 10             	mov    0x10(%ebp),%eax
80101fd7:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fdc:	8d 50 10             	lea    0x10(%eax),%edx
80101fdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fe2:	01 d0                	add    %edx,%eax
80101fe4:	8d 50 08             	lea    0x8(%eax),%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 9d 30 00 00       	call   8010509a <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 e8 13 00 00       	call   801033f0 <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 55 ff ff ff    	jb     80101f86 <writei+0xba>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18a>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18a>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 49 f6 ff ff       	call   8010169f <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	c9                   	leave  
8010205a:	c3                   	ret    

8010205b <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205b:	55                   	push   %ebp
8010205c:	89 e5                	mov    %esp,%ebp
8010205e:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102061:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
80102068:	00 
80102069:	8b 45 0c             	mov    0xc(%ebp),%eax
8010206c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102070:	8b 45 08             	mov    0x8(%ebp),%eax
80102073:	89 04 24             	mov    %eax,(%esp)
80102076:	e8 c2 30 00 00       	call   8010513d <strncmp>
}
8010207b:	c9                   	leave  
8010207c:	c3                   	ret    

8010207d <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
8010207d:	55                   	push   %ebp
8010207e:	89 e5                	mov    %esp,%ebp
80102080:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102083:	8b 45 08             	mov    0x8(%ebp),%eax
80102086:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208a:	66 83 f8 01          	cmp    $0x1,%ax
8010208e:	74 0c                	je     8010209c <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102090:	c7 04 24 b9 88 10 80 	movl   $0x801088b9,(%esp)
80102097:	e8 9e e4 ff ff       	call   8010053a <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
8010209c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a3:	e9 88 00 00 00       	jmp    80102130 <dirlookup+0xb3>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020a8:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020af:	00 
801020b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b3:	89 44 24 08          	mov    %eax,0x8(%esp)
801020b7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ba:	89 44 24 04          	mov    %eax,0x4(%esp)
801020be:	8b 45 08             	mov    0x8(%ebp),%eax
801020c1:	89 04 24             	mov    %eax,(%esp)
801020c4:	e8 9f fc ff ff       	call   80101d68 <readi>
801020c9:	83 f8 10             	cmp    $0x10,%eax
801020cc:	74 0c                	je     801020da <dirlookup+0x5d>
      panic("dirlink read");
801020ce:	c7 04 24 cb 88 10 80 	movl   $0x801088cb,(%esp)
801020d5:	e8 60 e4 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801020da:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020de:	66 85 c0             	test   %ax,%ax
801020e1:	75 02                	jne    801020e5 <dirlookup+0x68>
      continue;
801020e3:	eb 47                	jmp    8010212c <dirlookup+0xaf>
    if(namecmp(name, de.name) == 0){
801020e5:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020e8:	83 c0 02             	add    $0x2,%eax
801020eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801020ef:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f2:	89 04 24             	mov    %eax,(%esp)
801020f5:	e8 61 ff ff ff       	call   8010205b <namecmp>
801020fa:	85 c0                	test   %eax,%eax
801020fc:	75 2e                	jne    8010212c <dirlookup+0xaf>
      // entry matches path element
      if(poff)
801020fe:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102102:	74 08                	je     8010210c <dirlookup+0x8f>
        *poff = off;
80102104:	8b 45 10             	mov    0x10(%ebp),%eax
80102107:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210a:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210c:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102110:	0f b7 c0             	movzwl %ax,%eax
80102113:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102116:	8b 45 08             	mov    0x8(%ebp),%eax
80102119:	8b 00                	mov    (%eax),%eax
8010211b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010211e:	89 54 24 04          	mov    %edx,0x4(%esp)
80102122:	89 04 24             	mov    %eax,(%esp)
80102125:	e8 2d f6 ff ff       	call   80101757 <iget>
8010212a:	eb 18                	jmp    80102144 <dirlookup+0xc7>
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212c:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102130:	8b 45 08             	mov    0x8(%ebp),%eax
80102133:	8b 40 18             	mov    0x18(%eax),%eax
80102136:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102139:	0f 87 69 ff ff ff    	ja     801020a8 <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
8010213f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102144:	c9                   	leave  
80102145:	c3                   	ret    

80102146 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102146:	55                   	push   %ebp
80102147:	89 e5                	mov    %esp,%ebp
80102149:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214c:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102153:	00 
80102154:	8b 45 0c             	mov    0xc(%ebp),%eax
80102157:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215b:	8b 45 08             	mov    0x8(%ebp),%eax
8010215e:	89 04 24             	mov    %eax,(%esp)
80102161:	e8 17 ff ff ff       	call   8010207d <dirlookup>
80102166:	89 45 f0             	mov    %eax,-0x10(%ebp)
80102169:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010216d:	74 15                	je     80102184 <dirlink+0x3e>
    iput(ip);
8010216f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102172:	89 04 24             	mov    %eax,(%esp)
80102175:	e8 94 f8 ff ff       	call   80101a0e <iput>
    return -1;
8010217a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010217f:	e9 b7 00 00 00       	jmp    8010223b <dirlink+0xf5>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102184:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218b:	eb 46                	jmp    801021d3 <dirlink+0x8d>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010218d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102190:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102197:	00 
80102198:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219c:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010219f:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a3:	8b 45 08             	mov    0x8(%ebp),%eax
801021a6:	89 04 24             	mov    %eax,(%esp)
801021a9:	e8 ba fb ff ff       	call   80101d68 <readi>
801021ae:	83 f8 10             	cmp    $0x10,%eax
801021b1:	74 0c                	je     801021bf <dirlink+0x79>
      panic("dirlink read");
801021b3:	c7 04 24 cb 88 10 80 	movl   $0x801088cb,(%esp)
801021ba:	e8 7b e3 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801021bf:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c3:	66 85 c0             	test   %ax,%ax
801021c6:	75 02                	jne    801021ca <dirlink+0x84>
      break;
801021c8:	eb 16                	jmp    801021e0 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021cd:	83 c0 10             	add    $0x10,%eax
801021d0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d6:	8b 45 08             	mov    0x8(%ebp),%eax
801021d9:	8b 40 18             	mov    0x18(%eax),%eax
801021dc:	39 c2                	cmp    %eax,%edx
801021de:	72 ad                	jb     8010218d <dirlink+0x47>
      panic("dirlink read");
    if(de.inum == 0)
      break;
  }

  strncpy(de.name, name, DIRSIZ);
801021e0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021e7:	00 
801021e8:	8b 45 0c             	mov    0xc(%ebp),%eax
801021eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801021ef:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f2:	83 c0 02             	add    $0x2,%eax
801021f5:	89 04 24             	mov    %eax,(%esp)
801021f8:	e8 96 2f 00 00       	call   80105193 <strncpy>
  de.inum = inum;
801021fd:	8b 45 10             	mov    0x10(%ebp),%eax
80102200:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102204:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102207:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010220e:	00 
8010220f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102213:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102216:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221a:	8b 45 08             	mov    0x8(%ebp),%eax
8010221d:	89 04 24             	mov    %eax,(%esp)
80102220:	e8 a7 fc ff ff       	call   80101ecc <writei>
80102225:	83 f8 10             	cmp    $0x10,%eax
80102228:	74 0c                	je     80102236 <dirlink+0xf0>
    panic("dirlink");
8010222a:	c7 04 24 d8 88 10 80 	movl   $0x801088d8,(%esp)
80102231:	e8 04 e3 ff ff       	call   8010053a <panic>
  
  return 0;
80102236:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223b:	c9                   	leave  
8010223c:	c3                   	ret    

8010223d <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
8010223d:	55                   	push   %ebp
8010223e:	89 e5                	mov    %esp,%ebp
80102240:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102243:	eb 04                	jmp    80102249 <skipelem+0xc>
    path++;
80102245:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
80102249:	8b 45 08             	mov    0x8(%ebp),%eax
8010224c:	0f b6 00             	movzbl (%eax),%eax
8010224f:	3c 2f                	cmp    $0x2f,%al
80102251:	74 f2                	je     80102245 <skipelem+0x8>
    path++;
  if(*path == 0)
80102253:	8b 45 08             	mov    0x8(%ebp),%eax
80102256:	0f b6 00             	movzbl (%eax),%eax
80102259:	84 c0                	test   %al,%al
8010225b:	75 0a                	jne    80102267 <skipelem+0x2a>
    return 0;
8010225d:	b8 00 00 00 00       	mov    $0x0,%eax
80102262:	e9 86 00 00 00       	jmp    801022ed <skipelem+0xb0>
  s = path;
80102267:	8b 45 08             	mov    0x8(%ebp),%eax
8010226a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
8010226d:	eb 04                	jmp    80102273 <skipelem+0x36>
    path++;
8010226f:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102273:	8b 45 08             	mov    0x8(%ebp),%eax
80102276:	0f b6 00             	movzbl (%eax),%eax
80102279:	3c 2f                	cmp    $0x2f,%al
8010227b:	74 0a                	je     80102287 <skipelem+0x4a>
8010227d:	8b 45 08             	mov    0x8(%ebp),%eax
80102280:	0f b6 00             	movzbl (%eax),%eax
80102283:	84 c0                	test   %al,%al
80102285:	75 e8                	jne    8010226f <skipelem+0x32>
    path++;
  len = path - s;
80102287:	8b 55 08             	mov    0x8(%ebp),%edx
8010228a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010228d:	29 c2                	sub    %eax,%edx
8010228f:	89 d0                	mov    %edx,%eax
80102291:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
80102294:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
80102298:	7e 1c                	jle    801022b6 <skipelem+0x79>
    memmove(name, s, DIRSIZ);
8010229a:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a1:	00 
801022a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022a5:	89 44 24 04          	mov    %eax,0x4(%esp)
801022a9:	8b 45 0c             	mov    0xc(%ebp),%eax
801022ac:	89 04 24             	mov    %eax,(%esp)
801022af:	e8 e6 2d 00 00       	call   8010509a <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022b4:	eb 2a                	jmp    801022e0 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022b6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022b9:	89 44 24 08          	mov    %eax,0x8(%esp)
801022bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801022c4:	8b 45 0c             	mov    0xc(%ebp),%eax
801022c7:	89 04 24             	mov    %eax,(%esp)
801022ca:	e8 cb 2d 00 00       	call   8010509a <memmove>
    name[len] = 0;
801022cf:	8b 55 f0             	mov    -0x10(%ebp),%edx
801022d2:	8b 45 0c             	mov    0xc(%ebp),%eax
801022d5:	01 d0                	add    %edx,%eax
801022d7:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022da:	eb 04                	jmp    801022e0 <skipelem+0xa3>
    path++;
801022dc:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e0:	8b 45 08             	mov    0x8(%ebp),%eax
801022e3:	0f b6 00             	movzbl (%eax),%eax
801022e6:	3c 2f                	cmp    $0x2f,%al
801022e8:	74 f2                	je     801022dc <skipelem+0x9f>
    path++;
  return path;
801022ea:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022ed:	c9                   	leave  
801022ee:	c3                   	ret    

801022ef <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022ef:	55                   	push   %ebp
801022f0:	89 e5                	mov    %esp,%ebp
801022f2:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;

  if(*path == '/')
801022f5:	8b 45 08             	mov    0x8(%ebp),%eax
801022f8:	0f b6 00             	movzbl (%eax),%eax
801022fb:	3c 2f                	cmp    $0x2f,%al
801022fd:	75 1c                	jne    8010231b <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
801022ff:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102306:	00 
80102307:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010230e:	e8 44 f4 ff ff       	call   80101757 <iget>
80102313:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
80102316:	e9 af 00 00 00       	jmp    801023ca <namex+0xdb>
  struct inode *ip, *next;

  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102321:	8b 40 68             	mov    0x68(%eax),%eax
80102324:	89 04 24             	mov    %eax,(%esp)
80102327:	e8 fd f4 ff ff       	call   80101829 <idup>
8010232c:	89 45 f4             	mov    %eax,-0xc(%ebp)

  while((path = skipelem(path, name)) != 0){
8010232f:	e9 96 00 00 00       	jmp    801023ca <namex+0xdb>
    ilock(ip);
80102334:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102337:	89 04 24             	mov    %eax,(%esp)
8010233a:	e8 1c f5 ff ff       	call   8010185b <ilock>
    if(ip->type != T_DIR){
8010233f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102342:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80102346:	66 83 f8 01          	cmp    $0x1,%ax
8010234a:	74 15                	je     80102361 <namex+0x72>
      iunlockput(ip);
8010234c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010234f:	89 04 24             	mov    %eax,(%esp)
80102352:	e8 88 f7 ff ff       	call   80101adf <iunlockput>
      return 0;
80102357:	b8 00 00 00 00       	mov    $0x0,%eax
8010235c:	e9 a3 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102361:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102365:	74 1d                	je     80102384 <namex+0x95>
80102367:	8b 45 08             	mov    0x8(%ebp),%eax
8010236a:	0f b6 00             	movzbl (%eax),%eax
8010236d:	84 c0                	test   %al,%al
8010236f:	75 13                	jne    80102384 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102371:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102374:	89 04 24             	mov    %eax,(%esp)
80102377:	e8 2d f6 ff ff       	call   801019a9 <iunlock>
      return ip;
8010237c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010237f:	e9 80 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102384:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238b:	00 
8010238c:	8b 45 10             	mov    0x10(%ebp),%eax
8010238f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102393:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102396:	89 04 24             	mov    %eax,(%esp)
80102399:	e8 df fc ff ff       	call   8010207d <dirlookup>
8010239e:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a1:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a5:	75 12                	jne    801023b9 <namex+0xca>
      iunlockput(ip);
801023a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023aa:	89 04 24             	mov    %eax,(%esp)
801023ad:	e8 2d f7 ff ff       	call   80101adf <iunlockput>
      return 0;
801023b2:	b8 00 00 00 00       	mov    $0x0,%eax
801023b7:	eb 4b                	jmp    80102404 <namex+0x115>
    }
    iunlockput(ip);
801023b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023bc:	89 04 24             	mov    %eax,(%esp)
801023bf:	e8 1b f7 ff ff       	call   80101adf <iunlockput>
    ip = next;
801023c4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
801023ca:	8b 45 10             	mov    0x10(%ebp),%eax
801023cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d1:	8b 45 08             	mov    0x8(%ebp),%eax
801023d4:	89 04 24             	mov    %eax,(%esp)
801023d7:	e8 61 fe ff ff       	call   8010223d <skipelem>
801023dc:	89 45 08             	mov    %eax,0x8(%ebp)
801023df:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e3:	0f 85 4b ff ff ff    	jne    80102334 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023e9:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023ed:	74 12                	je     80102401 <namex+0x112>
    iput(ip);
801023ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f2:	89 04 24             	mov    %eax,(%esp)
801023f5:	e8 14 f6 ff ff       	call   80101a0e <iput>
    return 0;
801023fa:	b8 00 00 00 00       	mov    $0x0,%eax
801023ff:	eb 03                	jmp    80102404 <namex+0x115>
  }
  return ip;
80102401:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102404:	c9                   	leave  
80102405:	c3                   	ret    

80102406 <namei>:

struct inode*
namei(char *path)
{
80102406:	55                   	push   %ebp
80102407:	89 e5                	mov    %esp,%ebp
80102409:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
8010240c:	8d 45 ea             	lea    -0x16(%ebp),%eax
8010240f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102413:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241a:	00 
8010241b:	8b 45 08             	mov    0x8(%ebp),%eax
8010241e:	89 04 24             	mov    %eax,(%esp)
80102421:	e8 c9 fe ff ff       	call   801022ef <namex>
}
80102426:	c9                   	leave  
80102427:	c3                   	ret    

80102428 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102428:	55                   	push   %ebp
80102429:	89 e5                	mov    %esp,%ebp
8010242b:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
8010242e:	8b 45 0c             	mov    0xc(%ebp),%eax
80102431:	89 44 24 08          	mov    %eax,0x8(%esp)
80102435:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010243c:	00 
8010243d:	8b 45 08             	mov    0x8(%ebp),%eax
80102440:	89 04 24             	mov    %eax,(%esp)
80102443:	e8 a7 fe ff ff       	call   801022ef <namex>
}
80102448:	c9                   	leave  
80102449:	c3                   	ret    

8010244a <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
8010244a:	55                   	push   %ebp
8010244b:	89 e5                	mov    %esp,%ebp
8010244d:	83 ec 14             	sub    $0x14,%esp
80102450:	8b 45 08             	mov    0x8(%ebp),%eax
80102453:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102457:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
8010245b:	89 c2                	mov    %eax,%edx
8010245d:	ec                   	in     (%dx),%al
8010245e:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102461:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102465:	c9                   	leave  
80102466:	c3                   	ret    

80102467 <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
80102467:	55                   	push   %ebp
80102468:	89 e5                	mov    %esp,%ebp
8010246a:	57                   	push   %edi
8010246b:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010246c:	8b 55 08             	mov    0x8(%ebp),%edx
8010246f:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102472:	8b 45 10             	mov    0x10(%ebp),%eax
80102475:	89 cb                	mov    %ecx,%ebx
80102477:	89 df                	mov    %ebx,%edi
80102479:	89 c1                	mov    %eax,%ecx
8010247b:	fc                   	cld    
8010247c:	f3 6d                	rep insl (%dx),%es:(%edi)
8010247e:	89 c8                	mov    %ecx,%eax
80102480:	89 fb                	mov    %edi,%ebx
80102482:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102485:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
80102488:	5b                   	pop    %ebx
80102489:	5f                   	pop    %edi
8010248a:	5d                   	pop    %ebp
8010248b:	c3                   	ret    

8010248c <outb>:

static inline void
outb(ushort port, uchar data)
{
8010248c:	55                   	push   %ebp
8010248d:	89 e5                	mov    %esp,%ebp
8010248f:	83 ec 08             	sub    $0x8,%esp
80102492:	8b 55 08             	mov    0x8(%ebp),%edx
80102495:	8b 45 0c             	mov    0xc(%ebp),%eax
80102498:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010249c:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010249f:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024a3:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024a7:	ee                   	out    %al,(%dx)
}
801024a8:	c9                   	leave  
801024a9:	c3                   	ret    

801024aa <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024aa:	55                   	push   %ebp
801024ab:	89 e5                	mov    %esp,%ebp
801024ad:	56                   	push   %esi
801024ae:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024af:	8b 55 08             	mov    0x8(%ebp),%edx
801024b2:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024b5:	8b 45 10             	mov    0x10(%ebp),%eax
801024b8:	89 cb                	mov    %ecx,%ebx
801024ba:	89 de                	mov    %ebx,%esi
801024bc:	89 c1                	mov    %eax,%ecx
801024be:	fc                   	cld    
801024bf:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024c1:	89 c8                	mov    %ecx,%eax
801024c3:	89 f3                	mov    %esi,%ebx
801024c5:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024c8:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024cb:	5b                   	pop    %ebx
801024cc:	5e                   	pop    %esi
801024cd:	5d                   	pop    %ebp
801024ce:	c3                   	ret    

801024cf <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024cf:	55                   	push   %ebp
801024d0:	89 e5                	mov    %esp,%ebp
801024d2:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024d5:	90                   	nop
801024d6:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024dd:	e8 68 ff ff ff       	call   8010244a <inb>
801024e2:	0f b6 c0             	movzbl %al,%eax
801024e5:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024e8:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024eb:	25 c0 00 00 00       	and    $0xc0,%eax
801024f0:	83 f8 40             	cmp    $0x40,%eax
801024f3:	75 e1                	jne    801024d6 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
801024f5:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801024f9:	74 11                	je     8010250c <idewait+0x3d>
801024fb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024fe:	83 e0 21             	and    $0x21,%eax
80102501:	85 c0                	test   %eax,%eax
80102503:	74 07                	je     8010250c <idewait+0x3d>
    return -1;
80102505:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010250a:	eb 05                	jmp    80102511 <idewait+0x42>
  return 0;
8010250c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102511:	c9                   	leave  
80102512:	c3                   	ret    

80102513 <ideinit>:

void
ideinit(void)
{
80102513:	55                   	push   %ebp
80102514:	89 e5                	mov    %esp,%ebp
80102516:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102519:	c7 44 24 04 e0 88 10 	movl   $0x801088e0,0x4(%esp)
80102520:	80 
80102521:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
80102528:	e8 29 28 00 00       	call   80104d56 <initlock>
  picenable(IRQ_IDE);
8010252d:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102534:	e8 38 16 00 00       	call   80103b71 <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102539:	a1 80 29 11 80       	mov    0x80112980,%eax
8010253e:	83 e8 01             	sub    $0x1,%eax
80102541:	89 44 24 04          	mov    %eax,0x4(%esp)
80102545:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010254c:	e8 0c 04 00 00       	call   8010295d <ioapicenable>
  idewait(0);
80102551:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102558:	e8 72 ff ff ff       	call   801024cf <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
8010255d:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
80102564:	00 
80102565:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010256c:	e8 1b ff ff ff       	call   8010248c <outb>
  for(i=0; i<1000; i++){
80102571:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102578:	eb 20                	jmp    8010259a <ideinit+0x87>
    if(inb(0x1f7) != 0){
8010257a:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102581:	e8 c4 fe ff ff       	call   8010244a <inb>
80102586:	84 c0                	test   %al,%al
80102588:	74 0c                	je     80102596 <ideinit+0x83>
      havedisk1 = 1;
8010258a:	c7 05 78 b6 10 80 01 	movl   $0x1,0x8010b678
80102591:	00 00 00 
      break;
80102594:	eb 0d                	jmp    801025a3 <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
80102596:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010259a:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025a1:	7e d7                	jle    8010257a <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025a3:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025aa:	00 
801025ab:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025b2:	e8 d5 fe ff ff       	call   8010248c <outb>
}
801025b7:	c9                   	leave  
801025b8:	c3                   	ret    

801025b9 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025b9:	55                   	push   %ebp
801025ba:	89 e5                	mov    %esp,%ebp
801025bc:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025bf:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025c3:	75 0c                	jne    801025d1 <idestart+0x18>
    panic("idestart");
801025c5:	c7 04 24 e4 88 10 80 	movl   $0x801088e4,(%esp)
801025cc:	e8 69 df ff ff       	call   8010053a <panic>

  idewait(0);
801025d1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025d8:	e8 f2 fe ff ff       	call   801024cf <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025dd:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025e4:	00 
801025e5:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025ec:	e8 9b fe ff ff       	call   8010248c <outb>
  outb(0x1f2, 1);  // number of sectors
801025f1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801025f8:	00 
801025f9:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102600:	e8 87 fe ff ff       	call   8010248c <outb>
  outb(0x1f3, b->sector & 0xff);
80102605:	8b 45 08             	mov    0x8(%ebp),%eax
80102608:	8b 40 08             	mov    0x8(%eax),%eax
8010260b:	0f b6 c0             	movzbl %al,%eax
8010260e:	89 44 24 04          	mov    %eax,0x4(%esp)
80102612:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
80102619:	e8 6e fe ff ff       	call   8010248c <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
8010261e:	8b 45 08             	mov    0x8(%ebp),%eax
80102621:	8b 40 08             	mov    0x8(%eax),%eax
80102624:	c1 e8 08             	shr    $0x8,%eax
80102627:	0f b6 c0             	movzbl %al,%eax
8010262a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010262e:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102635:	e8 52 fe ff ff       	call   8010248c <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
8010263a:	8b 45 08             	mov    0x8(%ebp),%eax
8010263d:	8b 40 08             	mov    0x8(%eax),%eax
80102640:	c1 e8 10             	shr    $0x10,%eax
80102643:	0f b6 c0             	movzbl %al,%eax
80102646:	89 44 24 04          	mov    %eax,0x4(%esp)
8010264a:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
80102651:	e8 36 fe ff ff       	call   8010248c <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102656:	8b 45 08             	mov    0x8(%ebp),%eax
80102659:	8b 40 04             	mov    0x4(%eax),%eax
8010265c:	83 e0 01             	and    $0x1,%eax
8010265f:	c1 e0 04             	shl    $0x4,%eax
80102662:	89 c2                	mov    %eax,%edx
80102664:	8b 45 08             	mov    0x8(%ebp),%eax
80102667:	8b 40 08             	mov    0x8(%eax),%eax
8010266a:	c1 e8 18             	shr    $0x18,%eax
8010266d:	83 e0 0f             	and    $0xf,%eax
80102670:	09 d0                	or     %edx,%eax
80102672:	83 c8 e0             	or     $0xffffffe0,%eax
80102675:	0f b6 c0             	movzbl %al,%eax
80102678:	89 44 24 04          	mov    %eax,0x4(%esp)
8010267c:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102683:	e8 04 fe ff ff       	call   8010248c <outb>
  if(b->flags & B_DIRTY){
80102688:	8b 45 08             	mov    0x8(%ebp),%eax
8010268b:	8b 00                	mov    (%eax),%eax
8010268d:	83 e0 04             	and    $0x4,%eax
80102690:	85 c0                	test   %eax,%eax
80102692:	74 34                	je     801026c8 <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
80102694:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
8010269b:	00 
8010269c:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026a3:	e8 e4 fd ff ff       	call   8010248c <outb>
    outsl(0x1f0, b->data, 512/4);
801026a8:	8b 45 08             	mov    0x8(%ebp),%eax
801026ab:	83 c0 18             	add    $0x18,%eax
801026ae:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026b5:	00 
801026b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801026ba:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026c1:	e8 e4 fd ff ff       	call   801024aa <outsl>
801026c6:	eb 14                	jmp    801026dc <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026c8:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026cf:	00 
801026d0:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026d7:	e8 b0 fd ff ff       	call   8010248c <outb>
  }
}
801026dc:	c9                   	leave  
801026dd:	c3                   	ret    

801026de <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026de:	55                   	push   %ebp
801026df:	89 e5                	mov    %esp,%ebp
801026e1:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026e4:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
801026eb:	e8 87 26 00 00       	call   80104d77 <acquire>
  if((b = idequeue) == 0){
801026f0:	a1 74 b6 10 80       	mov    0x8010b674,%eax
801026f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
801026f8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801026fc:	75 11                	jne    8010270f <ideintr+0x31>
    release(&idelock);
801026fe:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
80102705:	e8 cf 26 00 00       	call   80104dd9 <release>
    // cprintf("spurious IDE interrupt\n");
    return;
8010270a:	e9 90 00 00 00       	jmp    8010279f <ideintr+0xc1>
  }
  idequeue = b->qnext;
8010270f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102712:	8b 40 14             	mov    0x14(%eax),%eax
80102715:	a3 74 b6 10 80       	mov    %eax,0x8010b674

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
8010271a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010271d:	8b 00                	mov    (%eax),%eax
8010271f:	83 e0 04             	and    $0x4,%eax
80102722:	85 c0                	test   %eax,%eax
80102724:	75 2e                	jne    80102754 <ideintr+0x76>
80102726:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010272d:	e8 9d fd ff ff       	call   801024cf <idewait>
80102732:	85 c0                	test   %eax,%eax
80102734:	78 1e                	js     80102754 <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102736:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102739:	83 c0 18             	add    $0x18,%eax
8010273c:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102743:	00 
80102744:	89 44 24 04          	mov    %eax,0x4(%esp)
80102748:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
8010274f:	e8 13 fd ff ff       	call   80102467 <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
80102754:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102757:	8b 00                	mov    (%eax),%eax
80102759:	83 c8 02             	or     $0x2,%eax
8010275c:	89 c2                	mov    %eax,%edx
8010275e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102761:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102763:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102766:	8b 00                	mov    (%eax),%eax
80102768:	83 e0 fb             	and    $0xfffffffb,%eax
8010276b:	89 c2                	mov    %eax,%edx
8010276d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102770:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80102772:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102775:	89 04 24             	mov    %eax,(%esp)
80102778:	e8 03 24 00 00       	call   80104b80 <wakeup>
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
8010277d:	a1 74 b6 10 80       	mov    0x8010b674,%eax
80102782:	85 c0                	test   %eax,%eax
80102784:	74 0d                	je     80102793 <ideintr+0xb5>
    idestart(idequeue);
80102786:	a1 74 b6 10 80       	mov    0x8010b674,%eax
8010278b:	89 04 24             	mov    %eax,(%esp)
8010278e:	e8 26 fe ff ff       	call   801025b9 <idestart>

  release(&idelock);
80102793:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
8010279a:	e8 3a 26 00 00       	call   80104dd9 <release>
}
8010279f:	c9                   	leave  
801027a0:	c3                   	ret    

801027a1 <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027a1:	55                   	push   %ebp
801027a2:	89 e5                	mov    %esp,%ebp
801027a4:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027a7:	8b 45 08             	mov    0x8(%ebp),%eax
801027aa:	8b 00                	mov    (%eax),%eax
801027ac:	83 e0 01             	and    $0x1,%eax
801027af:	85 c0                	test   %eax,%eax
801027b1:	75 0c                	jne    801027bf <iderw+0x1e>
    panic("iderw: buf not busy");
801027b3:	c7 04 24 ed 88 10 80 	movl   $0x801088ed,(%esp)
801027ba:	e8 7b dd ff ff       	call   8010053a <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027bf:	8b 45 08             	mov    0x8(%ebp),%eax
801027c2:	8b 00                	mov    (%eax),%eax
801027c4:	83 e0 06             	and    $0x6,%eax
801027c7:	83 f8 02             	cmp    $0x2,%eax
801027ca:	75 0c                	jne    801027d8 <iderw+0x37>
    panic("iderw: nothing to do");
801027cc:	c7 04 24 01 89 10 80 	movl   $0x80108901,(%esp)
801027d3:	e8 62 dd ff ff       	call   8010053a <panic>
  if(b->dev != 0 && !havedisk1)
801027d8:	8b 45 08             	mov    0x8(%ebp),%eax
801027db:	8b 40 04             	mov    0x4(%eax),%eax
801027de:	85 c0                	test   %eax,%eax
801027e0:	74 15                	je     801027f7 <iderw+0x56>
801027e2:	a1 78 b6 10 80       	mov    0x8010b678,%eax
801027e7:	85 c0                	test   %eax,%eax
801027e9:	75 0c                	jne    801027f7 <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027eb:	c7 04 24 16 89 10 80 	movl   $0x80108916,(%esp)
801027f2:	e8 43 dd ff ff       	call   8010053a <panic>

  acquire(&idelock);  //DOC:acquire-lock
801027f7:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
801027fe:	e8 74 25 00 00       	call   80104d77 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
80102803:	8b 45 08             	mov    0x8(%ebp),%eax
80102806:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
8010280d:	c7 45 f4 74 b6 10 80 	movl   $0x8010b674,-0xc(%ebp)
80102814:	eb 0b                	jmp    80102821 <iderw+0x80>
80102816:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102819:	8b 00                	mov    (%eax),%eax
8010281b:	83 c0 14             	add    $0x14,%eax
8010281e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102821:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102824:	8b 00                	mov    (%eax),%eax
80102826:	85 c0                	test   %eax,%eax
80102828:	75 ec                	jne    80102816 <iderw+0x75>
    ;
  *pp = b;
8010282a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282d:	8b 55 08             	mov    0x8(%ebp),%edx
80102830:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
80102832:	a1 74 b6 10 80       	mov    0x8010b674,%eax
80102837:	3b 45 08             	cmp    0x8(%ebp),%eax
8010283a:	75 0d                	jne    80102849 <iderw+0xa8>
    idestart(b);
8010283c:	8b 45 08             	mov    0x8(%ebp),%eax
8010283f:	89 04 24             	mov    %eax,(%esp)
80102842:	e8 72 fd ff ff       	call   801025b9 <idestart>
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102847:	eb 15                	jmp    8010285e <iderw+0xbd>
80102849:	eb 13                	jmp    8010285e <iderw+0xbd>
    sleep(b, &idelock);
8010284b:	c7 44 24 04 40 b6 10 	movl   $0x8010b640,0x4(%esp)
80102852:	80 
80102853:	8b 45 08             	mov    0x8(%ebp),%eax
80102856:	89 04 24             	mov    %eax,(%esp)
80102859:	e8 46 22 00 00       	call   80104aa4 <sleep>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010285e:	8b 45 08             	mov    0x8(%ebp),%eax
80102861:	8b 00                	mov    (%eax),%eax
80102863:	83 e0 06             	and    $0x6,%eax
80102866:	83 f8 02             	cmp    $0x2,%eax
80102869:	75 e0                	jne    8010284b <iderw+0xaa>
    sleep(b, &idelock);
  }

  release(&idelock);
8010286b:	c7 04 24 40 b6 10 80 	movl   $0x8010b640,(%esp)
80102872:	e8 62 25 00 00       	call   80104dd9 <release>
}
80102877:	c9                   	leave  
80102878:	c3                   	ret    

80102879 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102879:	55                   	push   %ebp
8010287a:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
8010287c:	a1 54 22 11 80       	mov    0x80112254,%eax
80102881:	8b 55 08             	mov    0x8(%ebp),%edx
80102884:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102886:	a1 54 22 11 80       	mov    0x80112254,%eax
8010288b:	8b 40 10             	mov    0x10(%eax),%eax
}
8010288e:	5d                   	pop    %ebp
8010288f:	c3                   	ret    

80102890 <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
80102890:	55                   	push   %ebp
80102891:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102893:	a1 54 22 11 80       	mov    0x80112254,%eax
80102898:	8b 55 08             	mov    0x8(%ebp),%edx
8010289b:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
8010289d:	a1 54 22 11 80       	mov    0x80112254,%eax
801028a2:	8b 55 0c             	mov    0xc(%ebp),%edx
801028a5:	89 50 10             	mov    %edx,0x10(%eax)
}
801028a8:	5d                   	pop    %ebp
801028a9:	c3                   	ret    

801028aa <ioapicinit>:

void
ioapicinit(void)
{
801028aa:	55                   	push   %ebp
801028ab:	89 e5                	mov    %esp,%ebp
801028ad:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028b0:	a1 84 23 11 80       	mov    0x80112384,%eax
801028b5:	85 c0                	test   %eax,%eax
801028b7:	75 05                	jne    801028be <ioapicinit+0x14>
    return;
801028b9:	e9 9d 00 00 00       	jmp    8010295b <ioapicinit+0xb1>

  ioapic = (volatile struct ioapic*)IOAPIC;
801028be:	c7 05 54 22 11 80 00 	movl   $0xfec00000,0x80112254
801028c5:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028c8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028cf:	e8 a5 ff ff ff       	call   80102879 <ioapicread>
801028d4:	c1 e8 10             	shr    $0x10,%eax
801028d7:	25 ff 00 00 00       	and    $0xff,%eax
801028dc:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028df:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028e6:	e8 8e ff ff ff       	call   80102879 <ioapicread>
801028eb:	c1 e8 18             	shr    $0x18,%eax
801028ee:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
801028f1:	0f b6 05 80 23 11 80 	movzbl 0x80112380,%eax
801028f8:	0f b6 c0             	movzbl %al,%eax
801028fb:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801028fe:	74 0c                	je     8010290c <ioapicinit+0x62>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102900:	c7 04 24 34 89 10 80 	movl   $0x80108934,(%esp)
80102907:	e8 94 da ff ff       	call   801003a0 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010290c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102913:	eb 3e                	jmp    80102953 <ioapicinit+0xa9>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102915:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102918:	83 c0 20             	add    $0x20,%eax
8010291b:	0d 00 00 01 00       	or     $0x10000,%eax
80102920:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102923:	83 c2 08             	add    $0x8,%edx
80102926:	01 d2                	add    %edx,%edx
80102928:	89 44 24 04          	mov    %eax,0x4(%esp)
8010292c:	89 14 24             	mov    %edx,(%esp)
8010292f:	e8 5c ff ff ff       	call   80102890 <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102934:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102937:	83 c0 08             	add    $0x8,%eax
8010293a:	01 c0                	add    %eax,%eax
8010293c:	83 c0 01             	add    $0x1,%eax
8010293f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102946:	00 
80102947:	89 04 24             	mov    %eax,(%esp)
8010294a:	e8 41 ff ff ff       	call   80102890 <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010294f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102953:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102956:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102959:	7e ba                	jle    80102915 <ioapicinit+0x6b>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010295b:	c9                   	leave  
8010295c:	c3                   	ret    

8010295d <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010295d:	55                   	push   %ebp
8010295e:	89 e5                	mov    %esp,%ebp
80102960:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102963:	a1 84 23 11 80       	mov    0x80112384,%eax
80102968:	85 c0                	test   %eax,%eax
8010296a:	75 02                	jne    8010296e <ioapicenable+0x11>
    return;
8010296c:	eb 37                	jmp    801029a5 <ioapicenable+0x48>

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010296e:	8b 45 08             	mov    0x8(%ebp),%eax
80102971:	83 c0 20             	add    $0x20,%eax
80102974:	8b 55 08             	mov    0x8(%ebp),%edx
80102977:	83 c2 08             	add    $0x8,%edx
8010297a:	01 d2                	add    %edx,%edx
8010297c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102980:	89 14 24             	mov    %edx,(%esp)
80102983:	e8 08 ff ff ff       	call   80102890 <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102988:	8b 45 0c             	mov    0xc(%ebp),%eax
8010298b:	c1 e0 18             	shl    $0x18,%eax
8010298e:	8b 55 08             	mov    0x8(%ebp),%edx
80102991:	83 c2 08             	add    $0x8,%edx
80102994:	01 d2                	add    %edx,%edx
80102996:	83 c2 01             	add    $0x1,%edx
80102999:	89 44 24 04          	mov    %eax,0x4(%esp)
8010299d:	89 14 24             	mov    %edx,(%esp)
801029a0:	e8 eb fe ff ff       	call   80102890 <ioapicwrite>
}
801029a5:	c9                   	leave  
801029a6:	c3                   	ret    

801029a7 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029a7:	55                   	push   %ebp
801029a8:	89 e5                	mov    %esp,%ebp
801029aa:	8b 45 08             	mov    0x8(%ebp),%eax
801029ad:	05 00 00 00 80       	add    $0x80000000,%eax
801029b2:	5d                   	pop    %ebp
801029b3:	c3                   	ret    

801029b4 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029b4:	55                   	push   %ebp
801029b5:	89 e5                	mov    %esp,%ebp
801029b7:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029ba:	c7 44 24 04 66 89 10 	movl   $0x80108966,0x4(%esp)
801029c1:	80 
801029c2:	c7 04 24 60 22 11 80 	movl   $0x80112260,(%esp)
801029c9:	e8 88 23 00 00       	call   80104d56 <initlock>
  kmem.use_lock = 0;
801029ce:	c7 05 94 22 11 80 00 	movl   $0x0,0x80112294
801029d5:	00 00 00 
  freerange(vstart, vend);
801029d8:	8b 45 0c             	mov    0xc(%ebp),%eax
801029db:	89 44 24 04          	mov    %eax,0x4(%esp)
801029df:	8b 45 08             	mov    0x8(%ebp),%eax
801029e2:	89 04 24             	mov    %eax,(%esp)
801029e5:	e8 26 00 00 00       	call   80102a10 <freerange>
}
801029ea:	c9                   	leave  
801029eb:	c3                   	ret    

801029ec <kinit2>:

void
kinit2(void *vstart, void *vend)
{
801029ec:	55                   	push   %ebp
801029ed:	89 e5                	mov    %esp,%ebp
801029ef:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
801029f2:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f5:	89 44 24 04          	mov    %eax,0x4(%esp)
801029f9:	8b 45 08             	mov    0x8(%ebp),%eax
801029fc:	89 04 24             	mov    %eax,(%esp)
801029ff:	e8 0c 00 00 00       	call   80102a10 <freerange>
  kmem.use_lock = 1;
80102a04:	c7 05 94 22 11 80 01 	movl   $0x1,0x80112294
80102a0b:	00 00 00 
}
80102a0e:	c9                   	leave  
80102a0f:	c3                   	ret    

80102a10 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a10:	55                   	push   %ebp
80102a11:	89 e5                	mov    %esp,%ebp
80102a13:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a16:	8b 45 08             	mov    0x8(%ebp),%eax
80102a19:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a1e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a23:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a26:	eb 12                	jmp    80102a3a <freerange+0x2a>
    kfree(p);
80102a28:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a2b:	89 04 24             	mov    %eax,(%esp)
80102a2e:	e8 16 00 00 00       	call   80102a49 <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a33:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a3d:	05 00 10 00 00       	add    $0x1000,%eax
80102a42:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a45:	76 e1                	jbe    80102a28 <freerange+0x18>
    kfree(p);
}
80102a47:	c9                   	leave  
80102a48:	c3                   	ret    

80102a49 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a49:	55                   	push   %ebp
80102a4a:	89 e5                	mov    %esp,%ebp
80102a4c:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a4f:	8b 45 08             	mov    0x8(%ebp),%eax
80102a52:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a57:	85 c0                	test   %eax,%eax
80102a59:	75 1b                	jne    80102a76 <kfree+0x2d>
80102a5b:	81 7d 08 7c 86 11 80 	cmpl   $0x8011867c,0x8(%ebp)
80102a62:	72 12                	jb     80102a76 <kfree+0x2d>
80102a64:	8b 45 08             	mov    0x8(%ebp),%eax
80102a67:	89 04 24             	mov    %eax,(%esp)
80102a6a:	e8 38 ff ff ff       	call   801029a7 <v2p>
80102a6f:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a74:	76 0c                	jbe    80102a82 <kfree+0x39>
    panic("kfree");
80102a76:	c7 04 24 6b 89 10 80 	movl   $0x8010896b,(%esp)
80102a7d:	e8 b8 da ff ff       	call   8010053a <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a82:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102a89:	00 
80102a8a:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102a91:	00 
80102a92:	8b 45 08             	mov    0x8(%ebp),%eax
80102a95:	89 04 24             	mov    %eax,(%esp)
80102a98:	e8 2e 25 00 00       	call   80104fcb <memset>

  if(kmem.use_lock)
80102a9d:	a1 94 22 11 80       	mov    0x80112294,%eax
80102aa2:	85 c0                	test   %eax,%eax
80102aa4:	74 0c                	je     80102ab2 <kfree+0x69>
    acquire(&kmem.lock);
80102aa6:	c7 04 24 60 22 11 80 	movl   $0x80112260,(%esp)
80102aad:	e8 c5 22 00 00       	call   80104d77 <acquire>
  r = (struct run*)v;
80102ab2:	8b 45 08             	mov    0x8(%ebp),%eax
80102ab5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102ab8:	8b 15 98 22 11 80    	mov    0x80112298,%edx
80102abe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac1:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ac3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac6:	a3 98 22 11 80       	mov    %eax,0x80112298
  if(kmem.use_lock)
80102acb:	a1 94 22 11 80       	mov    0x80112294,%eax
80102ad0:	85 c0                	test   %eax,%eax
80102ad2:	74 0c                	je     80102ae0 <kfree+0x97>
    release(&kmem.lock);
80102ad4:	c7 04 24 60 22 11 80 	movl   $0x80112260,(%esp)
80102adb:	e8 f9 22 00 00       	call   80104dd9 <release>
}
80102ae0:	c9                   	leave  
80102ae1:	c3                   	ret    

80102ae2 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102ae2:	55                   	push   %ebp
80102ae3:	89 e5                	mov    %esp,%ebp
80102ae5:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102ae8:	a1 94 22 11 80       	mov    0x80112294,%eax
80102aed:	85 c0                	test   %eax,%eax
80102aef:	74 0c                	je     80102afd <kalloc+0x1b>
    acquire(&kmem.lock);
80102af1:	c7 04 24 60 22 11 80 	movl   $0x80112260,(%esp)
80102af8:	e8 7a 22 00 00       	call   80104d77 <acquire>
  r = kmem.freelist;
80102afd:	a1 98 22 11 80       	mov    0x80112298,%eax
80102b02:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b05:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b09:	74 0a                	je     80102b15 <kalloc+0x33>
    kmem.freelist = r->next;
80102b0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b0e:	8b 00                	mov    (%eax),%eax
80102b10:	a3 98 22 11 80       	mov    %eax,0x80112298
  if(kmem.use_lock)
80102b15:	a1 94 22 11 80       	mov    0x80112294,%eax
80102b1a:	85 c0                	test   %eax,%eax
80102b1c:	74 0c                	je     80102b2a <kalloc+0x48>
    release(&kmem.lock);
80102b1e:	c7 04 24 60 22 11 80 	movl   $0x80112260,(%esp)
80102b25:	e8 af 22 00 00       	call   80104dd9 <release>
  return (char*)r;
80102b2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b2d:	c9                   	leave  
80102b2e:	c3                   	ret    

80102b2f <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b2f:	55                   	push   %ebp
80102b30:	89 e5                	mov    %esp,%ebp
80102b32:	83 ec 14             	sub    $0x14,%esp
80102b35:	8b 45 08             	mov    0x8(%ebp),%eax
80102b38:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b3c:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80102b40:	89 c2                	mov    %eax,%edx
80102b42:	ec                   	in     (%dx),%al
80102b43:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102b46:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102b4a:	c9                   	leave  
80102b4b:	c3                   	ret    

80102b4c <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b4c:	55                   	push   %ebp
80102b4d:	89 e5                	mov    %esp,%ebp
80102b4f:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b52:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b59:	e8 d1 ff ff ff       	call   80102b2f <inb>
80102b5e:	0f b6 c0             	movzbl %al,%eax
80102b61:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b64:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b67:	83 e0 01             	and    $0x1,%eax
80102b6a:	85 c0                	test   %eax,%eax
80102b6c:	75 0a                	jne    80102b78 <kbdgetc+0x2c>
    return -1;
80102b6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b73:	e9 25 01 00 00       	jmp    80102c9d <kbdgetc+0x151>
  data = inb(KBDATAP);
80102b78:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102b7f:	e8 ab ff ff ff       	call   80102b2f <inb>
80102b84:	0f b6 c0             	movzbl %al,%eax
80102b87:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102b8a:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102b91:	75 17                	jne    80102baa <kbdgetc+0x5e>
    shift |= E0ESC;
80102b93:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102b98:	83 c8 40             	or     $0x40,%eax
80102b9b:	a3 7c b6 10 80       	mov    %eax,0x8010b67c
    return 0;
80102ba0:	b8 00 00 00 00       	mov    $0x0,%eax
80102ba5:	e9 f3 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(data & 0x80){
80102baa:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bad:	25 80 00 00 00       	and    $0x80,%eax
80102bb2:	85 c0                	test   %eax,%eax
80102bb4:	74 45                	je     80102bfb <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102bb6:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102bbb:	83 e0 40             	and    $0x40,%eax
80102bbe:	85 c0                	test   %eax,%eax
80102bc0:	75 08                	jne    80102bca <kbdgetc+0x7e>
80102bc2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bc5:	83 e0 7f             	and    $0x7f,%eax
80102bc8:	eb 03                	jmp    80102bcd <kbdgetc+0x81>
80102bca:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bcd:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bd0:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bd3:	05 20 90 10 80       	add    $0x80109020,%eax
80102bd8:	0f b6 00             	movzbl (%eax),%eax
80102bdb:	83 c8 40             	or     $0x40,%eax
80102bde:	0f b6 c0             	movzbl %al,%eax
80102be1:	f7 d0                	not    %eax
80102be3:	89 c2                	mov    %eax,%edx
80102be5:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102bea:	21 d0                	and    %edx,%eax
80102bec:	a3 7c b6 10 80       	mov    %eax,0x8010b67c
    return 0;
80102bf1:	b8 00 00 00 00       	mov    $0x0,%eax
80102bf6:	e9 a2 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(shift & E0ESC){
80102bfb:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c00:	83 e0 40             	and    $0x40,%eax
80102c03:	85 c0                	test   %eax,%eax
80102c05:	74 14                	je     80102c1b <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c07:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c0e:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c13:	83 e0 bf             	and    $0xffffffbf,%eax
80102c16:	a3 7c b6 10 80       	mov    %eax,0x8010b67c
  }

  shift |= shiftcode[data];
80102c1b:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c1e:	05 20 90 10 80       	add    $0x80109020,%eax
80102c23:	0f b6 00             	movzbl (%eax),%eax
80102c26:	0f b6 d0             	movzbl %al,%edx
80102c29:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c2e:	09 d0                	or     %edx,%eax
80102c30:	a3 7c b6 10 80       	mov    %eax,0x8010b67c
  shift ^= togglecode[data];
80102c35:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c38:	05 20 91 10 80       	add    $0x80109120,%eax
80102c3d:	0f b6 00             	movzbl (%eax),%eax
80102c40:	0f b6 d0             	movzbl %al,%edx
80102c43:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c48:	31 d0                	xor    %edx,%eax
80102c4a:	a3 7c b6 10 80       	mov    %eax,0x8010b67c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c4f:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c54:	83 e0 03             	and    $0x3,%eax
80102c57:	8b 14 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%edx
80102c5e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c61:	01 d0                	add    %edx,%eax
80102c63:	0f b6 00             	movzbl (%eax),%eax
80102c66:	0f b6 c0             	movzbl %al,%eax
80102c69:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c6c:	a1 7c b6 10 80       	mov    0x8010b67c,%eax
80102c71:	83 e0 08             	and    $0x8,%eax
80102c74:	85 c0                	test   %eax,%eax
80102c76:	74 22                	je     80102c9a <kbdgetc+0x14e>
    if('a' <= c && c <= 'z')
80102c78:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102c7c:	76 0c                	jbe    80102c8a <kbdgetc+0x13e>
80102c7e:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102c82:	77 06                	ja     80102c8a <kbdgetc+0x13e>
      c += 'A' - 'a';
80102c84:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102c88:	eb 10                	jmp    80102c9a <kbdgetc+0x14e>
    else if('A' <= c && c <= 'Z')
80102c8a:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102c8e:	76 0a                	jbe    80102c9a <kbdgetc+0x14e>
80102c90:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102c94:	77 04                	ja     80102c9a <kbdgetc+0x14e>
      c += 'a' - 'A';
80102c96:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102c9a:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102c9d:	c9                   	leave  
80102c9e:	c3                   	ret    

80102c9f <kbdintr>:

void
kbdintr(void)
{
80102c9f:	55                   	push   %ebp
80102ca0:	89 e5                	mov    %esp,%ebp
80102ca2:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102ca5:	c7 04 24 4c 2b 10 80 	movl   $0x80102b4c,(%esp)
80102cac:	e8 fc da ff ff       	call   801007ad <consoleintr>
}
80102cb1:	c9                   	leave  
80102cb2:	c3                   	ret    

80102cb3 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cb3:	55                   	push   %ebp
80102cb4:	89 e5                	mov    %esp,%ebp
80102cb6:	83 ec 08             	sub    $0x8,%esp
80102cb9:	8b 55 08             	mov    0x8(%ebp),%edx
80102cbc:	8b 45 0c             	mov    0xc(%ebp),%eax
80102cbf:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102cc3:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cc6:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102cca:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102cce:	ee                   	out    %al,(%dx)
}
80102ccf:	c9                   	leave  
80102cd0:	c3                   	ret    

80102cd1 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cd1:	55                   	push   %ebp
80102cd2:	89 e5                	mov    %esp,%ebp
80102cd4:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102cd7:	9c                   	pushf  
80102cd8:	58                   	pop    %eax
80102cd9:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80102cdc:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80102cdf:	c9                   	leave  
80102ce0:	c3                   	ret    

80102ce1 <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102ce1:	55                   	push   %ebp
80102ce2:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102ce4:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102ce9:	8b 55 08             	mov    0x8(%ebp),%edx
80102cec:	c1 e2 02             	shl    $0x2,%edx
80102cef:	01 c2                	add    %eax,%edx
80102cf1:	8b 45 0c             	mov    0xc(%ebp),%eax
80102cf4:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102cf6:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102cfb:	83 c0 20             	add    $0x20,%eax
80102cfe:	8b 00                	mov    (%eax),%eax
}
80102d00:	5d                   	pop    %ebp
80102d01:	c3                   	ret    

80102d02 <lapicinit>:
//PAGEBREAK!

void
lapicinit(void)
{
80102d02:	55                   	push   %ebp
80102d03:	89 e5                	mov    %esp,%ebp
80102d05:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d08:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102d0d:	85 c0                	test   %eax,%eax
80102d0f:	75 05                	jne    80102d16 <lapicinit+0x14>
    return;
80102d11:	e9 43 01 00 00       	jmp    80102e59 <lapicinit+0x157>

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d16:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d1d:	00 
80102d1e:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d25:	e8 b7 ff ff ff       	call   80102ce1 <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d2a:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d31:	00 
80102d32:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d39:	e8 a3 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d3e:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d45:	00 
80102d46:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d4d:	e8 8f ff ff ff       	call   80102ce1 <lapicw>
  lapicw(TICR, 10000000); 
80102d52:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d59:	00 
80102d5a:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d61:	e8 7b ff ff ff       	call   80102ce1 <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d66:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d6d:	00 
80102d6e:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102d75:	e8 67 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(LINT1, MASKED);
80102d7a:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d81:	00 
80102d82:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102d89:	e8 53 ff ff ff       	call   80102ce1 <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102d8e:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102d93:	83 c0 30             	add    $0x30,%eax
80102d96:	8b 00                	mov    (%eax),%eax
80102d98:	c1 e8 10             	shr    $0x10,%eax
80102d9b:	0f b6 c0             	movzbl %al,%eax
80102d9e:	83 f8 03             	cmp    $0x3,%eax
80102da1:	76 14                	jbe    80102db7 <lapicinit+0xb5>
    lapicw(PCINT, MASKED);
80102da3:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102daa:	00 
80102dab:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102db2:	e8 2a ff ff ff       	call   80102ce1 <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102db7:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102dbe:	00 
80102dbf:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102dc6:	e8 16 ff ff ff       	call   80102ce1 <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102dcb:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102dd2:	00 
80102dd3:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102dda:	e8 02 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(ESR, 0);
80102ddf:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102de6:	00 
80102de7:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102dee:	e8 ee fe ff ff       	call   80102ce1 <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102df3:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102dfa:	00 
80102dfb:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e02:	e8 da fe ff ff       	call   80102ce1 <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e07:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e0e:	00 
80102e0f:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e16:	e8 c6 fe ff ff       	call   80102ce1 <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e1b:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e22:	00 
80102e23:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e2a:	e8 b2 fe ff ff       	call   80102ce1 <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e2f:	90                   	nop
80102e30:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102e35:	05 00 03 00 00       	add    $0x300,%eax
80102e3a:	8b 00                	mov    (%eax),%eax
80102e3c:	25 00 10 00 00       	and    $0x1000,%eax
80102e41:	85 c0                	test   %eax,%eax
80102e43:	75 eb                	jne    80102e30 <lapicinit+0x12e>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e45:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e4c:	00 
80102e4d:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e54:	e8 88 fe ff ff       	call   80102ce1 <lapicw>
}
80102e59:	c9                   	leave  
80102e5a:	c3                   	ret    

80102e5b <cpunum>:

int
cpunum(void)
{
80102e5b:	55                   	push   %ebp
80102e5c:	89 e5                	mov    %esp,%ebp
80102e5e:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e61:	e8 6b fe ff ff       	call   80102cd1 <readeflags>
80102e66:	25 00 02 00 00       	and    $0x200,%eax
80102e6b:	85 c0                	test   %eax,%eax
80102e6d:	74 25                	je     80102e94 <cpunum+0x39>
    static int n;
    if(n++ == 0)
80102e6f:	a1 80 b6 10 80       	mov    0x8010b680,%eax
80102e74:	8d 50 01             	lea    0x1(%eax),%edx
80102e77:	89 15 80 b6 10 80    	mov    %edx,0x8010b680
80102e7d:	85 c0                	test   %eax,%eax
80102e7f:	75 13                	jne    80102e94 <cpunum+0x39>
      cprintf("cpu called from %x with interrupts enabled\n",
80102e81:	8b 45 04             	mov    0x4(%ebp),%eax
80102e84:	89 44 24 04          	mov    %eax,0x4(%esp)
80102e88:	c7 04 24 74 89 10 80 	movl   $0x80108974,(%esp)
80102e8f:	e8 0c d5 ff ff       	call   801003a0 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102e94:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102e99:	85 c0                	test   %eax,%eax
80102e9b:	74 0f                	je     80102eac <cpunum+0x51>
    return lapic[ID]>>24;
80102e9d:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102ea2:	83 c0 20             	add    $0x20,%eax
80102ea5:	8b 00                	mov    (%eax),%eax
80102ea7:	c1 e8 18             	shr    $0x18,%eax
80102eaa:	eb 05                	jmp    80102eb1 <cpunum+0x56>
  return 0;
80102eac:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102eb1:	c9                   	leave  
80102eb2:	c3                   	ret    

80102eb3 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102eb3:	55                   	push   %ebp
80102eb4:	89 e5                	mov    %esp,%ebp
80102eb6:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102eb9:	a1 9c 22 11 80       	mov    0x8011229c,%eax
80102ebe:	85 c0                	test   %eax,%eax
80102ec0:	74 14                	je     80102ed6 <lapiceoi+0x23>
    lapicw(EOI, 0);
80102ec2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ec9:	00 
80102eca:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102ed1:	e8 0b fe ff ff       	call   80102ce1 <lapicw>
}
80102ed6:	c9                   	leave  
80102ed7:	c3                   	ret    

80102ed8 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102ed8:	55                   	push   %ebp
80102ed9:	89 e5                	mov    %esp,%ebp
}
80102edb:	5d                   	pop    %ebp
80102edc:	c3                   	ret    

80102edd <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102edd:	55                   	push   %ebp
80102ede:	89 e5                	mov    %esp,%ebp
80102ee0:	83 ec 1c             	sub    $0x1c,%esp
80102ee3:	8b 45 08             	mov    0x8(%ebp),%eax
80102ee6:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
80102ee9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102ef0:	00 
80102ef1:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102ef8:	e8 b6 fd ff ff       	call   80102cb3 <outb>
  outb(IO_RTC+1, 0x0A);
80102efd:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f04:	00 
80102f05:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f0c:	e8 a2 fd ff ff       	call   80102cb3 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f11:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f18:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f1b:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f20:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f23:	8d 50 02             	lea    0x2(%eax),%edx
80102f26:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f29:	c1 e8 04             	shr    $0x4,%eax
80102f2c:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f2f:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f33:	c1 e0 18             	shl    $0x18,%eax
80102f36:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f3a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f41:	e8 9b fd ff ff       	call   80102ce1 <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f46:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f4d:	00 
80102f4e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f55:	e8 87 fd ff ff       	call   80102ce1 <lapicw>
  microdelay(200);
80102f5a:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f61:	e8 72 ff ff ff       	call   80102ed8 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f66:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102f6d:	00 
80102f6e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f75:	e8 67 fd ff ff       	call   80102ce1 <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102f7a:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102f81:	e8 52 ff ff ff       	call   80102ed8 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102f86:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102f8d:	eb 40                	jmp    80102fcf <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102f8f:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f93:	c1 e0 18             	shl    $0x18,%eax
80102f96:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f9a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fa1:	e8 3b fd ff ff       	call   80102ce1 <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fa6:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fa9:	c1 e8 0c             	shr    $0xc,%eax
80102fac:	80 cc 06             	or     $0x6,%ah
80102faf:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fb3:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fba:	e8 22 fd ff ff       	call   80102ce1 <lapicw>
    microdelay(200);
80102fbf:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102fc6:	e8 0d ff ff ff       	call   80102ed8 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fcb:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80102fcf:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80102fd3:	7e ba                	jle    80102f8f <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80102fd5:	c9                   	leave  
80102fd6:	c3                   	ret    

80102fd7 <initlog>:
static void recover_from_log(void);
static void commit();

void
initlog(void)
{
80102fd7:	55                   	push   %ebp
80102fd8:	89 e5                	mov    %esp,%ebp
80102fda:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80102fdd:	c7 44 24 04 a0 89 10 	movl   $0x801089a0,0x4(%esp)
80102fe4:	80 
80102fe5:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
80102fec:	e8 65 1d 00 00       	call   80104d56 <initlock>
  readsb(ROOTDEV, &sb);
80102ff1:	8d 45 e8             	lea    -0x18(%ebp),%eax
80102ff4:	89 44 24 04          	mov    %eax,0x4(%esp)
80102ff8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102fff:	e8 ed e2 ff ff       	call   801012f1 <readsb>
  log.start = sb.size - sb.nlog;
80103004:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103007:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010300a:	29 c2                	sub    %eax,%edx
8010300c:	89 d0                	mov    %edx,%eax
8010300e:	a3 d4 22 11 80       	mov    %eax,0x801122d4
  log.size = sb.nlog;
80103013:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103016:	a3 d8 22 11 80       	mov    %eax,0x801122d8
  log.dev = ROOTDEV;
8010301b:	c7 05 e4 22 11 80 01 	movl   $0x1,0x801122e4
80103022:	00 00 00 
  recover_from_log();
80103025:	e8 9a 01 00 00       	call   801031c4 <recover_from_log>
}
8010302a:	c9                   	leave  
8010302b:	c3                   	ret    

8010302c <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
8010302c:	55                   	push   %ebp
8010302d:	89 e5                	mov    %esp,%ebp
8010302f:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103032:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103039:	e9 8c 00 00 00       	jmp    801030ca <install_trans+0x9e>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
8010303e:	8b 15 d4 22 11 80    	mov    0x801122d4,%edx
80103044:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103047:	01 d0                	add    %edx,%eax
80103049:	83 c0 01             	add    $0x1,%eax
8010304c:	89 c2                	mov    %eax,%edx
8010304e:	a1 e4 22 11 80       	mov    0x801122e4,%eax
80103053:	89 54 24 04          	mov    %edx,0x4(%esp)
80103057:	89 04 24             	mov    %eax,(%esp)
8010305a:	e8 47 d1 ff ff       	call   801001a6 <bread>
8010305f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
80103062:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103065:	83 c0 10             	add    $0x10,%eax
80103068:	8b 04 85 ac 22 11 80 	mov    -0x7feedd54(,%eax,4),%eax
8010306f:	89 c2                	mov    %eax,%edx
80103071:	a1 e4 22 11 80       	mov    0x801122e4,%eax
80103076:	89 54 24 04          	mov    %edx,0x4(%esp)
8010307a:	89 04 24             	mov    %eax,(%esp)
8010307d:	e8 24 d1 ff ff       	call   801001a6 <bread>
80103082:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
80103085:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103088:	8d 50 18             	lea    0x18(%eax),%edx
8010308b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010308e:	83 c0 18             	add    $0x18,%eax
80103091:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103098:	00 
80103099:	89 54 24 04          	mov    %edx,0x4(%esp)
8010309d:	89 04 24             	mov    %eax,(%esp)
801030a0:	e8 f5 1f 00 00       	call   8010509a <memmove>
    bwrite(dbuf);  // write dst to disk
801030a5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030a8:	89 04 24             	mov    %eax,(%esp)
801030ab:	e8 2d d1 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801030b0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030b3:	89 04 24             	mov    %eax,(%esp)
801030b6:	e8 5c d1 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801030bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030be:	89 04 24             	mov    %eax,(%esp)
801030c1:	e8 51 d1 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801030c6:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801030ca:	a1 e8 22 11 80       	mov    0x801122e8,%eax
801030cf:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801030d2:	0f 8f 66 ff ff ff    	jg     8010303e <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
801030d8:	c9                   	leave  
801030d9:	c3                   	ret    

801030da <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
801030da:	55                   	push   %ebp
801030db:	89 e5                	mov    %esp,%ebp
801030dd:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
801030e0:	a1 d4 22 11 80       	mov    0x801122d4,%eax
801030e5:	89 c2                	mov    %eax,%edx
801030e7:	a1 e4 22 11 80       	mov    0x801122e4,%eax
801030ec:	89 54 24 04          	mov    %edx,0x4(%esp)
801030f0:	89 04 24             	mov    %eax,(%esp)
801030f3:	e8 ae d0 ff ff       	call   801001a6 <bread>
801030f8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
801030fb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030fe:	83 c0 18             	add    $0x18,%eax
80103101:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
80103104:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103107:	8b 00                	mov    (%eax),%eax
80103109:	a3 e8 22 11 80       	mov    %eax,0x801122e8
  for (i = 0; i < log.lh.n; i++) {
8010310e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103115:	eb 1b                	jmp    80103132 <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
80103117:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010311a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010311d:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103121:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103124:	83 c2 10             	add    $0x10,%edx
80103127:	89 04 95 ac 22 11 80 	mov    %eax,-0x7feedd54(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
8010312e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103132:	a1 e8 22 11 80       	mov    0x801122e8,%eax
80103137:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010313a:	7f db                	jg     80103117 <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
8010313c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010313f:	89 04 24             	mov    %eax,(%esp)
80103142:	e8 d0 d0 ff ff       	call   80100217 <brelse>
}
80103147:	c9                   	leave  
80103148:	c3                   	ret    

80103149 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103149:	55                   	push   %ebp
8010314a:	89 e5                	mov    %esp,%ebp
8010314c:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010314f:	a1 d4 22 11 80       	mov    0x801122d4,%eax
80103154:	89 c2                	mov    %eax,%edx
80103156:	a1 e4 22 11 80       	mov    0x801122e4,%eax
8010315b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010315f:	89 04 24             	mov    %eax,(%esp)
80103162:	e8 3f d0 ff ff       	call   801001a6 <bread>
80103167:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
8010316a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010316d:	83 c0 18             	add    $0x18,%eax
80103170:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
80103173:	8b 15 e8 22 11 80    	mov    0x801122e8,%edx
80103179:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010317c:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
8010317e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103185:	eb 1b                	jmp    801031a2 <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
80103187:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010318a:	83 c0 10             	add    $0x10,%eax
8010318d:	8b 0c 85 ac 22 11 80 	mov    -0x7feedd54(,%eax,4),%ecx
80103194:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103197:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010319a:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
8010319e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801031a2:	a1 e8 22 11 80       	mov    0x801122e8,%eax
801031a7:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801031aa:	7f db                	jg     80103187 <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801031ac:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031af:	89 04 24             	mov    %eax,(%esp)
801031b2:	e8 26 d0 ff ff       	call   801001dd <bwrite>
  brelse(buf);
801031b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031ba:	89 04 24             	mov    %eax,(%esp)
801031bd:	e8 55 d0 ff ff       	call   80100217 <brelse>
}
801031c2:	c9                   	leave  
801031c3:	c3                   	ret    

801031c4 <recover_from_log>:

static void
recover_from_log(void)
{
801031c4:	55                   	push   %ebp
801031c5:	89 e5                	mov    %esp,%ebp
801031c7:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801031ca:	e8 0b ff ff ff       	call   801030da <read_head>
  install_trans(); // if committed, copy from log to disk
801031cf:	e8 58 fe ff ff       	call   8010302c <install_trans>
  log.lh.n = 0;
801031d4:	c7 05 e8 22 11 80 00 	movl   $0x0,0x801122e8
801031db:	00 00 00 
  write_head(); // clear the log
801031de:	e8 66 ff ff ff       	call   80103149 <write_head>
}
801031e3:	c9                   	leave  
801031e4:	c3                   	ret    

801031e5 <begin_op>:

// called at the start of each FS system call.
void
begin_op(void)
{
801031e5:	55                   	push   %ebp
801031e6:	89 e5                	mov    %esp,%ebp
801031e8:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
801031eb:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
801031f2:	e8 80 1b 00 00       	call   80104d77 <acquire>
  while(1){
    if(log.committing){
801031f7:	a1 e0 22 11 80       	mov    0x801122e0,%eax
801031fc:	85 c0                	test   %eax,%eax
801031fe:	74 16                	je     80103216 <begin_op+0x31>
      sleep(&log, &log.lock);
80103200:	c7 44 24 04 a0 22 11 	movl   $0x801122a0,0x4(%esp)
80103207:	80 
80103208:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
8010320f:	e8 90 18 00 00       	call   80104aa4 <sleep>
80103214:	eb 4f                	jmp    80103265 <begin_op+0x80>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
80103216:	8b 0d e8 22 11 80    	mov    0x801122e8,%ecx
8010321c:	a1 dc 22 11 80       	mov    0x801122dc,%eax
80103221:	8d 50 01             	lea    0x1(%eax),%edx
80103224:	89 d0                	mov    %edx,%eax
80103226:	c1 e0 02             	shl    $0x2,%eax
80103229:	01 d0                	add    %edx,%eax
8010322b:	01 c0                	add    %eax,%eax
8010322d:	01 c8                	add    %ecx,%eax
8010322f:	83 f8 1e             	cmp    $0x1e,%eax
80103232:	7e 16                	jle    8010324a <begin_op+0x65>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
80103234:	c7 44 24 04 a0 22 11 	movl   $0x801122a0,0x4(%esp)
8010323b:	80 
8010323c:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
80103243:	e8 5c 18 00 00       	call   80104aa4 <sleep>
80103248:	eb 1b                	jmp    80103265 <begin_op+0x80>
    } else {
      log.outstanding += 1;
8010324a:	a1 dc 22 11 80       	mov    0x801122dc,%eax
8010324f:	83 c0 01             	add    $0x1,%eax
80103252:	a3 dc 22 11 80       	mov    %eax,0x801122dc
      release(&log.lock);
80103257:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
8010325e:	e8 76 1b 00 00       	call   80104dd9 <release>
      break;
80103263:	eb 02                	jmp    80103267 <begin_op+0x82>
    }
  }
80103265:	eb 90                	jmp    801031f7 <begin_op+0x12>
}
80103267:	c9                   	leave  
80103268:	c3                   	ret    

80103269 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
80103269:	55                   	push   %ebp
8010326a:	89 e5                	mov    %esp,%ebp
8010326c:	83 ec 28             	sub    $0x28,%esp
  int do_commit = 0;
8010326f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

  acquire(&log.lock);
80103276:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
8010327d:	e8 f5 1a 00 00       	call   80104d77 <acquire>
  log.outstanding -= 1;
80103282:	a1 dc 22 11 80       	mov    0x801122dc,%eax
80103287:	83 e8 01             	sub    $0x1,%eax
8010328a:	a3 dc 22 11 80       	mov    %eax,0x801122dc
  if(log.committing)
8010328f:	a1 e0 22 11 80       	mov    0x801122e0,%eax
80103294:	85 c0                	test   %eax,%eax
80103296:	74 0c                	je     801032a4 <end_op+0x3b>
    panic("log.committing");
80103298:	c7 04 24 a4 89 10 80 	movl   $0x801089a4,(%esp)
8010329f:	e8 96 d2 ff ff       	call   8010053a <panic>
  if(log.outstanding == 0){
801032a4:	a1 dc 22 11 80       	mov    0x801122dc,%eax
801032a9:	85 c0                	test   %eax,%eax
801032ab:	75 13                	jne    801032c0 <end_op+0x57>
    do_commit = 1;
801032ad:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
    log.committing = 1;
801032b4:	c7 05 e0 22 11 80 01 	movl   $0x1,0x801122e0
801032bb:	00 00 00 
801032be:	eb 0c                	jmp    801032cc <end_op+0x63>
  } else {
    // begin_op() may be waiting for log space.
    wakeup(&log);
801032c0:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
801032c7:	e8 b4 18 00 00       	call   80104b80 <wakeup>
  }
  release(&log.lock);
801032cc:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
801032d3:	e8 01 1b 00 00       	call   80104dd9 <release>

  if(do_commit){
801032d8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801032dc:	74 33                	je     80103311 <end_op+0xa8>
    // call commit w/o holding locks, since not allowed
    // to sleep with locks.
    commit();
801032de:	e8 de 00 00 00       	call   801033c1 <commit>
    acquire(&log.lock);
801032e3:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
801032ea:	e8 88 1a 00 00       	call   80104d77 <acquire>
    log.committing = 0;
801032ef:	c7 05 e0 22 11 80 00 	movl   $0x0,0x801122e0
801032f6:	00 00 00 
    wakeup(&log);
801032f9:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
80103300:	e8 7b 18 00 00       	call   80104b80 <wakeup>
    release(&log.lock);
80103305:	c7 04 24 a0 22 11 80 	movl   $0x801122a0,(%esp)
8010330c:	e8 c8 1a 00 00       	call   80104dd9 <release>
  }
}
80103311:	c9                   	leave  
80103312:	c3                   	ret    

80103313 <write_log>:

// Copy modified blocks from cache to log.
static void 
write_log(void)
{
80103313:	55                   	push   %ebp
80103314:	89 e5                	mov    %esp,%ebp
80103316:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103319:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103320:	e9 8c 00 00 00       	jmp    801033b1 <write_log+0x9e>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80103325:	8b 15 d4 22 11 80    	mov    0x801122d4,%edx
8010332b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010332e:	01 d0                	add    %edx,%eax
80103330:	83 c0 01             	add    $0x1,%eax
80103333:	89 c2                	mov    %eax,%edx
80103335:	a1 e4 22 11 80       	mov    0x801122e4,%eax
8010333a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010333e:	89 04 24             	mov    %eax,(%esp)
80103341:	e8 60 ce ff ff       	call   801001a6 <bread>
80103346:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *from = bread(log.dev, log.lh.sector[tail]); // cache block
80103349:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010334c:	83 c0 10             	add    $0x10,%eax
8010334f:	8b 04 85 ac 22 11 80 	mov    -0x7feedd54(,%eax,4),%eax
80103356:	89 c2                	mov    %eax,%edx
80103358:	a1 e4 22 11 80       	mov    0x801122e4,%eax
8010335d:	89 54 24 04          	mov    %edx,0x4(%esp)
80103361:	89 04 24             	mov    %eax,(%esp)
80103364:	e8 3d ce ff ff       	call   801001a6 <bread>
80103369:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(to->data, from->data, BSIZE);
8010336c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010336f:	8d 50 18             	lea    0x18(%eax),%edx
80103372:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103375:	83 c0 18             	add    $0x18,%eax
80103378:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010337f:	00 
80103380:	89 54 24 04          	mov    %edx,0x4(%esp)
80103384:	89 04 24             	mov    %eax,(%esp)
80103387:	e8 0e 1d 00 00       	call   8010509a <memmove>
    bwrite(to);  // write the log
8010338c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010338f:	89 04 24             	mov    %eax,(%esp)
80103392:	e8 46 ce ff ff       	call   801001dd <bwrite>
    brelse(from); 
80103397:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010339a:	89 04 24             	mov    %eax,(%esp)
8010339d:	e8 75 ce ff ff       	call   80100217 <brelse>
    brelse(to);
801033a2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801033a5:	89 04 24             	mov    %eax,(%esp)
801033a8:	e8 6a ce ff ff       	call   80100217 <brelse>
static void 
write_log(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801033ad:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801033b1:	a1 e8 22 11 80       	mov    0x801122e8,%eax
801033b6:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801033b9:	0f 8f 66 ff ff ff    	jg     80103325 <write_log+0x12>
    memmove(to->data, from->data, BSIZE);
    bwrite(to);  // write the log
    brelse(from); 
    brelse(to);
  }
}
801033bf:	c9                   	leave  
801033c0:	c3                   	ret    

801033c1 <commit>:

static void
commit()
{
801033c1:	55                   	push   %ebp
801033c2:	89 e5                	mov    %esp,%ebp
801033c4:	83 ec 08             	sub    $0x8,%esp
  if (log.lh.n > 0) {
801033c7:	a1 e8 22 11 80       	mov    0x801122e8,%eax
801033cc:	85 c0                	test   %eax,%eax
801033ce:	7e 1e                	jle    801033ee <commit+0x2d>
    write_log();     // Write modified blocks from cache to log
801033d0:	e8 3e ff ff ff       	call   80103313 <write_log>
    write_head();    // Write header to disk -- the real commit
801033d5:	e8 6f fd ff ff       	call   80103149 <write_head>
    install_trans(); // Now install writes to home locations
801033da:	e8 4d fc ff ff       	call   8010302c <install_trans>
    log.lh.n = 0; 
801033df:	c7 05 e8 22 11 80 00 	movl   $0x0,0x801122e8
801033e6:	00 00 00 
    write_head();    // Erase the transaction from the log
801033e9:	e8 5b fd ff ff       	call   80103149 <write_head>
  }
}
801033ee:	c9                   	leave  
801033ef:	c3                   	ret    

801033f0 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
801033f0:	55                   	push   %ebp
801033f1:	89 e5                	mov    %esp,%ebp
801033f3:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
801033f6:	a1 e8 22 11 80       	mov    0x801122e8,%eax
801033fb:	83 f8 1d             	cmp    $0x1d,%eax
801033fe:	7f 12                	jg     80103412 <log_write+0x22>
80103400:	a1 e8 22 11 80       	mov    0x801122e8,%eax
80103405:	8b 15 d8 22 11 80    	mov    0x801122d8,%edx
8010340b:	83 ea 01             	sub    $0x1,%edx
8010340e:	39 d0                	cmp    %edx,%eax
80103410:	7c 0c                	jl     8010341e <log_write+0x2e>
    panic("too big a transaction");
80103412:	c7 04 24 b3 89 10 80 	movl   $0x801089b3,(%esp)
80103419:	e8 1c d1 ff ff       	call   8010053a <panic>
  if (log.outstanding < 1)
8010341e:	a1 dc 22 11 80       	mov    0x801122dc,%eax
80103423:	85 c0                	test   %eax,%eax
80103425:	7f 0c                	jg     80103433 <log_write+0x43>
    panic("log_write outside of trans");
80103427:	c7 04 24 c9 89 10 80 	movl   $0x801089c9,(%esp)
8010342e:	e8 07 d1 ff ff       	call   8010053a <panic>

  for (i = 0; i < log.lh.n; i++) {
80103433:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010343a:	eb 1f                	jmp    8010345b <log_write+0x6b>
    if (log.lh.sector[i] == b->sector)   // log absorbtion
8010343c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010343f:	83 c0 10             	add    $0x10,%eax
80103442:	8b 04 85 ac 22 11 80 	mov    -0x7feedd54(,%eax,4),%eax
80103449:	89 c2                	mov    %eax,%edx
8010344b:	8b 45 08             	mov    0x8(%ebp),%eax
8010344e:	8b 40 08             	mov    0x8(%eax),%eax
80103451:	39 c2                	cmp    %eax,%edx
80103453:	75 02                	jne    80103457 <log_write+0x67>
      break;
80103455:	eb 0e                	jmp    80103465 <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (log.outstanding < 1)
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
80103457:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010345b:	a1 e8 22 11 80       	mov    0x801122e8,%eax
80103460:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103463:	7f d7                	jg     8010343c <log_write+0x4c>
    if (log.lh.sector[i] == b->sector)   // log absorbtion
      break;
  }
  log.lh.sector[i] = b->sector;
80103465:	8b 45 08             	mov    0x8(%ebp),%eax
80103468:	8b 40 08             	mov    0x8(%eax),%eax
8010346b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010346e:	83 c2 10             	add    $0x10,%edx
80103471:	89 04 95 ac 22 11 80 	mov    %eax,-0x7feedd54(,%edx,4)
  if (i == log.lh.n)
80103478:	a1 e8 22 11 80       	mov    0x801122e8,%eax
8010347d:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103480:	75 0d                	jne    8010348f <log_write+0x9f>
    log.lh.n++;
80103482:	a1 e8 22 11 80       	mov    0x801122e8,%eax
80103487:	83 c0 01             	add    $0x1,%eax
8010348a:	a3 e8 22 11 80       	mov    %eax,0x801122e8
  b->flags |= B_DIRTY; // prevent eviction
8010348f:	8b 45 08             	mov    0x8(%ebp),%eax
80103492:	8b 00                	mov    (%eax),%eax
80103494:	83 c8 04             	or     $0x4,%eax
80103497:	89 c2                	mov    %eax,%edx
80103499:	8b 45 08             	mov    0x8(%ebp),%eax
8010349c:	89 10                	mov    %edx,(%eax)
}
8010349e:	c9                   	leave  
8010349f:	c3                   	ret    

801034a0 <v2p>:
801034a0:	55                   	push   %ebp
801034a1:	89 e5                	mov    %esp,%ebp
801034a3:	8b 45 08             	mov    0x8(%ebp),%eax
801034a6:	05 00 00 00 80       	add    $0x80000000,%eax
801034ab:	5d                   	pop    %ebp
801034ac:	c3                   	ret    

801034ad <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801034ad:	55                   	push   %ebp
801034ae:	89 e5                	mov    %esp,%ebp
801034b0:	8b 45 08             	mov    0x8(%ebp),%eax
801034b3:	05 00 00 00 80       	add    $0x80000000,%eax
801034b8:	5d                   	pop    %ebp
801034b9:	c3                   	ret    

801034ba <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801034ba:	55                   	push   %ebp
801034bb:	89 e5                	mov    %esp,%ebp
801034bd:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801034c0:	8b 55 08             	mov    0x8(%ebp),%edx
801034c3:	8b 45 0c             	mov    0xc(%ebp),%eax
801034c6:	8b 4d 08             	mov    0x8(%ebp),%ecx
801034c9:	f0 87 02             	lock xchg %eax,(%edx)
801034cc:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801034cf:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801034d2:	c9                   	leave  
801034d3:	c3                   	ret    

801034d4 <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801034d4:	55                   	push   %ebp
801034d5:	89 e5                	mov    %esp,%ebp
801034d7:	83 e4 f0             	and    $0xfffffff0,%esp
801034da:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
801034dd:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
801034e4:	80 
801034e5:	c7 04 24 7c 86 11 80 	movl   $0x8011867c,(%esp)
801034ec:	e8 c3 f4 ff ff       	call   801029b4 <kinit1>
  kvmalloc();      // kernel page table
801034f1:	e8 f2 4a 00 00       	call   80107fe8 <kvmalloc>
  mpinit();        // collect info about this machine
801034f6:	e8 46 04 00 00       	call   80103941 <mpinit>
  lapicinit();
801034fb:	e8 02 f8 ff ff       	call   80102d02 <lapicinit>
  seginit();       // set up segments
80103500:	e8 76 44 00 00       	call   8010797b <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103505:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010350b:	0f b6 00             	movzbl (%eax),%eax
8010350e:	0f b6 c0             	movzbl %al,%eax
80103511:	89 44 24 04          	mov    %eax,0x4(%esp)
80103515:	c7 04 24 e4 89 10 80 	movl   $0x801089e4,(%esp)
8010351c:	e8 7f ce ff ff       	call   801003a0 <cprintf>
  picinit();       // interrupt controller
80103521:	e8 79 06 00 00       	call   80103b9f <picinit>
  ioapicinit();    // another interrupt controller
80103526:	e8 7f f3 ff ff       	call   801028aa <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
8010352b:	e8 51 d5 ff ff       	call   80100a81 <consoleinit>
  uartinit();      // serial port
80103530:	e8 95 37 00 00       	call   80106cca <uartinit>
  pinit();         // process table
80103535:	e8 6f 0b 00 00       	call   801040a9 <pinit>
  tvinit();        // trap vectors
8010353a:	e8 3d 33 00 00       	call   8010687c <tvinit>
  binit();         // buffer cache
8010353f:	e8 f0 ca ff ff       	call   80100034 <binit>
  fileinit();      // file table
80103544:	e8 c1 d9 ff ff       	call   80100f0a <fileinit>
  iinit();         // inode cache
80103549:	e8 56 e0 ff ff       	call   801015a4 <iinit>
  ideinit();       // disk
8010354e:	e8 c0 ef ff ff       	call   80102513 <ideinit>
  if(!ismp)
80103553:	a1 84 23 11 80       	mov    0x80112384,%eax
80103558:	85 c0                	test   %eax,%eax
8010355a:	75 05                	jne    80103561 <main+0x8d>
    timerinit();   // uniprocessor timer
8010355c:	e8 66 32 00 00       	call   801067c7 <timerinit>
  startothers();   // start other processors
80103561:	e8 7f 00 00 00       	call   801035e5 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103566:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
8010356d:	8e 
8010356e:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
80103575:	e8 72 f4 ff ff       	call   801029ec <kinit2>
  userinit();      // first user process
8010357a:	e8 48 0c 00 00       	call   801041c7 <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
8010357f:	e8 1a 00 00 00       	call   8010359e <mpmain>

80103584 <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
80103584:	55                   	push   %ebp
80103585:	89 e5                	mov    %esp,%ebp
80103587:	83 ec 08             	sub    $0x8,%esp
  switchkvm(); 
8010358a:	e8 70 4a 00 00       	call   80107fff <switchkvm>
  seginit();
8010358f:	e8 e7 43 00 00       	call   8010797b <seginit>
  lapicinit();
80103594:	e8 69 f7 ff ff       	call   80102d02 <lapicinit>
  mpmain();
80103599:	e8 00 00 00 00       	call   8010359e <mpmain>

8010359e <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
8010359e:	55                   	push   %ebp
8010359f:	89 e5                	mov    %esp,%ebp
801035a1:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
801035a4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801035aa:	0f b6 00             	movzbl (%eax),%eax
801035ad:	0f b6 c0             	movzbl %al,%eax
801035b0:	89 44 24 04          	mov    %eax,0x4(%esp)
801035b4:	c7 04 24 fb 89 10 80 	movl   $0x801089fb,(%esp)
801035bb:	e8 e0 cd ff ff       	call   801003a0 <cprintf>
  idtinit();       // load idt register
801035c0:	e8 2b 34 00 00       	call   801069f0 <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
801035c5:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801035cb:	05 a8 00 00 00       	add    $0xa8,%eax
801035d0:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801035d7:	00 
801035d8:	89 04 24             	mov    %eax,(%esp)
801035db:	e8 da fe ff ff       	call   801034ba <xchg>
  scheduler();     // start running processes
801035e0:	e8 14 13 00 00       	call   801048f9 <scheduler>

801035e5 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
801035e5:	55                   	push   %ebp
801035e6:	89 e5                	mov    %esp,%ebp
801035e8:	53                   	push   %ebx
801035e9:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
801035ec:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
801035f3:	e8 b5 fe ff ff       	call   801034ad <p2v>
801035f8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
801035fb:	b8 8a 00 00 00       	mov    $0x8a,%eax
80103600:	89 44 24 08          	mov    %eax,0x8(%esp)
80103604:	c7 44 24 04 4c b5 10 	movl   $0x8010b54c,0x4(%esp)
8010360b:	80 
8010360c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010360f:	89 04 24             	mov    %eax,(%esp)
80103612:	e8 83 1a 00 00       	call   8010509a <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103617:	c7 45 f4 a0 23 11 80 	movl   $0x801123a0,-0xc(%ebp)
8010361e:	e9 85 00 00 00       	jmp    801036a8 <startothers+0xc3>
    if(c == cpus+cpunum())  // We've started already.
80103623:	e8 33 f8 ff ff       	call   80102e5b <cpunum>
80103628:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
8010362e:	05 a0 23 11 80       	add    $0x801123a0,%eax
80103633:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103636:	75 02                	jne    8010363a <startothers+0x55>
      continue;
80103638:	eb 67                	jmp    801036a1 <startothers+0xbc>

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
8010363a:	e8 a3 f4 ff ff       	call   80102ae2 <kalloc>
8010363f:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
80103642:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103645:	83 e8 04             	sub    $0x4,%eax
80103648:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010364b:	81 c2 00 10 00 00    	add    $0x1000,%edx
80103651:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
80103653:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103656:	83 e8 08             	sub    $0x8,%eax
80103659:	c7 00 84 35 10 80    	movl   $0x80103584,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
8010365f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103662:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103665:	c7 04 24 00 a0 10 80 	movl   $0x8010a000,(%esp)
8010366c:	e8 2f fe ff ff       	call   801034a0 <v2p>
80103671:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
80103673:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103676:	89 04 24             	mov    %eax,(%esp)
80103679:	e8 22 fe ff ff       	call   801034a0 <v2p>
8010367e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103681:	0f b6 12             	movzbl (%edx),%edx
80103684:	0f b6 d2             	movzbl %dl,%edx
80103687:	89 44 24 04          	mov    %eax,0x4(%esp)
8010368b:	89 14 24             	mov    %edx,(%esp)
8010368e:	e8 4a f8 ff ff       	call   80102edd <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103693:	90                   	nop
80103694:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103697:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
8010369d:	85 c0                	test   %eax,%eax
8010369f:	74 f3                	je     80103694 <startothers+0xaf>
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
801036a1:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
801036a8:	a1 80 29 11 80       	mov    0x80112980,%eax
801036ad:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801036b3:	05 a0 23 11 80       	add    $0x801123a0,%eax
801036b8:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801036bb:	0f 87 62 ff ff ff    	ja     80103623 <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801036c1:	83 c4 24             	add    $0x24,%esp
801036c4:	5b                   	pop    %ebx
801036c5:	5d                   	pop    %ebp
801036c6:	c3                   	ret    

801036c7 <p2v>:
801036c7:	55                   	push   %ebp
801036c8:	89 e5                	mov    %esp,%ebp
801036ca:	8b 45 08             	mov    0x8(%ebp),%eax
801036cd:	05 00 00 00 80       	add    $0x80000000,%eax
801036d2:	5d                   	pop    %ebp
801036d3:	c3                   	ret    

801036d4 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801036d4:	55                   	push   %ebp
801036d5:	89 e5                	mov    %esp,%ebp
801036d7:	83 ec 14             	sub    $0x14,%esp
801036da:	8b 45 08             	mov    0x8(%ebp),%eax
801036dd:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801036e1:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
801036e5:	89 c2                	mov    %eax,%edx
801036e7:	ec                   	in     (%dx),%al
801036e8:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801036eb:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
801036ef:	c9                   	leave  
801036f0:	c3                   	ret    

801036f1 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801036f1:	55                   	push   %ebp
801036f2:	89 e5                	mov    %esp,%ebp
801036f4:	83 ec 08             	sub    $0x8,%esp
801036f7:	8b 55 08             	mov    0x8(%ebp),%edx
801036fa:	8b 45 0c             	mov    0xc(%ebp),%eax
801036fd:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103701:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103704:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103708:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
8010370c:	ee                   	out    %al,(%dx)
}
8010370d:	c9                   	leave  
8010370e:	c3                   	ret    

8010370f <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
8010370f:	55                   	push   %ebp
80103710:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
80103712:	a1 84 b6 10 80       	mov    0x8010b684,%eax
80103717:	89 c2                	mov    %eax,%edx
80103719:	b8 a0 23 11 80       	mov    $0x801123a0,%eax
8010371e:	29 c2                	sub    %eax,%edx
80103720:	89 d0                	mov    %edx,%eax
80103722:	c1 f8 02             	sar    $0x2,%eax
80103725:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
8010372b:	5d                   	pop    %ebp
8010372c:	c3                   	ret    

8010372d <sum>:

static uchar
sum(uchar *addr, int len)
{
8010372d:	55                   	push   %ebp
8010372e:	89 e5                	mov    %esp,%ebp
80103730:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
80103733:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
8010373a:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103741:	eb 15                	jmp    80103758 <sum+0x2b>
    sum += addr[i];
80103743:	8b 55 fc             	mov    -0x4(%ebp),%edx
80103746:	8b 45 08             	mov    0x8(%ebp),%eax
80103749:	01 d0                	add    %edx,%eax
8010374b:	0f b6 00             	movzbl (%eax),%eax
8010374e:	0f b6 c0             	movzbl %al,%eax
80103751:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
80103754:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103758:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010375b:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010375e:	7c e3                	jl     80103743 <sum+0x16>
    sum += addr[i];
  return sum;
80103760:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103763:	c9                   	leave  
80103764:	c3                   	ret    

80103765 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103765:	55                   	push   %ebp
80103766:	89 e5                	mov    %esp,%ebp
80103768:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
8010376b:	8b 45 08             	mov    0x8(%ebp),%eax
8010376e:	89 04 24             	mov    %eax,(%esp)
80103771:	e8 51 ff ff ff       	call   801036c7 <p2v>
80103776:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
80103779:	8b 55 0c             	mov    0xc(%ebp),%edx
8010377c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010377f:	01 d0                	add    %edx,%eax
80103781:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
80103784:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103787:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010378a:	eb 3f                	jmp    801037cb <mpsearch1+0x66>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
8010378c:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103793:	00 
80103794:	c7 44 24 04 0c 8a 10 	movl   $0x80108a0c,0x4(%esp)
8010379b:	80 
8010379c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010379f:	89 04 24             	mov    %eax,(%esp)
801037a2:	e8 9b 18 00 00       	call   80105042 <memcmp>
801037a7:	85 c0                	test   %eax,%eax
801037a9:	75 1c                	jne    801037c7 <mpsearch1+0x62>
801037ab:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801037b2:	00 
801037b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037b6:	89 04 24             	mov    %eax,(%esp)
801037b9:	e8 6f ff ff ff       	call   8010372d <sum>
801037be:	84 c0                	test   %al,%al
801037c0:	75 05                	jne    801037c7 <mpsearch1+0x62>
      return (struct mp*)p;
801037c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037c5:	eb 11                	jmp    801037d8 <mpsearch1+0x73>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
801037c7:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
801037cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037ce:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801037d1:	72 b9                	jb     8010378c <mpsearch1+0x27>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
801037d3:	b8 00 00 00 00       	mov    $0x0,%eax
}
801037d8:	c9                   	leave  
801037d9:	c3                   	ret    

801037da <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
801037da:	55                   	push   %ebp
801037db:	89 e5                	mov    %esp,%ebp
801037dd:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
801037e0:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
801037e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037ea:	83 c0 0f             	add    $0xf,%eax
801037ed:	0f b6 00             	movzbl (%eax),%eax
801037f0:	0f b6 c0             	movzbl %al,%eax
801037f3:	c1 e0 08             	shl    $0x8,%eax
801037f6:	89 c2                	mov    %eax,%edx
801037f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037fb:	83 c0 0e             	add    $0xe,%eax
801037fe:	0f b6 00             	movzbl (%eax),%eax
80103801:	0f b6 c0             	movzbl %al,%eax
80103804:	09 d0                	or     %edx,%eax
80103806:	c1 e0 04             	shl    $0x4,%eax
80103809:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010380c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103810:	74 21                	je     80103833 <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
80103812:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103819:	00 
8010381a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010381d:	89 04 24             	mov    %eax,(%esp)
80103820:	e8 40 ff ff ff       	call   80103765 <mpsearch1>
80103825:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103828:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010382c:	74 50                	je     8010387e <mpsearch+0xa4>
      return mp;
8010382e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103831:	eb 5f                	jmp    80103892 <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103833:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103836:	83 c0 14             	add    $0x14,%eax
80103839:	0f b6 00             	movzbl (%eax),%eax
8010383c:	0f b6 c0             	movzbl %al,%eax
8010383f:	c1 e0 08             	shl    $0x8,%eax
80103842:	89 c2                	mov    %eax,%edx
80103844:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103847:	83 c0 13             	add    $0x13,%eax
8010384a:	0f b6 00             	movzbl (%eax),%eax
8010384d:	0f b6 c0             	movzbl %al,%eax
80103850:	09 d0                	or     %edx,%eax
80103852:	c1 e0 0a             	shl    $0xa,%eax
80103855:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103858:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010385b:	2d 00 04 00 00       	sub    $0x400,%eax
80103860:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103867:	00 
80103868:	89 04 24             	mov    %eax,(%esp)
8010386b:	e8 f5 fe ff ff       	call   80103765 <mpsearch1>
80103870:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103873:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103877:	74 05                	je     8010387e <mpsearch+0xa4>
      return mp;
80103879:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010387c:	eb 14                	jmp    80103892 <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
8010387e:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103885:	00 
80103886:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
8010388d:	e8 d3 fe ff ff       	call   80103765 <mpsearch1>
}
80103892:	c9                   	leave  
80103893:	c3                   	ret    

80103894 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
80103894:	55                   	push   %ebp
80103895:	89 e5                	mov    %esp,%ebp
80103897:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
8010389a:	e8 3b ff ff ff       	call   801037da <mpsearch>
8010389f:	89 45 f4             	mov    %eax,-0xc(%ebp)
801038a2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801038a6:	74 0a                	je     801038b2 <mpconfig+0x1e>
801038a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038ab:	8b 40 04             	mov    0x4(%eax),%eax
801038ae:	85 c0                	test   %eax,%eax
801038b0:	75 0a                	jne    801038bc <mpconfig+0x28>
    return 0;
801038b2:	b8 00 00 00 00       	mov    $0x0,%eax
801038b7:	e9 83 00 00 00       	jmp    8010393f <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
801038bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038bf:	8b 40 04             	mov    0x4(%eax),%eax
801038c2:	89 04 24             	mov    %eax,(%esp)
801038c5:	e8 fd fd ff ff       	call   801036c7 <p2v>
801038ca:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
801038cd:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801038d4:	00 
801038d5:	c7 44 24 04 11 8a 10 	movl   $0x80108a11,0x4(%esp)
801038dc:	80 
801038dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038e0:	89 04 24             	mov    %eax,(%esp)
801038e3:	e8 5a 17 00 00       	call   80105042 <memcmp>
801038e8:	85 c0                	test   %eax,%eax
801038ea:	74 07                	je     801038f3 <mpconfig+0x5f>
    return 0;
801038ec:	b8 00 00 00 00       	mov    $0x0,%eax
801038f1:	eb 4c                	jmp    8010393f <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
801038f3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038f6:	0f b6 40 06          	movzbl 0x6(%eax),%eax
801038fa:	3c 01                	cmp    $0x1,%al
801038fc:	74 12                	je     80103910 <mpconfig+0x7c>
801038fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103901:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103905:	3c 04                	cmp    $0x4,%al
80103907:	74 07                	je     80103910 <mpconfig+0x7c>
    return 0;
80103909:	b8 00 00 00 00       	mov    $0x0,%eax
8010390e:	eb 2f                	jmp    8010393f <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
80103910:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103913:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103917:	0f b7 c0             	movzwl %ax,%eax
8010391a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010391e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103921:	89 04 24             	mov    %eax,(%esp)
80103924:	e8 04 fe ff ff       	call   8010372d <sum>
80103929:	84 c0                	test   %al,%al
8010392b:	74 07                	je     80103934 <mpconfig+0xa0>
    return 0;
8010392d:	b8 00 00 00 00       	mov    $0x0,%eax
80103932:	eb 0b                	jmp    8010393f <mpconfig+0xab>
  *pmp = mp;
80103934:	8b 45 08             	mov    0x8(%ebp),%eax
80103937:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010393a:	89 10                	mov    %edx,(%eax)
  return conf;
8010393c:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010393f:	c9                   	leave  
80103940:	c3                   	ret    

80103941 <mpinit>:

void
mpinit(void)
{
80103941:	55                   	push   %ebp
80103942:	89 e5                	mov    %esp,%ebp
80103944:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80103947:	c7 05 84 b6 10 80 a0 	movl   $0x801123a0,0x8010b684
8010394e:	23 11 80 
  if((conf = mpconfig(&mp)) == 0)
80103951:	8d 45 e0             	lea    -0x20(%ebp),%eax
80103954:	89 04 24             	mov    %eax,(%esp)
80103957:	e8 38 ff ff ff       	call   80103894 <mpconfig>
8010395c:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010395f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103963:	75 05                	jne    8010396a <mpinit+0x29>
    return;
80103965:	e9 9c 01 00 00       	jmp    80103b06 <mpinit+0x1c5>
  ismp = 1;
8010396a:	c7 05 84 23 11 80 01 	movl   $0x1,0x80112384
80103971:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
80103974:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103977:	8b 40 24             	mov    0x24(%eax),%eax
8010397a:	a3 9c 22 11 80       	mov    %eax,0x8011229c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
8010397f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103982:	83 c0 2c             	add    $0x2c,%eax
80103985:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103988:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010398b:	0f b7 40 04          	movzwl 0x4(%eax),%eax
8010398f:	0f b7 d0             	movzwl %ax,%edx
80103992:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103995:	01 d0                	add    %edx,%eax
80103997:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010399a:	e9 f4 00 00 00       	jmp    80103a93 <mpinit+0x152>
    switch(*p){
8010399f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039a2:	0f b6 00             	movzbl (%eax),%eax
801039a5:	0f b6 c0             	movzbl %al,%eax
801039a8:	83 f8 04             	cmp    $0x4,%eax
801039ab:	0f 87 bf 00 00 00    	ja     80103a70 <mpinit+0x12f>
801039b1:	8b 04 85 54 8a 10 80 	mov    -0x7fef75ac(,%eax,4),%eax
801039b8:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
801039ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039bd:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
801039c0:	8b 45 e8             	mov    -0x18(%ebp),%eax
801039c3:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801039c7:	0f b6 d0             	movzbl %al,%edx
801039ca:	a1 80 29 11 80       	mov    0x80112980,%eax
801039cf:	39 c2                	cmp    %eax,%edx
801039d1:	74 2d                	je     80103a00 <mpinit+0xbf>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
801039d3:	8b 45 e8             	mov    -0x18(%ebp),%eax
801039d6:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801039da:	0f b6 d0             	movzbl %al,%edx
801039dd:	a1 80 29 11 80       	mov    0x80112980,%eax
801039e2:	89 54 24 08          	mov    %edx,0x8(%esp)
801039e6:	89 44 24 04          	mov    %eax,0x4(%esp)
801039ea:	c7 04 24 16 8a 10 80 	movl   $0x80108a16,(%esp)
801039f1:	e8 aa c9 ff ff       	call   801003a0 <cprintf>
        ismp = 0;
801039f6:	c7 05 84 23 11 80 00 	movl   $0x0,0x80112384
801039fd:	00 00 00 
      }
      if(proc->flags & MPBOOT)
80103a00:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103a03:	0f b6 40 03          	movzbl 0x3(%eax),%eax
80103a07:	0f b6 c0             	movzbl %al,%eax
80103a0a:	83 e0 02             	and    $0x2,%eax
80103a0d:	85 c0                	test   %eax,%eax
80103a0f:	74 15                	je     80103a26 <mpinit+0xe5>
        bcpu = &cpus[ncpu];
80103a11:	a1 80 29 11 80       	mov    0x80112980,%eax
80103a16:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103a1c:	05 a0 23 11 80       	add    $0x801123a0,%eax
80103a21:	a3 84 b6 10 80       	mov    %eax,0x8010b684
      cpus[ncpu].id = ncpu;
80103a26:	8b 15 80 29 11 80    	mov    0x80112980,%edx
80103a2c:	a1 80 29 11 80       	mov    0x80112980,%eax
80103a31:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80103a37:	81 c2 a0 23 11 80    	add    $0x801123a0,%edx
80103a3d:	88 02                	mov    %al,(%edx)
      ncpu++;
80103a3f:	a1 80 29 11 80       	mov    0x80112980,%eax
80103a44:	83 c0 01             	add    $0x1,%eax
80103a47:	a3 80 29 11 80       	mov    %eax,0x80112980
      p += sizeof(struct mpproc);
80103a4c:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103a50:	eb 41                	jmp    80103a93 <mpinit+0x152>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
80103a52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a55:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
80103a58:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103a5b:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103a5f:	a2 80 23 11 80       	mov    %al,0x80112380
      p += sizeof(struct mpioapic);
80103a64:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103a68:	eb 29                	jmp    80103a93 <mpinit+0x152>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
80103a6a:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103a6e:	eb 23                	jmp    80103a93 <mpinit+0x152>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
80103a70:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a73:	0f b6 00             	movzbl (%eax),%eax
80103a76:	0f b6 c0             	movzbl %al,%eax
80103a79:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a7d:	c7 04 24 34 8a 10 80 	movl   $0x80108a34,(%esp)
80103a84:	e8 17 c9 ff ff       	call   801003a0 <cprintf>
      ismp = 0;
80103a89:	c7 05 84 23 11 80 00 	movl   $0x0,0x80112384
80103a90:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103a93:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a96:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103a99:	0f 82 00 ff ff ff    	jb     8010399f <mpinit+0x5e>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
80103a9f:	a1 84 23 11 80       	mov    0x80112384,%eax
80103aa4:	85 c0                	test   %eax,%eax
80103aa6:	75 1d                	jne    80103ac5 <mpinit+0x184>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
80103aa8:	c7 05 80 29 11 80 01 	movl   $0x1,0x80112980
80103aaf:	00 00 00 
    lapic = 0;
80103ab2:	c7 05 9c 22 11 80 00 	movl   $0x0,0x8011229c
80103ab9:	00 00 00 
    ioapicid = 0;
80103abc:	c6 05 80 23 11 80 00 	movb   $0x0,0x80112380
    return;
80103ac3:	eb 41                	jmp    80103b06 <mpinit+0x1c5>
  }

  if(mp->imcrp){
80103ac5:	8b 45 e0             	mov    -0x20(%ebp),%eax
80103ac8:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
80103acc:	84 c0                	test   %al,%al
80103ace:	74 36                	je     80103b06 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80103ad0:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103ad7:	00 
80103ad8:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103adf:	e8 0d fc ff ff       	call   801036f1 <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103ae4:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103aeb:	e8 e4 fb ff ff       	call   801036d4 <inb>
80103af0:	83 c8 01             	or     $0x1,%eax
80103af3:	0f b6 c0             	movzbl %al,%eax
80103af6:	89 44 24 04          	mov    %eax,0x4(%esp)
80103afa:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103b01:	e8 eb fb ff ff       	call   801036f1 <outb>
  }
}
80103b06:	c9                   	leave  
80103b07:	c3                   	ret    

80103b08 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103b08:	55                   	push   %ebp
80103b09:	89 e5                	mov    %esp,%ebp
80103b0b:	83 ec 08             	sub    $0x8,%esp
80103b0e:	8b 55 08             	mov    0x8(%ebp),%edx
80103b11:	8b 45 0c             	mov    0xc(%ebp),%eax
80103b14:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103b18:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103b1b:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103b1f:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103b23:	ee                   	out    %al,(%dx)
}
80103b24:	c9                   	leave  
80103b25:	c3                   	ret    

80103b26 <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80103b26:	55                   	push   %ebp
80103b27:	89 e5                	mov    %esp,%ebp
80103b29:	83 ec 0c             	sub    $0xc,%esp
80103b2c:	8b 45 08             	mov    0x8(%ebp),%eax
80103b2f:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
80103b33:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103b37:	66 a3 00 b0 10 80    	mov    %ax,0x8010b000
  outb(IO_PIC1+1, mask);
80103b3d:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103b41:	0f b6 c0             	movzbl %al,%eax
80103b44:	89 44 24 04          	mov    %eax,0x4(%esp)
80103b48:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b4f:	e8 b4 ff ff ff       	call   80103b08 <outb>
  outb(IO_PIC2+1, mask >> 8);
80103b54:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103b58:	66 c1 e8 08          	shr    $0x8,%ax
80103b5c:	0f b6 c0             	movzbl %al,%eax
80103b5f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103b63:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b6a:	e8 99 ff ff ff       	call   80103b08 <outb>
}
80103b6f:	c9                   	leave  
80103b70:	c3                   	ret    

80103b71 <picenable>:

void
picenable(int irq)
{
80103b71:	55                   	push   %ebp
80103b72:	89 e5                	mov    %esp,%ebp
80103b74:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
80103b77:	8b 45 08             	mov    0x8(%ebp),%eax
80103b7a:	ba 01 00 00 00       	mov    $0x1,%edx
80103b7f:	89 c1                	mov    %eax,%ecx
80103b81:	d3 e2                	shl    %cl,%edx
80103b83:	89 d0                	mov    %edx,%eax
80103b85:	f7 d0                	not    %eax
80103b87:	89 c2                	mov    %eax,%edx
80103b89:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103b90:	21 d0                	and    %edx,%eax
80103b92:	0f b7 c0             	movzwl %ax,%eax
80103b95:	89 04 24             	mov    %eax,(%esp)
80103b98:	e8 89 ff ff ff       	call   80103b26 <picsetmask>
}
80103b9d:	c9                   	leave  
80103b9e:	c3                   	ret    

80103b9f <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
80103b9f:	55                   	push   %ebp
80103ba0:	89 e5                	mov    %esp,%ebp
80103ba2:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103ba5:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103bac:	00 
80103bad:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103bb4:	e8 4f ff ff ff       	call   80103b08 <outb>
  outb(IO_PIC2+1, 0xFF);
80103bb9:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103bc0:	00 
80103bc1:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103bc8:	e8 3b ff ff ff       	call   80103b08 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80103bcd:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103bd4:	00 
80103bd5:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bdc:	e8 27 ff ff ff       	call   80103b08 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80103be1:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103be8:	00 
80103be9:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103bf0:	e8 13 ff ff ff       	call   80103b08 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80103bf5:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103bfc:	00 
80103bfd:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103c04:	e8 ff fe ff ff       	call   80103b08 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
80103c09:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103c10:	00 
80103c11:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103c18:	e8 eb fe ff ff       	call   80103b08 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80103c1d:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103c24:	00 
80103c25:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103c2c:	e8 d7 fe ff ff       	call   80103b08 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80103c31:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103c38:	00 
80103c39:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103c40:	e8 c3 fe ff ff       	call   80103b08 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80103c45:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103c4c:	00 
80103c4d:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103c54:	e8 af fe ff ff       	call   80103b08 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
80103c59:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103c60:	00 
80103c61:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103c68:	e8 9b fe ff ff       	call   80103b08 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
80103c6d:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103c74:	00 
80103c75:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103c7c:	e8 87 fe ff ff       	call   80103b08 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
80103c81:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103c88:	00 
80103c89:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103c90:	e8 73 fe ff ff       	call   80103b08 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
80103c95:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103c9c:	00 
80103c9d:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103ca4:	e8 5f fe ff ff       	call   80103b08 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
80103ca9:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103cb0:	00 
80103cb1:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103cb8:	e8 4b fe ff ff       	call   80103b08 <outb>

  if(irqmask != 0xFFFF)
80103cbd:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103cc4:	66 83 f8 ff          	cmp    $0xffff,%ax
80103cc8:	74 12                	je     80103cdc <picinit+0x13d>
    picsetmask(irqmask);
80103cca:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103cd1:	0f b7 c0             	movzwl %ax,%eax
80103cd4:	89 04 24             	mov    %eax,(%esp)
80103cd7:	e8 4a fe ff ff       	call   80103b26 <picsetmask>
}
80103cdc:	c9                   	leave  
80103cdd:	c3                   	ret    

80103cde <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103cde:	55                   	push   %ebp
80103cdf:	89 e5                	mov    %esp,%ebp
80103ce1:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103ce4:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103ceb:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cee:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103cf4:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cf7:	8b 10                	mov    (%eax),%edx
80103cf9:	8b 45 08             	mov    0x8(%ebp),%eax
80103cfc:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103cfe:	e8 23 d2 ff ff       	call   80100f26 <filealloc>
80103d03:	8b 55 08             	mov    0x8(%ebp),%edx
80103d06:	89 02                	mov    %eax,(%edx)
80103d08:	8b 45 08             	mov    0x8(%ebp),%eax
80103d0b:	8b 00                	mov    (%eax),%eax
80103d0d:	85 c0                	test   %eax,%eax
80103d0f:	0f 84 c8 00 00 00    	je     80103ddd <pipealloc+0xff>
80103d15:	e8 0c d2 ff ff       	call   80100f26 <filealloc>
80103d1a:	8b 55 0c             	mov    0xc(%ebp),%edx
80103d1d:	89 02                	mov    %eax,(%edx)
80103d1f:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d22:	8b 00                	mov    (%eax),%eax
80103d24:	85 c0                	test   %eax,%eax
80103d26:	0f 84 b1 00 00 00    	je     80103ddd <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103d2c:	e8 b1 ed ff ff       	call   80102ae2 <kalloc>
80103d31:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103d34:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103d38:	75 05                	jne    80103d3f <pipealloc+0x61>
    goto bad;
80103d3a:	e9 9e 00 00 00       	jmp    80103ddd <pipealloc+0xff>
  p->readopen = 1;
80103d3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d42:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103d49:	00 00 00 
  p->writeopen = 1;
80103d4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d4f:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103d56:	00 00 00 
  p->nwrite = 0;
80103d59:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d5c:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103d63:	00 00 00 
  p->nread = 0;
80103d66:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d69:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103d70:	00 00 00 
  initlock(&p->lock, "pipe");
80103d73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d76:	c7 44 24 04 68 8a 10 	movl   $0x80108a68,0x4(%esp)
80103d7d:	80 
80103d7e:	89 04 24             	mov    %eax,(%esp)
80103d81:	e8 d0 0f 00 00       	call   80104d56 <initlock>
  (*f0)->type = FD_PIPE;
80103d86:	8b 45 08             	mov    0x8(%ebp),%eax
80103d89:	8b 00                	mov    (%eax),%eax
80103d8b:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103d91:	8b 45 08             	mov    0x8(%ebp),%eax
80103d94:	8b 00                	mov    (%eax),%eax
80103d96:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103d9a:	8b 45 08             	mov    0x8(%ebp),%eax
80103d9d:	8b 00                	mov    (%eax),%eax
80103d9f:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103da3:	8b 45 08             	mov    0x8(%ebp),%eax
80103da6:	8b 00                	mov    (%eax),%eax
80103da8:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103dab:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103dae:	8b 45 0c             	mov    0xc(%ebp),%eax
80103db1:	8b 00                	mov    (%eax),%eax
80103db3:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103db9:	8b 45 0c             	mov    0xc(%ebp),%eax
80103dbc:	8b 00                	mov    (%eax),%eax
80103dbe:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103dc2:	8b 45 0c             	mov    0xc(%ebp),%eax
80103dc5:	8b 00                	mov    (%eax),%eax
80103dc7:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103dcb:	8b 45 0c             	mov    0xc(%ebp),%eax
80103dce:	8b 00                	mov    (%eax),%eax
80103dd0:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103dd3:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103dd6:	b8 00 00 00 00       	mov    $0x0,%eax
80103ddb:	eb 42                	jmp    80103e1f <pipealloc+0x141>

//PAGEBREAK: 20
 bad:
  if(p)
80103ddd:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103de1:	74 0b                	je     80103dee <pipealloc+0x110>
    kfree((char*)p);
80103de3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103de6:	89 04 24             	mov    %eax,(%esp)
80103de9:	e8 5b ec ff ff       	call   80102a49 <kfree>
  if(*f0)
80103dee:	8b 45 08             	mov    0x8(%ebp),%eax
80103df1:	8b 00                	mov    (%eax),%eax
80103df3:	85 c0                	test   %eax,%eax
80103df5:	74 0d                	je     80103e04 <pipealloc+0x126>
    fileclose(*f0);
80103df7:	8b 45 08             	mov    0x8(%ebp),%eax
80103dfa:	8b 00                	mov    (%eax),%eax
80103dfc:	89 04 24             	mov    %eax,(%esp)
80103dff:	e8 ca d1 ff ff       	call   80100fce <fileclose>
  if(*f1)
80103e04:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e07:	8b 00                	mov    (%eax),%eax
80103e09:	85 c0                	test   %eax,%eax
80103e0b:	74 0d                	je     80103e1a <pipealloc+0x13c>
    fileclose(*f1);
80103e0d:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e10:	8b 00                	mov    (%eax),%eax
80103e12:	89 04 24             	mov    %eax,(%esp)
80103e15:	e8 b4 d1 ff ff       	call   80100fce <fileclose>
  return -1;
80103e1a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103e1f:	c9                   	leave  
80103e20:	c3                   	ret    

80103e21 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103e21:	55                   	push   %ebp
80103e22:	89 e5                	mov    %esp,%ebp
80103e24:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103e27:	8b 45 08             	mov    0x8(%ebp),%eax
80103e2a:	89 04 24             	mov    %eax,(%esp)
80103e2d:	e8 45 0f 00 00       	call   80104d77 <acquire>
  if(writable){
80103e32:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103e36:	74 1f                	je     80103e57 <pipeclose+0x36>
    p->writeopen = 0;
80103e38:	8b 45 08             	mov    0x8(%ebp),%eax
80103e3b:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103e42:	00 00 00 
    wakeup(&p->nread);
80103e45:	8b 45 08             	mov    0x8(%ebp),%eax
80103e48:	05 34 02 00 00       	add    $0x234,%eax
80103e4d:	89 04 24             	mov    %eax,(%esp)
80103e50:	e8 2b 0d 00 00       	call   80104b80 <wakeup>
80103e55:	eb 1d                	jmp    80103e74 <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103e57:	8b 45 08             	mov    0x8(%ebp),%eax
80103e5a:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103e61:	00 00 00 
    wakeup(&p->nwrite);
80103e64:	8b 45 08             	mov    0x8(%ebp),%eax
80103e67:	05 38 02 00 00       	add    $0x238,%eax
80103e6c:	89 04 24             	mov    %eax,(%esp)
80103e6f:	e8 0c 0d 00 00       	call   80104b80 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103e74:	8b 45 08             	mov    0x8(%ebp),%eax
80103e77:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103e7d:	85 c0                	test   %eax,%eax
80103e7f:	75 25                	jne    80103ea6 <pipeclose+0x85>
80103e81:	8b 45 08             	mov    0x8(%ebp),%eax
80103e84:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103e8a:	85 c0                	test   %eax,%eax
80103e8c:	75 18                	jne    80103ea6 <pipeclose+0x85>
    release(&p->lock);
80103e8e:	8b 45 08             	mov    0x8(%ebp),%eax
80103e91:	89 04 24             	mov    %eax,(%esp)
80103e94:	e8 40 0f 00 00       	call   80104dd9 <release>
    kfree((char*)p);
80103e99:	8b 45 08             	mov    0x8(%ebp),%eax
80103e9c:	89 04 24             	mov    %eax,(%esp)
80103e9f:	e8 a5 eb ff ff       	call   80102a49 <kfree>
80103ea4:	eb 0b                	jmp    80103eb1 <pipeclose+0x90>
  } else
    release(&p->lock);
80103ea6:	8b 45 08             	mov    0x8(%ebp),%eax
80103ea9:	89 04 24             	mov    %eax,(%esp)
80103eac:	e8 28 0f 00 00       	call   80104dd9 <release>
}
80103eb1:	c9                   	leave  
80103eb2:	c3                   	ret    

80103eb3 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103eb3:	55                   	push   %ebp
80103eb4:	89 e5                	mov    %esp,%ebp
80103eb6:	83 ec 28             	sub    $0x28,%esp
  int i;

  acquire(&p->lock);
80103eb9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ebc:	89 04 24             	mov    %eax,(%esp)
80103ebf:	e8 b3 0e 00 00       	call   80104d77 <acquire>
  for(i = 0; i < n; i++){
80103ec4:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103ecb:	e9 a6 00 00 00       	jmp    80103f76 <pipewrite+0xc3>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103ed0:	eb 57                	jmp    80103f29 <pipewrite+0x76>
      if(p->readopen == 0 || proc->killed){
80103ed2:	8b 45 08             	mov    0x8(%ebp),%eax
80103ed5:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103edb:	85 c0                	test   %eax,%eax
80103edd:	74 0d                	je     80103eec <pipewrite+0x39>
80103edf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103ee5:	8b 40 24             	mov    0x24(%eax),%eax
80103ee8:	85 c0                	test   %eax,%eax
80103eea:	74 15                	je     80103f01 <pipewrite+0x4e>
        release(&p->lock);
80103eec:	8b 45 08             	mov    0x8(%ebp),%eax
80103eef:	89 04 24             	mov    %eax,(%esp)
80103ef2:	e8 e2 0e 00 00       	call   80104dd9 <release>
        return -1;
80103ef7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103efc:	e9 9f 00 00 00       	jmp    80103fa0 <pipewrite+0xed>
      }
      wakeup(&p->nread);
80103f01:	8b 45 08             	mov    0x8(%ebp),%eax
80103f04:	05 34 02 00 00       	add    $0x234,%eax
80103f09:	89 04 24             	mov    %eax,(%esp)
80103f0c:	e8 6f 0c 00 00       	call   80104b80 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103f11:	8b 45 08             	mov    0x8(%ebp),%eax
80103f14:	8b 55 08             	mov    0x8(%ebp),%edx
80103f17:	81 c2 38 02 00 00    	add    $0x238,%edx
80103f1d:	89 44 24 04          	mov    %eax,0x4(%esp)
80103f21:	89 14 24             	mov    %edx,(%esp)
80103f24:	e8 7b 0b 00 00       	call   80104aa4 <sleep>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103f29:	8b 45 08             	mov    0x8(%ebp),%eax
80103f2c:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103f32:	8b 45 08             	mov    0x8(%ebp),%eax
80103f35:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103f3b:	05 00 02 00 00       	add    $0x200,%eax
80103f40:	39 c2                	cmp    %eax,%edx
80103f42:	74 8e                	je     80103ed2 <pipewrite+0x1f>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103f44:	8b 45 08             	mov    0x8(%ebp),%eax
80103f47:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f4d:	8d 48 01             	lea    0x1(%eax),%ecx
80103f50:	8b 55 08             	mov    0x8(%ebp),%edx
80103f53:	89 8a 38 02 00 00    	mov    %ecx,0x238(%edx)
80103f59:	25 ff 01 00 00       	and    $0x1ff,%eax
80103f5e:	89 c1                	mov    %eax,%ecx
80103f60:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103f63:	8b 45 0c             	mov    0xc(%ebp),%eax
80103f66:	01 d0                	add    %edx,%eax
80103f68:	0f b6 10             	movzbl (%eax),%edx
80103f6b:	8b 45 08             	mov    0x8(%ebp),%eax
80103f6e:	88 54 08 34          	mov    %dl,0x34(%eax,%ecx,1)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
80103f72:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103f76:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f79:	3b 45 10             	cmp    0x10(%ebp),%eax
80103f7c:	0f 8c 4e ff ff ff    	jl     80103ed0 <pipewrite+0x1d>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103f82:	8b 45 08             	mov    0x8(%ebp),%eax
80103f85:	05 34 02 00 00       	add    $0x234,%eax
80103f8a:	89 04 24             	mov    %eax,(%esp)
80103f8d:	e8 ee 0b 00 00       	call   80104b80 <wakeup>
  release(&p->lock);
80103f92:	8b 45 08             	mov    0x8(%ebp),%eax
80103f95:	89 04 24             	mov    %eax,(%esp)
80103f98:	e8 3c 0e 00 00       	call   80104dd9 <release>
  return n;
80103f9d:	8b 45 10             	mov    0x10(%ebp),%eax
}
80103fa0:	c9                   	leave  
80103fa1:	c3                   	ret    

80103fa2 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103fa2:	55                   	push   %ebp
80103fa3:	89 e5                	mov    %esp,%ebp
80103fa5:	53                   	push   %ebx
80103fa6:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103fa9:	8b 45 08             	mov    0x8(%ebp),%eax
80103fac:	89 04 24             	mov    %eax,(%esp)
80103faf:	e8 c3 0d 00 00       	call   80104d77 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103fb4:	eb 3a                	jmp    80103ff0 <piperead+0x4e>
    if(proc->killed){
80103fb6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103fbc:	8b 40 24             	mov    0x24(%eax),%eax
80103fbf:	85 c0                	test   %eax,%eax
80103fc1:	74 15                	je     80103fd8 <piperead+0x36>
      release(&p->lock);
80103fc3:	8b 45 08             	mov    0x8(%ebp),%eax
80103fc6:	89 04 24             	mov    %eax,(%esp)
80103fc9:	e8 0b 0e 00 00       	call   80104dd9 <release>
      return -1;
80103fce:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103fd3:	e9 b5 00 00 00       	jmp    8010408d <piperead+0xeb>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103fd8:	8b 45 08             	mov    0x8(%ebp),%eax
80103fdb:	8b 55 08             	mov    0x8(%ebp),%edx
80103fde:	81 c2 34 02 00 00    	add    $0x234,%edx
80103fe4:	89 44 24 04          	mov    %eax,0x4(%esp)
80103fe8:	89 14 24             	mov    %edx,(%esp)
80103feb:	e8 b4 0a 00 00       	call   80104aa4 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103ff0:	8b 45 08             	mov    0x8(%ebp),%eax
80103ff3:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103ff9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ffc:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104002:	39 c2                	cmp    %eax,%edx
80104004:	75 0d                	jne    80104013 <piperead+0x71>
80104006:	8b 45 08             	mov    0x8(%ebp),%eax
80104009:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
8010400f:	85 c0                	test   %eax,%eax
80104011:	75 a3                	jne    80103fb6 <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80104013:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010401a:	eb 4b                	jmp    80104067 <piperead+0xc5>
    if(p->nread == p->nwrite)
8010401c:	8b 45 08             	mov    0x8(%ebp),%eax
8010401f:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80104025:	8b 45 08             	mov    0x8(%ebp),%eax
80104028:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
8010402e:	39 c2                	cmp    %eax,%edx
80104030:	75 02                	jne    80104034 <piperead+0x92>
      break;
80104032:	eb 3b                	jmp    8010406f <piperead+0xcd>
    addr[i] = p->data[p->nread++ % PIPESIZE];
80104034:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104037:	8b 45 0c             	mov    0xc(%ebp),%eax
8010403a:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
8010403d:	8b 45 08             	mov    0x8(%ebp),%eax
80104040:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80104046:	8d 48 01             	lea    0x1(%eax),%ecx
80104049:	8b 55 08             	mov    0x8(%ebp),%edx
8010404c:	89 8a 34 02 00 00    	mov    %ecx,0x234(%edx)
80104052:	25 ff 01 00 00       	and    $0x1ff,%eax
80104057:	89 c2                	mov    %eax,%edx
80104059:	8b 45 08             	mov    0x8(%ebp),%eax
8010405c:	0f b6 44 10 34       	movzbl 0x34(%eax,%edx,1),%eax
80104061:	88 03                	mov    %al,(%ebx)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80104063:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104067:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010406a:	3b 45 10             	cmp    0x10(%ebp),%eax
8010406d:	7c ad                	jl     8010401c <piperead+0x7a>
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
8010406f:	8b 45 08             	mov    0x8(%ebp),%eax
80104072:	05 38 02 00 00       	add    $0x238,%eax
80104077:	89 04 24             	mov    %eax,(%esp)
8010407a:	e8 01 0b 00 00       	call   80104b80 <wakeup>
  release(&p->lock);
8010407f:	8b 45 08             	mov    0x8(%ebp),%eax
80104082:	89 04 24             	mov    %eax,(%esp)
80104085:	e8 4f 0d 00 00       	call   80104dd9 <release>
  return i;
8010408a:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010408d:	83 c4 24             	add    $0x24,%esp
80104090:	5b                   	pop    %ebx
80104091:	5d                   	pop    %ebp
80104092:	c3                   	ret    

80104093 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104093:	55                   	push   %ebp
80104094:	89 e5                	mov    %esp,%ebp
80104096:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104099:	9c                   	pushf  
8010409a:	58                   	pop    %eax
8010409b:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
8010409e:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801040a1:	c9                   	leave  
801040a2:	c3                   	ret    

801040a3 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
801040a3:	55                   	push   %ebp
801040a4:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
801040a6:	fb                   	sti    
}
801040a7:	5d                   	pop    %ebp
801040a8:	c3                   	ret    

801040a9 <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
801040a9:	55                   	push   %ebp
801040aa:	89 e5                	mov    %esp,%ebp
801040ac:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
801040af:	c7 44 24 04 6d 8a 10 	movl   $0x80108a6d,0x4(%esp)
801040b6:	80 
801040b7:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
801040be:	e8 93 0c 00 00       	call   80104d56 <initlock>
}
801040c3:	c9                   	leave  
801040c4:	c3                   	ret    

801040c5 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
801040c5:	55                   	push   %ebp
801040c6:	89 e5                	mov    %esp,%ebp
801040c8:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
801040cb:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
801040d2:	e8 a0 0c 00 00       	call   80104d77 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801040d7:	c7 45 f4 d4 29 11 80 	movl   $0x801129d4,-0xc(%ebp)
801040de:	eb 53                	jmp    80104133 <allocproc+0x6e>
    if(p->state == UNUSED)
801040e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040e3:	8b 40 0c             	mov    0xc(%eax),%eax
801040e6:	85 c0                	test   %eax,%eax
801040e8:	75 42                	jne    8010412c <allocproc+0x67>
      goto found;
801040ea:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
801040eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040ee:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
801040f5:	a1 04 b0 10 80       	mov    0x8010b004,%eax
801040fa:	8d 50 01             	lea    0x1(%eax),%edx
801040fd:	89 15 04 b0 10 80    	mov    %edx,0x8010b004
80104103:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104106:	89 42 10             	mov    %eax,0x10(%edx)
  release(&ptable.lock);
80104109:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104110:	e8 c4 0c 00 00       	call   80104dd9 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
80104115:	e8 c8 e9 ff ff       	call   80102ae2 <kalloc>
8010411a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010411d:	89 42 08             	mov    %eax,0x8(%edx)
80104120:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104123:	8b 40 08             	mov    0x8(%eax),%eax
80104126:	85 c0                	test   %eax,%eax
80104128:	75 36                	jne    80104160 <allocproc+0x9b>
8010412a:	eb 23                	jmp    8010414f <allocproc+0x8a>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010412c:	81 45 f4 50 01 00 00 	addl   $0x150,-0xc(%ebp)
80104133:	81 7d f4 d4 7d 11 80 	cmpl   $0x80117dd4,-0xc(%ebp)
8010413a:	72 a4                	jb     801040e0 <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
8010413c:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104143:	e8 91 0c 00 00       	call   80104dd9 <release>
  return 0;
80104148:	b8 00 00 00 00       	mov    $0x0,%eax
8010414d:	eb 76                	jmp    801041c5 <allocproc+0x100>
  p->pid = nextpid++;
  release(&ptable.lock);

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
    p->state = UNUSED;
8010414f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104152:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
80104159:	b8 00 00 00 00       	mov    $0x0,%eax
8010415e:	eb 65                	jmp    801041c5 <allocproc+0x100>
  }
  sp = p->kstack + KSTACKSIZE;
80104160:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104163:	8b 40 08             	mov    0x8(%eax),%eax
80104166:	05 00 10 00 00       	add    $0x1000,%eax
8010416b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
8010416e:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
80104172:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104175:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104178:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
8010417b:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
8010417f:	ba 37 68 10 80       	mov    $0x80106837,%edx
80104184:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104187:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
80104189:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
8010418d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104190:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104193:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
80104196:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104199:	8b 40 1c             	mov    0x1c(%eax),%eax
8010419c:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801041a3:	00 
801041a4:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801041ab:	00 
801041ac:	89 04 24             	mov    %eax,(%esp)
801041af:	e8 17 0e 00 00       	call   80104fcb <memset>
  p->context->eip = (uint)forkret;
801041b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041b7:	8b 40 1c             	mov    0x1c(%eax),%eax
801041ba:	ba 78 4a 10 80       	mov    $0x80104a78,%edx
801041bf:	89 50 10             	mov    %edx,0x10(%eax)

  return p;
801041c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801041c5:	c9                   	leave  
801041c6:	c3                   	ret    

801041c7 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
801041c7:	55                   	push   %ebp
801041c8:	89 e5                	mov    %esp,%ebp
801041ca:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
801041cd:	e8 f3 fe ff ff       	call   801040c5 <allocproc>
801041d2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
801041d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041d8:	a3 88 b6 10 80       	mov    %eax,0x8010b688
  if((p->pgdir = setupkvm()) == 0)
801041dd:	e8 49 3d 00 00       	call   80107f2b <setupkvm>
801041e2:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041e5:	89 42 04             	mov    %eax,0x4(%edx)
801041e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041eb:	8b 40 04             	mov    0x4(%eax),%eax
801041ee:	85 c0                	test   %eax,%eax
801041f0:	75 0c                	jne    801041fe <userinit+0x37>
    panic("userinit: out of memory?");
801041f2:	c7 04 24 74 8a 10 80 	movl   $0x80108a74,(%esp)
801041f9:	e8 3c c3 ff ff       	call   8010053a <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
801041fe:	ba 2c 00 00 00       	mov    $0x2c,%edx
80104203:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104206:	8b 40 04             	mov    0x4(%eax),%eax
80104209:	89 54 24 08          	mov    %edx,0x8(%esp)
8010420d:	c7 44 24 04 20 b5 10 	movl   $0x8010b520,0x4(%esp)
80104214:	80 
80104215:	89 04 24             	mov    %eax,(%esp)
80104218:	e8 66 3f 00 00       	call   80108183 <inituvm>
  p->sz = PGSIZE;
8010421d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104220:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
80104226:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104229:	8b 40 18             	mov    0x18(%eax),%eax
8010422c:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
80104233:	00 
80104234:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010423b:	00 
8010423c:	89 04 24             	mov    %eax,(%esp)
8010423f:	e8 87 0d 00 00       	call   80104fcb <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104244:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104247:	8b 40 18             	mov    0x18(%eax),%eax
8010424a:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80104250:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104253:	8b 40 18             	mov    0x18(%eax),%eax
80104256:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
8010425c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010425f:	8b 40 18             	mov    0x18(%eax),%eax
80104262:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104265:	8b 52 18             	mov    0x18(%edx),%edx
80104268:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
8010426c:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
80104270:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104273:	8b 40 18             	mov    0x18(%eax),%eax
80104276:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104279:	8b 52 18             	mov    0x18(%edx),%edx
8010427c:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
80104280:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
80104284:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104287:	8b 40 18             	mov    0x18(%eax),%eax
8010428a:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80104291:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104294:	8b 40 18             	mov    0x18(%eax),%eax
80104297:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
8010429e:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042a1:	8b 40 18             	mov    0x18(%eax),%eax
801042a4:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
801042ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042ae:	83 c0 6c             	add    $0x6c,%eax
801042b1:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
801042b8:	00 
801042b9:	c7 44 24 04 8d 8a 10 	movl   $0x80108a8d,0x4(%esp)
801042c0:	80 
801042c1:	89 04 24             	mov    %eax,(%esp)
801042c4:	e8 22 0f 00 00       	call   801051eb <safestrcpy>
  p->cwd = namei("/");
801042c9:	c7 04 24 96 8a 10 80 	movl   $0x80108a96,(%esp)
801042d0:	e8 31 e1 ff ff       	call   80102406 <namei>
801042d5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801042d8:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
801042db:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042de:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
}
801042e5:	c9                   	leave  
801042e6:	c3                   	ret    

801042e7 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
801042e7:	55                   	push   %ebp
801042e8:	89 e5                	mov    %esp,%ebp
801042ea:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
801042ed:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042f3:	8b 00                	mov    (%eax),%eax
801042f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
801042f8:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801042fc:	7e 34                	jle    80104332 <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
801042fe:	8b 55 08             	mov    0x8(%ebp),%edx
80104301:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104304:	01 c2                	add    %eax,%edx
80104306:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010430c:	8b 40 04             	mov    0x4(%eax),%eax
8010430f:	89 54 24 08          	mov    %edx,0x8(%esp)
80104313:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104316:	89 54 24 04          	mov    %edx,0x4(%esp)
8010431a:	89 04 24             	mov    %eax,(%esp)
8010431d:	e8 d7 3f 00 00       	call   801082f9 <allocuvm>
80104322:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104325:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104329:	75 41                	jne    8010436c <growproc+0x85>
      return -1;
8010432b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104330:	eb 58                	jmp    8010438a <growproc+0xa3>
  } else if(n < 0){
80104332:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104336:	79 34                	jns    8010436c <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
80104338:	8b 55 08             	mov    0x8(%ebp),%edx
8010433b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010433e:	01 c2                	add    %eax,%edx
80104340:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104346:	8b 40 04             	mov    0x4(%eax),%eax
80104349:	89 54 24 08          	mov    %edx,0x8(%esp)
8010434d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104350:	89 54 24 04          	mov    %edx,0x4(%esp)
80104354:	89 04 24             	mov    %eax,(%esp)
80104357:	e8 77 40 00 00       	call   801083d3 <deallocuvm>
8010435c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010435f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104363:	75 07                	jne    8010436c <growproc+0x85>
      return -1;
80104365:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010436a:	eb 1e                	jmp    8010438a <growproc+0xa3>
  }
  proc->sz = sz;
8010436c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104372:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104375:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
80104377:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010437d:	89 04 24             	mov    %eax,(%esp)
80104380:	e8 97 3c 00 00       	call   8010801c <switchuvm>
  return 0;
80104385:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010438a:	c9                   	leave  
8010438b:	c3                   	ret    

8010438c <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(char *ustack, uint wrapperaddr, uint arg1, uint arg2)
{
8010438c:	55                   	push   %ebp
8010438d:	89 e5                	mov    %esp,%ebp
8010438f:	57                   	push   %edi
80104390:	56                   	push   %esi
80104391:	53                   	push   %ebx
80104392:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
80104395:	e8 2b fd ff ff       	call   801040c5 <allocproc>
8010439a:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010439d:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
801043a1:	75 0a                	jne    801043ad <fork+0x21>
    return -1;
801043a3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801043a8:	e9 b2 02 00 00       	jmp    8010465f <fork+0x2d3>

  if(proc->ctflag){
801043ad:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043b3:	8b 40 7c             	mov    0x7c(%eax),%eax
801043b6:	85 c0                	test   %eax,%eax
801043b8:	74 21                	je     801043db <fork+0x4f>

    np->pgdir = proc->pgdir;
801043ba:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043c0:	8b 50 04             	mov    0x4(%eax),%edx
801043c3:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043c6:	89 50 04             	mov    %edx,0x4(%eax)
    //np->kstack = proc->cStack;
    np->type = 1;
801043c9:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043cc:	c7 80 80 00 00 00 01 	movl   $0x1,0x80(%eax)
801043d3:	00 00 00 
801043d6:	e9 d9 00 00 00       	jmp    801044b4 <fork+0x128>


} else {

    // Copy process state from p.
    if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
801043db:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043e1:	8b 10                	mov    (%eax),%edx
801043e3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043e9:	8b 40 04             	mov    0x4(%eax),%eax
801043ec:	89 54 24 04          	mov    %edx,0x4(%esp)
801043f0:	89 04 24             	mov    %eax,(%esp)
801043f3:	e8 77 41 00 00       	call   8010856f <copyuvm>
801043f8:	8b 55 e0             	mov    -0x20(%ebp),%edx
801043fb:	89 42 04             	mov    %eax,0x4(%edx)
801043fe:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104401:	8b 40 04             	mov    0x4(%eax),%eax
80104404:	85 c0                	test   %eax,%eax
80104406:	75 2c                	jne    80104434 <fork+0xa8>
      kfree(np->kstack);
80104408:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010440b:	8b 40 08             	mov    0x8(%eax),%eax
8010440e:	89 04 24             	mov    %eax,(%esp)
80104411:	e8 33 e6 ff ff       	call   80102a49 <kfree>
      np->kstack = 0;
80104416:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104419:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
      np->state = UNUSED;
80104420:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104423:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
      return -1;
8010442a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010442f:	e9 2b 02 00 00       	jmp    8010465f <fork+0x2d3>
    }

    np->type = 0;
80104434:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104437:	c7 80 80 00 00 00 00 	movl   $0x0,0x80(%eax)
8010443e:	00 00 00 

    for(i=0;i<NMUTX;i++){
80104441:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80104448:	eb 64                	jmp    801044ae <fork+0x122>
	proc->mutexlist[i].id = -1;
8010444a:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80104451:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104454:	89 d0                	mov    %edx,%eax
80104456:	01 c0                	add    %eax,%eax
80104458:	01 d0                	add    %edx,%eax
8010445a:	c1 e0 02             	shl    $0x2,%eax
8010445d:	01 c8                	add    %ecx,%eax
8010445f:	05 90 00 00 00       	add    $0x90,%eax
80104464:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
	proc->mutexlist[i].state = -1;
8010446a:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80104471:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104474:	89 d0                	mov    %edx,%eax
80104476:	01 c0                	add    %eax,%eax
80104478:	01 d0                	add    %edx,%eax
8010447a:	c1 e0 02             	shl    $0x2,%eax
8010447d:	01 c8                	add    %ecx,%eax
8010447f:	05 94 00 00 00       	add    $0x94,%eax
80104484:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
        proc->mutexlist[i].lockingthread = -1;
8010448a:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80104491:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80104494:	89 d0                	mov    %edx,%eax
80104496:	01 c0                	add    %eax,%eax
80104498:	01 d0                	add    %edx,%eax
8010449a:	c1 e0 02             	shl    $0x2,%eax
8010449d:	01 c8                	add    %ecx,%eax
8010449f:	05 98 00 00 00       	add    $0x98,%eax
801044a4:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
      return -1;
    }

    np->type = 0;

    for(i=0;i<NMUTX;i++){
801044aa:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
801044ae:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
801044b2:	7e 96                	jle    8010444a <fork+0xbe>
	proc->mutexlist[i].state = -1;
        proc->mutexlist[i].lockingthread = -1;
    }
  }

  np->sz = proc->sz;
801044b4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044ba:	8b 10                	mov    (%eax),%edx
801044bc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801044bf:	89 10                	mov    %edx,(%eax)

  if(proc->type && proc->ctflag)
801044c1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044c7:	8b 80 80 00 00 00    	mov    0x80(%eax),%eax
801044cd:	85 c0                	test   %eax,%eax
801044cf:	74 1e                	je     801044ef <fork+0x163>
801044d1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044d7:	8b 40 7c             	mov    0x7c(%eax),%eax
801044da:	85 c0                	test   %eax,%eax
801044dc:	74 11                	je     801044ef <fork+0x163>
    np->parent = proc->parent;
801044de:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044e4:	8b 50 14             	mov    0x14(%eax),%edx
801044e7:	8b 45 e0             	mov    -0x20(%ebp),%eax
801044ea:	89 50 14             	mov    %edx,0x14(%eax)
801044ed:	eb 0d                	jmp    801044fc <fork+0x170>
  else
    np->parent = proc;
801044ef:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801044f6:	8b 45 e0             	mov    -0x20(%ebp),%eax
801044f9:	89 50 14             	mov    %edx,0x14(%eax)

  *np->tf = *proc->tf;
801044fc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801044ff:	8b 50 18             	mov    0x18(%eax),%edx
80104502:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104508:	8b 40 18             	mov    0x18(%eax),%eax
8010450b:	89 c3                	mov    %eax,%ebx
8010450d:	b8 13 00 00 00       	mov    $0x13,%eax
80104512:	89 d7                	mov    %edx,%edi
80104514:	89 de                	mov    %ebx,%esi
80104516:	89 c1                	mov    %eax,%ecx
80104518:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

if(proc->ctflag){
8010451a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104520:	8b 40 7c             	mov    0x7c(%eax),%eax
80104523:	85 c0                	test   %eax,%eax
80104525:	74 77                	je     8010459e <fork+0x212>

  np->tf->eip = wrapperaddr;
80104527:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010452a:	8b 40 18             	mov    0x18(%eax),%eax
8010452d:	8b 55 0c             	mov    0xc(%ebp),%edx
80104530:	89 50 38             	mov    %edx,0x38(%eax)

  proc->ustack = ustack;
80104533:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104539:	8b 55 08             	mov    0x8(%ebp),%edx
8010453c:	89 90 88 00 00 00    	mov    %edx,0x88(%eax)

  uint *sp = (uint*)proc->ustack;
80104542:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104548:	8b 80 88 00 00 00    	mov    0x88(%eax),%eax
8010454e:	89 45 dc             	mov    %eax,-0x24(%ebp)

  *(--sp) = arg2;
80104551:	83 6d dc 04          	subl   $0x4,-0x24(%ebp)
80104555:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104558:	8b 55 14             	mov    0x14(%ebp),%edx
8010455b:	89 10                	mov    %edx,(%eax)
  *(--sp) = arg1;
8010455d:	83 6d dc 04          	subl   $0x4,-0x24(%ebp)
80104561:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104564:	8b 55 10             	mov    0x10(%ebp),%edx
80104567:	89 10                	mov    %edx,(%eax)
  *(--sp) = 0;
80104569:	83 6d dc 04          	subl   $0x4,-0x24(%ebp)
8010456d:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104570:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  np->tf->esp = (uint)sp;
80104576:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104579:	8b 40 18             	mov    0x18(%eax),%eax
8010457c:	8b 55 dc             	mov    -0x24(%ebp),%edx
8010457f:	89 50 44             	mov    %edx,0x44(%eax)
  proc->tcount+=1;
80104582:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104588:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
8010458f:	8b 92 8c 00 00 00    	mov    0x8c(%edx),%edx
80104595:	83 c2 01             	add    $0x1,%edx
80104598:	89 90 8c 00 00 00    	mov    %edx,0x8c(%eax)
}

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
8010459e:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045a1:	8b 40 18             	mov    0x18(%eax),%eax
801045a4:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
801045ab:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801045b2:	eb 3d                	jmp    801045f1 <fork+0x265>
    if(proc->ofile[i])
801045b4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045ba:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801045bd:	83 c2 08             	add    $0x8,%edx
801045c0:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801045c4:	85 c0                	test   %eax,%eax
801045c6:	74 25                	je     801045ed <fork+0x261>
      np->ofile[i] = filedup(proc->ofile[i]);
801045c8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045ce:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801045d1:	83 c2 08             	add    $0x8,%edx
801045d4:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801045d8:	89 04 24             	mov    %eax,(%esp)
801045db:	e8 a6 c9 ff ff       	call   80100f86 <filedup>
801045e0:	8b 55 e0             	mov    -0x20(%ebp),%edx
801045e3:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
801045e6:	83 c1 08             	add    $0x8,%ecx
801045e9:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
}

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
801045ed:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
801045f1:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
801045f5:	7e bd                	jle    801045b4 <fork+0x228>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
801045f7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045fd:	8b 40 68             	mov    0x68(%eax),%eax
80104600:	89 04 24             	mov    %eax,(%esp)
80104603:	e8 21 d2 ff ff       	call   80101829 <idup>
80104608:	8b 55 e0             	mov    -0x20(%ebp),%edx
8010460b:	89 42 68             	mov    %eax,0x68(%edx)

  safestrcpy(np->name, proc->name, sizeof(proc->name));
8010460e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104614:	8d 50 6c             	lea    0x6c(%eax),%edx
80104617:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010461a:	83 c0 6c             	add    $0x6c,%eax
8010461d:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104624:	00 
80104625:	89 54 24 04          	mov    %edx,0x4(%esp)
80104629:	89 04 24             	mov    %eax,(%esp)
8010462c:	e8 ba 0b 00 00       	call   801051eb <safestrcpy>
 
  pid = np->pid;
80104631:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104634:	8b 40 10             	mov    0x10(%eax),%eax
80104637:	89 45 d8             	mov    %eax,-0x28(%ebp)

  // lock to force the compiler to emit the np->state write last.
  acquire(&ptable.lock);
8010463a:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104641:	e8 31 07 00 00       	call   80104d77 <acquire>
  np->state = RUNNABLE;
80104646:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104649:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  release(&ptable.lock);
80104650:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104657:	e8 7d 07 00 00       	call   80104dd9 <release>

  return pid;
8010465c:	8b 45 d8             	mov    -0x28(%ebp),%eax

}
8010465f:	83 c4 2c             	add    $0x2c,%esp
80104662:	5b                   	pop    %ebx
80104663:	5e                   	pop    %esi
80104664:	5f                   	pop    %edi
80104665:	5d                   	pop    %ebp
80104666:	c3                   	ret    

80104667 <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
80104667:	55                   	push   %ebp
80104668:	89 e5                	mov    %esp,%ebp
8010466a:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
8010466d:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104674:	a1 88 b6 10 80       	mov    0x8010b688,%eax
80104679:	39 c2                	cmp    %eax,%edx
8010467b:	75 0c                	jne    80104689 <exit+0x22>
    panic("init exiting");
8010467d:	c7 04 24 98 8a 10 80 	movl   $0x80108a98,(%esp)
80104684:	e8 b1 be ff ff       	call   8010053a <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104689:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104690:	eb 44                	jmp    801046d6 <exit+0x6f>
    if(proc->ofile[fd]){
80104692:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104698:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010469b:	83 c2 08             	add    $0x8,%edx
8010469e:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046a2:	85 c0                	test   %eax,%eax
801046a4:	74 2c                	je     801046d2 <exit+0x6b>
      fileclose(proc->ofile[fd]);
801046a6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046ac:	8b 55 f0             	mov    -0x10(%ebp),%edx
801046af:	83 c2 08             	add    $0x8,%edx
801046b2:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046b6:	89 04 24             	mov    %eax,(%esp)
801046b9:	e8 10 c9 ff ff       	call   80100fce <fileclose>
      proc->ofile[fd] = 0;
801046be:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046c4:	8b 55 f0             	mov    -0x10(%ebp),%edx
801046c7:	83 c2 08             	add    $0x8,%edx
801046ca:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801046d1:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801046d2:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
801046d6:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801046da:	7e b6                	jle    80104692 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  begin_op();
801046dc:	e8 04 eb ff ff       	call   801031e5 <begin_op>
  iput(proc->cwd);
801046e1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046e7:	8b 40 68             	mov    0x68(%eax),%eax
801046ea:	89 04 24             	mov    %eax,(%esp)
801046ed:	e8 1c d3 ff ff       	call   80101a0e <iput>
  end_op();
801046f2:	e8 72 eb ff ff       	call   80103269 <end_op>
  proc->cwd = 0;
801046f7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801046fd:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
80104704:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
8010470b:	e8 67 06 00 00       	call   80104d77 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
80104710:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104716:	8b 40 14             	mov    0x14(%eax),%eax
80104719:	89 04 24             	mov    %eax,(%esp)
8010471c:	e8 1e 04 00 00       	call   80104b3f <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104721:	c7 45 f4 d4 29 11 80 	movl   $0x801129d4,-0xc(%ebp)
80104728:	eb 3b                	jmp    80104765 <exit+0xfe>
    if(p->parent == proc){
8010472a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010472d:	8b 50 14             	mov    0x14(%eax),%edx
80104730:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104736:	39 c2                	cmp    %eax,%edx
80104738:	75 24                	jne    8010475e <exit+0xf7>
      p->parent = initproc;
8010473a:	8b 15 88 b6 10 80    	mov    0x8010b688,%edx
80104740:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104743:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
80104746:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104749:	8b 40 0c             	mov    0xc(%eax),%eax
8010474c:	83 f8 05             	cmp    $0x5,%eax
8010474f:	75 0d                	jne    8010475e <exit+0xf7>
        wakeup1(initproc);
80104751:	a1 88 b6 10 80       	mov    0x8010b688,%eax
80104756:	89 04 24             	mov    %eax,(%esp)
80104759:	e8 e1 03 00 00       	call   80104b3f <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010475e:	81 45 f4 50 01 00 00 	addl   $0x150,-0xc(%ebp)
80104765:	81 7d f4 d4 7d 11 80 	cmpl   $0x80117dd4,-0xc(%ebp)
8010476c:	72 bc                	jb     8010472a <exit+0xc3>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
8010476e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104774:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
8010477b:	e8 14 02 00 00       	call   80104994 <sched>
  panic("zombie exit");
80104780:	c7 04 24 a5 8a 10 80 	movl   $0x80108aa5,(%esp)
80104787:	e8 ae bd ff ff       	call   8010053a <panic>

8010478c <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(uint tid)
{
8010478c:	55                   	push   %ebp
8010478d:	89 e5                	mov    %esp,%ebp
8010478f:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
80104792:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104799:	e8 d9 05 00 00       	call   80104d77 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
8010479e:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801047a5:	c7 45 f4 d4 29 11 80 	movl   $0x801129d4,-0xc(%ebp)
801047ac:	e9 f8 00 00 00       	jmp    801048a9 <wait+0x11d>
      if(p->parent != proc)
801047b1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047b4:	8b 50 14             	mov    0x14(%eax),%edx
801047b7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047bd:	39 c2                	cmp    %eax,%edx
801047bf:	74 05                	je     801047c6 <wait+0x3a>
        continue;
801047c1:	e9 dc 00 00 00       	jmp    801048a2 <wait+0x116>
      
      if(tid)
801047c6:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801047ca:	74 10                	je     801047dc <wait+0x50>
	if(tid != p->pid)
801047cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047cf:	8b 40 10             	mov    0x10(%eax),%eax
801047d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801047d5:	74 05                	je     801047dc <wait+0x50>
	  continue;
801047d7:	e9 c6 00 00 00       	jmp    801048a2 <wait+0x116>
     
      havekids = 1;
801047dc:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
 
      if(p->state == ZOMBIE){
801047e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047e6:	8b 40 0c             	mov    0xc(%eax),%eax
801047e9:	83 f8 05             	cmp    $0x5,%eax
801047ec:	0f 85 b0 00 00 00    	jne    801048a2 <wait+0x116>
        // Found one.
        pid = p->pid;
801047f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047f5:	8b 40 10             	mov    0x10(%eax),%eax
801047f8:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
801047fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047fe:	8b 40 08             	mov    0x8(%eax),%eax
80104801:	89 04 24             	mov    %eax,(%esp)
80104804:	e8 40 e2 ff ff       	call   80102a49 <kfree>
        p->kstack = 0;
80104809:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010480c:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)

	if(!p->type && (p->tcount == 0))
80104813:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104816:	8b 80 80 00 00 00    	mov    0x80(%eax),%eax
8010481c:	85 c0                	test   %eax,%eax
8010481e:	75 1d                	jne    8010483d <wait+0xb1>
80104820:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104823:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80104829:	85 c0                	test   %eax,%eax
8010482b:	75 10                	jne    8010483d <wait+0xb1>
           freevm(p->pgdir);
8010482d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104830:	8b 40 04             	mov    0x4(%eax),%eax
80104833:	89 04 24             	mov    %eax,(%esp)
80104836:	e8 54 3c 00 00       	call   8010848f <freevm>
8010483b:	eb 25                	jmp    80104862 <wait+0xd6>
	else {
	   p->pgdir = 0;
8010483d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104840:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
	   p->parent->tcount-=1;
80104847:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010484a:	8b 40 14             	mov    0x14(%eax),%eax
8010484d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104850:	8b 52 14             	mov    0x14(%edx),%edx
80104853:	8b 92 8c 00 00 00    	mov    0x8c(%edx),%edx
80104859:	83 ea 01             	sub    $0x1,%edx
8010485c:	89 90 8c 00 00 00    	mov    %edx,0x8c(%eax)
	}

        p->state = UNUSED;
80104862:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104865:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
8010486c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010486f:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
80104876:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104879:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
80104880:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104883:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
80104887:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010488a:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
80104891:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104898:	e8 3c 05 00 00       	call   80104dd9 <release>
        return pid;
8010489d:	8b 45 ec             	mov    -0x14(%ebp),%eax
801048a0:	eb 55                	jmp    801048f7 <wait+0x16b>

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801048a2:	81 45 f4 50 01 00 00 	addl   $0x150,-0xc(%ebp)
801048a9:	81 7d f4 d4 7d 11 80 	cmpl   $0x80117dd4,-0xc(%ebp)
801048b0:	0f 82 fb fe ff ff    	jb     801047b1 <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
801048b6:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801048ba:	74 0d                	je     801048c9 <wait+0x13d>
801048bc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048c2:	8b 40 24             	mov    0x24(%eax),%eax
801048c5:	85 c0                	test   %eax,%eax
801048c7:	74 13                	je     801048dc <wait+0x150>
      release(&ptable.lock);
801048c9:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
801048d0:	e8 04 05 00 00       	call   80104dd9 <release>
      return -1;
801048d5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801048da:	eb 1b                	jmp    801048f7 <wait+0x16b>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
801048dc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801048e2:	c7 44 24 04 a0 29 11 	movl   $0x801129a0,0x4(%esp)
801048e9:	80 
801048ea:	89 04 24             	mov    %eax,(%esp)
801048ed:	e8 b2 01 00 00       	call   80104aa4 <sleep>
  }
801048f2:	e9 a7 fe ff ff       	jmp    8010479e <wait+0x12>
}
801048f7:	c9                   	leave  
801048f8:	c3                   	ret    

801048f9 <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
801048f9:	55                   	push   %ebp
801048fa:	89 e5                	mov    %esp,%ebp
801048fc:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  for(;;){
    // Enable interrupts on this processor.
    sti();
801048ff:	e8 9f f7 ff ff       	call   801040a3 <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
80104904:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
8010490b:	e8 67 04 00 00       	call   80104d77 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104910:	c7 45 f4 d4 29 11 80 	movl   $0x801129d4,-0xc(%ebp)
80104917:	eb 61                	jmp    8010497a <scheduler+0x81>
      if(p->state != RUNNABLE)
80104919:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010491c:	8b 40 0c             	mov    0xc(%eax),%eax
8010491f:	83 f8 03             	cmp    $0x3,%eax
80104922:	74 02                	je     80104926 <scheduler+0x2d>
        continue;
80104924:	eb 4d                	jmp    80104973 <scheduler+0x7a>

      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
80104926:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104929:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
8010492f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104932:	89 04 24             	mov    %eax,(%esp)
80104935:	e8 e2 36 00 00       	call   8010801c <switchuvm>
      p->state = RUNNING;
8010493a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010493d:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
80104944:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010494a:	8b 40 1c             	mov    0x1c(%eax),%eax
8010494d:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104954:	83 c2 04             	add    $0x4,%edx
80104957:	89 44 24 04          	mov    %eax,0x4(%esp)
8010495b:	89 14 24             	mov    %edx,(%esp)
8010495e:	e8 f9 08 00 00       	call   8010525c <swtch>
      switchkvm();
80104963:	e8 97 36 00 00       	call   80107fff <switchkvm>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
80104968:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
8010496f:	00 00 00 00 
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104973:	81 45 f4 50 01 00 00 	addl   $0x150,-0xc(%ebp)
8010497a:	81 7d f4 d4 7d 11 80 	cmpl   $0x80117dd4,-0xc(%ebp)
80104981:	72 96                	jb     80104919 <scheduler+0x20>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
80104983:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
8010498a:	e8 4a 04 00 00       	call   80104dd9 <release>

  }
8010498f:	e9 6b ff ff ff       	jmp    801048ff <scheduler+0x6>

80104994 <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
80104994:	55                   	push   %ebp
80104995:	89 e5                	mov    %esp,%ebp
80104997:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
8010499a:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
801049a1:	e8 fb 04 00 00       	call   80104ea1 <holding>
801049a6:	85 c0                	test   %eax,%eax
801049a8:	75 0c                	jne    801049b6 <sched+0x22>
    panic("sched ptable.lock");
801049aa:	c7 04 24 b1 8a 10 80 	movl   $0x80108ab1,(%esp)
801049b1:	e8 84 bb ff ff       	call   8010053a <panic>
  if(cpu->ncli != 1)
801049b6:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801049bc:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
801049c2:	83 f8 01             	cmp    $0x1,%eax
801049c5:	74 0c                	je     801049d3 <sched+0x3f>
    panic("sched locks");
801049c7:	c7 04 24 c3 8a 10 80 	movl   $0x80108ac3,(%esp)
801049ce:	e8 67 bb ff ff       	call   8010053a <panic>
  if(proc->state == RUNNING)
801049d3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801049d9:	8b 40 0c             	mov    0xc(%eax),%eax
801049dc:	83 f8 04             	cmp    $0x4,%eax
801049df:	75 0c                	jne    801049ed <sched+0x59>
    panic("sched running");
801049e1:	c7 04 24 cf 8a 10 80 	movl   $0x80108acf,(%esp)
801049e8:	e8 4d bb ff ff       	call   8010053a <panic>
  if(readeflags()&FL_IF)
801049ed:	e8 a1 f6 ff ff       	call   80104093 <readeflags>
801049f2:	25 00 02 00 00       	and    $0x200,%eax
801049f7:	85 c0                	test   %eax,%eax
801049f9:	74 0c                	je     80104a07 <sched+0x73>
    panic("sched interruptible");
801049fb:	c7 04 24 dd 8a 10 80 	movl   $0x80108add,(%esp)
80104a02:	e8 33 bb ff ff       	call   8010053a <panic>
  intena = cpu->intena;
80104a07:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a0d:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104a13:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80104a16:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a1c:	8b 40 04             	mov    0x4(%eax),%eax
80104a1f:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104a26:	83 c2 1c             	add    $0x1c,%edx
80104a29:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a2d:	89 14 24             	mov    %edx,(%esp)
80104a30:	e8 27 08 00 00       	call   8010525c <swtch>
  cpu->intena = intena;
80104a35:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104a3b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104a3e:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104a44:	c9                   	leave  
80104a45:	c3                   	ret    

80104a46 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
80104a46:	55                   	push   %ebp
80104a47:	89 e5                	mov    %esp,%ebp
80104a49:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104a4c:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104a53:	e8 1f 03 00 00       	call   80104d77 <acquire>
  proc->state = RUNNABLE;
80104a58:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104a5e:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
80104a65:	e8 2a ff ff ff       	call   80104994 <sched>
  release(&ptable.lock);
80104a6a:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104a71:	e8 63 03 00 00       	call   80104dd9 <release>
}
80104a76:	c9                   	leave  
80104a77:	c3                   	ret    

80104a78 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80104a78:	55                   	push   %ebp
80104a79:	89 e5                	mov    %esp,%ebp
80104a7b:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80104a7e:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104a85:	e8 4f 03 00 00       	call   80104dd9 <release>

  if (first) {
80104a8a:	a1 08 b0 10 80       	mov    0x8010b008,%eax
80104a8f:	85 c0                	test   %eax,%eax
80104a91:	74 0f                	je     80104aa2 <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
80104a93:	c7 05 08 b0 10 80 00 	movl   $0x0,0x8010b008
80104a9a:	00 00 00 
    initlog();
80104a9d:	e8 35 e5 ff ff       	call   80102fd7 <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
80104aa2:	c9                   	leave  
80104aa3:	c3                   	ret    

80104aa4 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80104aa4:	55                   	push   %ebp
80104aa5:	89 e5                	mov    %esp,%ebp
80104aa7:	83 ec 18             	sub    $0x18,%esp
  if(proc == 0)
80104aaa:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104ab0:	85 c0                	test   %eax,%eax
80104ab2:	75 0c                	jne    80104ac0 <sleep+0x1c>
    panic("sleep");
80104ab4:	c7 04 24 f1 8a 10 80 	movl   $0x80108af1,(%esp)
80104abb:	e8 7a ba ff ff       	call   8010053a <panic>

  if(lk == 0)
80104ac0:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104ac4:	75 0c                	jne    80104ad2 <sleep+0x2e>
    panic("sleep without lk");
80104ac6:	c7 04 24 f7 8a 10 80 	movl   $0x80108af7,(%esp)
80104acd:	e8 68 ba ff ff       	call   8010053a <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104ad2:	81 7d 0c a0 29 11 80 	cmpl   $0x801129a0,0xc(%ebp)
80104ad9:	74 17                	je     80104af2 <sleep+0x4e>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104adb:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104ae2:	e8 90 02 00 00       	call   80104d77 <acquire>
    release(lk);
80104ae7:	8b 45 0c             	mov    0xc(%ebp),%eax
80104aea:	89 04 24             	mov    %eax,(%esp)
80104aed:	e8 e7 02 00 00       	call   80104dd9 <release>
  }

  // Go to sleep.
  proc->chan = chan;
80104af2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104af8:	8b 55 08             	mov    0x8(%ebp),%edx
80104afb:	89 50 20             	mov    %edx,0x20(%eax)
  proc->state = SLEEPING;
80104afe:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b04:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
  sched();
80104b0b:	e8 84 fe ff ff       	call   80104994 <sched>

  // Tidy up.
  proc->chan = 0;
80104b10:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104b16:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104b1d:	81 7d 0c a0 29 11 80 	cmpl   $0x801129a0,0xc(%ebp)
80104b24:	74 17                	je     80104b3d <sleep+0x99>
    release(&ptable.lock);
80104b26:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104b2d:	e8 a7 02 00 00       	call   80104dd9 <release>
    acquire(lk);
80104b32:	8b 45 0c             	mov    0xc(%ebp),%eax
80104b35:	89 04 24             	mov    %eax,(%esp)
80104b38:	e8 3a 02 00 00       	call   80104d77 <acquire>
  }
}
80104b3d:	c9                   	leave  
80104b3e:	c3                   	ret    

80104b3f <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104b3f:	55                   	push   %ebp
80104b40:	89 e5                	mov    %esp,%ebp
80104b42:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104b45:	c7 45 fc d4 29 11 80 	movl   $0x801129d4,-0x4(%ebp)
80104b4c:	eb 27                	jmp    80104b75 <wakeup1+0x36>
    if(p->state == SLEEPING && p->chan == chan)
80104b4e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b51:	8b 40 0c             	mov    0xc(%eax),%eax
80104b54:	83 f8 02             	cmp    $0x2,%eax
80104b57:	75 15                	jne    80104b6e <wakeup1+0x2f>
80104b59:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b5c:	8b 40 20             	mov    0x20(%eax),%eax
80104b5f:	3b 45 08             	cmp    0x8(%ebp),%eax
80104b62:	75 0a                	jne    80104b6e <wakeup1+0x2f>
      p->state = RUNNABLE;
80104b64:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b67:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104b6e:	81 45 fc 50 01 00 00 	addl   $0x150,-0x4(%ebp)
80104b75:	81 7d fc d4 7d 11 80 	cmpl   $0x80117dd4,-0x4(%ebp)
80104b7c:	72 d0                	jb     80104b4e <wakeup1+0xf>
    if(p->state == SLEEPING && p->chan == chan)
      p->state = RUNNABLE;
}
80104b7e:	c9                   	leave  
80104b7f:	c3                   	ret    

80104b80 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104b80:	55                   	push   %ebp
80104b81:	89 e5                	mov    %esp,%ebp
80104b83:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
80104b86:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104b8d:	e8 e5 01 00 00       	call   80104d77 <acquire>
  wakeup1(chan);
80104b92:	8b 45 08             	mov    0x8(%ebp),%eax
80104b95:	89 04 24             	mov    %eax,(%esp)
80104b98:	e8 a2 ff ff ff       	call   80104b3f <wakeup1>
  release(&ptable.lock);
80104b9d:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104ba4:	e8 30 02 00 00       	call   80104dd9 <release>
}
80104ba9:	c9                   	leave  
80104baa:	c3                   	ret    

80104bab <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104bab:	55                   	push   %ebp
80104bac:	89 e5                	mov    %esp,%ebp
80104bae:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104bb1:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104bb8:	e8 ba 01 00 00       	call   80104d77 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104bbd:	c7 45 f4 d4 29 11 80 	movl   $0x801129d4,-0xc(%ebp)
80104bc4:	eb 44                	jmp    80104c0a <kill+0x5f>
    if(p->pid == pid){
80104bc6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104bc9:	8b 40 10             	mov    0x10(%eax),%eax
80104bcc:	3b 45 08             	cmp    0x8(%ebp),%eax
80104bcf:	75 32                	jne    80104c03 <kill+0x58>
      p->killed = 1;
80104bd1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104bd4:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104bdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104bde:	8b 40 0c             	mov    0xc(%eax),%eax
80104be1:	83 f8 02             	cmp    $0x2,%eax
80104be4:	75 0a                	jne    80104bf0 <kill+0x45>
        p->state = RUNNABLE;
80104be6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104be9:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104bf0:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104bf7:	e8 dd 01 00 00       	call   80104dd9 <release>
      return 0;
80104bfc:	b8 00 00 00 00       	mov    $0x0,%eax
80104c01:	eb 21                	jmp    80104c24 <kill+0x79>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104c03:	81 45 f4 50 01 00 00 	addl   $0x150,-0xc(%ebp)
80104c0a:	81 7d f4 d4 7d 11 80 	cmpl   $0x80117dd4,-0xc(%ebp)
80104c11:	72 b3                	jb     80104bc6 <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104c13:	c7 04 24 a0 29 11 80 	movl   $0x801129a0,(%esp)
80104c1a:	e8 ba 01 00 00       	call   80104dd9 <release>
  return -1;
80104c1f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104c24:	c9                   	leave  
80104c25:	c3                   	ret    

80104c26 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104c26:	55                   	push   %ebp
80104c27:	89 e5                	mov    %esp,%ebp
80104c29:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104c2c:	c7 45 f0 d4 29 11 80 	movl   $0x801129d4,-0x10(%ebp)
80104c33:	e9 d9 00 00 00       	jmp    80104d11 <procdump+0xeb>
    if(p->state == UNUSED)
80104c38:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c3b:	8b 40 0c             	mov    0xc(%eax),%eax
80104c3e:	85 c0                	test   %eax,%eax
80104c40:	75 05                	jne    80104c47 <procdump+0x21>
      continue;
80104c42:	e9 c3 00 00 00       	jmp    80104d0a <procdump+0xe4>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104c47:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c4a:	8b 40 0c             	mov    0xc(%eax),%eax
80104c4d:	83 f8 05             	cmp    $0x5,%eax
80104c50:	77 23                	ja     80104c75 <procdump+0x4f>
80104c52:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c55:	8b 40 0c             	mov    0xc(%eax),%eax
80104c58:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104c5f:	85 c0                	test   %eax,%eax
80104c61:	74 12                	je     80104c75 <procdump+0x4f>
      state = states[p->state];
80104c63:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c66:	8b 40 0c             	mov    0xc(%eax),%eax
80104c69:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104c70:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104c73:	eb 07                	jmp    80104c7c <procdump+0x56>
    else
      state = "???";
80104c75:	c7 45 ec 08 8b 10 80 	movl   $0x80108b08,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80104c7c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c7f:	8d 50 6c             	lea    0x6c(%eax),%edx
80104c82:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c85:	8b 40 10             	mov    0x10(%eax),%eax
80104c88:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104c8c:	8b 55 ec             	mov    -0x14(%ebp),%edx
80104c8f:	89 54 24 08          	mov    %edx,0x8(%esp)
80104c93:	89 44 24 04          	mov    %eax,0x4(%esp)
80104c97:	c7 04 24 0c 8b 10 80 	movl   $0x80108b0c,(%esp)
80104c9e:	e8 fd b6 ff ff       	call   801003a0 <cprintf>
    if(p->state == SLEEPING){
80104ca3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104ca6:	8b 40 0c             	mov    0xc(%eax),%eax
80104ca9:	83 f8 02             	cmp    $0x2,%eax
80104cac:	75 50                	jne    80104cfe <procdump+0xd8>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80104cae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104cb1:	8b 40 1c             	mov    0x1c(%eax),%eax
80104cb4:	8b 40 0c             	mov    0xc(%eax),%eax
80104cb7:	83 c0 08             	add    $0x8,%eax
80104cba:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80104cbd:	89 54 24 04          	mov    %edx,0x4(%esp)
80104cc1:	89 04 24             	mov    %eax,(%esp)
80104cc4:	e8 5f 01 00 00       	call   80104e28 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
80104cc9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104cd0:	eb 1b                	jmp    80104ced <procdump+0xc7>
        cprintf(" %p", pc[i]);
80104cd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cd5:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104cd9:	89 44 24 04          	mov    %eax,0x4(%esp)
80104cdd:	c7 04 24 15 8b 10 80 	movl   $0x80108b15,(%esp)
80104ce4:	e8 b7 b6 ff ff       	call   801003a0 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80104ce9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104ced:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104cf1:	7f 0b                	jg     80104cfe <procdump+0xd8>
80104cf3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104cf6:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104cfa:	85 c0                	test   %eax,%eax
80104cfc:	75 d4                	jne    80104cd2 <procdump+0xac>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104cfe:	c7 04 24 19 8b 10 80 	movl   $0x80108b19,(%esp)
80104d05:	e8 96 b6 ff ff       	call   801003a0 <cprintf>
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104d0a:	81 45 f0 50 01 00 00 	addl   $0x150,-0x10(%ebp)
80104d11:	81 7d f0 d4 7d 11 80 	cmpl   $0x80117dd4,-0x10(%ebp)
80104d18:	0f 82 1a ff ff ff    	jb     80104c38 <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
80104d1e:	c9                   	leave  
80104d1f:	c3                   	ret    

80104d20 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104d20:	55                   	push   %ebp
80104d21:	89 e5                	mov    %esp,%ebp
80104d23:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104d26:	9c                   	pushf  
80104d27:	58                   	pop    %eax
80104d28:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80104d2b:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104d2e:	c9                   	leave  
80104d2f:	c3                   	ret    

80104d30 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80104d30:	55                   	push   %ebp
80104d31:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80104d33:	fa                   	cli    
}
80104d34:	5d                   	pop    %ebp
80104d35:	c3                   	ret    

80104d36 <sti>:

static inline void
sti(void)
{
80104d36:	55                   	push   %ebp
80104d37:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104d39:	fb                   	sti    
}
80104d3a:	5d                   	pop    %ebp
80104d3b:	c3                   	ret    

80104d3c <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80104d3c:	55                   	push   %ebp
80104d3d:	89 e5                	mov    %esp,%ebp
80104d3f:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104d42:	8b 55 08             	mov    0x8(%ebp),%edx
80104d45:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d48:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104d4b:	f0 87 02             	lock xchg %eax,(%edx)
80104d4e:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80104d51:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104d54:	c9                   	leave  
80104d55:	c3                   	ret    

80104d56 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104d56:	55                   	push   %ebp
80104d57:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80104d59:	8b 45 08             	mov    0x8(%ebp),%eax
80104d5c:	8b 55 0c             	mov    0xc(%ebp),%edx
80104d5f:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80104d62:	8b 45 08             	mov    0x8(%ebp),%eax
80104d65:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80104d6b:	8b 45 08             	mov    0x8(%ebp),%eax
80104d6e:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104d75:	5d                   	pop    %ebp
80104d76:	c3                   	ret    

80104d77 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80104d77:	55                   	push   %ebp
80104d78:	89 e5                	mov    %esp,%ebp
80104d7a:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80104d7d:	e8 49 01 00 00       	call   80104ecb <pushcli>
  if(holding(lk))
80104d82:	8b 45 08             	mov    0x8(%ebp),%eax
80104d85:	89 04 24             	mov    %eax,(%esp)
80104d88:	e8 14 01 00 00       	call   80104ea1 <holding>
80104d8d:	85 c0                	test   %eax,%eax
80104d8f:	74 0c                	je     80104d9d <acquire+0x26>
    panic("acquire");
80104d91:	c7 04 24 45 8b 10 80 	movl   $0x80108b45,(%esp)
80104d98:	e8 9d b7 ff ff       	call   8010053a <panic>

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80104d9d:	90                   	nop
80104d9e:	8b 45 08             	mov    0x8(%ebp),%eax
80104da1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80104da8:	00 
80104da9:	89 04 24             	mov    %eax,(%esp)
80104dac:	e8 8b ff ff ff       	call   80104d3c <xchg>
80104db1:	85 c0                	test   %eax,%eax
80104db3:	75 e9                	jne    80104d9e <acquire+0x27>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80104db5:	8b 45 08             	mov    0x8(%ebp),%eax
80104db8:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104dbf:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
80104dc2:	8b 45 08             	mov    0x8(%ebp),%eax
80104dc5:	83 c0 0c             	add    $0xc,%eax
80104dc8:	89 44 24 04          	mov    %eax,0x4(%esp)
80104dcc:	8d 45 08             	lea    0x8(%ebp),%eax
80104dcf:	89 04 24             	mov    %eax,(%esp)
80104dd2:	e8 51 00 00 00       	call   80104e28 <getcallerpcs>
}
80104dd7:	c9                   	leave  
80104dd8:	c3                   	ret    

80104dd9 <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80104dd9:	55                   	push   %ebp
80104dda:	89 e5                	mov    %esp,%ebp
80104ddc:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80104ddf:	8b 45 08             	mov    0x8(%ebp),%eax
80104de2:	89 04 24             	mov    %eax,(%esp)
80104de5:	e8 b7 00 00 00       	call   80104ea1 <holding>
80104dea:	85 c0                	test   %eax,%eax
80104dec:	75 0c                	jne    80104dfa <release+0x21>
    panic("release");
80104dee:	c7 04 24 4d 8b 10 80 	movl   $0x80108b4d,(%esp)
80104df5:	e8 40 b7 ff ff       	call   8010053a <panic>

  lk->pcs[0] = 0;
80104dfa:	8b 45 08             	mov    0x8(%ebp),%eax
80104dfd:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80104e04:	8b 45 08             	mov    0x8(%ebp),%eax
80104e07:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
80104e0e:	8b 45 08             	mov    0x8(%ebp),%eax
80104e11:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104e18:	00 
80104e19:	89 04 24             	mov    %eax,(%esp)
80104e1c:	e8 1b ff ff ff       	call   80104d3c <xchg>

  popcli();
80104e21:	e8 e9 00 00 00       	call   80104f0f <popcli>
}
80104e26:	c9                   	leave  
80104e27:	c3                   	ret    

80104e28 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80104e28:	55                   	push   %ebp
80104e29:	89 e5                	mov    %esp,%ebp
80104e2b:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
80104e2e:	8b 45 08             	mov    0x8(%ebp),%eax
80104e31:	83 e8 08             	sub    $0x8,%eax
80104e34:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80104e37:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80104e3e:	eb 38                	jmp    80104e78 <getcallerpcs+0x50>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104e40:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80104e44:	74 38                	je     80104e7e <getcallerpcs+0x56>
80104e46:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80104e4d:	76 2f                	jbe    80104e7e <getcallerpcs+0x56>
80104e4f:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80104e53:	74 29                	je     80104e7e <getcallerpcs+0x56>
      break;
    pcs[i] = ebp[1];     // saved %eip
80104e55:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104e58:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104e5f:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e62:	01 c2                	add    %eax,%edx
80104e64:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104e67:	8b 40 04             	mov    0x4(%eax),%eax
80104e6a:	89 02                	mov    %eax,(%edx)
    ebp = (uint*)ebp[0]; // saved %ebp
80104e6c:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104e6f:	8b 00                	mov    (%eax),%eax
80104e71:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80104e74:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104e78:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104e7c:	7e c2                	jle    80104e40 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104e7e:	eb 19                	jmp    80104e99 <getcallerpcs+0x71>
    pcs[i] = 0;
80104e80:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104e83:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104e8a:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e8d:	01 d0                	add    %edx,%eax
80104e8f:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104e95:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104e99:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104e9d:	7e e1                	jle    80104e80 <getcallerpcs+0x58>
    pcs[i] = 0;
}
80104e9f:	c9                   	leave  
80104ea0:	c3                   	ret    

80104ea1 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80104ea1:	55                   	push   %ebp
80104ea2:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
80104ea4:	8b 45 08             	mov    0x8(%ebp),%eax
80104ea7:	8b 00                	mov    (%eax),%eax
80104ea9:	85 c0                	test   %eax,%eax
80104eab:	74 17                	je     80104ec4 <holding+0x23>
80104ead:	8b 45 08             	mov    0x8(%ebp),%eax
80104eb0:	8b 50 08             	mov    0x8(%eax),%edx
80104eb3:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104eb9:	39 c2                	cmp    %eax,%edx
80104ebb:	75 07                	jne    80104ec4 <holding+0x23>
80104ebd:	b8 01 00 00 00       	mov    $0x1,%eax
80104ec2:	eb 05                	jmp    80104ec9 <holding+0x28>
80104ec4:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104ec9:	5d                   	pop    %ebp
80104eca:	c3                   	ret    

80104ecb <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80104ecb:	55                   	push   %ebp
80104ecc:	89 e5                	mov    %esp,%ebp
80104ece:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
80104ed1:	e8 4a fe ff ff       	call   80104d20 <readeflags>
80104ed6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
80104ed9:	e8 52 fe ff ff       	call   80104d30 <cli>
  if(cpu->ncli++ == 0)
80104ede:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104ee5:	8b 82 ac 00 00 00    	mov    0xac(%edx),%eax
80104eeb:	8d 48 01             	lea    0x1(%eax),%ecx
80104eee:	89 8a ac 00 00 00    	mov    %ecx,0xac(%edx)
80104ef4:	85 c0                	test   %eax,%eax
80104ef6:	75 15                	jne    80104f0d <pushcli+0x42>
    cpu->intena = eflags & FL_IF;
80104ef8:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104efe:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104f01:	81 e2 00 02 00 00    	and    $0x200,%edx
80104f07:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104f0d:	c9                   	leave  
80104f0e:	c3                   	ret    

80104f0f <popcli>:

void
popcli(void)
{
80104f0f:	55                   	push   %ebp
80104f10:	89 e5                	mov    %esp,%ebp
80104f12:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80104f15:	e8 06 fe ff ff       	call   80104d20 <readeflags>
80104f1a:	25 00 02 00 00       	and    $0x200,%eax
80104f1f:	85 c0                	test   %eax,%eax
80104f21:	74 0c                	je     80104f2f <popcli+0x20>
    panic("popcli - interruptible");
80104f23:	c7 04 24 55 8b 10 80 	movl   $0x80108b55,(%esp)
80104f2a:	e8 0b b6 ff ff       	call   8010053a <panic>
  if(--cpu->ncli < 0)
80104f2f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104f35:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80104f3b:	83 ea 01             	sub    $0x1,%edx
80104f3e:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80104f44:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104f4a:	85 c0                	test   %eax,%eax
80104f4c:	79 0c                	jns    80104f5a <popcli+0x4b>
    panic("popcli");
80104f4e:	c7 04 24 6c 8b 10 80 	movl   $0x80108b6c,(%esp)
80104f55:	e8 e0 b5 ff ff       	call   8010053a <panic>
  if(cpu->ncli == 0 && cpu->intena)
80104f5a:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104f60:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104f66:	85 c0                	test   %eax,%eax
80104f68:	75 15                	jne    80104f7f <popcli+0x70>
80104f6a:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104f70:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104f76:	85 c0                	test   %eax,%eax
80104f78:	74 05                	je     80104f7f <popcli+0x70>
    sti();
80104f7a:	e8 b7 fd ff ff       	call   80104d36 <sti>
}
80104f7f:	c9                   	leave  
80104f80:	c3                   	ret    

80104f81 <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
80104f81:	55                   	push   %ebp
80104f82:	89 e5                	mov    %esp,%ebp
80104f84:	57                   	push   %edi
80104f85:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80104f86:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104f89:	8b 55 10             	mov    0x10(%ebp),%edx
80104f8c:	8b 45 0c             	mov    0xc(%ebp),%eax
80104f8f:	89 cb                	mov    %ecx,%ebx
80104f91:	89 df                	mov    %ebx,%edi
80104f93:	89 d1                	mov    %edx,%ecx
80104f95:	fc                   	cld    
80104f96:	f3 aa                	rep stos %al,%es:(%edi)
80104f98:	89 ca                	mov    %ecx,%edx
80104f9a:	89 fb                	mov    %edi,%ebx
80104f9c:	89 5d 08             	mov    %ebx,0x8(%ebp)
80104f9f:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80104fa2:	5b                   	pop    %ebx
80104fa3:	5f                   	pop    %edi
80104fa4:	5d                   	pop    %ebp
80104fa5:	c3                   	ret    

80104fa6 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
80104fa6:	55                   	push   %ebp
80104fa7:	89 e5                	mov    %esp,%ebp
80104fa9:	57                   	push   %edi
80104faa:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
80104fab:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104fae:	8b 55 10             	mov    0x10(%ebp),%edx
80104fb1:	8b 45 0c             	mov    0xc(%ebp),%eax
80104fb4:	89 cb                	mov    %ecx,%ebx
80104fb6:	89 df                	mov    %ebx,%edi
80104fb8:	89 d1                	mov    %edx,%ecx
80104fba:	fc                   	cld    
80104fbb:	f3 ab                	rep stos %eax,%es:(%edi)
80104fbd:	89 ca                	mov    %ecx,%edx
80104fbf:	89 fb                	mov    %edi,%ebx
80104fc1:	89 5d 08             	mov    %ebx,0x8(%ebp)
80104fc4:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80104fc7:	5b                   	pop    %ebx
80104fc8:	5f                   	pop    %edi
80104fc9:	5d                   	pop    %ebp
80104fca:	c3                   	ret    

80104fcb <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80104fcb:	55                   	push   %ebp
80104fcc:	89 e5                	mov    %esp,%ebp
80104fce:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
80104fd1:	8b 45 08             	mov    0x8(%ebp),%eax
80104fd4:	83 e0 03             	and    $0x3,%eax
80104fd7:	85 c0                	test   %eax,%eax
80104fd9:	75 49                	jne    80105024 <memset+0x59>
80104fdb:	8b 45 10             	mov    0x10(%ebp),%eax
80104fde:	83 e0 03             	and    $0x3,%eax
80104fe1:	85 c0                	test   %eax,%eax
80104fe3:	75 3f                	jne    80105024 <memset+0x59>
    c &= 0xFF;
80104fe5:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
80104fec:	8b 45 10             	mov    0x10(%ebp),%eax
80104fef:	c1 e8 02             	shr    $0x2,%eax
80104ff2:	89 c2                	mov    %eax,%edx
80104ff4:	8b 45 0c             	mov    0xc(%ebp),%eax
80104ff7:	c1 e0 18             	shl    $0x18,%eax
80104ffa:	89 c1                	mov    %eax,%ecx
80104ffc:	8b 45 0c             	mov    0xc(%ebp),%eax
80104fff:	c1 e0 10             	shl    $0x10,%eax
80105002:	09 c1                	or     %eax,%ecx
80105004:	8b 45 0c             	mov    0xc(%ebp),%eax
80105007:	c1 e0 08             	shl    $0x8,%eax
8010500a:	09 c8                	or     %ecx,%eax
8010500c:	0b 45 0c             	or     0xc(%ebp),%eax
8010500f:	89 54 24 08          	mov    %edx,0x8(%esp)
80105013:	89 44 24 04          	mov    %eax,0x4(%esp)
80105017:	8b 45 08             	mov    0x8(%ebp),%eax
8010501a:	89 04 24             	mov    %eax,(%esp)
8010501d:	e8 84 ff ff ff       	call   80104fa6 <stosl>
80105022:	eb 19                	jmp    8010503d <memset+0x72>
  } else
    stosb(dst, c, n);
80105024:	8b 45 10             	mov    0x10(%ebp),%eax
80105027:	89 44 24 08          	mov    %eax,0x8(%esp)
8010502b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010502e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105032:	8b 45 08             	mov    0x8(%ebp),%eax
80105035:	89 04 24             	mov    %eax,(%esp)
80105038:	e8 44 ff ff ff       	call   80104f81 <stosb>
  return dst;
8010503d:	8b 45 08             	mov    0x8(%ebp),%eax
}
80105040:	c9                   	leave  
80105041:	c3                   	ret    

80105042 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
80105042:	55                   	push   %ebp
80105043:	89 e5                	mov    %esp,%ebp
80105045:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80105048:	8b 45 08             	mov    0x8(%ebp),%eax
8010504b:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
8010504e:	8b 45 0c             	mov    0xc(%ebp),%eax
80105051:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
80105054:	eb 30                	jmp    80105086 <memcmp+0x44>
    if(*s1 != *s2)
80105056:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105059:	0f b6 10             	movzbl (%eax),%edx
8010505c:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010505f:	0f b6 00             	movzbl (%eax),%eax
80105062:	38 c2                	cmp    %al,%dl
80105064:	74 18                	je     8010507e <memcmp+0x3c>
      return *s1 - *s2;
80105066:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105069:	0f b6 00             	movzbl (%eax),%eax
8010506c:	0f b6 d0             	movzbl %al,%edx
8010506f:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105072:	0f b6 00             	movzbl (%eax),%eax
80105075:	0f b6 c0             	movzbl %al,%eax
80105078:	29 c2                	sub    %eax,%edx
8010507a:	89 d0                	mov    %edx,%eax
8010507c:	eb 1a                	jmp    80105098 <memcmp+0x56>
    s1++, s2++;
8010507e:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105082:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80105086:	8b 45 10             	mov    0x10(%ebp),%eax
80105089:	8d 50 ff             	lea    -0x1(%eax),%edx
8010508c:	89 55 10             	mov    %edx,0x10(%ebp)
8010508f:	85 c0                	test   %eax,%eax
80105091:	75 c3                	jne    80105056 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
80105093:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105098:	c9                   	leave  
80105099:	c3                   	ret    

8010509a <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
8010509a:	55                   	push   %ebp
8010509b:	89 e5                	mov    %esp,%ebp
8010509d:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
801050a0:	8b 45 0c             	mov    0xc(%ebp),%eax
801050a3:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
801050a6:	8b 45 08             	mov    0x8(%ebp),%eax
801050a9:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
801050ac:	8b 45 fc             	mov    -0x4(%ebp),%eax
801050af:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801050b2:	73 3d                	jae    801050f1 <memmove+0x57>
801050b4:	8b 45 10             	mov    0x10(%ebp),%eax
801050b7:	8b 55 fc             	mov    -0x4(%ebp),%edx
801050ba:	01 d0                	add    %edx,%eax
801050bc:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801050bf:	76 30                	jbe    801050f1 <memmove+0x57>
    s += n;
801050c1:	8b 45 10             	mov    0x10(%ebp),%eax
801050c4:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
801050c7:	8b 45 10             	mov    0x10(%ebp),%eax
801050ca:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
801050cd:	eb 13                	jmp    801050e2 <memmove+0x48>
      *--d = *--s;
801050cf:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
801050d3:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
801050d7:	8b 45 fc             	mov    -0x4(%ebp),%eax
801050da:	0f b6 10             	movzbl (%eax),%edx
801050dd:	8b 45 f8             	mov    -0x8(%ebp),%eax
801050e0:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
801050e2:	8b 45 10             	mov    0x10(%ebp),%eax
801050e5:	8d 50 ff             	lea    -0x1(%eax),%edx
801050e8:	89 55 10             	mov    %edx,0x10(%ebp)
801050eb:	85 c0                	test   %eax,%eax
801050ed:	75 e0                	jne    801050cf <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
801050ef:	eb 26                	jmp    80105117 <memmove+0x7d>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
801050f1:	eb 17                	jmp    8010510a <memmove+0x70>
      *d++ = *s++;
801050f3:	8b 45 f8             	mov    -0x8(%ebp),%eax
801050f6:	8d 50 01             	lea    0x1(%eax),%edx
801050f9:	89 55 f8             	mov    %edx,-0x8(%ebp)
801050fc:	8b 55 fc             	mov    -0x4(%ebp),%edx
801050ff:	8d 4a 01             	lea    0x1(%edx),%ecx
80105102:	89 4d fc             	mov    %ecx,-0x4(%ebp)
80105105:	0f b6 12             	movzbl (%edx),%edx
80105108:	88 10                	mov    %dl,(%eax)
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
8010510a:	8b 45 10             	mov    0x10(%ebp),%eax
8010510d:	8d 50 ff             	lea    -0x1(%eax),%edx
80105110:	89 55 10             	mov    %edx,0x10(%ebp)
80105113:	85 c0                	test   %eax,%eax
80105115:	75 dc                	jne    801050f3 <memmove+0x59>
      *d++ = *s++;

  return dst;
80105117:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010511a:	c9                   	leave  
8010511b:	c3                   	ret    

8010511c <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
8010511c:	55                   	push   %ebp
8010511d:	89 e5                	mov    %esp,%ebp
8010511f:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
80105122:	8b 45 10             	mov    0x10(%ebp),%eax
80105125:	89 44 24 08          	mov    %eax,0x8(%esp)
80105129:	8b 45 0c             	mov    0xc(%ebp),%eax
8010512c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105130:	8b 45 08             	mov    0x8(%ebp),%eax
80105133:	89 04 24             	mov    %eax,(%esp)
80105136:	e8 5f ff ff ff       	call   8010509a <memmove>
}
8010513b:	c9                   	leave  
8010513c:	c3                   	ret    

8010513d <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
8010513d:	55                   	push   %ebp
8010513e:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80105140:	eb 0c                	jmp    8010514e <strncmp+0x11>
    n--, p++, q++;
80105142:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105146:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010514a:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
8010514e:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105152:	74 1a                	je     8010516e <strncmp+0x31>
80105154:	8b 45 08             	mov    0x8(%ebp),%eax
80105157:	0f b6 00             	movzbl (%eax),%eax
8010515a:	84 c0                	test   %al,%al
8010515c:	74 10                	je     8010516e <strncmp+0x31>
8010515e:	8b 45 08             	mov    0x8(%ebp),%eax
80105161:	0f b6 10             	movzbl (%eax),%edx
80105164:	8b 45 0c             	mov    0xc(%ebp),%eax
80105167:	0f b6 00             	movzbl (%eax),%eax
8010516a:	38 c2                	cmp    %al,%dl
8010516c:	74 d4                	je     80105142 <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
8010516e:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105172:	75 07                	jne    8010517b <strncmp+0x3e>
    return 0;
80105174:	b8 00 00 00 00       	mov    $0x0,%eax
80105179:	eb 16                	jmp    80105191 <strncmp+0x54>
  return (uchar)*p - (uchar)*q;
8010517b:	8b 45 08             	mov    0x8(%ebp),%eax
8010517e:	0f b6 00             	movzbl (%eax),%eax
80105181:	0f b6 d0             	movzbl %al,%edx
80105184:	8b 45 0c             	mov    0xc(%ebp),%eax
80105187:	0f b6 00             	movzbl (%eax),%eax
8010518a:	0f b6 c0             	movzbl %al,%eax
8010518d:	29 c2                	sub    %eax,%edx
8010518f:	89 d0                	mov    %edx,%eax
}
80105191:	5d                   	pop    %ebp
80105192:	c3                   	ret    

80105193 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80105193:	55                   	push   %ebp
80105194:	89 e5                	mov    %esp,%ebp
80105196:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80105199:	8b 45 08             	mov    0x8(%ebp),%eax
8010519c:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
8010519f:	90                   	nop
801051a0:	8b 45 10             	mov    0x10(%ebp),%eax
801051a3:	8d 50 ff             	lea    -0x1(%eax),%edx
801051a6:	89 55 10             	mov    %edx,0x10(%ebp)
801051a9:	85 c0                	test   %eax,%eax
801051ab:	7e 1e                	jle    801051cb <strncpy+0x38>
801051ad:	8b 45 08             	mov    0x8(%ebp),%eax
801051b0:	8d 50 01             	lea    0x1(%eax),%edx
801051b3:	89 55 08             	mov    %edx,0x8(%ebp)
801051b6:	8b 55 0c             	mov    0xc(%ebp),%edx
801051b9:	8d 4a 01             	lea    0x1(%edx),%ecx
801051bc:	89 4d 0c             	mov    %ecx,0xc(%ebp)
801051bf:	0f b6 12             	movzbl (%edx),%edx
801051c2:	88 10                	mov    %dl,(%eax)
801051c4:	0f b6 00             	movzbl (%eax),%eax
801051c7:	84 c0                	test   %al,%al
801051c9:	75 d5                	jne    801051a0 <strncpy+0xd>
    ;
  while(n-- > 0)
801051cb:	eb 0c                	jmp    801051d9 <strncpy+0x46>
    *s++ = 0;
801051cd:	8b 45 08             	mov    0x8(%ebp),%eax
801051d0:	8d 50 01             	lea    0x1(%eax),%edx
801051d3:	89 55 08             	mov    %edx,0x8(%ebp)
801051d6:	c6 00 00             	movb   $0x0,(%eax)
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
801051d9:	8b 45 10             	mov    0x10(%ebp),%eax
801051dc:	8d 50 ff             	lea    -0x1(%eax),%edx
801051df:	89 55 10             	mov    %edx,0x10(%ebp)
801051e2:	85 c0                	test   %eax,%eax
801051e4:	7f e7                	jg     801051cd <strncpy+0x3a>
    *s++ = 0;
  return os;
801051e6:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801051e9:	c9                   	leave  
801051ea:	c3                   	ret    

801051eb <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
801051eb:	55                   	push   %ebp
801051ec:	89 e5                	mov    %esp,%ebp
801051ee:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
801051f1:	8b 45 08             	mov    0x8(%ebp),%eax
801051f4:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
801051f7:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801051fb:	7f 05                	jg     80105202 <safestrcpy+0x17>
    return os;
801051fd:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105200:	eb 31                	jmp    80105233 <safestrcpy+0x48>
  while(--n > 0 && (*s++ = *t++) != 0)
80105202:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105206:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010520a:	7e 1e                	jle    8010522a <safestrcpy+0x3f>
8010520c:	8b 45 08             	mov    0x8(%ebp),%eax
8010520f:	8d 50 01             	lea    0x1(%eax),%edx
80105212:	89 55 08             	mov    %edx,0x8(%ebp)
80105215:	8b 55 0c             	mov    0xc(%ebp),%edx
80105218:	8d 4a 01             	lea    0x1(%edx),%ecx
8010521b:	89 4d 0c             	mov    %ecx,0xc(%ebp)
8010521e:	0f b6 12             	movzbl (%edx),%edx
80105221:	88 10                	mov    %dl,(%eax)
80105223:	0f b6 00             	movzbl (%eax),%eax
80105226:	84 c0                	test   %al,%al
80105228:	75 d8                	jne    80105202 <safestrcpy+0x17>
    ;
  *s = 0;
8010522a:	8b 45 08             	mov    0x8(%ebp),%eax
8010522d:	c6 00 00             	movb   $0x0,(%eax)
  return os;
80105230:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80105233:	c9                   	leave  
80105234:	c3                   	ret    

80105235 <strlen>:

int
strlen(const char *s)
{
80105235:	55                   	push   %ebp
80105236:	89 e5                	mov    %esp,%ebp
80105238:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
8010523b:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80105242:	eb 04                	jmp    80105248 <strlen+0x13>
80105244:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105248:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010524b:	8b 45 08             	mov    0x8(%ebp),%eax
8010524e:	01 d0                	add    %edx,%eax
80105250:	0f b6 00             	movzbl (%eax),%eax
80105253:	84 c0                	test   %al,%al
80105255:	75 ed                	jne    80105244 <strlen+0xf>
    ;
  return n;
80105257:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010525a:	c9                   	leave  
8010525b:	c3                   	ret    

8010525c <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
8010525c:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80105260:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80105264:	55                   	push   %ebp
  pushl %ebx
80105265:	53                   	push   %ebx
  pushl %esi
80105266:	56                   	push   %esi
  pushl %edi
80105267:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
80105268:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
8010526a:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
8010526c:	5f                   	pop    %edi
  popl %esi
8010526d:	5e                   	pop    %esi
  popl %ebx
8010526e:	5b                   	pop    %ebx
  popl %ebp
8010526f:	5d                   	pop    %ebp
  ret
80105270:	c3                   	ret    

80105271 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80105271:	55                   	push   %ebp
80105272:	89 e5                	mov    %esp,%ebp
  if(addr >= proc->sz || addr+4 > proc->sz)
80105274:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010527a:	8b 00                	mov    (%eax),%eax
8010527c:	3b 45 08             	cmp    0x8(%ebp),%eax
8010527f:	76 12                	jbe    80105293 <fetchint+0x22>
80105281:	8b 45 08             	mov    0x8(%ebp),%eax
80105284:	8d 50 04             	lea    0x4(%eax),%edx
80105287:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010528d:	8b 00                	mov    (%eax),%eax
8010528f:	39 c2                	cmp    %eax,%edx
80105291:	76 07                	jbe    8010529a <fetchint+0x29>
    return -1;
80105293:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105298:	eb 0f                	jmp    801052a9 <fetchint+0x38>
  *ip = *(int*)(addr);
8010529a:	8b 45 08             	mov    0x8(%ebp),%eax
8010529d:	8b 10                	mov    (%eax),%edx
8010529f:	8b 45 0c             	mov    0xc(%ebp),%eax
801052a2:	89 10                	mov    %edx,(%eax)
  return 0;
801052a4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801052a9:	5d                   	pop    %ebp
801052aa:	c3                   	ret    

801052ab <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
801052ab:	55                   	push   %ebp
801052ac:	89 e5                	mov    %esp,%ebp
801052ae:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= proc->sz)
801052b1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052b7:	8b 00                	mov    (%eax),%eax
801052b9:	3b 45 08             	cmp    0x8(%ebp),%eax
801052bc:	77 07                	ja     801052c5 <fetchstr+0x1a>
    return -1;
801052be:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801052c3:	eb 46                	jmp    8010530b <fetchstr+0x60>
  *pp = (char*)addr;
801052c5:	8b 55 08             	mov    0x8(%ebp),%edx
801052c8:	8b 45 0c             	mov    0xc(%ebp),%eax
801052cb:	89 10                	mov    %edx,(%eax)
  ep = (char*)proc->sz;
801052cd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801052d3:	8b 00                	mov    (%eax),%eax
801052d5:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
801052d8:	8b 45 0c             	mov    0xc(%ebp),%eax
801052db:	8b 00                	mov    (%eax),%eax
801052dd:	89 45 fc             	mov    %eax,-0x4(%ebp)
801052e0:	eb 1c                	jmp    801052fe <fetchstr+0x53>
    if(*s == 0)
801052e2:	8b 45 fc             	mov    -0x4(%ebp),%eax
801052e5:	0f b6 00             	movzbl (%eax),%eax
801052e8:	84 c0                	test   %al,%al
801052ea:	75 0e                	jne    801052fa <fetchstr+0x4f>
      return s - *pp;
801052ec:	8b 55 fc             	mov    -0x4(%ebp),%edx
801052ef:	8b 45 0c             	mov    0xc(%ebp),%eax
801052f2:	8b 00                	mov    (%eax),%eax
801052f4:	29 c2                	sub    %eax,%edx
801052f6:	89 d0                	mov    %edx,%eax
801052f8:	eb 11                	jmp    8010530b <fetchstr+0x60>

  if(addr >= proc->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)proc->sz;
  for(s = *pp; s < ep; s++)
801052fa:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801052fe:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105301:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105304:	72 dc                	jb     801052e2 <fetchstr+0x37>
    if(*s == 0)
      return s - *pp;
  return -1;
80105306:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010530b:	c9                   	leave  
8010530c:	c3                   	ret    

8010530d <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
8010530d:	55                   	push   %ebp
8010530e:	89 e5                	mov    %esp,%ebp
80105310:	83 ec 08             	sub    $0x8,%esp
  return fetchint(proc->tf->esp + 4 + 4*n, ip);
80105313:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105319:	8b 40 18             	mov    0x18(%eax),%eax
8010531c:	8b 50 44             	mov    0x44(%eax),%edx
8010531f:	8b 45 08             	mov    0x8(%ebp),%eax
80105322:	c1 e0 02             	shl    $0x2,%eax
80105325:	01 d0                	add    %edx,%eax
80105327:	8d 50 04             	lea    0x4(%eax),%edx
8010532a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010532d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105331:	89 14 24             	mov    %edx,(%esp)
80105334:	e8 38 ff ff ff       	call   80105271 <fetchint>
}
80105339:	c9                   	leave  
8010533a:	c3                   	ret    

8010533b <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
8010533b:	55                   	push   %ebp
8010533c:	89 e5                	mov    %esp,%ebp
8010533e:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
80105341:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105344:	89 44 24 04          	mov    %eax,0x4(%esp)
80105348:	8b 45 08             	mov    0x8(%ebp),%eax
8010534b:	89 04 24             	mov    %eax,(%esp)
8010534e:	e8 ba ff ff ff       	call   8010530d <argint>
80105353:	85 c0                	test   %eax,%eax
80105355:	79 07                	jns    8010535e <argptr+0x23>
    return -1;
80105357:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010535c:	eb 3d                	jmp    8010539b <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
8010535e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105361:	89 c2                	mov    %eax,%edx
80105363:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105369:	8b 00                	mov    (%eax),%eax
8010536b:	39 c2                	cmp    %eax,%edx
8010536d:	73 16                	jae    80105385 <argptr+0x4a>
8010536f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105372:	89 c2                	mov    %eax,%edx
80105374:	8b 45 10             	mov    0x10(%ebp),%eax
80105377:	01 c2                	add    %eax,%edx
80105379:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010537f:	8b 00                	mov    (%eax),%eax
80105381:	39 c2                	cmp    %eax,%edx
80105383:	76 07                	jbe    8010538c <argptr+0x51>
    return -1;
80105385:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010538a:	eb 0f                	jmp    8010539b <argptr+0x60>
  *pp = (char*)i;
8010538c:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010538f:	89 c2                	mov    %eax,%edx
80105391:	8b 45 0c             	mov    0xc(%ebp),%eax
80105394:	89 10                	mov    %edx,(%eax)
  return 0;
80105396:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010539b:	c9                   	leave  
8010539c:	c3                   	ret    

8010539d <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
8010539d:	55                   	push   %ebp
8010539e:	89 e5                	mov    %esp,%ebp
801053a0:	83 ec 18             	sub    $0x18,%esp
  int addr;
  if(argint(n, &addr) < 0)
801053a3:	8d 45 fc             	lea    -0x4(%ebp),%eax
801053a6:	89 44 24 04          	mov    %eax,0x4(%esp)
801053aa:	8b 45 08             	mov    0x8(%ebp),%eax
801053ad:	89 04 24             	mov    %eax,(%esp)
801053b0:	e8 58 ff ff ff       	call   8010530d <argint>
801053b5:	85 c0                	test   %eax,%eax
801053b7:	79 07                	jns    801053c0 <argstr+0x23>
    return -1;
801053b9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801053be:	eb 12                	jmp    801053d2 <argstr+0x35>
  return fetchstr(addr, pp);
801053c0:	8b 45 fc             	mov    -0x4(%ebp),%eax
801053c3:	8b 55 0c             	mov    0xc(%ebp),%edx
801053c6:	89 54 24 04          	mov    %edx,0x4(%esp)
801053ca:	89 04 24             	mov    %eax,(%esp)
801053cd:	e8 d9 fe ff ff       	call   801052ab <fetchstr>
}
801053d2:	c9                   	leave  
801053d3:	c3                   	ret    

801053d4 <syscall>:
[SYS_kthread_cond_broadcast] sys_kthread_cond_broadcast,
};

void
syscall(void)
{
801053d4:	55                   	push   %ebp
801053d5:	89 e5                	mov    %esp,%ebp
801053d7:	53                   	push   %ebx
801053d8:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
801053db:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801053e1:	8b 40 18             	mov    0x18(%eax),%eax
801053e4:	8b 40 1c             	mov    0x1c(%eax),%eax
801053e7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
801053ea:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801053ee:	7e 30                	jle    80105420 <syscall+0x4c>
801053f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053f3:	83 f8 20             	cmp    $0x20,%eax
801053f6:	77 28                	ja     80105420 <syscall+0x4c>
801053f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053fb:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105402:	85 c0                	test   %eax,%eax
80105404:	74 1a                	je     80105420 <syscall+0x4c>
    proc->tf->eax = syscalls[num]();
80105406:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010540c:	8b 58 18             	mov    0x18(%eax),%ebx
8010540f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105412:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105419:	ff d0                	call   *%eax
8010541b:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010541e:	eb 3d                	jmp    8010545d <syscall+0x89>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
80105420:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105426:	8d 48 6c             	lea    0x6c(%eax),%ecx
80105429:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax

  num = proc->tf->eax;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
8010542f:	8b 40 10             	mov    0x10(%eax),%eax
80105432:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105435:	89 54 24 0c          	mov    %edx,0xc(%esp)
80105439:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010543d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105441:	c7 04 24 73 8b 10 80 	movl   $0x80108b73,(%esp)
80105448:	e8 53 af ff ff       	call   801003a0 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
8010544d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105453:	8b 40 18             	mov    0x18(%eax),%eax
80105456:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
8010545d:	83 c4 24             	add    $0x24,%esp
80105460:	5b                   	pop    %ebx
80105461:	5d                   	pop    %ebp
80105462:	c3                   	ret    

80105463 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
80105463:	55                   	push   %ebp
80105464:	89 e5                	mov    %esp,%ebp
80105466:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
80105469:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010546c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105470:	8b 45 08             	mov    0x8(%ebp),%eax
80105473:	89 04 24             	mov    %eax,(%esp)
80105476:	e8 92 fe ff ff       	call   8010530d <argint>
8010547b:	85 c0                	test   %eax,%eax
8010547d:	79 07                	jns    80105486 <argfd+0x23>
    return -1;
8010547f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105484:	eb 50                	jmp    801054d6 <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
80105486:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105489:	85 c0                	test   %eax,%eax
8010548b:	78 21                	js     801054ae <argfd+0x4b>
8010548d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105490:	83 f8 0f             	cmp    $0xf,%eax
80105493:	7f 19                	jg     801054ae <argfd+0x4b>
80105495:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010549b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010549e:	83 c2 08             	add    $0x8,%edx
801054a1:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801054a5:	89 45 f4             	mov    %eax,-0xc(%ebp)
801054a8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801054ac:	75 07                	jne    801054b5 <argfd+0x52>
    return -1;
801054ae:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054b3:	eb 21                	jmp    801054d6 <argfd+0x73>
  if(pfd)
801054b5:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801054b9:	74 08                	je     801054c3 <argfd+0x60>
    *pfd = fd;
801054bb:	8b 55 f0             	mov    -0x10(%ebp),%edx
801054be:	8b 45 0c             	mov    0xc(%ebp),%eax
801054c1:	89 10                	mov    %edx,(%eax)
  if(pf)
801054c3:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801054c7:	74 08                	je     801054d1 <argfd+0x6e>
    *pf = f;
801054c9:	8b 45 10             	mov    0x10(%ebp),%eax
801054cc:	8b 55 f4             	mov    -0xc(%ebp),%edx
801054cf:	89 10                	mov    %edx,(%eax)
  return 0;
801054d1:	b8 00 00 00 00       	mov    $0x0,%eax
}
801054d6:	c9                   	leave  
801054d7:	c3                   	ret    

801054d8 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
801054d8:	55                   	push   %ebp
801054d9:	89 e5                	mov    %esp,%ebp
801054db:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
801054de:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801054e5:	eb 30                	jmp    80105517 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
801054e7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801054ed:	8b 55 fc             	mov    -0x4(%ebp),%edx
801054f0:	83 c2 08             	add    $0x8,%edx
801054f3:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801054f7:	85 c0                	test   %eax,%eax
801054f9:	75 18                	jne    80105513 <fdalloc+0x3b>
      proc->ofile[fd] = f;
801054fb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105501:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105504:	8d 4a 08             	lea    0x8(%edx),%ecx
80105507:	8b 55 08             	mov    0x8(%ebp),%edx
8010550a:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
8010550e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105511:	eb 0f                	jmp    80105522 <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105513:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105517:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
8010551b:	7e ca                	jle    801054e7 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
8010551d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105522:	c9                   	leave  
80105523:	c3                   	ret    

80105524 <sys_dup>:

int
sys_dup(void)
{
80105524:	55                   	push   %ebp
80105525:	89 e5                	mov    %esp,%ebp
80105527:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
8010552a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010552d:	89 44 24 08          	mov    %eax,0x8(%esp)
80105531:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105538:	00 
80105539:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105540:	e8 1e ff ff ff       	call   80105463 <argfd>
80105545:	85 c0                	test   %eax,%eax
80105547:	79 07                	jns    80105550 <sys_dup+0x2c>
    return -1;
80105549:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010554e:	eb 29                	jmp    80105579 <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
80105550:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105553:	89 04 24             	mov    %eax,(%esp)
80105556:	e8 7d ff ff ff       	call   801054d8 <fdalloc>
8010555b:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010555e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105562:	79 07                	jns    8010556b <sys_dup+0x47>
    return -1;
80105564:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105569:	eb 0e                	jmp    80105579 <sys_dup+0x55>
  filedup(f);
8010556b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010556e:	89 04 24             	mov    %eax,(%esp)
80105571:	e8 10 ba ff ff       	call   80100f86 <filedup>
  return fd;
80105576:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80105579:	c9                   	leave  
8010557a:	c3                   	ret    

8010557b <sys_read>:

int
sys_read(void)
{
8010557b:	55                   	push   %ebp
8010557c:	89 e5                	mov    %esp,%ebp
8010557e:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105581:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105584:	89 44 24 08          	mov    %eax,0x8(%esp)
80105588:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010558f:	00 
80105590:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105597:	e8 c7 fe ff ff       	call   80105463 <argfd>
8010559c:	85 c0                	test   %eax,%eax
8010559e:	78 35                	js     801055d5 <sys_read+0x5a>
801055a0:	8d 45 f0             	lea    -0x10(%ebp),%eax
801055a3:	89 44 24 04          	mov    %eax,0x4(%esp)
801055a7:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801055ae:	e8 5a fd ff ff       	call   8010530d <argint>
801055b3:	85 c0                	test   %eax,%eax
801055b5:	78 1e                	js     801055d5 <sys_read+0x5a>
801055b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801055ba:	89 44 24 08          	mov    %eax,0x8(%esp)
801055be:	8d 45 ec             	lea    -0x14(%ebp),%eax
801055c1:	89 44 24 04          	mov    %eax,0x4(%esp)
801055c5:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801055cc:	e8 6a fd ff ff       	call   8010533b <argptr>
801055d1:	85 c0                	test   %eax,%eax
801055d3:	79 07                	jns    801055dc <sys_read+0x61>
    return -1;
801055d5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801055da:	eb 19                	jmp    801055f5 <sys_read+0x7a>
  return fileread(f, p, n);
801055dc:	8b 4d f0             	mov    -0x10(%ebp),%ecx
801055df:	8b 55 ec             	mov    -0x14(%ebp),%edx
801055e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055e5:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801055e9:	89 54 24 04          	mov    %edx,0x4(%esp)
801055ed:	89 04 24             	mov    %eax,(%esp)
801055f0:	e8 fe ba ff ff       	call   801010f3 <fileread>
}
801055f5:	c9                   	leave  
801055f6:	c3                   	ret    

801055f7 <sys_write>:

int
sys_write(void)
{
801055f7:	55                   	push   %ebp
801055f8:	89 e5                	mov    %esp,%ebp
801055fa:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801055fd:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105600:	89 44 24 08          	mov    %eax,0x8(%esp)
80105604:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010560b:	00 
8010560c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105613:	e8 4b fe ff ff       	call   80105463 <argfd>
80105618:	85 c0                	test   %eax,%eax
8010561a:	78 35                	js     80105651 <sys_write+0x5a>
8010561c:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010561f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105623:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010562a:	e8 de fc ff ff       	call   8010530d <argint>
8010562f:	85 c0                	test   %eax,%eax
80105631:	78 1e                	js     80105651 <sys_write+0x5a>
80105633:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105636:	89 44 24 08          	mov    %eax,0x8(%esp)
8010563a:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010563d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105641:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105648:	e8 ee fc ff ff       	call   8010533b <argptr>
8010564d:	85 c0                	test   %eax,%eax
8010564f:	79 07                	jns    80105658 <sys_write+0x61>
    return -1;
80105651:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105656:	eb 19                	jmp    80105671 <sys_write+0x7a>
  return filewrite(f, p, n);
80105658:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010565b:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010565e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105661:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105665:	89 54 24 04          	mov    %edx,0x4(%esp)
80105669:	89 04 24             	mov    %eax,(%esp)
8010566c:	e8 3e bb ff ff       	call   801011af <filewrite>
}
80105671:	c9                   	leave  
80105672:	c3                   	ret    

80105673 <sys_close>:

int
sys_close(void)
{
80105673:	55                   	push   %ebp
80105674:	89 e5                	mov    %esp,%ebp
80105676:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
80105679:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010567c:	89 44 24 08          	mov    %eax,0x8(%esp)
80105680:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105683:	89 44 24 04          	mov    %eax,0x4(%esp)
80105687:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010568e:	e8 d0 fd ff ff       	call   80105463 <argfd>
80105693:	85 c0                	test   %eax,%eax
80105695:	79 07                	jns    8010569e <sys_close+0x2b>
    return -1;
80105697:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010569c:	eb 24                	jmp    801056c2 <sys_close+0x4f>
  proc->ofile[fd] = 0;
8010569e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801056a4:	8b 55 f4             	mov    -0xc(%ebp),%edx
801056a7:	83 c2 08             	add    $0x8,%edx
801056aa:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801056b1:	00 
  fileclose(f);
801056b2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801056b5:	89 04 24             	mov    %eax,(%esp)
801056b8:	e8 11 b9 ff ff       	call   80100fce <fileclose>
  return 0;
801056bd:	b8 00 00 00 00       	mov    $0x0,%eax
}
801056c2:	c9                   	leave  
801056c3:	c3                   	ret    

801056c4 <sys_fstat>:

int
sys_fstat(void)
{
801056c4:	55                   	push   %ebp
801056c5:	89 e5                	mov    %esp,%ebp
801056c7:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
801056ca:	8d 45 f4             	lea    -0xc(%ebp),%eax
801056cd:	89 44 24 08          	mov    %eax,0x8(%esp)
801056d1:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801056d8:	00 
801056d9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801056e0:	e8 7e fd ff ff       	call   80105463 <argfd>
801056e5:	85 c0                	test   %eax,%eax
801056e7:	78 1f                	js     80105708 <sys_fstat+0x44>
801056e9:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801056f0:	00 
801056f1:	8d 45 f0             	lea    -0x10(%ebp),%eax
801056f4:	89 44 24 04          	mov    %eax,0x4(%esp)
801056f8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801056ff:	e8 37 fc ff ff       	call   8010533b <argptr>
80105704:	85 c0                	test   %eax,%eax
80105706:	79 07                	jns    8010570f <sys_fstat+0x4b>
    return -1;
80105708:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010570d:	eb 12                	jmp    80105721 <sys_fstat+0x5d>
  return filestat(f, st);
8010570f:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105712:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105715:	89 54 24 04          	mov    %edx,0x4(%esp)
80105719:	89 04 24             	mov    %eax,(%esp)
8010571c:	e8 83 b9 ff ff       	call   801010a4 <filestat>
}
80105721:	c9                   	leave  
80105722:	c3                   	ret    

80105723 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
80105723:	55                   	push   %ebp
80105724:	89 e5                	mov    %esp,%ebp
80105726:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105729:	8d 45 d8             	lea    -0x28(%ebp),%eax
8010572c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105730:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105737:	e8 61 fc ff ff       	call   8010539d <argstr>
8010573c:	85 c0                	test   %eax,%eax
8010573e:	78 17                	js     80105757 <sys_link+0x34>
80105740:	8d 45 dc             	lea    -0x24(%ebp),%eax
80105743:	89 44 24 04          	mov    %eax,0x4(%esp)
80105747:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010574e:	e8 4a fc ff ff       	call   8010539d <argstr>
80105753:	85 c0                	test   %eax,%eax
80105755:	79 0a                	jns    80105761 <sys_link+0x3e>
    return -1;
80105757:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010575c:	e9 42 01 00 00       	jmp    801058a3 <sys_link+0x180>

  begin_op();
80105761:	e8 7f da ff ff       	call   801031e5 <begin_op>
  if((ip = namei(old)) == 0){
80105766:	8b 45 d8             	mov    -0x28(%ebp),%eax
80105769:	89 04 24             	mov    %eax,(%esp)
8010576c:	e8 95 cc ff ff       	call   80102406 <namei>
80105771:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105774:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105778:	75 0f                	jne    80105789 <sys_link+0x66>
    end_op();
8010577a:	e8 ea da ff ff       	call   80103269 <end_op>
    return -1;
8010577f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105784:	e9 1a 01 00 00       	jmp    801058a3 <sys_link+0x180>
  }

  ilock(ip);
80105789:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010578c:	89 04 24             	mov    %eax,(%esp)
8010578f:	e8 c7 c0 ff ff       	call   8010185b <ilock>
  if(ip->type == T_DIR){
80105794:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105797:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010579b:	66 83 f8 01          	cmp    $0x1,%ax
8010579f:	75 1a                	jne    801057bb <sys_link+0x98>
    iunlockput(ip);
801057a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057a4:	89 04 24             	mov    %eax,(%esp)
801057a7:	e8 33 c3 ff ff       	call   80101adf <iunlockput>
    end_op();
801057ac:	e8 b8 da ff ff       	call   80103269 <end_op>
    return -1;
801057b1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057b6:	e9 e8 00 00 00       	jmp    801058a3 <sys_link+0x180>
  }

  ip->nlink++;
801057bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057be:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801057c2:	8d 50 01             	lea    0x1(%eax),%edx
801057c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057c8:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
801057cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057cf:	89 04 24             	mov    %eax,(%esp)
801057d2:	e8 c8 be ff ff       	call   8010169f <iupdate>
  iunlock(ip);
801057d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057da:	89 04 24             	mov    %eax,(%esp)
801057dd:	e8 c7 c1 ff ff       	call   801019a9 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
801057e2:	8b 45 dc             	mov    -0x24(%ebp),%eax
801057e5:	8d 55 e2             	lea    -0x1e(%ebp),%edx
801057e8:	89 54 24 04          	mov    %edx,0x4(%esp)
801057ec:	89 04 24             	mov    %eax,(%esp)
801057ef:	e8 34 cc ff ff       	call   80102428 <nameiparent>
801057f4:	89 45 f0             	mov    %eax,-0x10(%ebp)
801057f7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801057fb:	75 02                	jne    801057ff <sys_link+0xdc>
    goto bad;
801057fd:	eb 68                	jmp    80105867 <sys_link+0x144>
  ilock(dp);
801057ff:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105802:	89 04 24             	mov    %eax,(%esp)
80105805:	e8 51 c0 ff ff       	call   8010185b <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
8010580a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010580d:	8b 10                	mov    (%eax),%edx
8010580f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105812:	8b 00                	mov    (%eax),%eax
80105814:	39 c2                	cmp    %eax,%edx
80105816:	75 20                	jne    80105838 <sys_link+0x115>
80105818:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010581b:	8b 40 04             	mov    0x4(%eax),%eax
8010581e:	89 44 24 08          	mov    %eax,0x8(%esp)
80105822:	8d 45 e2             	lea    -0x1e(%ebp),%eax
80105825:	89 44 24 04          	mov    %eax,0x4(%esp)
80105829:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010582c:	89 04 24             	mov    %eax,(%esp)
8010582f:	e8 12 c9 ff ff       	call   80102146 <dirlink>
80105834:	85 c0                	test   %eax,%eax
80105836:	79 0d                	jns    80105845 <sys_link+0x122>
    iunlockput(dp);
80105838:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010583b:	89 04 24             	mov    %eax,(%esp)
8010583e:	e8 9c c2 ff ff       	call   80101adf <iunlockput>
    goto bad;
80105843:	eb 22                	jmp    80105867 <sys_link+0x144>
  }
  iunlockput(dp);
80105845:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105848:	89 04 24             	mov    %eax,(%esp)
8010584b:	e8 8f c2 ff ff       	call   80101adf <iunlockput>
  iput(ip);
80105850:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105853:	89 04 24             	mov    %eax,(%esp)
80105856:	e8 b3 c1 ff ff       	call   80101a0e <iput>

  end_op();
8010585b:	e8 09 da ff ff       	call   80103269 <end_op>

  return 0;
80105860:	b8 00 00 00 00       	mov    $0x0,%eax
80105865:	eb 3c                	jmp    801058a3 <sys_link+0x180>

bad:
  ilock(ip);
80105867:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010586a:	89 04 24             	mov    %eax,(%esp)
8010586d:	e8 e9 bf ff ff       	call   8010185b <ilock>
  ip->nlink--;
80105872:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105875:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105879:	8d 50 ff             	lea    -0x1(%eax),%edx
8010587c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010587f:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105883:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105886:	89 04 24             	mov    %eax,(%esp)
80105889:	e8 11 be ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
8010588e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105891:	89 04 24             	mov    %eax,(%esp)
80105894:	e8 46 c2 ff ff       	call   80101adf <iunlockput>
  end_op();
80105899:	e8 cb d9 ff ff       	call   80103269 <end_op>
  return -1;
8010589e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801058a3:	c9                   	leave  
801058a4:	c3                   	ret    

801058a5 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
801058a5:	55                   	push   %ebp
801058a6:	89 e5                	mov    %esp,%ebp
801058a8:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
801058ab:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
801058b2:	eb 4b                	jmp    801058ff <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801058b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058b7:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801058be:	00 
801058bf:	89 44 24 08          	mov    %eax,0x8(%esp)
801058c3:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801058c6:	89 44 24 04          	mov    %eax,0x4(%esp)
801058ca:	8b 45 08             	mov    0x8(%ebp),%eax
801058cd:	89 04 24             	mov    %eax,(%esp)
801058d0:	e8 93 c4 ff ff       	call   80101d68 <readi>
801058d5:	83 f8 10             	cmp    $0x10,%eax
801058d8:	74 0c                	je     801058e6 <isdirempty+0x41>
      panic("isdirempty: readi");
801058da:	c7 04 24 8f 8b 10 80 	movl   $0x80108b8f,(%esp)
801058e1:	e8 54 ac ff ff       	call   8010053a <panic>
    if(de.inum != 0)
801058e6:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
801058ea:	66 85 c0             	test   %ax,%ax
801058ed:	74 07                	je     801058f6 <isdirempty+0x51>
      return 0;
801058ef:	b8 00 00 00 00       	mov    $0x0,%eax
801058f4:	eb 1b                	jmp    80105911 <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
801058f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058f9:	83 c0 10             	add    $0x10,%eax
801058fc:	89 45 f4             	mov    %eax,-0xc(%ebp)
801058ff:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105902:	8b 45 08             	mov    0x8(%ebp),%eax
80105905:	8b 40 18             	mov    0x18(%eax),%eax
80105908:	39 c2                	cmp    %eax,%edx
8010590a:	72 a8                	jb     801058b4 <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
8010590c:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105911:	c9                   	leave  
80105912:	c3                   	ret    

80105913 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105913:	55                   	push   %ebp
80105914:	89 e5                	mov    %esp,%ebp
80105916:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80105919:	8d 45 cc             	lea    -0x34(%ebp),%eax
8010591c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105920:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105927:	e8 71 fa ff ff       	call   8010539d <argstr>
8010592c:	85 c0                	test   %eax,%eax
8010592e:	79 0a                	jns    8010593a <sys_unlink+0x27>
    return -1;
80105930:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105935:	e9 af 01 00 00       	jmp    80105ae9 <sys_unlink+0x1d6>

  begin_op();
8010593a:	e8 a6 d8 ff ff       	call   801031e5 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
8010593f:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105942:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105945:	89 54 24 04          	mov    %edx,0x4(%esp)
80105949:	89 04 24             	mov    %eax,(%esp)
8010594c:	e8 d7 ca ff ff       	call   80102428 <nameiparent>
80105951:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105954:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105958:	75 0f                	jne    80105969 <sys_unlink+0x56>
    end_op();
8010595a:	e8 0a d9 ff ff       	call   80103269 <end_op>
    return -1;
8010595f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105964:	e9 80 01 00 00       	jmp    80105ae9 <sys_unlink+0x1d6>
  }

  ilock(dp);
80105969:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010596c:	89 04 24             	mov    %eax,(%esp)
8010596f:	e8 e7 be ff ff       	call   8010185b <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105974:	c7 44 24 04 a1 8b 10 	movl   $0x80108ba1,0x4(%esp)
8010597b:	80 
8010597c:	8d 45 d2             	lea    -0x2e(%ebp),%eax
8010597f:	89 04 24             	mov    %eax,(%esp)
80105982:	e8 d4 c6 ff ff       	call   8010205b <namecmp>
80105987:	85 c0                	test   %eax,%eax
80105989:	0f 84 45 01 00 00    	je     80105ad4 <sys_unlink+0x1c1>
8010598f:	c7 44 24 04 a3 8b 10 	movl   $0x80108ba3,0x4(%esp)
80105996:	80 
80105997:	8d 45 d2             	lea    -0x2e(%ebp),%eax
8010599a:	89 04 24             	mov    %eax,(%esp)
8010599d:	e8 b9 c6 ff ff       	call   8010205b <namecmp>
801059a2:	85 c0                	test   %eax,%eax
801059a4:	0f 84 2a 01 00 00    	je     80105ad4 <sys_unlink+0x1c1>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
801059aa:	8d 45 c8             	lea    -0x38(%ebp),%eax
801059ad:	89 44 24 08          	mov    %eax,0x8(%esp)
801059b1:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801059b4:	89 44 24 04          	mov    %eax,0x4(%esp)
801059b8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059bb:	89 04 24             	mov    %eax,(%esp)
801059be:	e8 ba c6 ff ff       	call   8010207d <dirlookup>
801059c3:	89 45 f0             	mov    %eax,-0x10(%ebp)
801059c6:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801059ca:	75 05                	jne    801059d1 <sys_unlink+0xbe>
    goto bad;
801059cc:	e9 03 01 00 00       	jmp    80105ad4 <sys_unlink+0x1c1>
  ilock(ip);
801059d1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059d4:	89 04 24             	mov    %eax,(%esp)
801059d7:	e8 7f be ff ff       	call   8010185b <ilock>

  if(ip->nlink < 1)
801059dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059df:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801059e3:	66 85 c0             	test   %ax,%ax
801059e6:	7f 0c                	jg     801059f4 <sys_unlink+0xe1>
    panic("unlink: nlink < 1");
801059e8:	c7 04 24 a6 8b 10 80 	movl   $0x80108ba6,(%esp)
801059ef:	e8 46 ab ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
801059f4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059f7:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801059fb:	66 83 f8 01          	cmp    $0x1,%ax
801059ff:	75 1f                	jne    80105a20 <sys_unlink+0x10d>
80105a01:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a04:	89 04 24             	mov    %eax,(%esp)
80105a07:	e8 99 fe ff ff       	call   801058a5 <isdirempty>
80105a0c:	85 c0                	test   %eax,%eax
80105a0e:	75 10                	jne    80105a20 <sys_unlink+0x10d>
    iunlockput(ip);
80105a10:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a13:	89 04 24             	mov    %eax,(%esp)
80105a16:	e8 c4 c0 ff ff       	call   80101adf <iunlockput>
    goto bad;
80105a1b:	e9 b4 00 00 00       	jmp    80105ad4 <sys_unlink+0x1c1>
  }

  memset(&de, 0, sizeof(de));
80105a20:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105a27:	00 
80105a28:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105a2f:	00 
80105a30:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105a33:	89 04 24             	mov    %eax,(%esp)
80105a36:	e8 90 f5 ff ff       	call   80104fcb <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105a3b:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105a3e:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105a45:	00 
80105a46:	89 44 24 08          	mov    %eax,0x8(%esp)
80105a4a:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105a4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a51:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a54:	89 04 24             	mov    %eax,(%esp)
80105a57:	e8 70 c4 ff ff       	call   80101ecc <writei>
80105a5c:	83 f8 10             	cmp    $0x10,%eax
80105a5f:	74 0c                	je     80105a6d <sys_unlink+0x15a>
    panic("unlink: writei");
80105a61:	c7 04 24 b8 8b 10 80 	movl   $0x80108bb8,(%esp)
80105a68:	e8 cd aa ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR){
80105a6d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a70:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105a74:	66 83 f8 01          	cmp    $0x1,%ax
80105a78:	75 1c                	jne    80105a96 <sys_unlink+0x183>
    dp->nlink--;
80105a7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a7d:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105a81:	8d 50 ff             	lea    -0x1(%eax),%edx
80105a84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a87:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105a8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a8e:	89 04 24             	mov    %eax,(%esp)
80105a91:	e8 09 bc ff ff       	call   8010169f <iupdate>
  }
  iunlockput(dp);
80105a96:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a99:	89 04 24             	mov    %eax,(%esp)
80105a9c:	e8 3e c0 ff ff       	call   80101adf <iunlockput>

  ip->nlink--;
80105aa1:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105aa4:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105aa8:	8d 50 ff             	lea    -0x1(%eax),%edx
80105aab:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105aae:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105ab2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ab5:	89 04 24             	mov    %eax,(%esp)
80105ab8:	e8 e2 bb ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
80105abd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ac0:	89 04 24             	mov    %eax,(%esp)
80105ac3:	e8 17 c0 ff ff       	call   80101adf <iunlockput>

  end_op();
80105ac8:	e8 9c d7 ff ff       	call   80103269 <end_op>

  return 0;
80105acd:	b8 00 00 00 00       	mov    $0x0,%eax
80105ad2:	eb 15                	jmp    80105ae9 <sys_unlink+0x1d6>

bad:
  iunlockput(dp);
80105ad4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ad7:	89 04 24             	mov    %eax,(%esp)
80105ada:	e8 00 c0 ff ff       	call   80101adf <iunlockput>
  end_op();
80105adf:	e8 85 d7 ff ff       	call   80103269 <end_op>
  return -1;
80105ae4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105ae9:	c9                   	leave  
80105aea:	c3                   	ret    

80105aeb <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105aeb:	55                   	push   %ebp
80105aec:	89 e5                	mov    %esp,%ebp
80105aee:	83 ec 48             	sub    $0x48,%esp
80105af1:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105af4:	8b 55 10             	mov    0x10(%ebp),%edx
80105af7:	8b 45 14             	mov    0x14(%ebp),%eax
80105afa:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105afe:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105b02:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105b06:	8d 45 de             	lea    -0x22(%ebp),%eax
80105b09:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b0d:	8b 45 08             	mov    0x8(%ebp),%eax
80105b10:	89 04 24             	mov    %eax,(%esp)
80105b13:	e8 10 c9 ff ff       	call   80102428 <nameiparent>
80105b18:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105b1b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105b1f:	75 0a                	jne    80105b2b <create+0x40>
    return 0;
80105b21:	b8 00 00 00 00       	mov    $0x0,%eax
80105b26:	e9 7e 01 00 00       	jmp    80105ca9 <create+0x1be>
  ilock(dp);
80105b2b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b2e:	89 04 24             	mov    %eax,(%esp)
80105b31:	e8 25 bd ff ff       	call   8010185b <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
80105b36:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105b39:	89 44 24 08          	mov    %eax,0x8(%esp)
80105b3d:	8d 45 de             	lea    -0x22(%ebp),%eax
80105b40:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b44:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b47:	89 04 24             	mov    %eax,(%esp)
80105b4a:	e8 2e c5 ff ff       	call   8010207d <dirlookup>
80105b4f:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105b52:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105b56:	74 47                	je     80105b9f <create+0xb4>
    iunlockput(dp);
80105b58:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b5b:	89 04 24             	mov    %eax,(%esp)
80105b5e:	e8 7c bf ff ff       	call   80101adf <iunlockput>
    ilock(ip);
80105b63:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b66:	89 04 24             	mov    %eax,(%esp)
80105b69:	e8 ed bc ff ff       	call   8010185b <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105b6e:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105b73:	75 15                	jne    80105b8a <create+0x9f>
80105b75:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b78:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105b7c:	66 83 f8 02          	cmp    $0x2,%ax
80105b80:	75 08                	jne    80105b8a <create+0x9f>
      return ip;
80105b82:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b85:	e9 1f 01 00 00       	jmp    80105ca9 <create+0x1be>
    iunlockput(ip);
80105b8a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b8d:	89 04 24             	mov    %eax,(%esp)
80105b90:	e8 4a bf ff ff       	call   80101adf <iunlockput>
    return 0;
80105b95:	b8 00 00 00 00       	mov    $0x0,%eax
80105b9a:	e9 0a 01 00 00       	jmp    80105ca9 <create+0x1be>
  }

  if((ip = ialloc(dp->dev, type)) == 0)
80105b9f:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80105ba3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ba6:	8b 00                	mov    (%eax),%eax
80105ba8:	89 54 24 04          	mov    %edx,0x4(%esp)
80105bac:	89 04 24             	mov    %eax,(%esp)
80105baf:	e8 0c ba ff ff       	call   801015c0 <ialloc>
80105bb4:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105bb7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105bbb:	75 0c                	jne    80105bc9 <create+0xde>
    panic("create: ialloc");
80105bbd:	c7 04 24 c7 8b 10 80 	movl   $0x80108bc7,(%esp)
80105bc4:	e8 71 a9 ff ff       	call   8010053a <panic>

  ilock(ip);
80105bc9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105bcc:	89 04 24             	mov    %eax,(%esp)
80105bcf:	e8 87 bc ff ff       	call   8010185b <ilock>
  ip->major = major;
80105bd4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105bd7:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80105bdb:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80105bdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105be2:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80105be6:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80105bea:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105bed:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80105bf3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105bf6:	89 04 24             	mov    %eax,(%esp)
80105bf9:	e8 a1 ba ff ff       	call   8010169f <iupdate>

  if(type == T_DIR){  // Create . and .. entries.
80105bfe:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105c03:	75 6a                	jne    80105c6f <create+0x184>
    dp->nlink++;  // for ".."
80105c05:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c08:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105c0c:	8d 50 01             	lea    0x1(%eax),%edx
80105c0f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c12:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105c16:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c19:	89 04 24             	mov    %eax,(%esp)
80105c1c:	e8 7e ba ff ff       	call   8010169f <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105c21:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c24:	8b 40 04             	mov    0x4(%eax),%eax
80105c27:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c2b:	c7 44 24 04 a1 8b 10 	movl   $0x80108ba1,0x4(%esp)
80105c32:	80 
80105c33:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c36:	89 04 24             	mov    %eax,(%esp)
80105c39:	e8 08 c5 ff ff       	call   80102146 <dirlink>
80105c3e:	85 c0                	test   %eax,%eax
80105c40:	78 21                	js     80105c63 <create+0x178>
80105c42:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c45:	8b 40 04             	mov    0x4(%eax),%eax
80105c48:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c4c:	c7 44 24 04 a3 8b 10 	movl   $0x80108ba3,0x4(%esp)
80105c53:	80 
80105c54:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c57:	89 04 24             	mov    %eax,(%esp)
80105c5a:	e8 e7 c4 ff ff       	call   80102146 <dirlink>
80105c5f:	85 c0                	test   %eax,%eax
80105c61:	79 0c                	jns    80105c6f <create+0x184>
      panic("create dots");
80105c63:	c7 04 24 d6 8b 10 80 	movl   $0x80108bd6,(%esp)
80105c6a:	e8 cb a8 ff ff       	call   8010053a <panic>
  }

  if(dirlink(dp, name, ip->inum) < 0)
80105c6f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c72:	8b 40 04             	mov    0x4(%eax),%eax
80105c75:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c79:	8d 45 de             	lea    -0x22(%ebp),%eax
80105c7c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c80:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c83:	89 04 24             	mov    %eax,(%esp)
80105c86:	e8 bb c4 ff ff       	call   80102146 <dirlink>
80105c8b:	85 c0                	test   %eax,%eax
80105c8d:	79 0c                	jns    80105c9b <create+0x1b0>
    panic("create: dirlink");
80105c8f:	c7 04 24 e2 8b 10 80 	movl   $0x80108be2,(%esp)
80105c96:	e8 9f a8 ff ff       	call   8010053a <panic>

  iunlockput(dp);
80105c9b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c9e:	89 04 24             	mov    %eax,(%esp)
80105ca1:	e8 39 be ff ff       	call   80101adf <iunlockput>

  return ip;
80105ca6:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105ca9:	c9                   	leave  
80105caa:	c3                   	ret    

80105cab <sys_open>:

int
sys_open(void)
{
80105cab:	55                   	push   %ebp
80105cac:	89 e5                	mov    %esp,%ebp
80105cae:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105cb1:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105cb4:	89 44 24 04          	mov    %eax,0x4(%esp)
80105cb8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105cbf:	e8 d9 f6 ff ff       	call   8010539d <argstr>
80105cc4:	85 c0                	test   %eax,%eax
80105cc6:	78 17                	js     80105cdf <sys_open+0x34>
80105cc8:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105ccb:	89 44 24 04          	mov    %eax,0x4(%esp)
80105ccf:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105cd6:	e8 32 f6 ff ff       	call   8010530d <argint>
80105cdb:	85 c0                	test   %eax,%eax
80105cdd:	79 0a                	jns    80105ce9 <sys_open+0x3e>
    return -1;
80105cdf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105ce4:	e9 5c 01 00 00       	jmp    80105e45 <sys_open+0x19a>

  begin_op();
80105ce9:	e8 f7 d4 ff ff       	call   801031e5 <begin_op>

  if(omode & O_CREATE){
80105cee:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105cf1:	25 00 02 00 00       	and    $0x200,%eax
80105cf6:	85 c0                	test   %eax,%eax
80105cf8:	74 3b                	je     80105d35 <sys_open+0x8a>
    ip = create(path, T_FILE, 0, 0);
80105cfa:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105cfd:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105d04:	00 
80105d05:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105d0c:	00 
80105d0d:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105d14:	00 
80105d15:	89 04 24             	mov    %eax,(%esp)
80105d18:	e8 ce fd ff ff       	call   80105aeb <create>
80105d1d:	89 45 f4             	mov    %eax,-0xc(%ebp)
    if(ip == 0){
80105d20:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105d24:	75 6b                	jne    80105d91 <sys_open+0xe6>
      end_op();
80105d26:	e8 3e d5 ff ff       	call   80103269 <end_op>
      return -1;
80105d2b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d30:	e9 10 01 00 00       	jmp    80105e45 <sys_open+0x19a>
    }
  } else {
    if((ip = namei(path)) == 0){
80105d35:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105d38:	89 04 24             	mov    %eax,(%esp)
80105d3b:	e8 c6 c6 ff ff       	call   80102406 <namei>
80105d40:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105d43:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105d47:	75 0f                	jne    80105d58 <sys_open+0xad>
      end_op();
80105d49:	e8 1b d5 ff ff       	call   80103269 <end_op>
      return -1;
80105d4e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d53:	e9 ed 00 00 00       	jmp    80105e45 <sys_open+0x19a>
    }
    ilock(ip);
80105d58:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d5b:	89 04 24             	mov    %eax,(%esp)
80105d5e:	e8 f8 ba ff ff       	call   8010185b <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105d63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d66:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105d6a:	66 83 f8 01          	cmp    $0x1,%ax
80105d6e:	75 21                	jne    80105d91 <sys_open+0xe6>
80105d70:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105d73:	85 c0                	test   %eax,%eax
80105d75:	74 1a                	je     80105d91 <sys_open+0xe6>
      iunlockput(ip);
80105d77:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d7a:	89 04 24             	mov    %eax,(%esp)
80105d7d:	e8 5d bd ff ff       	call   80101adf <iunlockput>
      end_op();
80105d82:	e8 e2 d4 ff ff       	call   80103269 <end_op>
      return -1;
80105d87:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d8c:	e9 b4 00 00 00       	jmp    80105e45 <sys_open+0x19a>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80105d91:	e8 90 b1 ff ff       	call   80100f26 <filealloc>
80105d96:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105d99:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105d9d:	74 14                	je     80105db3 <sys_open+0x108>
80105d9f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105da2:	89 04 24             	mov    %eax,(%esp)
80105da5:	e8 2e f7 ff ff       	call   801054d8 <fdalloc>
80105daa:	89 45 ec             	mov    %eax,-0x14(%ebp)
80105dad:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80105db1:	79 28                	jns    80105ddb <sys_open+0x130>
    if(f)
80105db3:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105db7:	74 0b                	je     80105dc4 <sys_open+0x119>
      fileclose(f);
80105db9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dbc:	89 04 24             	mov    %eax,(%esp)
80105dbf:	e8 0a b2 ff ff       	call   80100fce <fileclose>
    iunlockput(ip);
80105dc4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dc7:	89 04 24             	mov    %eax,(%esp)
80105dca:	e8 10 bd ff ff       	call   80101adf <iunlockput>
    end_op();
80105dcf:	e8 95 d4 ff ff       	call   80103269 <end_op>
    return -1;
80105dd4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dd9:	eb 6a                	jmp    80105e45 <sys_open+0x19a>
  }
  iunlock(ip);
80105ddb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dde:	89 04 24             	mov    %eax,(%esp)
80105de1:	e8 c3 bb ff ff       	call   801019a9 <iunlock>
  end_op();
80105de6:	e8 7e d4 ff ff       	call   80103269 <end_op>

  f->type = FD_INODE;
80105deb:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dee:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80105df4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105df7:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105dfa:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80105dfd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e00:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80105e07:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105e0a:	83 e0 01             	and    $0x1,%eax
80105e0d:	85 c0                	test   %eax,%eax
80105e0f:	0f 94 c0             	sete   %al
80105e12:	89 c2                	mov    %eax,%edx
80105e14:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e17:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105e1a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105e1d:	83 e0 01             	and    $0x1,%eax
80105e20:	85 c0                	test   %eax,%eax
80105e22:	75 0a                	jne    80105e2e <sys_open+0x183>
80105e24:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105e27:	83 e0 02             	and    $0x2,%eax
80105e2a:	85 c0                	test   %eax,%eax
80105e2c:	74 07                	je     80105e35 <sys_open+0x18a>
80105e2e:	b8 01 00 00 00       	mov    $0x1,%eax
80105e33:	eb 05                	jmp    80105e3a <sys_open+0x18f>
80105e35:	b8 00 00 00 00       	mov    $0x0,%eax
80105e3a:	89 c2                	mov    %eax,%edx
80105e3c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e3f:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80105e42:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80105e45:	c9                   	leave  
80105e46:	c3                   	ret    

80105e47 <sys_mkdir>:

int
sys_mkdir(void)
{
80105e47:	55                   	push   %ebp
80105e48:	89 e5                	mov    %esp,%ebp
80105e4a:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_op();
80105e4d:	e8 93 d3 ff ff       	call   801031e5 <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80105e52:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105e55:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e59:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105e60:	e8 38 f5 ff ff       	call   8010539d <argstr>
80105e65:	85 c0                	test   %eax,%eax
80105e67:	78 2c                	js     80105e95 <sys_mkdir+0x4e>
80105e69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e6c:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105e73:	00 
80105e74:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105e7b:	00 
80105e7c:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80105e83:	00 
80105e84:	89 04 24             	mov    %eax,(%esp)
80105e87:	e8 5f fc ff ff       	call   80105aeb <create>
80105e8c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105e8f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105e93:	75 0c                	jne    80105ea1 <sys_mkdir+0x5a>
    end_op();
80105e95:	e8 cf d3 ff ff       	call   80103269 <end_op>
    return -1;
80105e9a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e9f:	eb 15                	jmp    80105eb6 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80105ea1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ea4:	89 04 24             	mov    %eax,(%esp)
80105ea7:	e8 33 bc ff ff       	call   80101adf <iunlockput>
  end_op();
80105eac:	e8 b8 d3 ff ff       	call   80103269 <end_op>
  return 0;
80105eb1:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105eb6:	c9                   	leave  
80105eb7:	c3                   	ret    

80105eb8 <sys_mknod>:

int
sys_mknod(void)
{
80105eb8:	55                   	push   %ebp
80105eb9:	89 e5                	mov    %esp,%ebp
80105ebb:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_op();
80105ebe:	e8 22 d3 ff ff       	call   801031e5 <begin_op>
  if((len=argstr(0, &path)) < 0 ||
80105ec3:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105ec6:	89 44 24 04          	mov    %eax,0x4(%esp)
80105eca:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105ed1:	e8 c7 f4 ff ff       	call   8010539d <argstr>
80105ed6:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105ed9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105edd:	78 5e                	js     80105f3d <sys_mknod+0x85>
     argint(1, &major) < 0 ||
80105edf:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105ee2:	89 44 24 04          	mov    %eax,0x4(%esp)
80105ee6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105eed:	e8 1b f4 ff ff       	call   8010530d <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
80105ef2:	85 c0                	test   %eax,%eax
80105ef4:	78 47                	js     80105f3d <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105ef6:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105ef9:	89 44 24 04          	mov    %eax,0x4(%esp)
80105efd:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105f04:	e8 04 f4 ff ff       	call   8010530d <argint>
  int len;
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
80105f09:	85 c0                	test   %eax,%eax
80105f0b:	78 30                	js     80105f3d <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
80105f0d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105f10:	0f bf c8             	movswl %ax,%ecx
80105f13:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105f16:	0f bf d0             	movswl %ax,%edx
80105f19:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_op();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105f1c:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80105f20:	89 54 24 08          	mov    %edx,0x8(%esp)
80105f24:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80105f2b:	00 
80105f2c:	89 04 24             	mov    %eax,(%esp)
80105f2f:	e8 b7 fb ff ff       	call   80105aeb <create>
80105f34:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105f37:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105f3b:	75 0c                	jne    80105f49 <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    end_op();
80105f3d:	e8 27 d3 ff ff       	call   80103269 <end_op>
    return -1;
80105f42:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f47:	eb 15                	jmp    80105f5e <sys_mknod+0xa6>
  }
  iunlockput(ip);
80105f49:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f4c:	89 04 24             	mov    %eax,(%esp)
80105f4f:	e8 8b bb ff ff       	call   80101adf <iunlockput>
  end_op();
80105f54:	e8 10 d3 ff ff       	call   80103269 <end_op>
  return 0;
80105f59:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105f5e:	c9                   	leave  
80105f5f:	c3                   	ret    

80105f60 <sys_chdir>:

int
sys_chdir(void)
{
80105f60:	55                   	push   %ebp
80105f61:	89 e5                	mov    %esp,%ebp
80105f63:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_op();
80105f66:	e8 7a d2 ff ff       	call   801031e5 <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
80105f6b:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105f6e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f72:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f79:	e8 1f f4 ff ff       	call   8010539d <argstr>
80105f7e:	85 c0                	test   %eax,%eax
80105f80:	78 14                	js     80105f96 <sys_chdir+0x36>
80105f82:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f85:	89 04 24             	mov    %eax,(%esp)
80105f88:	e8 79 c4 ff ff       	call   80102406 <namei>
80105f8d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105f90:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105f94:	75 0c                	jne    80105fa2 <sys_chdir+0x42>
    end_op();
80105f96:	e8 ce d2 ff ff       	call   80103269 <end_op>
    return -1;
80105f9b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fa0:	eb 61                	jmp    80106003 <sys_chdir+0xa3>
  }
  ilock(ip);
80105fa2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fa5:	89 04 24             	mov    %eax,(%esp)
80105fa8:	e8 ae b8 ff ff       	call   8010185b <ilock>
  if(ip->type != T_DIR){
80105fad:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fb0:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105fb4:	66 83 f8 01          	cmp    $0x1,%ax
80105fb8:	74 17                	je     80105fd1 <sys_chdir+0x71>
    iunlockput(ip);
80105fba:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fbd:	89 04 24             	mov    %eax,(%esp)
80105fc0:	e8 1a bb ff ff       	call   80101adf <iunlockput>
    end_op();
80105fc5:	e8 9f d2 ff ff       	call   80103269 <end_op>
    return -1;
80105fca:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fcf:	eb 32                	jmp    80106003 <sys_chdir+0xa3>
  }
  iunlock(ip);
80105fd1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105fd4:	89 04 24             	mov    %eax,(%esp)
80105fd7:	e8 cd b9 ff ff       	call   801019a9 <iunlock>
  iput(proc->cwd);
80105fdc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105fe2:	8b 40 68             	mov    0x68(%eax),%eax
80105fe5:	89 04 24             	mov    %eax,(%esp)
80105fe8:	e8 21 ba ff ff       	call   80101a0e <iput>
  end_op();
80105fed:	e8 77 d2 ff ff       	call   80103269 <end_op>
  proc->cwd = ip;
80105ff2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105ff8:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105ffb:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
80105ffe:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106003:	c9                   	leave  
80106004:	c3                   	ret    

80106005 <sys_exec>:

int
sys_exec(void)
{
80106005:	55                   	push   %ebp
80106006:	89 e5                	mov    %esp,%ebp
80106008:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
8010600e:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106011:	89 44 24 04          	mov    %eax,0x4(%esp)
80106015:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010601c:	e8 7c f3 ff ff       	call   8010539d <argstr>
80106021:	85 c0                	test   %eax,%eax
80106023:	78 1a                	js     8010603f <sys_exec+0x3a>
80106025:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
8010602b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010602f:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80106036:	e8 d2 f2 ff ff       	call   8010530d <argint>
8010603b:	85 c0                	test   %eax,%eax
8010603d:	79 0a                	jns    80106049 <sys_exec+0x44>
    return -1;
8010603f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106044:	e9 c8 00 00 00       	jmp    80106111 <sys_exec+0x10c>
  }
  memset(argv, 0, sizeof(argv));
80106049:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80106050:	00 
80106051:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106058:	00 
80106059:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
8010605f:	89 04 24             	mov    %eax,(%esp)
80106062:	e8 64 ef ff ff       	call   80104fcb <memset>
  for(i=0;; i++){
80106067:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
8010606e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106071:	83 f8 1f             	cmp    $0x1f,%eax
80106074:	76 0a                	jbe    80106080 <sys_exec+0x7b>
      return -1;
80106076:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010607b:	e9 91 00 00 00       	jmp    80106111 <sys_exec+0x10c>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
80106080:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106083:	c1 e0 02             	shl    $0x2,%eax
80106086:	89 c2                	mov    %eax,%edx
80106088:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
8010608e:	01 c2                	add    %eax,%edx
80106090:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80106096:	89 44 24 04          	mov    %eax,0x4(%esp)
8010609a:	89 14 24             	mov    %edx,(%esp)
8010609d:	e8 cf f1 ff ff       	call   80105271 <fetchint>
801060a2:	85 c0                	test   %eax,%eax
801060a4:	79 07                	jns    801060ad <sys_exec+0xa8>
      return -1;
801060a6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060ab:	eb 64                	jmp    80106111 <sys_exec+0x10c>
    if(uarg == 0){
801060ad:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
801060b3:	85 c0                	test   %eax,%eax
801060b5:	75 26                	jne    801060dd <sys_exec+0xd8>
      argv[i] = 0;
801060b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060ba:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
801060c1:	00 00 00 00 
      break;
801060c5:	90                   	nop
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
801060c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060c9:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
801060cf:	89 54 24 04          	mov    %edx,0x4(%esp)
801060d3:	89 04 24             	mov    %eax,(%esp)
801060d6:	e8 14 aa ff ff       	call   80100aef <exec>
801060db:	eb 34                	jmp    80106111 <sys_exec+0x10c>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
801060dd:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801060e3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801060e6:	c1 e2 02             	shl    $0x2,%edx
801060e9:	01 c2                	add    %eax,%edx
801060eb:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
801060f1:	89 54 24 04          	mov    %edx,0x4(%esp)
801060f5:	89 04 24             	mov    %eax,(%esp)
801060f8:	e8 ae f1 ff ff       	call   801052ab <fetchstr>
801060fd:	85 c0                	test   %eax,%eax
801060ff:	79 07                	jns    80106108 <sys_exec+0x103>
      return -1;
80106101:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106106:	eb 09                	jmp    80106111 <sys_exec+0x10c>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
80106108:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
8010610c:	e9 5d ff ff ff       	jmp    8010606e <sys_exec+0x69>
  return exec(path, argv);
}
80106111:	c9                   	leave  
80106112:	c3                   	ret    

80106113 <sys_pipe>:

int
sys_pipe(void)
{
80106113:	55                   	push   %ebp
80106114:	89 e5                	mov    %esp,%ebp
80106116:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80106119:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
80106120:	00 
80106121:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106124:	89 44 24 04          	mov    %eax,0x4(%esp)
80106128:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010612f:	e8 07 f2 ff ff       	call   8010533b <argptr>
80106134:	85 c0                	test   %eax,%eax
80106136:	79 0a                	jns    80106142 <sys_pipe+0x2f>
    return -1;
80106138:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010613d:	e9 9b 00 00 00       	jmp    801061dd <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
80106142:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106145:	89 44 24 04          	mov    %eax,0x4(%esp)
80106149:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010614c:	89 04 24             	mov    %eax,(%esp)
8010614f:	e8 8a db ff ff       	call   80103cde <pipealloc>
80106154:	85 c0                	test   %eax,%eax
80106156:	79 07                	jns    8010615f <sys_pipe+0x4c>
    return -1;
80106158:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010615d:	eb 7e                	jmp    801061dd <sys_pipe+0xca>
  fd0 = -1;
8010615f:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106166:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106169:	89 04 24             	mov    %eax,(%esp)
8010616c:	e8 67 f3 ff ff       	call   801054d8 <fdalloc>
80106171:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106174:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106178:	78 14                	js     8010618e <sys_pipe+0x7b>
8010617a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010617d:	89 04 24             	mov    %eax,(%esp)
80106180:	e8 53 f3 ff ff       	call   801054d8 <fdalloc>
80106185:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106188:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010618c:	79 37                	jns    801061c5 <sys_pipe+0xb2>
    if(fd0 >= 0)
8010618e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106192:	78 14                	js     801061a8 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
80106194:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010619a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010619d:	83 c2 08             	add    $0x8,%edx
801061a0:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801061a7:	00 
    fileclose(rf);
801061a8:	8b 45 e8             	mov    -0x18(%ebp),%eax
801061ab:	89 04 24             	mov    %eax,(%esp)
801061ae:	e8 1b ae ff ff       	call   80100fce <fileclose>
    fileclose(wf);
801061b3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801061b6:	89 04 24             	mov    %eax,(%esp)
801061b9:	e8 10 ae ff ff       	call   80100fce <fileclose>
    return -1;
801061be:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061c3:	eb 18                	jmp    801061dd <sys_pipe+0xca>
  }
  fd[0] = fd0;
801061c5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801061c8:	8b 55 f4             	mov    -0xc(%ebp),%edx
801061cb:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
801061cd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801061d0:	8d 50 04             	lea    0x4(%eax),%edx
801061d3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801061d6:	89 02                	mov    %eax,(%edx)
  return 0;
801061d8:	b8 00 00 00 00       	mov    $0x0,%eax
}
801061dd:	c9                   	leave  
801061de:	c3                   	ret    

801061df <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
801061df:	55                   	push   %ebp
801061e0:	89 e5                	mov    %esp,%ebp
801061e2:	83 ec 18             	sub    $0x18,%esp
  proc->ctflag = 0;
801061e5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801061eb:	c7 40 7c 00 00 00 00 	movl   $0x0,0x7c(%eax)
  return fork((char*)0, 0, 0, 0);
801061f2:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
801061f9:	00 
801061fa:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80106201:	00 
80106202:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106209:	00 
8010620a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106211:	e8 76 e1 ff ff       	call   8010438c <fork>
}
80106216:	c9                   	leave  
80106217:	c3                   	ret    

80106218 <sys_exit>:

int
sys_exit(void)
{
80106218:	55                   	push   %ebp
80106219:	89 e5                	mov    %esp,%ebp
8010621b:	83 ec 08             	sub    $0x8,%esp
  exit();
8010621e:	e8 44 e4 ff ff       	call   80104667 <exit>
  return 0;  // not reached
80106223:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106228:	c9                   	leave  
80106229:	c3                   	ret    

8010622a <sys_wait>:

int
sys_wait(void)
{
8010622a:	55                   	push   %ebp
8010622b:	89 e5                	mov    %esp,%ebp
8010622d:	83 ec 18             	sub    $0x18,%esp
  return wait(0);
80106230:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106237:	e8 50 e5 ff ff       	call   8010478c <wait>
}
8010623c:	c9                   	leave  
8010623d:	c3                   	ret    

8010623e <sys_kill>:

int
sys_kill(void)
{
8010623e:	55                   	push   %ebp
8010623f:	89 e5                	mov    %esp,%ebp
80106241:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
80106244:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106247:	89 44 24 04          	mov    %eax,0x4(%esp)
8010624b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106252:	e8 b6 f0 ff ff       	call   8010530d <argint>
80106257:	85 c0                	test   %eax,%eax
80106259:	79 07                	jns    80106262 <sys_kill+0x24>
    return -1;
8010625b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106260:	eb 0b                	jmp    8010626d <sys_kill+0x2f>
  return kill(pid);
80106262:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106265:	89 04 24             	mov    %eax,(%esp)
80106268:	e8 3e e9 ff ff       	call   80104bab <kill>
}
8010626d:	c9                   	leave  
8010626e:	c3                   	ret    

8010626f <sys_getpid>:

int
sys_getpid(void)
{
8010626f:	55                   	push   %ebp
80106270:	89 e5                	mov    %esp,%ebp
  return proc->pid;
80106272:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106278:	8b 40 10             	mov    0x10(%eax),%eax
}
8010627b:	5d                   	pop    %ebp
8010627c:	c3                   	ret    

8010627d <sys_sbrk>:

int
sys_sbrk(void)
{
8010627d:	55                   	push   %ebp
8010627e:	89 e5                	mov    %esp,%ebp
80106280:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
80106283:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106286:	89 44 24 04          	mov    %eax,0x4(%esp)
8010628a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106291:	e8 77 f0 ff ff       	call   8010530d <argint>
80106296:	85 c0                	test   %eax,%eax
80106298:	79 07                	jns    801062a1 <sys_sbrk+0x24>
    return -1;
8010629a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010629f:	eb 24                	jmp    801062c5 <sys_sbrk+0x48>
  addr = proc->sz;
801062a1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062a7:	8b 00                	mov    (%eax),%eax
801062a9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
801062ac:	8b 45 f0             	mov    -0x10(%ebp),%eax
801062af:	89 04 24             	mov    %eax,(%esp)
801062b2:	e8 30 e0 ff ff       	call   801042e7 <growproc>
801062b7:	85 c0                	test   %eax,%eax
801062b9:	79 07                	jns    801062c2 <sys_sbrk+0x45>
    return -1;
801062bb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062c0:	eb 03                	jmp    801062c5 <sys_sbrk+0x48>
  return addr;
801062c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801062c5:	c9                   	leave  
801062c6:	c3                   	ret    

801062c7 <sys_sleep>:

int
sys_sleep(void)
{
801062c7:	55                   	push   %ebp
801062c8:	89 e5                	mov    %esp,%ebp
801062ca:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
801062cd:	8d 45 f0             	lea    -0x10(%ebp),%eax
801062d0:	89 44 24 04          	mov    %eax,0x4(%esp)
801062d4:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801062db:	e8 2d f0 ff ff       	call   8010530d <argint>
801062e0:	85 c0                	test   %eax,%eax
801062e2:	79 07                	jns    801062eb <sys_sleep+0x24>
    return -1;
801062e4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062e9:	eb 6c                	jmp    80106357 <sys_sleep+0x90>
  acquire(&tickslock);
801062eb:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
801062f2:	e8 80 ea ff ff       	call   80104d77 <acquire>
  ticks0 = ticks;
801062f7:	a1 20 86 11 80       	mov    0x80118620,%eax
801062fc:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
801062ff:	eb 34                	jmp    80106335 <sys_sleep+0x6e>
    if(proc->killed){
80106301:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106307:	8b 40 24             	mov    0x24(%eax),%eax
8010630a:	85 c0                	test   %eax,%eax
8010630c:	74 13                	je     80106321 <sys_sleep+0x5a>
      release(&tickslock);
8010630e:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
80106315:	e8 bf ea ff ff       	call   80104dd9 <release>
      return -1;
8010631a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010631f:	eb 36                	jmp    80106357 <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
80106321:	c7 44 24 04 e0 7d 11 	movl   $0x80117de0,0x4(%esp)
80106328:	80 
80106329:	c7 04 24 20 86 11 80 	movl   $0x80118620,(%esp)
80106330:	e8 6f e7 ff ff       	call   80104aa4 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
80106335:	a1 20 86 11 80       	mov    0x80118620,%eax
8010633a:	2b 45 f4             	sub    -0xc(%ebp),%eax
8010633d:	89 c2                	mov    %eax,%edx
8010633f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106342:	39 c2                	cmp    %eax,%edx
80106344:	72 bb                	jb     80106301 <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
80106346:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
8010634d:	e8 87 ea ff ff       	call   80104dd9 <release>
  return 0;
80106352:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106357:	c9                   	leave  
80106358:	c3                   	ret    

80106359 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80106359:	55                   	push   %ebp
8010635a:	89 e5                	mov    %esp,%ebp
8010635c:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
8010635f:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
80106366:	e8 0c ea ff ff       	call   80104d77 <acquire>
  xticks = ticks;
8010636b:	a1 20 86 11 80       	mov    0x80118620,%eax
80106370:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
80106373:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
8010637a:	e8 5a ea ff ff       	call   80104dd9 <release>
  return xticks;
8010637f:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106382:	c9                   	leave  
80106383:	c3                   	ret    

80106384 <sys_getppid>:

int sys_getppid(void) {
80106384:	55                   	push   %ebp
80106385:	89 e5                	mov    %esp,%ebp

	if(proc->parent == 0)
80106387:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010638d:	8b 40 14             	mov    0x14(%eax),%eax
80106390:	85 c0                	test   %eax,%eax
80106392:	75 07                	jne    8010639b <sys_getppid+0x17>
		return 0;
80106394:	b8 00 00 00 00       	mov    $0x0,%eax
80106399:	eb 0c                	jmp    801063a7 <sys_getppid+0x23>
	return proc->parent->pid;
8010639b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801063a1:	8b 40 14             	mov    0x14(%eax),%eax
801063a4:	8b 40 10             	mov    0x10(%eax),%eax
}
801063a7:	5d                   	pop    %ebp
801063a8:	c3                   	ret    

801063a9 <sys_kthread_create>:

int sys_kthread_create(void){
801063a9:	55                   	push   %ebp
801063aa:	89 e5                	mov    %esp,%ebp
801063ac:	53                   	push   %ebx
801063ad:	83 ec 24             	sub    $0x24,%esp

	int ustack = 0;
801063b0:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	int wrapper = 0;
801063b7:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
	int arg1 = 0;
801063be:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
	int arg2 = 0;
801063c5:	c7 45 e8 00 00 00 00 	movl   $0x0,-0x18(%ebp)

	argint(0, &ustack);
801063cc:	8d 45 f4             	lea    -0xc(%ebp),%eax
801063cf:	89 44 24 04          	mov    %eax,0x4(%esp)
801063d3:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063da:	e8 2e ef ff ff       	call   8010530d <argint>
	argint(1, &wrapper);
801063df:	8d 45 f0             	lea    -0x10(%ebp),%eax
801063e2:	89 44 24 04          	mov    %eax,0x4(%esp)
801063e6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801063ed:	e8 1b ef ff ff       	call   8010530d <argint>
	argint(2, &arg1);
801063f2:	8d 45 ec             	lea    -0x14(%ebp),%eax
801063f5:	89 44 24 04          	mov    %eax,0x4(%esp)
801063f9:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80106400:	e8 08 ef ff ff       	call   8010530d <argint>
	argint(3, &arg2);
80106405:	8d 45 e8             	lea    -0x18(%ebp),%eax
80106408:	89 44 24 04          	mov    %eax,0x4(%esp)
8010640c:	c7 04 24 03 00 00 00 	movl   $0x3,(%esp)
80106413:	e8 f5 ee ff ff       	call   8010530d <argint>

	proc->ctflag = 1;
80106418:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010641e:	c7 40 7c 01 00 00 00 	movl   $0x1,0x7c(%eax)
	return fork((char*)ustack, (uint)wrapper, (uint)arg1, (uint)arg2);
80106425:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106428:	89 c3                	mov    %eax,%ebx
8010642a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010642d:	89 c1                	mov    %eax,%ecx
8010642f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106432:	89 c2                	mov    %eax,%edx
80106434:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106437:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
8010643b:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010643f:	89 54 24 04          	mov    %edx,0x4(%esp)
80106443:	89 04 24             	mov    %eax,(%esp)
80106446:	e8 41 df ff ff       	call   8010438c <fork>
}
8010644b:	83 c4 24             	add    $0x24,%esp
8010644e:	5b                   	pop    %ebx
8010644f:	5d                   	pop    %ebp
80106450:	c3                   	ret    

80106451 <sys_kthread_join>:

int sys_kthread_join(void){
80106451:	55                   	push   %ebp
80106452:	89 e5                	mov    %esp,%ebp
80106454:	83 ec 28             	sub    $0x28,%esp

	int tid = 0;
80106457:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	argint(0, &tid);
8010645e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106461:	89 44 24 04          	mov    %eax,0x4(%esp)
80106465:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010646c:	e8 9c ee ff ff       	call   8010530d <argint>
	return wait(tid);
80106471:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106474:	89 04 24             	mov    %eax,(%esp)
80106477:	e8 10 e3 ff ff       	call   8010478c <wait>

}
8010647c:	c9                   	leave  
8010647d:	c3                   	ret    

8010647e <sys_kthread_mutex_init>:

int sys_kthread_mutex_init(void){
8010647e:	55                   	push   %ebp
8010647f:	89 e5                	mov    %esp,%ebp
80106481:	53                   	push   %ebx
80106482:	83 ec 10             	sub    $0x10,%esp

	int i,  flag = -1;
80106485:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)

	for(i=0;i<NMUTX;i++){
8010648c:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80106493:	e9 ac 00 00 00       	jmp    80106544 <sys_kthread_mutex_init+0xc6>

		if(proc->mutexlist[i].id != -1){
80106498:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
8010649f:	8b 55 f8             	mov    -0x8(%ebp),%edx
801064a2:	89 d0                	mov    %edx,%eax
801064a4:	01 c0                	add    %eax,%eax
801064a6:	01 d0                	add    %edx,%eax
801064a8:	c1 e0 02             	shl    $0x2,%eax
801064ab:	01 c8                	add    %ecx,%eax
801064ad:	05 90 00 00 00       	add    $0x90,%eax
801064b2:	8b 00                	mov    (%eax),%eax
801064b4:	83 f8 ff             	cmp    $0xffffffff,%eax
801064b7:	0f 84 83 00 00 00    	je     80106540 <sys_kthread_mutex_init+0xc2>

			proc->mutexlist[i].id = (i+1);
801064bd:	65 8b 1d 04 00 00 00 	mov    %gs:0x4,%ebx
801064c4:	8b 45 f8             	mov    -0x8(%ebp),%eax
801064c7:	8d 48 01             	lea    0x1(%eax),%ecx
801064ca:	8b 55 f8             	mov    -0x8(%ebp),%edx
801064cd:	89 d0                	mov    %edx,%eax
801064cf:	01 c0                	add    %eax,%eax
801064d1:	01 d0                	add    %edx,%eax
801064d3:	c1 e0 02             	shl    $0x2,%eax
801064d6:	01 d8                	add    %ebx,%eax
801064d8:	05 90 00 00 00       	add    $0x90,%eax
801064dd:	89 08                	mov    %ecx,(%eax)
			proc->mutexlist[i].state = 0;
801064df:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
801064e6:	8b 55 f8             	mov    -0x8(%ebp),%edx
801064e9:	89 d0                	mov    %edx,%eax
801064eb:	01 c0                	add    %eax,%eax
801064ed:	01 d0                	add    %edx,%eax
801064ef:	c1 e0 02             	shl    $0x2,%eax
801064f2:	01 c8                	add    %ecx,%eax
801064f4:	05 94 00 00 00       	add    $0x94,%eax
801064f9:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
			proc->mutexlist[i].lockingthread = 0;
801064ff:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106506:	8b 55 f8             	mov    -0x8(%ebp),%edx
80106509:	89 d0                	mov    %edx,%eax
8010650b:	01 c0                	add    %eax,%eax
8010650d:	01 d0                	add    %edx,%eax
8010650f:	c1 e0 02             	shl    $0x2,%eax
80106512:	01 c8                	add    %ecx,%eax
80106514:	05 98 00 00 00       	add    $0x98,%eax
80106519:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
			flag = proc->mutexlist[i].id;
8010651f:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106526:	8b 55 f8             	mov    -0x8(%ebp),%edx
80106529:	89 d0                	mov    %edx,%eax
8010652b:	01 c0                	add    %eax,%eax
8010652d:	01 d0                	add    %edx,%eax
8010652f:	c1 e0 02             	shl    $0x2,%eax
80106532:	01 c8                	add    %ecx,%eax
80106534:	05 90 00 00 00       	add    $0x90,%eax
80106539:	8b 00                	mov    (%eax),%eax
8010653b:	89 45 f4             	mov    %eax,-0xc(%ebp)
			break;
8010653e:	eb 0e                	jmp    8010654e <sys_kthread_mutex_init+0xd0>

int sys_kthread_mutex_init(void){

	int i,  flag = -1;

	for(i=0;i<NMUTX;i++){
80106540:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80106544:	83 7d f8 0f          	cmpl   $0xf,-0x8(%ebp)
80106548:	0f 8e 4a ff ff ff    	jle    80106498 <sys_kthread_mutex_init+0x1a>
			flag = proc->mutexlist[i].id;
			break;
		}
	}	
	
	return flag;
8010654e:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106551:	83 c4 10             	add    $0x10,%esp
80106554:	5b                   	pop    %ebx
80106555:	5d                   	pop    %ebp
80106556:	c3                   	ret    

80106557 <sys_kthread_mutex_destroy>:

int sys_kthread_mutex_destroy(void){
80106557:	55                   	push   %ebp
80106558:	89 e5                	mov    %esp,%ebp
8010655a:	83 ec 28             	sub    $0x28,%esp

	int mutex;
	argint(0, &mutex);
8010655d:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106560:	89 44 24 04          	mov    %eax,0x4(%esp)
80106564:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010656b:	e8 9d ed ff ff       	call   8010530d <argint>

	if(proc->mutexlist[mutex-1].id != mutex)
80106570:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106577:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010657a:	8d 50 ff             	lea    -0x1(%eax),%edx
8010657d:	89 d0                	mov    %edx,%eax
8010657f:	01 c0                	add    %eax,%eax
80106581:	01 d0                	add    %edx,%eax
80106583:	c1 e0 02             	shl    $0x2,%eax
80106586:	01 c8                	add    %ecx,%eax
80106588:	05 90 00 00 00       	add    $0x90,%eax
8010658d:	8b 10                	mov    (%eax),%edx
8010658f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106592:	39 c2                	cmp    %eax,%edx
80106594:	74 07                	je     8010659d <sys_kthread_mutex_destroy+0x46>
		return -1;
80106596:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010659b:	eb 6e                	jmp    8010660b <sys_kthread_mutex_destroy+0xb4>
		
	proc->mutexlist[mutex-1].id = -1;
8010659d:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
801065a4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801065a7:	8d 50 ff             	lea    -0x1(%eax),%edx
801065aa:	89 d0                	mov    %edx,%eax
801065ac:	01 c0                	add    %eax,%eax
801065ae:	01 d0                	add    %edx,%eax
801065b0:	c1 e0 02             	shl    $0x2,%eax
801065b3:	01 c8                	add    %ecx,%eax
801065b5:	05 90 00 00 00       	add    $0x90,%eax
801065ba:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
	proc->mutexlist[mutex-1].state = -1;
801065c0:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
801065c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801065ca:	8d 50 ff             	lea    -0x1(%eax),%edx
801065cd:	89 d0                	mov    %edx,%eax
801065cf:	01 c0                	add    %eax,%eax
801065d1:	01 d0                	add    %edx,%eax
801065d3:	c1 e0 02             	shl    $0x2,%eax
801065d6:	01 c8                	add    %ecx,%eax
801065d8:	05 94 00 00 00       	add    $0x94,%eax
801065dd:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
	proc->mutexlist[mutex-1].lockingthread = -1;
801065e3:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
801065ea:	8b 45 f4             	mov    -0xc(%ebp),%eax
801065ed:	8d 50 ff             	lea    -0x1(%eax),%edx
801065f0:	89 d0                	mov    %edx,%eax
801065f2:	01 c0                	add    %eax,%eax
801065f4:	01 d0                	add    %edx,%eax
801065f6:	c1 e0 02             	shl    $0x2,%eax
801065f9:	01 c8                	add    %ecx,%eax
801065fb:	05 98 00 00 00       	add    $0x98,%eax
80106600:	c7 00 ff ff ff ff    	movl   $0xffffffff,(%eax)
	return 0;		
80106606:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010660b:	c9                   	leave  
8010660c:	c3                   	ret    

8010660d <sys_kthread_mutex_lock>:

int sys_kthread_mutex_lock(void){
8010660d:	55                   	push   %ebp
8010660e:	89 e5                	mov    %esp,%ebp
80106610:	53                   	push   %ebx
80106611:	83 ec 24             	sub    $0x24,%esp

	int mutex = 0;
80106614:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	argint(0, &mutex);
8010661b:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010661e:	89 44 24 04          	mov    %eax,0x4(%esp)
80106622:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106629:	e8 df ec ff ff       	call   8010530d <argint>

	if((mutex > 0) && (proc->mutexlist[mutex-1].id == mutex)){
8010662e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106631:	85 c0                	test   %eax,%eax
80106633:	0f 8e 94 00 00 00    	jle    801066cd <sys_kthread_mutex_lock+0xc0>
80106639:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106640:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106643:	8d 50 ff             	lea    -0x1(%eax),%edx
80106646:	89 d0                	mov    %edx,%eax
80106648:	01 c0                	add    %eax,%eax
8010664a:	01 d0                	add    %edx,%eax
8010664c:	c1 e0 02             	shl    $0x2,%eax
8010664f:	01 c8                	add    %ecx,%eax
80106651:	05 90 00 00 00       	add    $0x90,%eax
80106656:	8b 10                	mov    (%eax),%edx
80106658:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010665b:	39 c2                	cmp    %eax,%edx
8010665d:	75 6e                	jne    801066cd <sys_kthread_mutex_lock+0xc0>
		while(proc->mutexlist[mutex-1].state);
8010665f:	90                   	nop
80106660:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106667:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010666a:	8d 50 ff             	lea    -0x1(%eax),%edx
8010666d:	89 d0                	mov    %edx,%eax
8010666f:	01 c0                	add    %eax,%eax
80106671:	01 d0                	add    %edx,%eax
80106673:	c1 e0 02             	shl    $0x2,%eax
80106676:	01 c8                	add    %ecx,%eax
80106678:	05 94 00 00 00       	add    $0x94,%eax
8010667d:	8b 00                	mov    (%eax),%eax
8010667f:	85 c0                	test   %eax,%eax
80106681:	75 dd                	jne    80106660 <sys_kthread_mutex_lock+0x53>
		proc->mutexlist[mutex-1].state = 1;
80106683:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
8010668a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010668d:	8d 50 ff             	lea    -0x1(%eax),%edx
80106690:	89 d0                	mov    %edx,%eax
80106692:	01 c0                	add    %eax,%eax
80106694:	01 d0                	add    %edx,%eax
80106696:	c1 e0 02             	shl    $0x2,%eax
80106699:	01 c8                	add    %ecx,%eax
8010669b:	05 94 00 00 00       	add    $0x94,%eax
801066a0:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
		proc->mutexlist[mutex-1].lockingthread = mutex;	
801066a6:	65 8b 1d 04 00 00 00 	mov    %gs:0x4,%ebx
801066ad:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066b0:	8d 50 ff             	lea    -0x1(%eax),%edx
801066b3:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801066b6:	89 d0                	mov    %edx,%eax
801066b8:	01 c0                	add    %eax,%eax
801066ba:	01 d0                	add    %edx,%eax
801066bc:	c1 e0 02             	shl    $0x2,%eax
801066bf:	01 d8                	add    %ebx,%eax
801066c1:	05 98 00 00 00       	add    $0x98,%eax
801066c6:	89 08                	mov    %ecx,(%eax)
		return mutex;
801066c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066cb:	eb 05                	jmp    801066d2 <sys_kthread_mutex_lock+0xc5>
	}
	
	return -1;
801066cd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801066d2:	83 c4 24             	add    $0x24,%esp
801066d5:	5b                   	pop    %ebx
801066d6:	5d                   	pop    %ebp
801066d7:	c3                   	ret    

801066d8 <sys_kthread_mutex_unlock>:

int sys_kthread_mutex_unlock(void){
801066d8:	55                   	push   %ebp
801066d9:	89 e5                	mov    %esp,%ebp
801066db:	83 ec 28             	sub    $0x28,%esp

	int mutex = 0;
801066de:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	argint(0, &mutex);
801066e5:	8d 45 f4             	lea    -0xc(%ebp),%eax
801066e8:	89 44 24 04          	mov    %eax,0x4(%esp)
801066ec:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801066f3:	e8 15 ec ff ff       	call   8010530d <argint>

	if((mutex > 0) && (proc->mutexlist[mutex-1].id == mutex)){
801066f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801066fb:	85 c0                	test   %eax,%eax
801066fd:	7e 71                	jle    80106770 <sys_kthread_mutex_unlock+0x98>
801066ff:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
80106706:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106709:	8d 50 ff             	lea    -0x1(%eax),%edx
8010670c:	89 d0                	mov    %edx,%eax
8010670e:	01 c0                	add    %eax,%eax
80106710:	01 d0                	add    %edx,%eax
80106712:	c1 e0 02             	shl    $0x2,%eax
80106715:	01 c8                	add    %ecx,%eax
80106717:	05 90 00 00 00       	add    $0x90,%eax
8010671c:	8b 10                	mov    (%eax),%edx
8010671e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106721:	39 c2                	cmp    %eax,%edx
80106723:	75 4b                	jne    80106770 <sys_kthread_mutex_unlock+0x98>
		proc->mutexlist[mutex-1].state = 0;
80106725:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
8010672c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010672f:	8d 50 ff             	lea    -0x1(%eax),%edx
80106732:	89 d0                	mov    %edx,%eax
80106734:	01 c0                	add    %eax,%eax
80106736:	01 d0                	add    %edx,%eax
80106738:	c1 e0 02             	shl    $0x2,%eax
8010673b:	01 c8                	add    %ecx,%eax
8010673d:	05 94 00 00 00       	add    $0x94,%eax
80106742:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
		proc->mutexlist[mutex-1].lockingthread = 0;
80106748:	65 8b 0d 04 00 00 00 	mov    %gs:0x4,%ecx
8010674f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106752:	8d 50 ff             	lea    -0x1(%eax),%edx
80106755:	89 d0                	mov    %edx,%eax
80106757:	01 c0                	add    %eax,%eax
80106759:	01 d0                	add    %edx,%eax
8010675b:	c1 e0 02             	shl    $0x2,%eax
8010675e:	01 c8                	add    %ecx,%eax
80106760:	05 98 00 00 00       	add    $0x98,%eax
80106765:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
		return mutex;
8010676b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010676e:	eb 05                	jmp    80106775 <sys_kthread_mutex_unlock+0x9d>
	}

	return -1;
80106770:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106775:	c9                   	leave  
80106776:	c3                   	ret    

80106777 <sys_kthread_cond_init>:


int sys_kthread_cond_init(void){
80106777:	55                   	push   %ebp
80106778:	89 e5                	mov    %esp,%ebp

	return 0;
8010677a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010677f:	5d                   	pop    %ebp
80106780:	c3                   	ret    

80106781 <sys_kthread_cond_destroy>:

int sys_kthread_cond_destroy(void){
80106781:	55                   	push   %ebp
80106782:	89 e5                	mov    %esp,%ebp

	return 0;	
80106784:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106789:	5d                   	pop    %ebp
8010678a:	c3                   	ret    

8010678b <sys_kthread_cond_wait>:

int sys_kthread_cond_wait(void){
8010678b:	55                   	push   %ebp
8010678c:	89 e5                	mov    %esp,%ebp

	return 0;
8010678e:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106793:	5d                   	pop    %ebp
80106794:	c3                   	ret    

80106795 <sys_kthread_cond_signal>:

int sys_kthread_cond_signal(void){
80106795:	55                   	push   %ebp
80106796:	89 e5                	mov    %esp,%ebp

	return 0;
80106798:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010679d:	5d                   	pop    %ebp
8010679e:	c3                   	ret    

8010679f <sys_kthread_cond_broadcast>:

int sys_kthread_cond_broadcast(void){
8010679f:	55                   	push   %ebp
801067a0:	89 e5                	mov    %esp,%ebp

	return 0;
801067a2:	b8 00 00 00 00       	mov    $0x0,%eax
}
801067a7:	5d                   	pop    %ebp
801067a8:	c3                   	ret    

801067a9 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801067a9:	55                   	push   %ebp
801067aa:	89 e5                	mov    %esp,%ebp
801067ac:	83 ec 08             	sub    $0x8,%esp
801067af:	8b 55 08             	mov    0x8(%ebp),%edx
801067b2:	8b 45 0c             	mov    0xc(%ebp),%eax
801067b5:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801067b9:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801067bc:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801067c0:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801067c4:	ee                   	out    %al,(%dx)
}
801067c5:	c9                   	leave  
801067c6:	c3                   	ret    

801067c7 <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
801067c7:	55                   	push   %ebp
801067c8:	89 e5                	mov    %esp,%ebp
801067ca:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
801067cd:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
801067d4:	00 
801067d5:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
801067dc:	e8 c8 ff ff ff       	call   801067a9 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
801067e1:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
801067e8:	00 
801067e9:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
801067f0:	e8 b4 ff ff ff       	call   801067a9 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
801067f5:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
801067fc:	00 
801067fd:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
80106804:	e8 a0 ff ff ff       	call   801067a9 <outb>
  picenable(IRQ_TIMER);
80106809:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106810:	e8 5c d3 ff ff       	call   80103b71 <picenable>
}
80106815:	c9                   	leave  
80106816:	c3                   	ret    

80106817 <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
80106817:	1e                   	push   %ds
  pushl %es
80106818:	06                   	push   %es
  pushl %fs
80106819:	0f a0                	push   %fs
  pushl %gs
8010681b:	0f a8                	push   %gs
  pushal
8010681d:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
8010681e:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
80106822:	8e d8                	mov    %eax,%ds
  movw %ax, %es
80106824:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
80106826:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
8010682a:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
8010682c:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
8010682e:	54                   	push   %esp
  call trap
8010682f:	e8 d8 01 00 00       	call   80106a0c <trap>
  addl $4, %esp
80106834:	83 c4 04             	add    $0x4,%esp

80106837 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80106837:	61                   	popa   
  popl %gs
80106838:	0f a9                	pop    %gs
  popl %fs
8010683a:	0f a1                	pop    %fs
  popl %es
8010683c:	07                   	pop    %es
  popl %ds
8010683d:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
8010683e:	83 c4 08             	add    $0x8,%esp
  iret
80106841:	cf                   	iret   

80106842 <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
80106842:	55                   	push   %ebp
80106843:	89 e5                	mov    %esp,%ebp
80106845:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80106848:	8b 45 0c             	mov    0xc(%ebp),%eax
8010684b:	83 e8 01             	sub    $0x1,%eax
8010684e:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80106852:	8b 45 08             	mov    0x8(%ebp),%eax
80106855:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80106859:	8b 45 08             	mov    0x8(%ebp),%eax
8010685c:	c1 e8 10             	shr    $0x10,%eax
8010685f:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
80106863:	8d 45 fa             	lea    -0x6(%ebp),%eax
80106866:	0f 01 18             	lidtl  (%eax)
}
80106869:	c9                   	leave  
8010686a:	c3                   	ret    

8010686b <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
8010686b:	55                   	push   %ebp
8010686c:	89 e5                	mov    %esp,%ebp
8010686e:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80106871:	0f 20 d0             	mov    %cr2,%eax
80106874:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return val;
80106877:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010687a:	c9                   	leave  
8010687b:	c3                   	ret    

8010687c <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
8010687c:	55                   	push   %ebp
8010687d:	89 e5                	mov    %esp,%ebp
8010687f:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
80106882:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106889:	e9 c3 00 00 00       	jmp    80106951 <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
8010688e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106891:	8b 04 85 c4 b0 10 80 	mov    -0x7fef4f3c(,%eax,4),%eax
80106898:	89 c2                	mov    %eax,%edx
8010689a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010689d:	66 89 14 c5 20 7e 11 	mov    %dx,-0x7fee81e0(,%eax,8)
801068a4:	80 
801068a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068a8:	66 c7 04 c5 22 7e 11 	movw   $0x8,-0x7fee81de(,%eax,8)
801068af:	80 08 00 
801068b2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068b5:	0f b6 14 c5 24 7e 11 	movzbl -0x7fee81dc(,%eax,8),%edx
801068bc:	80 
801068bd:	83 e2 e0             	and    $0xffffffe0,%edx
801068c0:	88 14 c5 24 7e 11 80 	mov    %dl,-0x7fee81dc(,%eax,8)
801068c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068ca:	0f b6 14 c5 24 7e 11 	movzbl -0x7fee81dc(,%eax,8),%edx
801068d1:	80 
801068d2:	83 e2 1f             	and    $0x1f,%edx
801068d5:	88 14 c5 24 7e 11 80 	mov    %dl,-0x7fee81dc(,%eax,8)
801068dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068df:	0f b6 14 c5 25 7e 11 	movzbl -0x7fee81db(,%eax,8),%edx
801068e6:	80 
801068e7:	83 e2 f0             	and    $0xfffffff0,%edx
801068ea:	83 ca 0e             	or     $0xe,%edx
801068ed:	88 14 c5 25 7e 11 80 	mov    %dl,-0x7fee81db(,%eax,8)
801068f4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801068f7:	0f b6 14 c5 25 7e 11 	movzbl -0x7fee81db(,%eax,8),%edx
801068fe:	80 
801068ff:	83 e2 ef             	and    $0xffffffef,%edx
80106902:	88 14 c5 25 7e 11 80 	mov    %dl,-0x7fee81db(,%eax,8)
80106909:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010690c:	0f b6 14 c5 25 7e 11 	movzbl -0x7fee81db(,%eax,8),%edx
80106913:	80 
80106914:	83 e2 9f             	and    $0xffffff9f,%edx
80106917:	88 14 c5 25 7e 11 80 	mov    %dl,-0x7fee81db(,%eax,8)
8010691e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106921:	0f b6 14 c5 25 7e 11 	movzbl -0x7fee81db(,%eax,8),%edx
80106928:	80 
80106929:	83 ca 80             	or     $0xffffff80,%edx
8010692c:	88 14 c5 25 7e 11 80 	mov    %dl,-0x7fee81db(,%eax,8)
80106933:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106936:	8b 04 85 c4 b0 10 80 	mov    -0x7fef4f3c(,%eax,4),%eax
8010693d:	c1 e8 10             	shr    $0x10,%eax
80106940:	89 c2                	mov    %eax,%edx
80106942:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106945:	66 89 14 c5 26 7e 11 	mov    %dx,-0x7fee81da(,%eax,8)
8010694c:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
8010694d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106951:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
80106958:	0f 8e 30 ff ff ff    	jle    8010688e <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
8010695e:	a1 c4 b1 10 80       	mov    0x8010b1c4,%eax
80106963:	66 a3 20 80 11 80    	mov    %ax,0x80118020
80106969:	66 c7 05 22 80 11 80 	movw   $0x8,0x80118022
80106970:	08 00 
80106972:	0f b6 05 24 80 11 80 	movzbl 0x80118024,%eax
80106979:	83 e0 e0             	and    $0xffffffe0,%eax
8010697c:	a2 24 80 11 80       	mov    %al,0x80118024
80106981:	0f b6 05 24 80 11 80 	movzbl 0x80118024,%eax
80106988:	83 e0 1f             	and    $0x1f,%eax
8010698b:	a2 24 80 11 80       	mov    %al,0x80118024
80106990:	0f b6 05 25 80 11 80 	movzbl 0x80118025,%eax
80106997:	83 c8 0f             	or     $0xf,%eax
8010699a:	a2 25 80 11 80       	mov    %al,0x80118025
8010699f:	0f b6 05 25 80 11 80 	movzbl 0x80118025,%eax
801069a6:	83 e0 ef             	and    $0xffffffef,%eax
801069a9:	a2 25 80 11 80       	mov    %al,0x80118025
801069ae:	0f b6 05 25 80 11 80 	movzbl 0x80118025,%eax
801069b5:	83 c8 60             	or     $0x60,%eax
801069b8:	a2 25 80 11 80       	mov    %al,0x80118025
801069bd:	0f b6 05 25 80 11 80 	movzbl 0x80118025,%eax
801069c4:	83 c8 80             	or     $0xffffff80,%eax
801069c7:	a2 25 80 11 80       	mov    %al,0x80118025
801069cc:	a1 c4 b1 10 80       	mov    0x8010b1c4,%eax
801069d1:	c1 e8 10             	shr    $0x10,%eax
801069d4:	66 a3 26 80 11 80    	mov    %ax,0x80118026
  
  initlock(&tickslock, "time");
801069da:	c7 44 24 04 f4 8b 10 	movl   $0x80108bf4,0x4(%esp)
801069e1:	80 
801069e2:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
801069e9:	e8 68 e3 ff ff       	call   80104d56 <initlock>
}
801069ee:	c9                   	leave  
801069ef:	c3                   	ret    

801069f0 <idtinit>:

void
idtinit(void)
{
801069f0:	55                   	push   %ebp
801069f1:	89 e5                	mov    %esp,%ebp
801069f3:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
801069f6:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
801069fd:	00 
801069fe:	c7 04 24 20 7e 11 80 	movl   $0x80117e20,(%esp)
80106a05:	e8 38 fe ff ff       	call   80106842 <lidt>
}
80106a0a:	c9                   	leave  
80106a0b:	c3                   	ret    

80106a0c <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
80106a0c:	55                   	push   %ebp
80106a0d:	89 e5                	mov    %esp,%ebp
80106a0f:	57                   	push   %edi
80106a10:	56                   	push   %esi
80106a11:	53                   	push   %ebx
80106a12:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
80106a15:	8b 45 08             	mov    0x8(%ebp),%eax
80106a18:	8b 40 30             	mov    0x30(%eax),%eax
80106a1b:	83 f8 40             	cmp    $0x40,%eax
80106a1e:	75 3f                	jne    80106a5f <trap+0x53>
    if(proc->killed)
80106a20:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a26:	8b 40 24             	mov    0x24(%eax),%eax
80106a29:	85 c0                	test   %eax,%eax
80106a2b:	74 05                	je     80106a32 <trap+0x26>
      exit();
80106a2d:	e8 35 dc ff ff       	call   80104667 <exit>
    proc->tf = tf;
80106a32:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a38:	8b 55 08             	mov    0x8(%ebp),%edx
80106a3b:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
80106a3e:	e8 91 e9 ff ff       	call   801053d4 <syscall>
    if(proc->killed)
80106a43:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a49:	8b 40 24             	mov    0x24(%eax),%eax
80106a4c:	85 c0                	test   %eax,%eax
80106a4e:	74 0a                	je     80106a5a <trap+0x4e>
      exit();
80106a50:	e8 12 dc ff ff       	call   80104667 <exit>
    return;
80106a55:	e9 2d 02 00 00       	jmp    80106c87 <trap+0x27b>
80106a5a:	e9 28 02 00 00       	jmp    80106c87 <trap+0x27b>
  }

  switch(tf->trapno){
80106a5f:	8b 45 08             	mov    0x8(%ebp),%eax
80106a62:	8b 40 30             	mov    0x30(%eax),%eax
80106a65:	83 e8 20             	sub    $0x20,%eax
80106a68:	83 f8 1f             	cmp    $0x1f,%eax
80106a6b:	0f 87 bc 00 00 00    	ja     80106b2d <trap+0x121>
80106a71:	8b 04 85 9c 8c 10 80 	mov    -0x7fef7364(,%eax,4),%eax
80106a78:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
80106a7a:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106a80:	0f b6 00             	movzbl (%eax),%eax
80106a83:	84 c0                	test   %al,%al
80106a85:	75 31                	jne    80106ab8 <trap+0xac>
      acquire(&tickslock);
80106a87:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
80106a8e:	e8 e4 e2 ff ff       	call   80104d77 <acquire>
      ticks++;
80106a93:	a1 20 86 11 80       	mov    0x80118620,%eax
80106a98:	83 c0 01             	add    $0x1,%eax
80106a9b:	a3 20 86 11 80       	mov    %eax,0x80118620
      wakeup(&ticks);
80106aa0:	c7 04 24 20 86 11 80 	movl   $0x80118620,(%esp)
80106aa7:	e8 d4 e0 ff ff       	call   80104b80 <wakeup>
      release(&tickslock);
80106aac:	c7 04 24 e0 7d 11 80 	movl   $0x80117de0,(%esp)
80106ab3:	e8 21 e3 ff ff       	call   80104dd9 <release>
    }
    lapiceoi();
80106ab8:	e8 f6 c3 ff ff       	call   80102eb3 <lapiceoi>
    break;
80106abd:	e9 41 01 00 00       	jmp    80106c03 <trap+0x1f7>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
80106ac2:	e8 17 bc ff ff       	call   801026de <ideintr>
    lapiceoi();
80106ac7:	e8 e7 c3 ff ff       	call   80102eb3 <lapiceoi>
    break;
80106acc:	e9 32 01 00 00       	jmp    80106c03 <trap+0x1f7>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
80106ad1:	e8 c9 c1 ff ff       	call   80102c9f <kbdintr>
    lapiceoi();
80106ad6:	e8 d8 c3 ff ff       	call   80102eb3 <lapiceoi>
    break;
80106adb:	e9 23 01 00 00       	jmp    80106c03 <trap+0x1f7>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80106ae0:	e8 97 03 00 00       	call   80106e7c <uartintr>
    lapiceoi();
80106ae5:	e8 c9 c3 ff ff       	call   80102eb3 <lapiceoi>
    break;
80106aea:	e9 14 01 00 00       	jmp    80106c03 <trap+0x1f7>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106aef:	8b 45 08             	mov    0x8(%ebp),%eax
80106af2:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
80106af5:	8b 45 08             	mov    0x8(%ebp),%eax
80106af8:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106afc:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80106aff:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106b05:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106b08:	0f b6 c0             	movzbl %al,%eax
80106b0b:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106b0f:	89 54 24 08          	mov    %edx,0x8(%esp)
80106b13:	89 44 24 04          	mov    %eax,0x4(%esp)
80106b17:	c7 04 24 fc 8b 10 80 	movl   $0x80108bfc,(%esp)
80106b1e:	e8 7d 98 ff ff       	call   801003a0 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
80106b23:	e8 8b c3 ff ff       	call   80102eb3 <lapiceoi>
    break;
80106b28:	e9 d6 00 00 00       	jmp    80106c03 <trap+0x1f7>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
80106b2d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b33:	85 c0                	test   %eax,%eax
80106b35:	74 11                	je     80106b48 <trap+0x13c>
80106b37:	8b 45 08             	mov    0x8(%ebp),%eax
80106b3a:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106b3e:	0f b7 c0             	movzwl %ax,%eax
80106b41:	83 e0 03             	and    $0x3,%eax
80106b44:	85 c0                	test   %eax,%eax
80106b46:	75 46                	jne    80106b8e <trap+0x182>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106b48:	e8 1e fd ff ff       	call   8010686b <rcr2>
80106b4d:	8b 55 08             	mov    0x8(%ebp),%edx
80106b50:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106b53:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80106b5a:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106b5d:	0f b6 ca             	movzbl %dl,%ecx
80106b60:	8b 55 08             	mov    0x8(%ebp),%edx
80106b63:	8b 52 30             	mov    0x30(%edx),%edx
80106b66:	89 44 24 10          	mov    %eax,0x10(%esp)
80106b6a:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106b6e:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106b72:	89 54 24 04          	mov    %edx,0x4(%esp)
80106b76:	c7 04 24 20 8c 10 80 	movl   $0x80108c20,(%esp)
80106b7d:	e8 1e 98 ff ff       	call   801003a0 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80106b82:	c7 04 24 52 8c 10 80 	movl   $0x80108c52,(%esp)
80106b89:	e8 ac 99 ff ff       	call   8010053a <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106b8e:	e8 d8 fc ff ff       	call   8010686b <rcr2>
80106b93:	89 c2                	mov    %eax,%edx
80106b95:	8b 45 08             	mov    0x8(%ebp),%eax
80106b98:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106b9b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106ba1:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106ba4:	0f b6 f0             	movzbl %al,%esi
80106ba7:	8b 45 08             	mov    0x8(%ebp),%eax
80106baa:	8b 58 34             	mov    0x34(%eax),%ebx
80106bad:	8b 45 08             	mov    0x8(%ebp),%eax
80106bb0:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106bb3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106bb9:	83 c0 6c             	add    $0x6c,%eax
80106bbc:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106bbf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106bc5:	8b 40 10             	mov    0x10(%eax),%eax
80106bc8:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106bcc:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106bd0:	89 74 24 14          	mov    %esi,0x14(%esp)
80106bd4:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106bd8:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106bdc:	8b 75 e4             	mov    -0x1c(%ebp),%esi
80106bdf:	89 74 24 08          	mov    %esi,0x8(%esp)
80106be3:	89 44 24 04          	mov    %eax,0x4(%esp)
80106be7:	c7 04 24 58 8c 10 80 	movl   $0x80108c58,(%esp)
80106bee:	e8 ad 97 ff ff       	call   801003a0 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80106bf3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106bf9:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80106c00:	eb 01                	jmp    80106c03 <trap+0x1f7>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80106c02:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106c03:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c09:	85 c0                	test   %eax,%eax
80106c0b:	74 24                	je     80106c31 <trap+0x225>
80106c0d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c13:	8b 40 24             	mov    0x24(%eax),%eax
80106c16:	85 c0                	test   %eax,%eax
80106c18:	74 17                	je     80106c31 <trap+0x225>
80106c1a:	8b 45 08             	mov    0x8(%ebp),%eax
80106c1d:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106c21:	0f b7 c0             	movzwl %ax,%eax
80106c24:	83 e0 03             	and    $0x3,%eax
80106c27:	83 f8 03             	cmp    $0x3,%eax
80106c2a:	75 05                	jne    80106c31 <trap+0x225>
    exit();
80106c2c:	e8 36 da ff ff       	call   80104667 <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80106c31:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c37:	85 c0                	test   %eax,%eax
80106c39:	74 1e                	je     80106c59 <trap+0x24d>
80106c3b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c41:	8b 40 0c             	mov    0xc(%eax),%eax
80106c44:	83 f8 04             	cmp    $0x4,%eax
80106c47:	75 10                	jne    80106c59 <trap+0x24d>
80106c49:	8b 45 08             	mov    0x8(%ebp),%eax
80106c4c:	8b 40 30             	mov    0x30(%eax),%eax
80106c4f:	83 f8 20             	cmp    $0x20,%eax
80106c52:	75 05                	jne    80106c59 <trap+0x24d>
    yield();
80106c54:	e8 ed dd ff ff       	call   80104a46 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106c59:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c5f:	85 c0                	test   %eax,%eax
80106c61:	74 24                	je     80106c87 <trap+0x27b>
80106c63:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106c69:	8b 40 24             	mov    0x24(%eax),%eax
80106c6c:	85 c0                	test   %eax,%eax
80106c6e:	74 17                	je     80106c87 <trap+0x27b>
80106c70:	8b 45 08             	mov    0x8(%ebp),%eax
80106c73:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106c77:	0f b7 c0             	movzwl %ax,%eax
80106c7a:	83 e0 03             	and    $0x3,%eax
80106c7d:	83 f8 03             	cmp    $0x3,%eax
80106c80:	75 05                	jne    80106c87 <trap+0x27b>
    exit();
80106c82:	e8 e0 d9 ff ff       	call   80104667 <exit>
}
80106c87:	83 c4 3c             	add    $0x3c,%esp
80106c8a:	5b                   	pop    %ebx
80106c8b:	5e                   	pop    %esi
80106c8c:	5f                   	pop    %edi
80106c8d:	5d                   	pop    %ebp
80106c8e:	c3                   	ret    

80106c8f <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106c8f:	55                   	push   %ebp
80106c90:	89 e5                	mov    %esp,%ebp
80106c92:	83 ec 14             	sub    $0x14,%esp
80106c95:	8b 45 08             	mov    0x8(%ebp),%eax
80106c98:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106c9c:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80106ca0:	89 c2                	mov    %eax,%edx
80106ca2:	ec                   	in     (%dx),%al
80106ca3:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80106ca6:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80106caa:	c9                   	leave  
80106cab:	c3                   	ret    

80106cac <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106cac:	55                   	push   %ebp
80106cad:	89 e5                	mov    %esp,%ebp
80106caf:	83 ec 08             	sub    $0x8,%esp
80106cb2:	8b 55 08             	mov    0x8(%ebp),%edx
80106cb5:	8b 45 0c             	mov    0xc(%ebp),%eax
80106cb8:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106cbc:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106cbf:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106cc3:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106cc7:	ee                   	out    %al,(%dx)
}
80106cc8:	c9                   	leave  
80106cc9:	c3                   	ret    

80106cca <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106cca:	55                   	push   %ebp
80106ccb:	89 e5                	mov    %esp,%ebp
80106ccd:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106cd0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106cd7:	00 
80106cd8:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106cdf:	e8 c8 ff ff ff       	call   80106cac <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106ce4:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80106ceb:	00 
80106cec:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106cf3:	e8 b4 ff ff ff       	call   80106cac <outb>
  outb(COM1+0, 115200/9600);
80106cf8:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80106cff:	00 
80106d00:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106d07:	e8 a0 ff ff ff       	call   80106cac <outb>
  outb(COM1+1, 0);
80106d0c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106d13:	00 
80106d14:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106d1b:	e8 8c ff ff ff       	call   80106cac <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80106d20:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80106d27:	00 
80106d28:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106d2f:	e8 78 ff ff ff       	call   80106cac <outb>
  outb(COM1+4, 0);
80106d34:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106d3b:	00 
80106d3c:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80106d43:	e8 64 ff ff ff       	call   80106cac <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106d48:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106d4f:	00 
80106d50:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106d57:	e8 50 ff ff ff       	call   80106cac <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106d5c:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106d63:	e8 27 ff ff ff       	call   80106c8f <inb>
80106d68:	3c ff                	cmp    $0xff,%al
80106d6a:	75 02                	jne    80106d6e <uartinit+0xa4>
    return;
80106d6c:	eb 6a                	jmp    80106dd8 <uartinit+0x10e>
  uart = 1;
80106d6e:	c7 05 8c b6 10 80 01 	movl   $0x1,0x8010b68c
80106d75:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106d78:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106d7f:	e8 0b ff ff ff       	call   80106c8f <inb>
  inb(COM1+0);
80106d84:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106d8b:	e8 ff fe ff ff       	call   80106c8f <inb>
  picenable(IRQ_COM1);
80106d90:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106d97:	e8 d5 cd ff ff       	call   80103b71 <picenable>
  ioapicenable(IRQ_COM1, 0);
80106d9c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106da3:	00 
80106da4:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106dab:	e8 ad bb ff ff       	call   8010295d <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106db0:	c7 45 f4 1c 8d 10 80 	movl   $0x80108d1c,-0xc(%ebp)
80106db7:	eb 15                	jmp    80106dce <uartinit+0x104>
    uartputc(*p);
80106db9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106dbc:	0f b6 00             	movzbl (%eax),%eax
80106dbf:	0f be c0             	movsbl %al,%eax
80106dc2:	89 04 24             	mov    %eax,(%esp)
80106dc5:	e8 10 00 00 00       	call   80106dda <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106dca:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106dce:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106dd1:	0f b6 00             	movzbl (%eax),%eax
80106dd4:	84 c0                	test   %al,%al
80106dd6:	75 e1                	jne    80106db9 <uartinit+0xef>
    uartputc(*p);
}
80106dd8:	c9                   	leave  
80106dd9:	c3                   	ret    

80106dda <uartputc>:

void
uartputc(int c)
{
80106dda:	55                   	push   %ebp
80106ddb:	89 e5                	mov    %esp,%ebp
80106ddd:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106de0:	a1 8c b6 10 80       	mov    0x8010b68c,%eax
80106de5:	85 c0                	test   %eax,%eax
80106de7:	75 02                	jne    80106deb <uartputc+0x11>
    return;
80106de9:	eb 4b                	jmp    80106e36 <uartputc+0x5c>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106deb:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106df2:	eb 10                	jmp    80106e04 <uartputc+0x2a>
    microdelay(10);
80106df4:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80106dfb:	e8 d8 c0 ff ff       	call   80102ed8 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106e00:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106e04:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106e08:	7f 16                	jg     80106e20 <uartputc+0x46>
80106e0a:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106e11:	e8 79 fe ff ff       	call   80106c8f <inb>
80106e16:	0f b6 c0             	movzbl %al,%eax
80106e19:	83 e0 20             	and    $0x20,%eax
80106e1c:	85 c0                	test   %eax,%eax
80106e1e:	74 d4                	je     80106df4 <uartputc+0x1a>
    microdelay(10);
  outb(COM1+0, c);
80106e20:	8b 45 08             	mov    0x8(%ebp),%eax
80106e23:	0f b6 c0             	movzbl %al,%eax
80106e26:	89 44 24 04          	mov    %eax,0x4(%esp)
80106e2a:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106e31:	e8 76 fe ff ff       	call   80106cac <outb>
}
80106e36:	c9                   	leave  
80106e37:	c3                   	ret    

80106e38 <uartgetc>:

static int
uartgetc(void)
{
80106e38:	55                   	push   %ebp
80106e39:	89 e5                	mov    %esp,%ebp
80106e3b:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106e3e:	a1 8c b6 10 80       	mov    0x8010b68c,%eax
80106e43:	85 c0                	test   %eax,%eax
80106e45:	75 07                	jne    80106e4e <uartgetc+0x16>
    return -1;
80106e47:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106e4c:	eb 2c                	jmp    80106e7a <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106e4e:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106e55:	e8 35 fe ff ff       	call   80106c8f <inb>
80106e5a:	0f b6 c0             	movzbl %al,%eax
80106e5d:	83 e0 01             	and    $0x1,%eax
80106e60:	85 c0                	test   %eax,%eax
80106e62:	75 07                	jne    80106e6b <uartgetc+0x33>
    return -1;
80106e64:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106e69:	eb 0f                	jmp    80106e7a <uartgetc+0x42>
  return inb(COM1+0);
80106e6b:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106e72:	e8 18 fe ff ff       	call   80106c8f <inb>
80106e77:	0f b6 c0             	movzbl %al,%eax
}
80106e7a:	c9                   	leave  
80106e7b:	c3                   	ret    

80106e7c <uartintr>:

void
uartintr(void)
{
80106e7c:	55                   	push   %ebp
80106e7d:	89 e5                	mov    %esp,%ebp
80106e7f:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80106e82:	c7 04 24 38 6e 10 80 	movl   $0x80106e38,(%esp)
80106e89:	e8 1f 99 ff ff       	call   801007ad <consoleintr>
}
80106e8e:	c9                   	leave  
80106e8f:	c3                   	ret    

80106e90 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80106e90:	6a 00                	push   $0x0
  pushl $0
80106e92:	6a 00                	push   $0x0
  jmp alltraps
80106e94:	e9 7e f9 ff ff       	jmp    80106817 <alltraps>

80106e99 <vector1>:
.globl vector1
vector1:
  pushl $0
80106e99:	6a 00                	push   $0x0
  pushl $1
80106e9b:	6a 01                	push   $0x1
  jmp alltraps
80106e9d:	e9 75 f9 ff ff       	jmp    80106817 <alltraps>

80106ea2 <vector2>:
.globl vector2
vector2:
  pushl $0
80106ea2:	6a 00                	push   $0x0
  pushl $2
80106ea4:	6a 02                	push   $0x2
  jmp alltraps
80106ea6:	e9 6c f9 ff ff       	jmp    80106817 <alltraps>

80106eab <vector3>:
.globl vector3
vector3:
  pushl $0
80106eab:	6a 00                	push   $0x0
  pushl $3
80106ead:	6a 03                	push   $0x3
  jmp alltraps
80106eaf:	e9 63 f9 ff ff       	jmp    80106817 <alltraps>

80106eb4 <vector4>:
.globl vector4
vector4:
  pushl $0
80106eb4:	6a 00                	push   $0x0
  pushl $4
80106eb6:	6a 04                	push   $0x4
  jmp alltraps
80106eb8:	e9 5a f9 ff ff       	jmp    80106817 <alltraps>

80106ebd <vector5>:
.globl vector5
vector5:
  pushl $0
80106ebd:	6a 00                	push   $0x0
  pushl $5
80106ebf:	6a 05                	push   $0x5
  jmp alltraps
80106ec1:	e9 51 f9 ff ff       	jmp    80106817 <alltraps>

80106ec6 <vector6>:
.globl vector6
vector6:
  pushl $0
80106ec6:	6a 00                	push   $0x0
  pushl $6
80106ec8:	6a 06                	push   $0x6
  jmp alltraps
80106eca:	e9 48 f9 ff ff       	jmp    80106817 <alltraps>

80106ecf <vector7>:
.globl vector7
vector7:
  pushl $0
80106ecf:	6a 00                	push   $0x0
  pushl $7
80106ed1:	6a 07                	push   $0x7
  jmp alltraps
80106ed3:	e9 3f f9 ff ff       	jmp    80106817 <alltraps>

80106ed8 <vector8>:
.globl vector8
vector8:
  pushl $8
80106ed8:	6a 08                	push   $0x8
  jmp alltraps
80106eda:	e9 38 f9 ff ff       	jmp    80106817 <alltraps>

80106edf <vector9>:
.globl vector9
vector9:
  pushl $0
80106edf:	6a 00                	push   $0x0
  pushl $9
80106ee1:	6a 09                	push   $0x9
  jmp alltraps
80106ee3:	e9 2f f9 ff ff       	jmp    80106817 <alltraps>

80106ee8 <vector10>:
.globl vector10
vector10:
  pushl $10
80106ee8:	6a 0a                	push   $0xa
  jmp alltraps
80106eea:	e9 28 f9 ff ff       	jmp    80106817 <alltraps>

80106eef <vector11>:
.globl vector11
vector11:
  pushl $11
80106eef:	6a 0b                	push   $0xb
  jmp alltraps
80106ef1:	e9 21 f9 ff ff       	jmp    80106817 <alltraps>

80106ef6 <vector12>:
.globl vector12
vector12:
  pushl $12
80106ef6:	6a 0c                	push   $0xc
  jmp alltraps
80106ef8:	e9 1a f9 ff ff       	jmp    80106817 <alltraps>

80106efd <vector13>:
.globl vector13
vector13:
  pushl $13
80106efd:	6a 0d                	push   $0xd
  jmp alltraps
80106eff:	e9 13 f9 ff ff       	jmp    80106817 <alltraps>

80106f04 <vector14>:
.globl vector14
vector14:
  pushl $14
80106f04:	6a 0e                	push   $0xe
  jmp alltraps
80106f06:	e9 0c f9 ff ff       	jmp    80106817 <alltraps>

80106f0b <vector15>:
.globl vector15
vector15:
  pushl $0
80106f0b:	6a 00                	push   $0x0
  pushl $15
80106f0d:	6a 0f                	push   $0xf
  jmp alltraps
80106f0f:	e9 03 f9 ff ff       	jmp    80106817 <alltraps>

80106f14 <vector16>:
.globl vector16
vector16:
  pushl $0
80106f14:	6a 00                	push   $0x0
  pushl $16
80106f16:	6a 10                	push   $0x10
  jmp alltraps
80106f18:	e9 fa f8 ff ff       	jmp    80106817 <alltraps>

80106f1d <vector17>:
.globl vector17
vector17:
  pushl $17
80106f1d:	6a 11                	push   $0x11
  jmp alltraps
80106f1f:	e9 f3 f8 ff ff       	jmp    80106817 <alltraps>

80106f24 <vector18>:
.globl vector18
vector18:
  pushl $0
80106f24:	6a 00                	push   $0x0
  pushl $18
80106f26:	6a 12                	push   $0x12
  jmp alltraps
80106f28:	e9 ea f8 ff ff       	jmp    80106817 <alltraps>

80106f2d <vector19>:
.globl vector19
vector19:
  pushl $0
80106f2d:	6a 00                	push   $0x0
  pushl $19
80106f2f:	6a 13                	push   $0x13
  jmp alltraps
80106f31:	e9 e1 f8 ff ff       	jmp    80106817 <alltraps>

80106f36 <vector20>:
.globl vector20
vector20:
  pushl $0
80106f36:	6a 00                	push   $0x0
  pushl $20
80106f38:	6a 14                	push   $0x14
  jmp alltraps
80106f3a:	e9 d8 f8 ff ff       	jmp    80106817 <alltraps>

80106f3f <vector21>:
.globl vector21
vector21:
  pushl $0
80106f3f:	6a 00                	push   $0x0
  pushl $21
80106f41:	6a 15                	push   $0x15
  jmp alltraps
80106f43:	e9 cf f8 ff ff       	jmp    80106817 <alltraps>

80106f48 <vector22>:
.globl vector22
vector22:
  pushl $0
80106f48:	6a 00                	push   $0x0
  pushl $22
80106f4a:	6a 16                	push   $0x16
  jmp alltraps
80106f4c:	e9 c6 f8 ff ff       	jmp    80106817 <alltraps>

80106f51 <vector23>:
.globl vector23
vector23:
  pushl $0
80106f51:	6a 00                	push   $0x0
  pushl $23
80106f53:	6a 17                	push   $0x17
  jmp alltraps
80106f55:	e9 bd f8 ff ff       	jmp    80106817 <alltraps>

80106f5a <vector24>:
.globl vector24
vector24:
  pushl $0
80106f5a:	6a 00                	push   $0x0
  pushl $24
80106f5c:	6a 18                	push   $0x18
  jmp alltraps
80106f5e:	e9 b4 f8 ff ff       	jmp    80106817 <alltraps>

80106f63 <vector25>:
.globl vector25
vector25:
  pushl $0
80106f63:	6a 00                	push   $0x0
  pushl $25
80106f65:	6a 19                	push   $0x19
  jmp alltraps
80106f67:	e9 ab f8 ff ff       	jmp    80106817 <alltraps>

80106f6c <vector26>:
.globl vector26
vector26:
  pushl $0
80106f6c:	6a 00                	push   $0x0
  pushl $26
80106f6e:	6a 1a                	push   $0x1a
  jmp alltraps
80106f70:	e9 a2 f8 ff ff       	jmp    80106817 <alltraps>

80106f75 <vector27>:
.globl vector27
vector27:
  pushl $0
80106f75:	6a 00                	push   $0x0
  pushl $27
80106f77:	6a 1b                	push   $0x1b
  jmp alltraps
80106f79:	e9 99 f8 ff ff       	jmp    80106817 <alltraps>

80106f7e <vector28>:
.globl vector28
vector28:
  pushl $0
80106f7e:	6a 00                	push   $0x0
  pushl $28
80106f80:	6a 1c                	push   $0x1c
  jmp alltraps
80106f82:	e9 90 f8 ff ff       	jmp    80106817 <alltraps>

80106f87 <vector29>:
.globl vector29
vector29:
  pushl $0
80106f87:	6a 00                	push   $0x0
  pushl $29
80106f89:	6a 1d                	push   $0x1d
  jmp alltraps
80106f8b:	e9 87 f8 ff ff       	jmp    80106817 <alltraps>

80106f90 <vector30>:
.globl vector30
vector30:
  pushl $0
80106f90:	6a 00                	push   $0x0
  pushl $30
80106f92:	6a 1e                	push   $0x1e
  jmp alltraps
80106f94:	e9 7e f8 ff ff       	jmp    80106817 <alltraps>

80106f99 <vector31>:
.globl vector31
vector31:
  pushl $0
80106f99:	6a 00                	push   $0x0
  pushl $31
80106f9b:	6a 1f                	push   $0x1f
  jmp alltraps
80106f9d:	e9 75 f8 ff ff       	jmp    80106817 <alltraps>

80106fa2 <vector32>:
.globl vector32
vector32:
  pushl $0
80106fa2:	6a 00                	push   $0x0
  pushl $32
80106fa4:	6a 20                	push   $0x20
  jmp alltraps
80106fa6:	e9 6c f8 ff ff       	jmp    80106817 <alltraps>

80106fab <vector33>:
.globl vector33
vector33:
  pushl $0
80106fab:	6a 00                	push   $0x0
  pushl $33
80106fad:	6a 21                	push   $0x21
  jmp alltraps
80106faf:	e9 63 f8 ff ff       	jmp    80106817 <alltraps>

80106fb4 <vector34>:
.globl vector34
vector34:
  pushl $0
80106fb4:	6a 00                	push   $0x0
  pushl $34
80106fb6:	6a 22                	push   $0x22
  jmp alltraps
80106fb8:	e9 5a f8 ff ff       	jmp    80106817 <alltraps>

80106fbd <vector35>:
.globl vector35
vector35:
  pushl $0
80106fbd:	6a 00                	push   $0x0
  pushl $35
80106fbf:	6a 23                	push   $0x23
  jmp alltraps
80106fc1:	e9 51 f8 ff ff       	jmp    80106817 <alltraps>

80106fc6 <vector36>:
.globl vector36
vector36:
  pushl $0
80106fc6:	6a 00                	push   $0x0
  pushl $36
80106fc8:	6a 24                	push   $0x24
  jmp alltraps
80106fca:	e9 48 f8 ff ff       	jmp    80106817 <alltraps>

80106fcf <vector37>:
.globl vector37
vector37:
  pushl $0
80106fcf:	6a 00                	push   $0x0
  pushl $37
80106fd1:	6a 25                	push   $0x25
  jmp alltraps
80106fd3:	e9 3f f8 ff ff       	jmp    80106817 <alltraps>

80106fd8 <vector38>:
.globl vector38
vector38:
  pushl $0
80106fd8:	6a 00                	push   $0x0
  pushl $38
80106fda:	6a 26                	push   $0x26
  jmp alltraps
80106fdc:	e9 36 f8 ff ff       	jmp    80106817 <alltraps>

80106fe1 <vector39>:
.globl vector39
vector39:
  pushl $0
80106fe1:	6a 00                	push   $0x0
  pushl $39
80106fe3:	6a 27                	push   $0x27
  jmp alltraps
80106fe5:	e9 2d f8 ff ff       	jmp    80106817 <alltraps>

80106fea <vector40>:
.globl vector40
vector40:
  pushl $0
80106fea:	6a 00                	push   $0x0
  pushl $40
80106fec:	6a 28                	push   $0x28
  jmp alltraps
80106fee:	e9 24 f8 ff ff       	jmp    80106817 <alltraps>

80106ff3 <vector41>:
.globl vector41
vector41:
  pushl $0
80106ff3:	6a 00                	push   $0x0
  pushl $41
80106ff5:	6a 29                	push   $0x29
  jmp alltraps
80106ff7:	e9 1b f8 ff ff       	jmp    80106817 <alltraps>

80106ffc <vector42>:
.globl vector42
vector42:
  pushl $0
80106ffc:	6a 00                	push   $0x0
  pushl $42
80106ffe:	6a 2a                	push   $0x2a
  jmp alltraps
80107000:	e9 12 f8 ff ff       	jmp    80106817 <alltraps>

80107005 <vector43>:
.globl vector43
vector43:
  pushl $0
80107005:	6a 00                	push   $0x0
  pushl $43
80107007:	6a 2b                	push   $0x2b
  jmp alltraps
80107009:	e9 09 f8 ff ff       	jmp    80106817 <alltraps>

8010700e <vector44>:
.globl vector44
vector44:
  pushl $0
8010700e:	6a 00                	push   $0x0
  pushl $44
80107010:	6a 2c                	push   $0x2c
  jmp alltraps
80107012:	e9 00 f8 ff ff       	jmp    80106817 <alltraps>

80107017 <vector45>:
.globl vector45
vector45:
  pushl $0
80107017:	6a 00                	push   $0x0
  pushl $45
80107019:	6a 2d                	push   $0x2d
  jmp alltraps
8010701b:	e9 f7 f7 ff ff       	jmp    80106817 <alltraps>

80107020 <vector46>:
.globl vector46
vector46:
  pushl $0
80107020:	6a 00                	push   $0x0
  pushl $46
80107022:	6a 2e                	push   $0x2e
  jmp alltraps
80107024:	e9 ee f7 ff ff       	jmp    80106817 <alltraps>

80107029 <vector47>:
.globl vector47
vector47:
  pushl $0
80107029:	6a 00                	push   $0x0
  pushl $47
8010702b:	6a 2f                	push   $0x2f
  jmp alltraps
8010702d:	e9 e5 f7 ff ff       	jmp    80106817 <alltraps>

80107032 <vector48>:
.globl vector48
vector48:
  pushl $0
80107032:	6a 00                	push   $0x0
  pushl $48
80107034:	6a 30                	push   $0x30
  jmp alltraps
80107036:	e9 dc f7 ff ff       	jmp    80106817 <alltraps>

8010703b <vector49>:
.globl vector49
vector49:
  pushl $0
8010703b:	6a 00                	push   $0x0
  pushl $49
8010703d:	6a 31                	push   $0x31
  jmp alltraps
8010703f:	e9 d3 f7 ff ff       	jmp    80106817 <alltraps>

80107044 <vector50>:
.globl vector50
vector50:
  pushl $0
80107044:	6a 00                	push   $0x0
  pushl $50
80107046:	6a 32                	push   $0x32
  jmp alltraps
80107048:	e9 ca f7 ff ff       	jmp    80106817 <alltraps>

8010704d <vector51>:
.globl vector51
vector51:
  pushl $0
8010704d:	6a 00                	push   $0x0
  pushl $51
8010704f:	6a 33                	push   $0x33
  jmp alltraps
80107051:	e9 c1 f7 ff ff       	jmp    80106817 <alltraps>

80107056 <vector52>:
.globl vector52
vector52:
  pushl $0
80107056:	6a 00                	push   $0x0
  pushl $52
80107058:	6a 34                	push   $0x34
  jmp alltraps
8010705a:	e9 b8 f7 ff ff       	jmp    80106817 <alltraps>

8010705f <vector53>:
.globl vector53
vector53:
  pushl $0
8010705f:	6a 00                	push   $0x0
  pushl $53
80107061:	6a 35                	push   $0x35
  jmp alltraps
80107063:	e9 af f7 ff ff       	jmp    80106817 <alltraps>

80107068 <vector54>:
.globl vector54
vector54:
  pushl $0
80107068:	6a 00                	push   $0x0
  pushl $54
8010706a:	6a 36                	push   $0x36
  jmp alltraps
8010706c:	e9 a6 f7 ff ff       	jmp    80106817 <alltraps>

80107071 <vector55>:
.globl vector55
vector55:
  pushl $0
80107071:	6a 00                	push   $0x0
  pushl $55
80107073:	6a 37                	push   $0x37
  jmp alltraps
80107075:	e9 9d f7 ff ff       	jmp    80106817 <alltraps>

8010707a <vector56>:
.globl vector56
vector56:
  pushl $0
8010707a:	6a 00                	push   $0x0
  pushl $56
8010707c:	6a 38                	push   $0x38
  jmp alltraps
8010707e:	e9 94 f7 ff ff       	jmp    80106817 <alltraps>

80107083 <vector57>:
.globl vector57
vector57:
  pushl $0
80107083:	6a 00                	push   $0x0
  pushl $57
80107085:	6a 39                	push   $0x39
  jmp alltraps
80107087:	e9 8b f7 ff ff       	jmp    80106817 <alltraps>

8010708c <vector58>:
.globl vector58
vector58:
  pushl $0
8010708c:	6a 00                	push   $0x0
  pushl $58
8010708e:	6a 3a                	push   $0x3a
  jmp alltraps
80107090:	e9 82 f7 ff ff       	jmp    80106817 <alltraps>

80107095 <vector59>:
.globl vector59
vector59:
  pushl $0
80107095:	6a 00                	push   $0x0
  pushl $59
80107097:	6a 3b                	push   $0x3b
  jmp alltraps
80107099:	e9 79 f7 ff ff       	jmp    80106817 <alltraps>

8010709e <vector60>:
.globl vector60
vector60:
  pushl $0
8010709e:	6a 00                	push   $0x0
  pushl $60
801070a0:	6a 3c                	push   $0x3c
  jmp alltraps
801070a2:	e9 70 f7 ff ff       	jmp    80106817 <alltraps>

801070a7 <vector61>:
.globl vector61
vector61:
  pushl $0
801070a7:	6a 00                	push   $0x0
  pushl $61
801070a9:	6a 3d                	push   $0x3d
  jmp alltraps
801070ab:	e9 67 f7 ff ff       	jmp    80106817 <alltraps>

801070b0 <vector62>:
.globl vector62
vector62:
  pushl $0
801070b0:	6a 00                	push   $0x0
  pushl $62
801070b2:	6a 3e                	push   $0x3e
  jmp alltraps
801070b4:	e9 5e f7 ff ff       	jmp    80106817 <alltraps>

801070b9 <vector63>:
.globl vector63
vector63:
  pushl $0
801070b9:	6a 00                	push   $0x0
  pushl $63
801070bb:	6a 3f                	push   $0x3f
  jmp alltraps
801070bd:	e9 55 f7 ff ff       	jmp    80106817 <alltraps>

801070c2 <vector64>:
.globl vector64
vector64:
  pushl $0
801070c2:	6a 00                	push   $0x0
  pushl $64
801070c4:	6a 40                	push   $0x40
  jmp alltraps
801070c6:	e9 4c f7 ff ff       	jmp    80106817 <alltraps>

801070cb <vector65>:
.globl vector65
vector65:
  pushl $0
801070cb:	6a 00                	push   $0x0
  pushl $65
801070cd:	6a 41                	push   $0x41
  jmp alltraps
801070cf:	e9 43 f7 ff ff       	jmp    80106817 <alltraps>

801070d4 <vector66>:
.globl vector66
vector66:
  pushl $0
801070d4:	6a 00                	push   $0x0
  pushl $66
801070d6:	6a 42                	push   $0x42
  jmp alltraps
801070d8:	e9 3a f7 ff ff       	jmp    80106817 <alltraps>

801070dd <vector67>:
.globl vector67
vector67:
  pushl $0
801070dd:	6a 00                	push   $0x0
  pushl $67
801070df:	6a 43                	push   $0x43
  jmp alltraps
801070e1:	e9 31 f7 ff ff       	jmp    80106817 <alltraps>

801070e6 <vector68>:
.globl vector68
vector68:
  pushl $0
801070e6:	6a 00                	push   $0x0
  pushl $68
801070e8:	6a 44                	push   $0x44
  jmp alltraps
801070ea:	e9 28 f7 ff ff       	jmp    80106817 <alltraps>

801070ef <vector69>:
.globl vector69
vector69:
  pushl $0
801070ef:	6a 00                	push   $0x0
  pushl $69
801070f1:	6a 45                	push   $0x45
  jmp alltraps
801070f3:	e9 1f f7 ff ff       	jmp    80106817 <alltraps>

801070f8 <vector70>:
.globl vector70
vector70:
  pushl $0
801070f8:	6a 00                	push   $0x0
  pushl $70
801070fa:	6a 46                	push   $0x46
  jmp alltraps
801070fc:	e9 16 f7 ff ff       	jmp    80106817 <alltraps>

80107101 <vector71>:
.globl vector71
vector71:
  pushl $0
80107101:	6a 00                	push   $0x0
  pushl $71
80107103:	6a 47                	push   $0x47
  jmp alltraps
80107105:	e9 0d f7 ff ff       	jmp    80106817 <alltraps>

8010710a <vector72>:
.globl vector72
vector72:
  pushl $0
8010710a:	6a 00                	push   $0x0
  pushl $72
8010710c:	6a 48                	push   $0x48
  jmp alltraps
8010710e:	e9 04 f7 ff ff       	jmp    80106817 <alltraps>

80107113 <vector73>:
.globl vector73
vector73:
  pushl $0
80107113:	6a 00                	push   $0x0
  pushl $73
80107115:	6a 49                	push   $0x49
  jmp alltraps
80107117:	e9 fb f6 ff ff       	jmp    80106817 <alltraps>

8010711c <vector74>:
.globl vector74
vector74:
  pushl $0
8010711c:	6a 00                	push   $0x0
  pushl $74
8010711e:	6a 4a                	push   $0x4a
  jmp alltraps
80107120:	e9 f2 f6 ff ff       	jmp    80106817 <alltraps>

80107125 <vector75>:
.globl vector75
vector75:
  pushl $0
80107125:	6a 00                	push   $0x0
  pushl $75
80107127:	6a 4b                	push   $0x4b
  jmp alltraps
80107129:	e9 e9 f6 ff ff       	jmp    80106817 <alltraps>

8010712e <vector76>:
.globl vector76
vector76:
  pushl $0
8010712e:	6a 00                	push   $0x0
  pushl $76
80107130:	6a 4c                	push   $0x4c
  jmp alltraps
80107132:	e9 e0 f6 ff ff       	jmp    80106817 <alltraps>

80107137 <vector77>:
.globl vector77
vector77:
  pushl $0
80107137:	6a 00                	push   $0x0
  pushl $77
80107139:	6a 4d                	push   $0x4d
  jmp alltraps
8010713b:	e9 d7 f6 ff ff       	jmp    80106817 <alltraps>

80107140 <vector78>:
.globl vector78
vector78:
  pushl $0
80107140:	6a 00                	push   $0x0
  pushl $78
80107142:	6a 4e                	push   $0x4e
  jmp alltraps
80107144:	e9 ce f6 ff ff       	jmp    80106817 <alltraps>

80107149 <vector79>:
.globl vector79
vector79:
  pushl $0
80107149:	6a 00                	push   $0x0
  pushl $79
8010714b:	6a 4f                	push   $0x4f
  jmp alltraps
8010714d:	e9 c5 f6 ff ff       	jmp    80106817 <alltraps>

80107152 <vector80>:
.globl vector80
vector80:
  pushl $0
80107152:	6a 00                	push   $0x0
  pushl $80
80107154:	6a 50                	push   $0x50
  jmp alltraps
80107156:	e9 bc f6 ff ff       	jmp    80106817 <alltraps>

8010715b <vector81>:
.globl vector81
vector81:
  pushl $0
8010715b:	6a 00                	push   $0x0
  pushl $81
8010715d:	6a 51                	push   $0x51
  jmp alltraps
8010715f:	e9 b3 f6 ff ff       	jmp    80106817 <alltraps>

80107164 <vector82>:
.globl vector82
vector82:
  pushl $0
80107164:	6a 00                	push   $0x0
  pushl $82
80107166:	6a 52                	push   $0x52
  jmp alltraps
80107168:	e9 aa f6 ff ff       	jmp    80106817 <alltraps>

8010716d <vector83>:
.globl vector83
vector83:
  pushl $0
8010716d:	6a 00                	push   $0x0
  pushl $83
8010716f:	6a 53                	push   $0x53
  jmp alltraps
80107171:	e9 a1 f6 ff ff       	jmp    80106817 <alltraps>

80107176 <vector84>:
.globl vector84
vector84:
  pushl $0
80107176:	6a 00                	push   $0x0
  pushl $84
80107178:	6a 54                	push   $0x54
  jmp alltraps
8010717a:	e9 98 f6 ff ff       	jmp    80106817 <alltraps>

8010717f <vector85>:
.globl vector85
vector85:
  pushl $0
8010717f:	6a 00                	push   $0x0
  pushl $85
80107181:	6a 55                	push   $0x55
  jmp alltraps
80107183:	e9 8f f6 ff ff       	jmp    80106817 <alltraps>

80107188 <vector86>:
.globl vector86
vector86:
  pushl $0
80107188:	6a 00                	push   $0x0
  pushl $86
8010718a:	6a 56                	push   $0x56
  jmp alltraps
8010718c:	e9 86 f6 ff ff       	jmp    80106817 <alltraps>

80107191 <vector87>:
.globl vector87
vector87:
  pushl $0
80107191:	6a 00                	push   $0x0
  pushl $87
80107193:	6a 57                	push   $0x57
  jmp alltraps
80107195:	e9 7d f6 ff ff       	jmp    80106817 <alltraps>

8010719a <vector88>:
.globl vector88
vector88:
  pushl $0
8010719a:	6a 00                	push   $0x0
  pushl $88
8010719c:	6a 58                	push   $0x58
  jmp alltraps
8010719e:	e9 74 f6 ff ff       	jmp    80106817 <alltraps>

801071a3 <vector89>:
.globl vector89
vector89:
  pushl $0
801071a3:	6a 00                	push   $0x0
  pushl $89
801071a5:	6a 59                	push   $0x59
  jmp alltraps
801071a7:	e9 6b f6 ff ff       	jmp    80106817 <alltraps>

801071ac <vector90>:
.globl vector90
vector90:
  pushl $0
801071ac:	6a 00                	push   $0x0
  pushl $90
801071ae:	6a 5a                	push   $0x5a
  jmp alltraps
801071b0:	e9 62 f6 ff ff       	jmp    80106817 <alltraps>

801071b5 <vector91>:
.globl vector91
vector91:
  pushl $0
801071b5:	6a 00                	push   $0x0
  pushl $91
801071b7:	6a 5b                	push   $0x5b
  jmp alltraps
801071b9:	e9 59 f6 ff ff       	jmp    80106817 <alltraps>

801071be <vector92>:
.globl vector92
vector92:
  pushl $0
801071be:	6a 00                	push   $0x0
  pushl $92
801071c0:	6a 5c                	push   $0x5c
  jmp alltraps
801071c2:	e9 50 f6 ff ff       	jmp    80106817 <alltraps>

801071c7 <vector93>:
.globl vector93
vector93:
  pushl $0
801071c7:	6a 00                	push   $0x0
  pushl $93
801071c9:	6a 5d                	push   $0x5d
  jmp alltraps
801071cb:	e9 47 f6 ff ff       	jmp    80106817 <alltraps>

801071d0 <vector94>:
.globl vector94
vector94:
  pushl $0
801071d0:	6a 00                	push   $0x0
  pushl $94
801071d2:	6a 5e                	push   $0x5e
  jmp alltraps
801071d4:	e9 3e f6 ff ff       	jmp    80106817 <alltraps>

801071d9 <vector95>:
.globl vector95
vector95:
  pushl $0
801071d9:	6a 00                	push   $0x0
  pushl $95
801071db:	6a 5f                	push   $0x5f
  jmp alltraps
801071dd:	e9 35 f6 ff ff       	jmp    80106817 <alltraps>

801071e2 <vector96>:
.globl vector96
vector96:
  pushl $0
801071e2:	6a 00                	push   $0x0
  pushl $96
801071e4:	6a 60                	push   $0x60
  jmp alltraps
801071e6:	e9 2c f6 ff ff       	jmp    80106817 <alltraps>

801071eb <vector97>:
.globl vector97
vector97:
  pushl $0
801071eb:	6a 00                	push   $0x0
  pushl $97
801071ed:	6a 61                	push   $0x61
  jmp alltraps
801071ef:	e9 23 f6 ff ff       	jmp    80106817 <alltraps>

801071f4 <vector98>:
.globl vector98
vector98:
  pushl $0
801071f4:	6a 00                	push   $0x0
  pushl $98
801071f6:	6a 62                	push   $0x62
  jmp alltraps
801071f8:	e9 1a f6 ff ff       	jmp    80106817 <alltraps>

801071fd <vector99>:
.globl vector99
vector99:
  pushl $0
801071fd:	6a 00                	push   $0x0
  pushl $99
801071ff:	6a 63                	push   $0x63
  jmp alltraps
80107201:	e9 11 f6 ff ff       	jmp    80106817 <alltraps>

80107206 <vector100>:
.globl vector100
vector100:
  pushl $0
80107206:	6a 00                	push   $0x0
  pushl $100
80107208:	6a 64                	push   $0x64
  jmp alltraps
8010720a:	e9 08 f6 ff ff       	jmp    80106817 <alltraps>

8010720f <vector101>:
.globl vector101
vector101:
  pushl $0
8010720f:	6a 00                	push   $0x0
  pushl $101
80107211:	6a 65                	push   $0x65
  jmp alltraps
80107213:	e9 ff f5 ff ff       	jmp    80106817 <alltraps>

80107218 <vector102>:
.globl vector102
vector102:
  pushl $0
80107218:	6a 00                	push   $0x0
  pushl $102
8010721a:	6a 66                	push   $0x66
  jmp alltraps
8010721c:	e9 f6 f5 ff ff       	jmp    80106817 <alltraps>

80107221 <vector103>:
.globl vector103
vector103:
  pushl $0
80107221:	6a 00                	push   $0x0
  pushl $103
80107223:	6a 67                	push   $0x67
  jmp alltraps
80107225:	e9 ed f5 ff ff       	jmp    80106817 <alltraps>

8010722a <vector104>:
.globl vector104
vector104:
  pushl $0
8010722a:	6a 00                	push   $0x0
  pushl $104
8010722c:	6a 68                	push   $0x68
  jmp alltraps
8010722e:	e9 e4 f5 ff ff       	jmp    80106817 <alltraps>

80107233 <vector105>:
.globl vector105
vector105:
  pushl $0
80107233:	6a 00                	push   $0x0
  pushl $105
80107235:	6a 69                	push   $0x69
  jmp alltraps
80107237:	e9 db f5 ff ff       	jmp    80106817 <alltraps>

8010723c <vector106>:
.globl vector106
vector106:
  pushl $0
8010723c:	6a 00                	push   $0x0
  pushl $106
8010723e:	6a 6a                	push   $0x6a
  jmp alltraps
80107240:	e9 d2 f5 ff ff       	jmp    80106817 <alltraps>

80107245 <vector107>:
.globl vector107
vector107:
  pushl $0
80107245:	6a 00                	push   $0x0
  pushl $107
80107247:	6a 6b                	push   $0x6b
  jmp alltraps
80107249:	e9 c9 f5 ff ff       	jmp    80106817 <alltraps>

8010724e <vector108>:
.globl vector108
vector108:
  pushl $0
8010724e:	6a 00                	push   $0x0
  pushl $108
80107250:	6a 6c                	push   $0x6c
  jmp alltraps
80107252:	e9 c0 f5 ff ff       	jmp    80106817 <alltraps>

80107257 <vector109>:
.globl vector109
vector109:
  pushl $0
80107257:	6a 00                	push   $0x0
  pushl $109
80107259:	6a 6d                	push   $0x6d
  jmp alltraps
8010725b:	e9 b7 f5 ff ff       	jmp    80106817 <alltraps>

80107260 <vector110>:
.globl vector110
vector110:
  pushl $0
80107260:	6a 00                	push   $0x0
  pushl $110
80107262:	6a 6e                	push   $0x6e
  jmp alltraps
80107264:	e9 ae f5 ff ff       	jmp    80106817 <alltraps>

80107269 <vector111>:
.globl vector111
vector111:
  pushl $0
80107269:	6a 00                	push   $0x0
  pushl $111
8010726b:	6a 6f                	push   $0x6f
  jmp alltraps
8010726d:	e9 a5 f5 ff ff       	jmp    80106817 <alltraps>

80107272 <vector112>:
.globl vector112
vector112:
  pushl $0
80107272:	6a 00                	push   $0x0
  pushl $112
80107274:	6a 70                	push   $0x70
  jmp alltraps
80107276:	e9 9c f5 ff ff       	jmp    80106817 <alltraps>

8010727b <vector113>:
.globl vector113
vector113:
  pushl $0
8010727b:	6a 00                	push   $0x0
  pushl $113
8010727d:	6a 71                	push   $0x71
  jmp alltraps
8010727f:	e9 93 f5 ff ff       	jmp    80106817 <alltraps>

80107284 <vector114>:
.globl vector114
vector114:
  pushl $0
80107284:	6a 00                	push   $0x0
  pushl $114
80107286:	6a 72                	push   $0x72
  jmp alltraps
80107288:	e9 8a f5 ff ff       	jmp    80106817 <alltraps>

8010728d <vector115>:
.globl vector115
vector115:
  pushl $0
8010728d:	6a 00                	push   $0x0
  pushl $115
8010728f:	6a 73                	push   $0x73
  jmp alltraps
80107291:	e9 81 f5 ff ff       	jmp    80106817 <alltraps>

80107296 <vector116>:
.globl vector116
vector116:
  pushl $0
80107296:	6a 00                	push   $0x0
  pushl $116
80107298:	6a 74                	push   $0x74
  jmp alltraps
8010729a:	e9 78 f5 ff ff       	jmp    80106817 <alltraps>

8010729f <vector117>:
.globl vector117
vector117:
  pushl $0
8010729f:	6a 00                	push   $0x0
  pushl $117
801072a1:	6a 75                	push   $0x75
  jmp alltraps
801072a3:	e9 6f f5 ff ff       	jmp    80106817 <alltraps>

801072a8 <vector118>:
.globl vector118
vector118:
  pushl $0
801072a8:	6a 00                	push   $0x0
  pushl $118
801072aa:	6a 76                	push   $0x76
  jmp alltraps
801072ac:	e9 66 f5 ff ff       	jmp    80106817 <alltraps>

801072b1 <vector119>:
.globl vector119
vector119:
  pushl $0
801072b1:	6a 00                	push   $0x0
  pushl $119
801072b3:	6a 77                	push   $0x77
  jmp alltraps
801072b5:	e9 5d f5 ff ff       	jmp    80106817 <alltraps>

801072ba <vector120>:
.globl vector120
vector120:
  pushl $0
801072ba:	6a 00                	push   $0x0
  pushl $120
801072bc:	6a 78                	push   $0x78
  jmp alltraps
801072be:	e9 54 f5 ff ff       	jmp    80106817 <alltraps>

801072c3 <vector121>:
.globl vector121
vector121:
  pushl $0
801072c3:	6a 00                	push   $0x0
  pushl $121
801072c5:	6a 79                	push   $0x79
  jmp alltraps
801072c7:	e9 4b f5 ff ff       	jmp    80106817 <alltraps>

801072cc <vector122>:
.globl vector122
vector122:
  pushl $0
801072cc:	6a 00                	push   $0x0
  pushl $122
801072ce:	6a 7a                	push   $0x7a
  jmp alltraps
801072d0:	e9 42 f5 ff ff       	jmp    80106817 <alltraps>

801072d5 <vector123>:
.globl vector123
vector123:
  pushl $0
801072d5:	6a 00                	push   $0x0
  pushl $123
801072d7:	6a 7b                	push   $0x7b
  jmp alltraps
801072d9:	e9 39 f5 ff ff       	jmp    80106817 <alltraps>

801072de <vector124>:
.globl vector124
vector124:
  pushl $0
801072de:	6a 00                	push   $0x0
  pushl $124
801072e0:	6a 7c                	push   $0x7c
  jmp alltraps
801072e2:	e9 30 f5 ff ff       	jmp    80106817 <alltraps>

801072e7 <vector125>:
.globl vector125
vector125:
  pushl $0
801072e7:	6a 00                	push   $0x0
  pushl $125
801072e9:	6a 7d                	push   $0x7d
  jmp alltraps
801072eb:	e9 27 f5 ff ff       	jmp    80106817 <alltraps>

801072f0 <vector126>:
.globl vector126
vector126:
  pushl $0
801072f0:	6a 00                	push   $0x0
  pushl $126
801072f2:	6a 7e                	push   $0x7e
  jmp alltraps
801072f4:	e9 1e f5 ff ff       	jmp    80106817 <alltraps>

801072f9 <vector127>:
.globl vector127
vector127:
  pushl $0
801072f9:	6a 00                	push   $0x0
  pushl $127
801072fb:	6a 7f                	push   $0x7f
  jmp alltraps
801072fd:	e9 15 f5 ff ff       	jmp    80106817 <alltraps>

80107302 <vector128>:
.globl vector128
vector128:
  pushl $0
80107302:	6a 00                	push   $0x0
  pushl $128
80107304:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80107309:	e9 09 f5 ff ff       	jmp    80106817 <alltraps>

8010730e <vector129>:
.globl vector129
vector129:
  pushl $0
8010730e:	6a 00                	push   $0x0
  pushl $129
80107310:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80107315:	e9 fd f4 ff ff       	jmp    80106817 <alltraps>

8010731a <vector130>:
.globl vector130
vector130:
  pushl $0
8010731a:	6a 00                	push   $0x0
  pushl $130
8010731c:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80107321:	e9 f1 f4 ff ff       	jmp    80106817 <alltraps>

80107326 <vector131>:
.globl vector131
vector131:
  pushl $0
80107326:	6a 00                	push   $0x0
  pushl $131
80107328:	68 83 00 00 00       	push   $0x83
  jmp alltraps
8010732d:	e9 e5 f4 ff ff       	jmp    80106817 <alltraps>

80107332 <vector132>:
.globl vector132
vector132:
  pushl $0
80107332:	6a 00                	push   $0x0
  pushl $132
80107334:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80107339:	e9 d9 f4 ff ff       	jmp    80106817 <alltraps>

8010733e <vector133>:
.globl vector133
vector133:
  pushl $0
8010733e:	6a 00                	push   $0x0
  pushl $133
80107340:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80107345:	e9 cd f4 ff ff       	jmp    80106817 <alltraps>

8010734a <vector134>:
.globl vector134
vector134:
  pushl $0
8010734a:	6a 00                	push   $0x0
  pushl $134
8010734c:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80107351:	e9 c1 f4 ff ff       	jmp    80106817 <alltraps>

80107356 <vector135>:
.globl vector135
vector135:
  pushl $0
80107356:	6a 00                	push   $0x0
  pushl $135
80107358:	68 87 00 00 00       	push   $0x87
  jmp alltraps
8010735d:	e9 b5 f4 ff ff       	jmp    80106817 <alltraps>

80107362 <vector136>:
.globl vector136
vector136:
  pushl $0
80107362:	6a 00                	push   $0x0
  pushl $136
80107364:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80107369:	e9 a9 f4 ff ff       	jmp    80106817 <alltraps>

8010736e <vector137>:
.globl vector137
vector137:
  pushl $0
8010736e:	6a 00                	push   $0x0
  pushl $137
80107370:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80107375:	e9 9d f4 ff ff       	jmp    80106817 <alltraps>

8010737a <vector138>:
.globl vector138
vector138:
  pushl $0
8010737a:	6a 00                	push   $0x0
  pushl $138
8010737c:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80107381:	e9 91 f4 ff ff       	jmp    80106817 <alltraps>

80107386 <vector139>:
.globl vector139
vector139:
  pushl $0
80107386:	6a 00                	push   $0x0
  pushl $139
80107388:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
8010738d:	e9 85 f4 ff ff       	jmp    80106817 <alltraps>

80107392 <vector140>:
.globl vector140
vector140:
  pushl $0
80107392:	6a 00                	push   $0x0
  pushl $140
80107394:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80107399:	e9 79 f4 ff ff       	jmp    80106817 <alltraps>

8010739e <vector141>:
.globl vector141
vector141:
  pushl $0
8010739e:	6a 00                	push   $0x0
  pushl $141
801073a0:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
801073a5:	e9 6d f4 ff ff       	jmp    80106817 <alltraps>

801073aa <vector142>:
.globl vector142
vector142:
  pushl $0
801073aa:	6a 00                	push   $0x0
  pushl $142
801073ac:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
801073b1:	e9 61 f4 ff ff       	jmp    80106817 <alltraps>

801073b6 <vector143>:
.globl vector143
vector143:
  pushl $0
801073b6:	6a 00                	push   $0x0
  pushl $143
801073b8:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
801073bd:	e9 55 f4 ff ff       	jmp    80106817 <alltraps>

801073c2 <vector144>:
.globl vector144
vector144:
  pushl $0
801073c2:	6a 00                	push   $0x0
  pushl $144
801073c4:	68 90 00 00 00       	push   $0x90
  jmp alltraps
801073c9:	e9 49 f4 ff ff       	jmp    80106817 <alltraps>

801073ce <vector145>:
.globl vector145
vector145:
  pushl $0
801073ce:	6a 00                	push   $0x0
  pushl $145
801073d0:	68 91 00 00 00       	push   $0x91
  jmp alltraps
801073d5:	e9 3d f4 ff ff       	jmp    80106817 <alltraps>

801073da <vector146>:
.globl vector146
vector146:
  pushl $0
801073da:	6a 00                	push   $0x0
  pushl $146
801073dc:	68 92 00 00 00       	push   $0x92
  jmp alltraps
801073e1:	e9 31 f4 ff ff       	jmp    80106817 <alltraps>

801073e6 <vector147>:
.globl vector147
vector147:
  pushl $0
801073e6:	6a 00                	push   $0x0
  pushl $147
801073e8:	68 93 00 00 00       	push   $0x93
  jmp alltraps
801073ed:	e9 25 f4 ff ff       	jmp    80106817 <alltraps>

801073f2 <vector148>:
.globl vector148
vector148:
  pushl $0
801073f2:	6a 00                	push   $0x0
  pushl $148
801073f4:	68 94 00 00 00       	push   $0x94
  jmp alltraps
801073f9:	e9 19 f4 ff ff       	jmp    80106817 <alltraps>

801073fe <vector149>:
.globl vector149
vector149:
  pushl $0
801073fe:	6a 00                	push   $0x0
  pushl $149
80107400:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80107405:	e9 0d f4 ff ff       	jmp    80106817 <alltraps>

8010740a <vector150>:
.globl vector150
vector150:
  pushl $0
8010740a:	6a 00                	push   $0x0
  pushl $150
8010740c:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80107411:	e9 01 f4 ff ff       	jmp    80106817 <alltraps>

80107416 <vector151>:
.globl vector151
vector151:
  pushl $0
80107416:	6a 00                	push   $0x0
  pushl $151
80107418:	68 97 00 00 00       	push   $0x97
  jmp alltraps
8010741d:	e9 f5 f3 ff ff       	jmp    80106817 <alltraps>

80107422 <vector152>:
.globl vector152
vector152:
  pushl $0
80107422:	6a 00                	push   $0x0
  pushl $152
80107424:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80107429:	e9 e9 f3 ff ff       	jmp    80106817 <alltraps>

8010742e <vector153>:
.globl vector153
vector153:
  pushl $0
8010742e:	6a 00                	push   $0x0
  pushl $153
80107430:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80107435:	e9 dd f3 ff ff       	jmp    80106817 <alltraps>

8010743a <vector154>:
.globl vector154
vector154:
  pushl $0
8010743a:	6a 00                	push   $0x0
  pushl $154
8010743c:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80107441:	e9 d1 f3 ff ff       	jmp    80106817 <alltraps>

80107446 <vector155>:
.globl vector155
vector155:
  pushl $0
80107446:	6a 00                	push   $0x0
  pushl $155
80107448:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
8010744d:	e9 c5 f3 ff ff       	jmp    80106817 <alltraps>

80107452 <vector156>:
.globl vector156
vector156:
  pushl $0
80107452:	6a 00                	push   $0x0
  pushl $156
80107454:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80107459:	e9 b9 f3 ff ff       	jmp    80106817 <alltraps>

8010745e <vector157>:
.globl vector157
vector157:
  pushl $0
8010745e:	6a 00                	push   $0x0
  pushl $157
80107460:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80107465:	e9 ad f3 ff ff       	jmp    80106817 <alltraps>

8010746a <vector158>:
.globl vector158
vector158:
  pushl $0
8010746a:	6a 00                	push   $0x0
  pushl $158
8010746c:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80107471:	e9 a1 f3 ff ff       	jmp    80106817 <alltraps>

80107476 <vector159>:
.globl vector159
vector159:
  pushl $0
80107476:	6a 00                	push   $0x0
  pushl $159
80107478:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
8010747d:	e9 95 f3 ff ff       	jmp    80106817 <alltraps>

80107482 <vector160>:
.globl vector160
vector160:
  pushl $0
80107482:	6a 00                	push   $0x0
  pushl $160
80107484:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80107489:	e9 89 f3 ff ff       	jmp    80106817 <alltraps>

8010748e <vector161>:
.globl vector161
vector161:
  pushl $0
8010748e:	6a 00                	push   $0x0
  pushl $161
80107490:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80107495:	e9 7d f3 ff ff       	jmp    80106817 <alltraps>

8010749a <vector162>:
.globl vector162
vector162:
  pushl $0
8010749a:	6a 00                	push   $0x0
  pushl $162
8010749c:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
801074a1:	e9 71 f3 ff ff       	jmp    80106817 <alltraps>

801074a6 <vector163>:
.globl vector163
vector163:
  pushl $0
801074a6:	6a 00                	push   $0x0
  pushl $163
801074a8:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
801074ad:	e9 65 f3 ff ff       	jmp    80106817 <alltraps>

801074b2 <vector164>:
.globl vector164
vector164:
  pushl $0
801074b2:	6a 00                	push   $0x0
  pushl $164
801074b4:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
801074b9:	e9 59 f3 ff ff       	jmp    80106817 <alltraps>

801074be <vector165>:
.globl vector165
vector165:
  pushl $0
801074be:	6a 00                	push   $0x0
  pushl $165
801074c0:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
801074c5:	e9 4d f3 ff ff       	jmp    80106817 <alltraps>

801074ca <vector166>:
.globl vector166
vector166:
  pushl $0
801074ca:	6a 00                	push   $0x0
  pushl $166
801074cc:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
801074d1:	e9 41 f3 ff ff       	jmp    80106817 <alltraps>

801074d6 <vector167>:
.globl vector167
vector167:
  pushl $0
801074d6:	6a 00                	push   $0x0
  pushl $167
801074d8:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
801074dd:	e9 35 f3 ff ff       	jmp    80106817 <alltraps>

801074e2 <vector168>:
.globl vector168
vector168:
  pushl $0
801074e2:	6a 00                	push   $0x0
  pushl $168
801074e4:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
801074e9:	e9 29 f3 ff ff       	jmp    80106817 <alltraps>

801074ee <vector169>:
.globl vector169
vector169:
  pushl $0
801074ee:	6a 00                	push   $0x0
  pushl $169
801074f0:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
801074f5:	e9 1d f3 ff ff       	jmp    80106817 <alltraps>

801074fa <vector170>:
.globl vector170
vector170:
  pushl $0
801074fa:	6a 00                	push   $0x0
  pushl $170
801074fc:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80107501:	e9 11 f3 ff ff       	jmp    80106817 <alltraps>

80107506 <vector171>:
.globl vector171
vector171:
  pushl $0
80107506:	6a 00                	push   $0x0
  pushl $171
80107508:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
8010750d:	e9 05 f3 ff ff       	jmp    80106817 <alltraps>

80107512 <vector172>:
.globl vector172
vector172:
  pushl $0
80107512:	6a 00                	push   $0x0
  pushl $172
80107514:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
80107519:	e9 f9 f2 ff ff       	jmp    80106817 <alltraps>

8010751e <vector173>:
.globl vector173
vector173:
  pushl $0
8010751e:	6a 00                	push   $0x0
  pushl $173
80107520:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
80107525:	e9 ed f2 ff ff       	jmp    80106817 <alltraps>

8010752a <vector174>:
.globl vector174
vector174:
  pushl $0
8010752a:	6a 00                	push   $0x0
  pushl $174
8010752c:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80107531:	e9 e1 f2 ff ff       	jmp    80106817 <alltraps>

80107536 <vector175>:
.globl vector175
vector175:
  pushl $0
80107536:	6a 00                	push   $0x0
  pushl $175
80107538:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
8010753d:	e9 d5 f2 ff ff       	jmp    80106817 <alltraps>

80107542 <vector176>:
.globl vector176
vector176:
  pushl $0
80107542:	6a 00                	push   $0x0
  pushl $176
80107544:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
80107549:	e9 c9 f2 ff ff       	jmp    80106817 <alltraps>

8010754e <vector177>:
.globl vector177
vector177:
  pushl $0
8010754e:	6a 00                	push   $0x0
  pushl $177
80107550:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
80107555:	e9 bd f2 ff ff       	jmp    80106817 <alltraps>

8010755a <vector178>:
.globl vector178
vector178:
  pushl $0
8010755a:	6a 00                	push   $0x0
  pushl $178
8010755c:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80107561:	e9 b1 f2 ff ff       	jmp    80106817 <alltraps>

80107566 <vector179>:
.globl vector179
vector179:
  pushl $0
80107566:	6a 00                	push   $0x0
  pushl $179
80107568:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
8010756d:	e9 a5 f2 ff ff       	jmp    80106817 <alltraps>

80107572 <vector180>:
.globl vector180
vector180:
  pushl $0
80107572:	6a 00                	push   $0x0
  pushl $180
80107574:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
80107579:	e9 99 f2 ff ff       	jmp    80106817 <alltraps>

8010757e <vector181>:
.globl vector181
vector181:
  pushl $0
8010757e:	6a 00                	push   $0x0
  pushl $181
80107580:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80107585:	e9 8d f2 ff ff       	jmp    80106817 <alltraps>

8010758a <vector182>:
.globl vector182
vector182:
  pushl $0
8010758a:	6a 00                	push   $0x0
  pushl $182
8010758c:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80107591:	e9 81 f2 ff ff       	jmp    80106817 <alltraps>

80107596 <vector183>:
.globl vector183
vector183:
  pushl $0
80107596:	6a 00                	push   $0x0
  pushl $183
80107598:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
8010759d:	e9 75 f2 ff ff       	jmp    80106817 <alltraps>

801075a2 <vector184>:
.globl vector184
vector184:
  pushl $0
801075a2:	6a 00                	push   $0x0
  pushl $184
801075a4:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
801075a9:	e9 69 f2 ff ff       	jmp    80106817 <alltraps>

801075ae <vector185>:
.globl vector185
vector185:
  pushl $0
801075ae:	6a 00                	push   $0x0
  pushl $185
801075b0:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
801075b5:	e9 5d f2 ff ff       	jmp    80106817 <alltraps>

801075ba <vector186>:
.globl vector186
vector186:
  pushl $0
801075ba:	6a 00                	push   $0x0
  pushl $186
801075bc:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
801075c1:	e9 51 f2 ff ff       	jmp    80106817 <alltraps>

801075c6 <vector187>:
.globl vector187
vector187:
  pushl $0
801075c6:	6a 00                	push   $0x0
  pushl $187
801075c8:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
801075cd:	e9 45 f2 ff ff       	jmp    80106817 <alltraps>

801075d2 <vector188>:
.globl vector188
vector188:
  pushl $0
801075d2:	6a 00                	push   $0x0
  pushl $188
801075d4:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
801075d9:	e9 39 f2 ff ff       	jmp    80106817 <alltraps>

801075de <vector189>:
.globl vector189
vector189:
  pushl $0
801075de:	6a 00                	push   $0x0
  pushl $189
801075e0:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
801075e5:	e9 2d f2 ff ff       	jmp    80106817 <alltraps>

801075ea <vector190>:
.globl vector190
vector190:
  pushl $0
801075ea:	6a 00                	push   $0x0
  pushl $190
801075ec:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
801075f1:	e9 21 f2 ff ff       	jmp    80106817 <alltraps>

801075f6 <vector191>:
.globl vector191
vector191:
  pushl $0
801075f6:	6a 00                	push   $0x0
  pushl $191
801075f8:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
801075fd:	e9 15 f2 ff ff       	jmp    80106817 <alltraps>

80107602 <vector192>:
.globl vector192
vector192:
  pushl $0
80107602:	6a 00                	push   $0x0
  pushl $192
80107604:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
80107609:	e9 09 f2 ff ff       	jmp    80106817 <alltraps>

8010760e <vector193>:
.globl vector193
vector193:
  pushl $0
8010760e:	6a 00                	push   $0x0
  pushl $193
80107610:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
80107615:	e9 fd f1 ff ff       	jmp    80106817 <alltraps>

8010761a <vector194>:
.globl vector194
vector194:
  pushl $0
8010761a:	6a 00                	push   $0x0
  pushl $194
8010761c:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80107621:	e9 f1 f1 ff ff       	jmp    80106817 <alltraps>

80107626 <vector195>:
.globl vector195
vector195:
  pushl $0
80107626:	6a 00                	push   $0x0
  pushl $195
80107628:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
8010762d:	e9 e5 f1 ff ff       	jmp    80106817 <alltraps>

80107632 <vector196>:
.globl vector196
vector196:
  pushl $0
80107632:	6a 00                	push   $0x0
  pushl $196
80107634:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
80107639:	e9 d9 f1 ff ff       	jmp    80106817 <alltraps>

8010763e <vector197>:
.globl vector197
vector197:
  pushl $0
8010763e:	6a 00                	push   $0x0
  pushl $197
80107640:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
80107645:	e9 cd f1 ff ff       	jmp    80106817 <alltraps>

8010764a <vector198>:
.globl vector198
vector198:
  pushl $0
8010764a:	6a 00                	push   $0x0
  pushl $198
8010764c:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80107651:	e9 c1 f1 ff ff       	jmp    80106817 <alltraps>

80107656 <vector199>:
.globl vector199
vector199:
  pushl $0
80107656:	6a 00                	push   $0x0
  pushl $199
80107658:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
8010765d:	e9 b5 f1 ff ff       	jmp    80106817 <alltraps>

80107662 <vector200>:
.globl vector200
vector200:
  pushl $0
80107662:	6a 00                	push   $0x0
  pushl $200
80107664:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
80107669:	e9 a9 f1 ff ff       	jmp    80106817 <alltraps>

8010766e <vector201>:
.globl vector201
vector201:
  pushl $0
8010766e:	6a 00                	push   $0x0
  pushl $201
80107670:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
80107675:	e9 9d f1 ff ff       	jmp    80106817 <alltraps>

8010767a <vector202>:
.globl vector202
vector202:
  pushl $0
8010767a:	6a 00                	push   $0x0
  pushl $202
8010767c:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80107681:	e9 91 f1 ff ff       	jmp    80106817 <alltraps>

80107686 <vector203>:
.globl vector203
vector203:
  pushl $0
80107686:	6a 00                	push   $0x0
  pushl $203
80107688:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
8010768d:	e9 85 f1 ff ff       	jmp    80106817 <alltraps>

80107692 <vector204>:
.globl vector204
vector204:
  pushl $0
80107692:	6a 00                	push   $0x0
  pushl $204
80107694:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80107699:	e9 79 f1 ff ff       	jmp    80106817 <alltraps>

8010769e <vector205>:
.globl vector205
vector205:
  pushl $0
8010769e:	6a 00                	push   $0x0
  pushl $205
801076a0:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
801076a5:	e9 6d f1 ff ff       	jmp    80106817 <alltraps>

801076aa <vector206>:
.globl vector206
vector206:
  pushl $0
801076aa:	6a 00                	push   $0x0
  pushl $206
801076ac:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
801076b1:	e9 61 f1 ff ff       	jmp    80106817 <alltraps>

801076b6 <vector207>:
.globl vector207
vector207:
  pushl $0
801076b6:	6a 00                	push   $0x0
  pushl $207
801076b8:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
801076bd:	e9 55 f1 ff ff       	jmp    80106817 <alltraps>

801076c2 <vector208>:
.globl vector208
vector208:
  pushl $0
801076c2:	6a 00                	push   $0x0
  pushl $208
801076c4:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
801076c9:	e9 49 f1 ff ff       	jmp    80106817 <alltraps>

801076ce <vector209>:
.globl vector209
vector209:
  pushl $0
801076ce:	6a 00                	push   $0x0
  pushl $209
801076d0:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
801076d5:	e9 3d f1 ff ff       	jmp    80106817 <alltraps>

801076da <vector210>:
.globl vector210
vector210:
  pushl $0
801076da:	6a 00                	push   $0x0
  pushl $210
801076dc:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
801076e1:	e9 31 f1 ff ff       	jmp    80106817 <alltraps>

801076e6 <vector211>:
.globl vector211
vector211:
  pushl $0
801076e6:	6a 00                	push   $0x0
  pushl $211
801076e8:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
801076ed:	e9 25 f1 ff ff       	jmp    80106817 <alltraps>

801076f2 <vector212>:
.globl vector212
vector212:
  pushl $0
801076f2:	6a 00                	push   $0x0
  pushl $212
801076f4:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
801076f9:	e9 19 f1 ff ff       	jmp    80106817 <alltraps>

801076fe <vector213>:
.globl vector213
vector213:
  pushl $0
801076fe:	6a 00                	push   $0x0
  pushl $213
80107700:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
80107705:	e9 0d f1 ff ff       	jmp    80106817 <alltraps>

8010770a <vector214>:
.globl vector214
vector214:
  pushl $0
8010770a:	6a 00                	push   $0x0
  pushl $214
8010770c:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
80107711:	e9 01 f1 ff ff       	jmp    80106817 <alltraps>

80107716 <vector215>:
.globl vector215
vector215:
  pushl $0
80107716:	6a 00                	push   $0x0
  pushl $215
80107718:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
8010771d:	e9 f5 f0 ff ff       	jmp    80106817 <alltraps>

80107722 <vector216>:
.globl vector216
vector216:
  pushl $0
80107722:	6a 00                	push   $0x0
  pushl $216
80107724:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
80107729:	e9 e9 f0 ff ff       	jmp    80106817 <alltraps>

8010772e <vector217>:
.globl vector217
vector217:
  pushl $0
8010772e:	6a 00                	push   $0x0
  pushl $217
80107730:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
80107735:	e9 dd f0 ff ff       	jmp    80106817 <alltraps>

8010773a <vector218>:
.globl vector218
vector218:
  pushl $0
8010773a:	6a 00                	push   $0x0
  pushl $218
8010773c:	68 da 00 00 00       	push   $0xda
  jmp alltraps
80107741:	e9 d1 f0 ff ff       	jmp    80106817 <alltraps>

80107746 <vector219>:
.globl vector219
vector219:
  pushl $0
80107746:	6a 00                	push   $0x0
  pushl $219
80107748:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
8010774d:	e9 c5 f0 ff ff       	jmp    80106817 <alltraps>

80107752 <vector220>:
.globl vector220
vector220:
  pushl $0
80107752:	6a 00                	push   $0x0
  pushl $220
80107754:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
80107759:	e9 b9 f0 ff ff       	jmp    80106817 <alltraps>

8010775e <vector221>:
.globl vector221
vector221:
  pushl $0
8010775e:	6a 00                	push   $0x0
  pushl $221
80107760:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
80107765:	e9 ad f0 ff ff       	jmp    80106817 <alltraps>

8010776a <vector222>:
.globl vector222
vector222:
  pushl $0
8010776a:	6a 00                	push   $0x0
  pushl $222
8010776c:	68 de 00 00 00       	push   $0xde
  jmp alltraps
80107771:	e9 a1 f0 ff ff       	jmp    80106817 <alltraps>

80107776 <vector223>:
.globl vector223
vector223:
  pushl $0
80107776:	6a 00                	push   $0x0
  pushl $223
80107778:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
8010777d:	e9 95 f0 ff ff       	jmp    80106817 <alltraps>

80107782 <vector224>:
.globl vector224
vector224:
  pushl $0
80107782:	6a 00                	push   $0x0
  pushl $224
80107784:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
80107789:	e9 89 f0 ff ff       	jmp    80106817 <alltraps>

8010778e <vector225>:
.globl vector225
vector225:
  pushl $0
8010778e:	6a 00                	push   $0x0
  pushl $225
80107790:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
80107795:	e9 7d f0 ff ff       	jmp    80106817 <alltraps>

8010779a <vector226>:
.globl vector226
vector226:
  pushl $0
8010779a:	6a 00                	push   $0x0
  pushl $226
8010779c:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
801077a1:	e9 71 f0 ff ff       	jmp    80106817 <alltraps>

801077a6 <vector227>:
.globl vector227
vector227:
  pushl $0
801077a6:	6a 00                	push   $0x0
  pushl $227
801077a8:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
801077ad:	e9 65 f0 ff ff       	jmp    80106817 <alltraps>

801077b2 <vector228>:
.globl vector228
vector228:
  pushl $0
801077b2:	6a 00                	push   $0x0
  pushl $228
801077b4:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
801077b9:	e9 59 f0 ff ff       	jmp    80106817 <alltraps>

801077be <vector229>:
.globl vector229
vector229:
  pushl $0
801077be:	6a 00                	push   $0x0
  pushl $229
801077c0:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
801077c5:	e9 4d f0 ff ff       	jmp    80106817 <alltraps>

801077ca <vector230>:
.globl vector230
vector230:
  pushl $0
801077ca:	6a 00                	push   $0x0
  pushl $230
801077cc:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
801077d1:	e9 41 f0 ff ff       	jmp    80106817 <alltraps>

801077d6 <vector231>:
.globl vector231
vector231:
  pushl $0
801077d6:	6a 00                	push   $0x0
  pushl $231
801077d8:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
801077dd:	e9 35 f0 ff ff       	jmp    80106817 <alltraps>

801077e2 <vector232>:
.globl vector232
vector232:
  pushl $0
801077e2:	6a 00                	push   $0x0
  pushl $232
801077e4:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
801077e9:	e9 29 f0 ff ff       	jmp    80106817 <alltraps>

801077ee <vector233>:
.globl vector233
vector233:
  pushl $0
801077ee:	6a 00                	push   $0x0
  pushl $233
801077f0:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
801077f5:	e9 1d f0 ff ff       	jmp    80106817 <alltraps>

801077fa <vector234>:
.globl vector234
vector234:
  pushl $0
801077fa:	6a 00                	push   $0x0
  pushl $234
801077fc:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
80107801:	e9 11 f0 ff ff       	jmp    80106817 <alltraps>

80107806 <vector235>:
.globl vector235
vector235:
  pushl $0
80107806:	6a 00                	push   $0x0
  pushl $235
80107808:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
8010780d:	e9 05 f0 ff ff       	jmp    80106817 <alltraps>

80107812 <vector236>:
.globl vector236
vector236:
  pushl $0
80107812:	6a 00                	push   $0x0
  pushl $236
80107814:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
80107819:	e9 f9 ef ff ff       	jmp    80106817 <alltraps>

8010781e <vector237>:
.globl vector237
vector237:
  pushl $0
8010781e:	6a 00                	push   $0x0
  pushl $237
80107820:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
80107825:	e9 ed ef ff ff       	jmp    80106817 <alltraps>

8010782a <vector238>:
.globl vector238
vector238:
  pushl $0
8010782a:	6a 00                	push   $0x0
  pushl $238
8010782c:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
80107831:	e9 e1 ef ff ff       	jmp    80106817 <alltraps>

80107836 <vector239>:
.globl vector239
vector239:
  pushl $0
80107836:	6a 00                	push   $0x0
  pushl $239
80107838:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
8010783d:	e9 d5 ef ff ff       	jmp    80106817 <alltraps>

80107842 <vector240>:
.globl vector240
vector240:
  pushl $0
80107842:	6a 00                	push   $0x0
  pushl $240
80107844:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
80107849:	e9 c9 ef ff ff       	jmp    80106817 <alltraps>

8010784e <vector241>:
.globl vector241
vector241:
  pushl $0
8010784e:	6a 00                	push   $0x0
  pushl $241
80107850:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
80107855:	e9 bd ef ff ff       	jmp    80106817 <alltraps>

8010785a <vector242>:
.globl vector242
vector242:
  pushl $0
8010785a:	6a 00                	push   $0x0
  pushl $242
8010785c:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
80107861:	e9 b1 ef ff ff       	jmp    80106817 <alltraps>

80107866 <vector243>:
.globl vector243
vector243:
  pushl $0
80107866:	6a 00                	push   $0x0
  pushl $243
80107868:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
8010786d:	e9 a5 ef ff ff       	jmp    80106817 <alltraps>

80107872 <vector244>:
.globl vector244
vector244:
  pushl $0
80107872:	6a 00                	push   $0x0
  pushl $244
80107874:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
80107879:	e9 99 ef ff ff       	jmp    80106817 <alltraps>

8010787e <vector245>:
.globl vector245
vector245:
  pushl $0
8010787e:	6a 00                	push   $0x0
  pushl $245
80107880:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
80107885:	e9 8d ef ff ff       	jmp    80106817 <alltraps>

8010788a <vector246>:
.globl vector246
vector246:
  pushl $0
8010788a:	6a 00                	push   $0x0
  pushl $246
8010788c:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107891:	e9 81 ef ff ff       	jmp    80106817 <alltraps>

80107896 <vector247>:
.globl vector247
vector247:
  pushl $0
80107896:	6a 00                	push   $0x0
  pushl $247
80107898:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
8010789d:	e9 75 ef ff ff       	jmp    80106817 <alltraps>

801078a2 <vector248>:
.globl vector248
vector248:
  pushl $0
801078a2:	6a 00                	push   $0x0
  pushl $248
801078a4:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
801078a9:	e9 69 ef ff ff       	jmp    80106817 <alltraps>

801078ae <vector249>:
.globl vector249
vector249:
  pushl $0
801078ae:	6a 00                	push   $0x0
  pushl $249
801078b0:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
801078b5:	e9 5d ef ff ff       	jmp    80106817 <alltraps>

801078ba <vector250>:
.globl vector250
vector250:
  pushl $0
801078ba:	6a 00                	push   $0x0
  pushl $250
801078bc:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
801078c1:	e9 51 ef ff ff       	jmp    80106817 <alltraps>

801078c6 <vector251>:
.globl vector251
vector251:
  pushl $0
801078c6:	6a 00                	push   $0x0
  pushl $251
801078c8:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
801078cd:	e9 45 ef ff ff       	jmp    80106817 <alltraps>

801078d2 <vector252>:
.globl vector252
vector252:
  pushl $0
801078d2:	6a 00                	push   $0x0
  pushl $252
801078d4:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
801078d9:	e9 39 ef ff ff       	jmp    80106817 <alltraps>

801078de <vector253>:
.globl vector253
vector253:
  pushl $0
801078de:	6a 00                	push   $0x0
  pushl $253
801078e0:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
801078e5:	e9 2d ef ff ff       	jmp    80106817 <alltraps>

801078ea <vector254>:
.globl vector254
vector254:
  pushl $0
801078ea:	6a 00                	push   $0x0
  pushl $254
801078ec:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
801078f1:	e9 21 ef ff ff       	jmp    80106817 <alltraps>

801078f6 <vector255>:
.globl vector255
vector255:
  pushl $0
801078f6:	6a 00                	push   $0x0
  pushl $255
801078f8:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
801078fd:	e9 15 ef ff ff       	jmp    80106817 <alltraps>

80107902 <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80107902:	55                   	push   %ebp
80107903:	89 e5                	mov    %esp,%ebp
80107905:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80107908:	8b 45 0c             	mov    0xc(%ebp),%eax
8010790b:	83 e8 01             	sub    $0x1,%eax
8010790e:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80107912:	8b 45 08             	mov    0x8(%ebp),%eax
80107915:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80107919:	8b 45 08             	mov    0x8(%ebp),%eax
8010791c:	c1 e8 10             	shr    $0x10,%eax
8010791f:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
80107923:	8d 45 fa             	lea    -0x6(%ebp),%eax
80107926:	0f 01 10             	lgdtl  (%eax)
}
80107929:	c9                   	leave  
8010792a:	c3                   	ret    

8010792b <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
8010792b:	55                   	push   %ebp
8010792c:	89 e5                	mov    %esp,%ebp
8010792e:	83 ec 04             	sub    $0x4,%esp
80107931:	8b 45 08             	mov    0x8(%ebp),%eax
80107934:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
80107938:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
8010793c:	0f 00 d8             	ltr    %ax
}
8010793f:	c9                   	leave  
80107940:	c3                   	ret    

80107941 <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
80107941:	55                   	push   %ebp
80107942:	89 e5                	mov    %esp,%ebp
80107944:	83 ec 04             	sub    $0x4,%esp
80107947:	8b 45 08             	mov    0x8(%ebp),%eax
8010794a:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
8010794e:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107952:	8e e8                	mov    %eax,%gs
}
80107954:	c9                   	leave  
80107955:	c3                   	ret    

80107956 <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
80107956:	55                   	push   %ebp
80107957:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107959:	8b 45 08             	mov    0x8(%ebp),%eax
8010795c:	0f 22 d8             	mov    %eax,%cr3
}
8010795f:	5d                   	pop    %ebp
80107960:	c3                   	ret    

80107961 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
80107961:	55                   	push   %ebp
80107962:	89 e5                	mov    %esp,%ebp
80107964:	8b 45 08             	mov    0x8(%ebp),%eax
80107967:	05 00 00 00 80       	add    $0x80000000,%eax
8010796c:	5d                   	pop    %ebp
8010796d:	c3                   	ret    

8010796e <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
8010796e:	55                   	push   %ebp
8010796f:	89 e5                	mov    %esp,%ebp
80107971:	8b 45 08             	mov    0x8(%ebp),%eax
80107974:	05 00 00 00 80       	add    $0x80000000,%eax
80107979:	5d                   	pop    %ebp
8010797a:	c3                   	ret    

8010797b <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
8010797b:	55                   	push   %ebp
8010797c:	89 e5                	mov    %esp,%ebp
8010797e:	53                   	push   %ebx
8010797f:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
80107982:	e8 d4 b4 ff ff       	call   80102e5b <cpunum>
80107987:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
8010798d:	05 a0 23 11 80       	add    $0x801123a0,%eax
80107992:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
80107995:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107998:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
8010799e:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079a1:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
801079a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079aa:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
801079ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079b1:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801079b5:	83 e2 f0             	and    $0xfffffff0,%edx
801079b8:	83 ca 0a             	or     $0xa,%edx
801079bb:	88 50 7d             	mov    %dl,0x7d(%eax)
801079be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079c1:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801079c5:	83 ca 10             	or     $0x10,%edx
801079c8:	88 50 7d             	mov    %dl,0x7d(%eax)
801079cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079ce:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801079d2:	83 e2 9f             	and    $0xffffff9f,%edx
801079d5:	88 50 7d             	mov    %dl,0x7d(%eax)
801079d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079db:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801079df:	83 ca 80             	or     $0xffffff80,%edx
801079e2:	88 50 7d             	mov    %dl,0x7d(%eax)
801079e5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079e8:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801079ec:	83 ca 0f             	or     $0xf,%edx
801079ef:	88 50 7e             	mov    %dl,0x7e(%eax)
801079f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079f5:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801079f9:	83 e2 ef             	and    $0xffffffef,%edx
801079fc:	88 50 7e             	mov    %dl,0x7e(%eax)
801079ff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a02:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107a06:	83 e2 df             	and    $0xffffffdf,%edx
80107a09:	88 50 7e             	mov    %dl,0x7e(%eax)
80107a0c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a0f:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107a13:	83 ca 40             	or     $0x40,%edx
80107a16:	88 50 7e             	mov    %dl,0x7e(%eax)
80107a19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a1c:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107a20:	83 ca 80             	or     $0xffffff80,%edx
80107a23:	88 50 7e             	mov    %dl,0x7e(%eax)
80107a26:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a29:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80107a2d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a30:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
80107a37:	ff ff 
80107a39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a3c:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
80107a43:	00 00 
80107a45:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a48:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80107a4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a52:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107a59:	83 e2 f0             	and    $0xfffffff0,%edx
80107a5c:	83 ca 02             	or     $0x2,%edx
80107a5f:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107a65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a68:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107a6f:	83 ca 10             	or     $0x10,%edx
80107a72:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107a78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a7b:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107a82:	83 e2 9f             	and    $0xffffff9f,%edx
80107a85:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107a8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a8e:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107a95:	83 ca 80             	or     $0xffffff80,%edx
80107a98:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107a9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aa1:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107aa8:	83 ca 0f             	or     $0xf,%edx
80107aab:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107ab1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ab4:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107abb:	83 e2 ef             	and    $0xffffffef,%edx
80107abe:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107ac4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ac7:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107ace:	83 e2 df             	and    $0xffffffdf,%edx
80107ad1:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107ad7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ada:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107ae1:	83 ca 40             	or     $0x40,%edx
80107ae4:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107aea:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aed:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107af4:	83 ca 80             	or     $0xffffff80,%edx
80107af7:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107afd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b00:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80107b07:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b0a:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
80107b11:	ff ff 
80107b13:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b16:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80107b1d:	00 00 
80107b1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b22:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80107b29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b2c:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107b33:	83 e2 f0             	and    $0xfffffff0,%edx
80107b36:	83 ca 0a             	or     $0xa,%edx
80107b39:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107b3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b42:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107b49:	83 ca 10             	or     $0x10,%edx
80107b4c:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107b52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b55:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107b5c:	83 ca 60             	or     $0x60,%edx
80107b5f:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107b65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b68:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107b6f:	83 ca 80             	or     $0xffffff80,%edx
80107b72:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107b78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b7b:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107b82:	83 ca 0f             	or     $0xf,%edx
80107b85:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107b8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b8e:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107b95:	83 e2 ef             	and    $0xffffffef,%edx
80107b98:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107b9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ba1:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107ba8:	83 e2 df             	and    $0xffffffdf,%edx
80107bab:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107bb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bb4:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107bbb:	83 ca 40             	or     $0x40,%edx
80107bbe:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107bc4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bc7:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107bce:	83 ca 80             	or     $0xffffff80,%edx
80107bd1:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107bd7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bda:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107be1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be4:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80107beb:	ff ff 
80107bed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bf0:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80107bf7:	00 00 
80107bf9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bfc:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80107c03:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c06:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107c0d:	83 e2 f0             	and    $0xfffffff0,%edx
80107c10:	83 ca 02             	or     $0x2,%edx
80107c13:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107c19:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c1c:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107c23:	83 ca 10             	or     $0x10,%edx
80107c26:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107c2c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c2f:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107c36:	83 ca 60             	or     $0x60,%edx
80107c39:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107c3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c42:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107c49:	83 ca 80             	or     $0xffffff80,%edx
80107c4c:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107c52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c55:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107c5c:	83 ca 0f             	or     $0xf,%edx
80107c5f:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107c65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c68:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107c6f:	83 e2 ef             	and    $0xffffffef,%edx
80107c72:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107c78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c7b:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107c82:	83 e2 df             	and    $0xffffffdf,%edx
80107c85:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107c8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c8e:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107c95:	83 ca 40             	or     $0x40,%edx
80107c98:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107c9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ca1:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107ca8:	83 ca 80             	or     $0xffffff80,%edx
80107cab:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107cb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cb4:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107cbb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cbe:	05 b4 00 00 00       	add    $0xb4,%eax
80107cc3:	89 c3                	mov    %eax,%ebx
80107cc5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cc8:	05 b4 00 00 00       	add    $0xb4,%eax
80107ccd:	c1 e8 10             	shr    $0x10,%eax
80107cd0:	89 c1                	mov    %eax,%ecx
80107cd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cd5:	05 b4 00 00 00       	add    $0xb4,%eax
80107cda:	c1 e8 18             	shr    $0x18,%eax
80107cdd:	89 c2                	mov    %eax,%edx
80107cdf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ce2:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80107ce9:	00 00 
80107ceb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cee:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80107cf5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cf8:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80107cfe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d01:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107d08:	83 e1 f0             	and    $0xfffffff0,%ecx
80107d0b:	83 c9 02             	or     $0x2,%ecx
80107d0e:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107d14:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d17:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107d1e:	83 c9 10             	or     $0x10,%ecx
80107d21:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107d27:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d2a:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107d31:	83 e1 9f             	and    $0xffffff9f,%ecx
80107d34:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107d3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d3d:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107d44:	83 c9 80             	or     $0xffffff80,%ecx
80107d47:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107d4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d50:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107d57:	83 e1 f0             	and    $0xfffffff0,%ecx
80107d5a:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107d60:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d63:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107d6a:	83 e1 ef             	and    $0xffffffef,%ecx
80107d6d:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107d73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d76:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107d7d:	83 e1 df             	and    $0xffffffdf,%ecx
80107d80:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107d86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d89:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107d90:	83 c9 40             	or     $0x40,%ecx
80107d93:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107d99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d9c:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107da3:	83 c9 80             	or     $0xffffff80,%ecx
80107da6:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107dac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107daf:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80107db5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107db8:	83 c0 70             	add    $0x70,%eax
80107dbb:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80107dc2:	00 
80107dc3:	89 04 24             	mov    %eax,(%esp)
80107dc6:	e8 37 fb ff ff       	call   80107902 <lgdt>
  loadgs(SEG_KCPU << 3);
80107dcb:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80107dd2:	e8 6a fb ff ff       	call   80107941 <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80107dd7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dda:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80107de0:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80107de7:	00 00 00 00 
}
80107deb:	83 c4 24             	add    $0x24,%esp
80107dee:	5b                   	pop    %ebx
80107def:	5d                   	pop    %ebp
80107df0:	c3                   	ret    

80107df1 <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80107df1:	55                   	push   %ebp
80107df2:	89 e5                	mov    %esp,%ebp
80107df4:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107df7:	8b 45 0c             	mov    0xc(%ebp),%eax
80107dfa:	c1 e8 16             	shr    $0x16,%eax
80107dfd:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107e04:	8b 45 08             	mov    0x8(%ebp),%eax
80107e07:	01 d0                	add    %edx,%eax
80107e09:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107e0c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e0f:	8b 00                	mov    (%eax),%eax
80107e11:	83 e0 01             	and    $0x1,%eax
80107e14:	85 c0                	test   %eax,%eax
80107e16:	74 17                	je     80107e2f <walkpgdir+0x3e>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80107e18:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e1b:	8b 00                	mov    (%eax),%eax
80107e1d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107e22:	89 04 24             	mov    %eax,(%esp)
80107e25:	e8 44 fb ff ff       	call   8010796e <p2v>
80107e2a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107e2d:	eb 4b                	jmp    80107e7a <walkpgdir+0x89>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107e2f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80107e33:	74 0e                	je     80107e43 <walkpgdir+0x52>
80107e35:	e8 a8 ac ff ff       	call   80102ae2 <kalloc>
80107e3a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107e3d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107e41:	75 07                	jne    80107e4a <walkpgdir+0x59>
      return 0;
80107e43:	b8 00 00 00 00       	mov    $0x0,%eax
80107e48:	eb 47                	jmp    80107e91 <walkpgdir+0xa0>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107e4a:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107e51:	00 
80107e52:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107e59:	00 
80107e5a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e5d:	89 04 24             	mov    %eax,(%esp)
80107e60:	e8 66 d1 ff ff       	call   80104fcb <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80107e65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e68:	89 04 24             	mov    %eax,(%esp)
80107e6b:	e8 f1 fa ff ff       	call   80107961 <v2p>
80107e70:	83 c8 07             	or     $0x7,%eax
80107e73:	89 c2                	mov    %eax,%edx
80107e75:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e78:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80107e7a:	8b 45 0c             	mov    0xc(%ebp),%eax
80107e7d:	c1 e8 0c             	shr    $0xc,%eax
80107e80:	25 ff 03 00 00       	and    $0x3ff,%eax
80107e85:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107e8c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e8f:	01 d0                	add    %edx,%eax
}
80107e91:	c9                   	leave  
80107e92:	c3                   	ret    

80107e93 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80107e93:	55                   	push   %ebp
80107e94:	89 e5                	mov    %esp,%ebp
80107e96:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80107e99:	8b 45 0c             	mov    0xc(%ebp),%eax
80107e9c:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107ea1:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80107ea4:	8b 55 0c             	mov    0xc(%ebp),%edx
80107ea7:	8b 45 10             	mov    0x10(%ebp),%eax
80107eaa:	01 d0                	add    %edx,%eax
80107eac:	83 e8 01             	sub    $0x1,%eax
80107eaf:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107eb4:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80107eb7:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80107ebe:	00 
80107ebf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ec2:	89 44 24 04          	mov    %eax,0x4(%esp)
80107ec6:	8b 45 08             	mov    0x8(%ebp),%eax
80107ec9:	89 04 24             	mov    %eax,(%esp)
80107ecc:	e8 20 ff ff ff       	call   80107df1 <walkpgdir>
80107ed1:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107ed4:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107ed8:	75 07                	jne    80107ee1 <mappages+0x4e>
      return -1;
80107eda:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107edf:	eb 48                	jmp    80107f29 <mappages+0x96>
    if(*pte & PTE_P)
80107ee1:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107ee4:	8b 00                	mov    (%eax),%eax
80107ee6:	83 e0 01             	and    $0x1,%eax
80107ee9:	85 c0                	test   %eax,%eax
80107eeb:	74 0c                	je     80107ef9 <mappages+0x66>
      panic("remap");
80107eed:	c7 04 24 24 8d 10 80 	movl   $0x80108d24,(%esp)
80107ef4:	e8 41 86 ff ff       	call   8010053a <panic>
    *pte = pa | perm | PTE_P;
80107ef9:	8b 45 18             	mov    0x18(%ebp),%eax
80107efc:	0b 45 14             	or     0x14(%ebp),%eax
80107eff:	83 c8 01             	or     $0x1,%eax
80107f02:	89 c2                	mov    %eax,%edx
80107f04:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107f07:	89 10                	mov    %edx,(%eax)
    if(a == last)
80107f09:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f0c:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107f0f:	75 08                	jne    80107f19 <mappages+0x86>
      break;
80107f11:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80107f12:	b8 00 00 00 00       	mov    $0x0,%eax
80107f17:	eb 10                	jmp    80107f29 <mappages+0x96>
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
    a += PGSIZE;
80107f19:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80107f20:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
80107f27:	eb 8e                	jmp    80107eb7 <mappages+0x24>
  return 0;
}
80107f29:	c9                   	leave  
80107f2a:	c3                   	ret    

80107f2b <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm(void)
{
80107f2b:	55                   	push   %ebp
80107f2c:	89 e5                	mov    %esp,%ebp
80107f2e:	53                   	push   %ebx
80107f2f:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
80107f32:	e8 ab ab ff ff       	call   80102ae2 <kalloc>
80107f37:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107f3a:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107f3e:	75 0a                	jne    80107f4a <setupkvm+0x1f>
    return 0;
80107f40:	b8 00 00 00 00       	mov    $0x0,%eax
80107f45:	e9 98 00 00 00       	jmp    80107fe2 <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
80107f4a:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107f51:	00 
80107f52:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107f59:	00 
80107f5a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f5d:	89 04 24             	mov    %eax,(%esp)
80107f60:	e8 66 d0 ff ff       	call   80104fcb <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
80107f65:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80107f6c:	e8 fd f9 ff ff       	call   8010796e <p2v>
80107f71:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
80107f76:	76 0c                	jbe    80107f84 <setupkvm+0x59>
    panic("PHYSTOP too high");
80107f78:	c7 04 24 2a 8d 10 80 	movl   $0x80108d2a,(%esp)
80107f7f:	e8 b6 85 ff ff       	call   8010053a <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107f84:	c7 45 f4 e0 b4 10 80 	movl   $0x8010b4e0,-0xc(%ebp)
80107f8b:	eb 49                	jmp    80107fd6 <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107f8d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f90:	8b 48 0c             	mov    0xc(%eax),%ecx
80107f93:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f96:	8b 50 04             	mov    0x4(%eax),%edx
80107f99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f9c:	8b 58 08             	mov    0x8(%eax),%ebx
80107f9f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fa2:	8b 40 04             	mov    0x4(%eax),%eax
80107fa5:	29 c3                	sub    %eax,%ebx
80107fa7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107faa:	8b 00                	mov    (%eax),%eax
80107fac:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80107fb0:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107fb4:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80107fb8:	89 44 24 04          	mov    %eax,0x4(%esp)
80107fbc:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107fbf:	89 04 24             	mov    %eax,(%esp)
80107fc2:	e8 cc fe ff ff       	call   80107e93 <mappages>
80107fc7:	85 c0                	test   %eax,%eax
80107fc9:	79 07                	jns    80107fd2 <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80107fcb:	b8 00 00 00 00       	mov    $0x0,%eax
80107fd0:	eb 10                	jmp    80107fe2 <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107fd2:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80107fd6:	81 7d f4 20 b5 10 80 	cmpl   $0x8010b520,-0xc(%ebp)
80107fdd:	72 ae                	jb     80107f8d <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
80107fdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80107fe2:	83 c4 34             	add    $0x34,%esp
80107fe5:	5b                   	pop    %ebx
80107fe6:	5d                   	pop    %ebp
80107fe7:	c3                   	ret    

80107fe8 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80107fe8:	55                   	push   %ebp
80107fe9:	89 e5                	mov    %esp,%ebp
80107feb:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80107fee:	e8 38 ff ff ff       	call   80107f2b <setupkvm>
80107ff3:	a3 78 86 11 80       	mov    %eax,0x80118678
  switchkvm();
80107ff8:	e8 02 00 00 00       	call   80107fff <switchkvm>
}
80107ffd:	c9                   	leave  
80107ffe:	c3                   	ret    

80107fff <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
80107fff:	55                   	push   %ebp
80108000:	89 e5                	mov    %esp,%ebp
80108002:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
80108005:	a1 78 86 11 80       	mov    0x80118678,%eax
8010800a:	89 04 24             	mov    %eax,(%esp)
8010800d:	e8 4f f9 ff ff       	call   80107961 <v2p>
80108012:	89 04 24             	mov    %eax,(%esp)
80108015:	e8 3c f9 ff ff       	call   80107956 <lcr3>
}
8010801a:	c9                   	leave  
8010801b:	c3                   	ret    

8010801c <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
8010801c:	55                   	push   %ebp
8010801d:	89 e5                	mov    %esp,%ebp
8010801f:	53                   	push   %ebx
80108020:	83 ec 14             	sub    $0x14,%esp
  pushcli();
80108023:	e8 a3 ce ff ff       	call   80104ecb <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
80108028:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010802e:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108035:	83 c2 08             	add    $0x8,%edx
80108038:	89 d3                	mov    %edx,%ebx
8010803a:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108041:	83 c2 08             	add    $0x8,%edx
80108044:	c1 ea 10             	shr    $0x10,%edx
80108047:	89 d1                	mov    %edx,%ecx
80108049:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80108050:	83 c2 08             	add    $0x8,%edx
80108053:	c1 ea 18             	shr    $0x18,%edx
80108056:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
8010805d:	67 00 
8010805f:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
80108066:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
8010806c:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108073:	83 e1 f0             	and    $0xfffffff0,%ecx
80108076:	83 c9 09             	or     $0x9,%ecx
80108079:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010807f:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108086:	83 c9 10             	or     $0x10,%ecx
80108089:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010808f:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80108096:	83 e1 9f             	and    $0xffffff9f,%ecx
80108099:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
8010809f:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
801080a6:	83 c9 80             	or     $0xffffff80,%ecx
801080a9:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
801080af:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801080b6:	83 e1 f0             	and    $0xfffffff0,%ecx
801080b9:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801080bf:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801080c6:	83 e1 ef             	and    $0xffffffef,%ecx
801080c9:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801080cf:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801080d6:	83 e1 df             	and    $0xffffffdf,%ecx
801080d9:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801080df:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801080e6:	83 c9 40             	or     $0x40,%ecx
801080e9:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801080ef:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801080f6:	83 e1 7f             	and    $0x7f,%ecx
801080f9:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801080ff:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
80108105:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010810b:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
80108112:	83 e2 ef             	and    $0xffffffef,%edx
80108115:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
8010811b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108121:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
80108127:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010812d:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80108134:	8b 52 08             	mov    0x8(%edx),%edx
80108137:	81 c2 00 10 00 00    	add    $0x1000,%edx
8010813d:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
80108140:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
80108147:	e8 df f7 ff ff       	call   8010792b <ltr>
  if(p->pgdir == 0)
8010814c:	8b 45 08             	mov    0x8(%ebp),%eax
8010814f:	8b 40 04             	mov    0x4(%eax),%eax
80108152:	85 c0                	test   %eax,%eax
80108154:	75 0c                	jne    80108162 <switchuvm+0x146>
    panic("switchuvm: no pgdir");
80108156:	c7 04 24 3b 8d 10 80 	movl   $0x80108d3b,(%esp)
8010815d:	e8 d8 83 ff ff       	call   8010053a <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
80108162:	8b 45 08             	mov    0x8(%ebp),%eax
80108165:	8b 40 04             	mov    0x4(%eax),%eax
80108168:	89 04 24             	mov    %eax,(%esp)
8010816b:	e8 f1 f7 ff ff       	call   80107961 <v2p>
80108170:	89 04 24             	mov    %eax,(%esp)
80108173:	e8 de f7 ff ff       	call   80107956 <lcr3>
  popcli();
80108178:	e8 92 cd ff ff       	call   80104f0f <popcli>
}
8010817d:	83 c4 14             	add    $0x14,%esp
80108180:	5b                   	pop    %ebx
80108181:	5d                   	pop    %ebp
80108182:	c3                   	ret    

80108183 <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
80108183:	55                   	push   %ebp
80108184:	89 e5                	mov    %esp,%ebp
80108186:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
80108189:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
80108190:	76 0c                	jbe    8010819e <inituvm+0x1b>
    panic("inituvm: more than a page");
80108192:	c7 04 24 4f 8d 10 80 	movl   $0x80108d4f,(%esp)
80108199:	e8 9c 83 ff ff       	call   8010053a <panic>
  mem = kalloc();
8010819e:	e8 3f a9 ff ff       	call   80102ae2 <kalloc>
801081a3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
801081a6:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801081ad:	00 
801081ae:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801081b5:	00 
801081b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081b9:	89 04 24             	mov    %eax,(%esp)
801081bc:	e8 0a ce ff ff       	call   80104fcb <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
801081c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081c4:	89 04 24             	mov    %eax,(%esp)
801081c7:	e8 95 f7 ff ff       	call   80107961 <v2p>
801081cc:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
801081d3:	00 
801081d4:	89 44 24 0c          	mov    %eax,0xc(%esp)
801081d8:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801081df:	00 
801081e0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801081e7:	00 
801081e8:	8b 45 08             	mov    0x8(%ebp),%eax
801081eb:	89 04 24             	mov    %eax,(%esp)
801081ee:	e8 a0 fc ff ff       	call   80107e93 <mappages>
  memmove(mem, init, sz);
801081f3:	8b 45 10             	mov    0x10(%ebp),%eax
801081f6:	89 44 24 08          	mov    %eax,0x8(%esp)
801081fa:	8b 45 0c             	mov    0xc(%ebp),%eax
801081fd:	89 44 24 04          	mov    %eax,0x4(%esp)
80108201:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108204:	89 04 24             	mov    %eax,(%esp)
80108207:	e8 8e ce ff ff       	call   8010509a <memmove>
}
8010820c:	c9                   	leave  
8010820d:	c3                   	ret    

8010820e <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
8010820e:	55                   	push   %ebp
8010820f:	89 e5                	mov    %esp,%ebp
80108211:	53                   	push   %ebx
80108212:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
80108215:	8b 45 0c             	mov    0xc(%ebp),%eax
80108218:	25 ff 0f 00 00       	and    $0xfff,%eax
8010821d:	85 c0                	test   %eax,%eax
8010821f:	74 0c                	je     8010822d <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
80108221:	c7 04 24 6c 8d 10 80 	movl   $0x80108d6c,(%esp)
80108228:	e8 0d 83 ff ff       	call   8010053a <panic>
  for(i = 0; i < sz; i += PGSIZE){
8010822d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108234:	e9 a9 00 00 00       	jmp    801082e2 <loaduvm+0xd4>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80108239:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010823c:	8b 55 0c             	mov    0xc(%ebp),%edx
8010823f:	01 d0                	add    %edx,%eax
80108241:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108248:	00 
80108249:	89 44 24 04          	mov    %eax,0x4(%esp)
8010824d:	8b 45 08             	mov    0x8(%ebp),%eax
80108250:	89 04 24             	mov    %eax,(%esp)
80108253:	e8 99 fb ff ff       	call   80107df1 <walkpgdir>
80108258:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010825b:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010825f:	75 0c                	jne    8010826d <loaduvm+0x5f>
      panic("loaduvm: address should exist");
80108261:	c7 04 24 8f 8d 10 80 	movl   $0x80108d8f,(%esp)
80108268:	e8 cd 82 ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
8010826d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108270:	8b 00                	mov    (%eax),%eax
80108272:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108277:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
8010827a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010827d:	8b 55 18             	mov    0x18(%ebp),%edx
80108280:	29 c2                	sub    %eax,%edx
80108282:	89 d0                	mov    %edx,%eax
80108284:	3d ff 0f 00 00       	cmp    $0xfff,%eax
80108289:	77 0f                	ja     8010829a <loaduvm+0x8c>
      n = sz - i;
8010828b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010828e:	8b 55 18             	mov    0x18(%ebp),%edx
80108291:	29 c2                	sub    %eax,%edx
80108293:	89 d0                	mov    %edx,%eax
80108295:	89 45 f0             	mov    %eax,-0x10(%ebp)
80108298:	eb 07                	jmp    801082a1 <loaduvm+0x93>
    else
      n = PGSIZE;
8010829a:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
801082a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801082a4:	8b 55 14             	mov    0x14(%ebp),%edx
801082a7:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
801082aa:	8b 45 e8             	mov    -0x18(%ebp),%eax
801082ad:	89 04 24             	mov    %eax,(%esp)
801082b0:	e8 b9 f6 ff ff       	call   8010796e <p2v>
801082b5:	8b 55 f0             	mov    -0x10(%ebp),%edx
801082b8:	89 54 24 0c          	mov    %edx,0xc(%esp)
801082bc:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801082c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801082c4:	8b 45 10             	mov    0x10(%ebp),%eax
801082c7:	89 04 24             	mov    %eax,(%esp)
801082ca:	e8 99 9a ff ff       	call   80101d68 <readi>
801082cf:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801082d2:	74 07                	je     801082db <loaduvm+0xcd>
      return -1;
801082d4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801082d9:	eb 18                	jmp    801082f3 <loaduvm+0xe5>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
801082db:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801082e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801082e5:	3b 45 18             	cmp    0x18(%ebp),%eax
801082e8:	0f 82 4b ff ff ff    	jb     80108239 <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
801082ee:	b8 00 00 00 00       	mov    $0x0,%eax
}
801082f3:	83 c4 24             	add    $0x24,%esp
801082f6:	5b                   	pop    %ebx
801082f7:	5d                   	pop    %ebp
801082f8:	c3                   	ret    

801082f9 <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
801082f9:	55                   	push   %ebp
801082fa:	89 e5                	mov    %esp,%ebp
801082fc:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
801082ff:	8b 45 10             	mov    0x10(%ebp),%eax
80108302:	85 c0                	test   %eax,%eax
80108304:	79 0a                	jns    80108310 <allocuvm+0x17>
    return 0;
80108306:	b8 00 00 00 00       	mov    $0x0,%eax
8010830b:	e9 c1 00 00 00       	jmp    801083d1 <allocuvm+0xd8>
  if(newsz < oldsz)
80108310:	8b 45 10             	mov    0x10(%ebp),%eax
80108313:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108316:	73 08                	jae    80108320 <allocuvm+0x27>
    return oldsz;
80108318:	8b 45 0c             	mov    0xc(%ebp),%eax
8010831b:	e9 b1 00 00 00       	jmp    801083d1 <allocuvm+0xd8>

  a = PGROUNDUP(oldsz);
80108320:	8b 45 0c             	mov    0xc(%ebp),%eax
80108323:	05 ff 0f 00 00       	add    $0xfff,%eax
80108328:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010832d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
80108330:	e9 8d 00 00 00       	jmp    801083c2 <allocuvm+0xc9>
    mem = kalloc();
80108335:	e8 a8 a7 ff ff       	call   80102ae2 <kalloc>
8010833a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
8010833d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108341:	75 2c                	jne    8010836f <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
80108343:	c7 04 24 ad 8d 10 80 	movl   $0x80108dad,(%esp)
8010834a:	e8 51 80 ff ff       	call   801003a0 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
8010834f:	8b 45 0c             	mov    0xc(%ebp),%eax
80108352:	89 44 24 08          	mov    %eax,0x8(%esp)
80108356:	8b 45 10             	mov    0x10(%ebp),%eax
80108359:	89 44 24 04          	mov    %eax,0x4(%esp)
8010835d:	8b 45 08             	mov    0x8(%ebp),%eax
80108360:	89 04 24             	mov    %eax,(%esp)
80108363:	e8 6b 00 00 00       	call   801083d3 <deallocuvm>
      return 0;
80108368:	b8 00 00 00 00       	mov    $0x0,%eax
8010836d:	eb 62                	jmp    801083d1 <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
8010836f:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108376:	00 
80108377:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010837e:	00 
8010837f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108382:	89 04 24             	mov    %eax,(%esp)
80108385:	e8 41 cc ff ff       	call   80104fcb <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
8010838a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010838d:	89 04 24             	mov    %eax,(%esp)
80108390:	e8 cc f5 ff ff       	call   80107961 <v2p>
80108395:	8b 55 f4             	mov    -0xc(%ebp),%edx
80108398:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
8010839f:	00 
801083a0:	89 44 24 0c          	mov    %eax,0xc(%esp)
801083a4:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801083ab:	00 
801083ac:	89 54 24 04          	mov    %edx,0x4(%esp)
801083b0:	8b 45 08             	mov    0x8(%ebp),%eax
801083b3:	89 04 24             	mov    %eax,(%esp)
801083b6:	e8 d8 fa ff ff       	call   80107e93 <mappages>
    return 0;
  if(newsz < oldsz)
    return oldsz;

  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
801083bb:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801083c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083c5:	3b 45 10             	cmp    0x10(%ebp),%eax
801083c8:	0f 82 67 ff ff ff    	jb     80108335 <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
801083ce:	8b 45 10             	mov    0x10(%ebp),%eax
}
801083d1:	c9                   	leave  
801083d2:	c3                   	ret    

801083d3 <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
801083d3:	55                   	push   %ebp
801083d4:	89 e5                	mov    %esp,%ebp
801083d6:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
801083d9:	8b 45 10             	mov    0x10(%ebp),%eax
801083dc:	3b 45 0c             	cmp    0xc(%ebp),%eax
801083df:	72 08                	jb     801083e9 <deallocuvm+0x16>
    return oldsz;
801083e1:	8b 45 0c             	mov    0xc(%ebp),%eax
801083e4:	e9 a4 00 00 00       	jmp    8010848d <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
801083e9:	8b 45 10             	mov    0x10(%ebp),%eax
801083ec:	05 ff 0f 00 00       	add    $0xfff,%eax
801083f1:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801083f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
801083f9:	e9 80 00 00 00       	jmp    8010847e <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
801083fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108401:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108408:	00 
80108409:	89 44 24 04          	mov    %eax,0x4(%esp)
8010840d:	8b 45 08             	mov    0x8(%ebp),%eax
80108410:	89 04 24             	mov    %eax,(%esp)
80108413:	e8 d9 f9 ff ff       	call   80107df1 <walkpgdir>
80108418:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
8010841b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010841f:	75 09                	jne    8010842a <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
80108421:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
80108428:	eb 4d                	jmp    80108477 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
8010842a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010842d:	8b 00                	mov    (%eax),%eax
8010842f:	83 e0 01             	and    $0x1,%eax
80108432:	85 c0                	test   %eax,%eax
80108434:	74 41                	je     80108477 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
80108436:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108439:	8b 00                	mov    (%eax),%eax
8010843b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108440:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
80108443:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108447:	75 0c                	jne    80108455 <deallocuvm+0x82>
        panic("kfree");
80108449:	c7 04 24 c5 8d 10 80 	movl   $0x80108dc5,(%esp)
80108450:	e8 e5 80 ff ff       	call   8010053a <panic>
      char *v = p2v(pa);
80108455:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108458:	89 04 24             	mov    %eax,(%esp)
8010845b:	e8 0e f5 ff ff       	call   8010796e <p2v>
80108460:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
80108463:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108466:	89 04 24             	mov    %eax,(%esp)
80108469:	e8 db a5 ff ff       	call   80102a49 <kfree>
      *pte = 0;
8010846e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108471:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80108477:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010847e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108481:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108484:	0f 82 74 ff ff ff    	jb     801083fe <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
8010848a:	8b 45 10             	mov    0x10(%ebp),%eax
}
8010848d:	c9                   	leave  
8010848e:	c3                   	ret    

8010848f <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
8010848f:	55                   	push   %ebp
80108490:	89 e5                	mov    %esp,%ebp
80108492:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80108495:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80108499:	75 0c                	jne    801084a7 <freevm+0x18>
    panic("freevm: no pgdir");
8010849b:	c7 04 24 cb 8d 10 80 	movl   $0x80108dcb,(%esp)
801084a2:	e8 93 80 ff ff       	call   8010053a <panic>
  deallocuvm(pgdir, KERNBASE, 0);
801084a7:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801084ae:	00 
801084af:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
801084b6:	80 
801084b7:	8b 45 08             	mov    0x8(%ebp),%eax
801084ba:	89 04 24             	mov    %eax,(%esp)
801084bd:	e8 11 ff ff ff       	call   801083d3 <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
801084c2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801084c9:	eb 48                	jmp    80108513 <freevm+0x84>
    if(pgdir[i] & PTE_P){
801084cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801084ce:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
801084d5:	8b 45 08             	mov    0x8(%ebp),%eax
801084d8:	01 d0                	add    %edx,%eax
801084da:	8b 00                	mov    (%eax),%eax
801084dc:	83 e0 01             	and    $0x1,%eax
801084df:	85 c0                	test   %eax,%eax
801084e1:	74 2c                	je     8010850f <freevm+0x80>
      char * v = p2v(PTE_ADDR(pgdir[i]));
801084e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801084e6:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
801084ed:	8b 45 08             	mov    0x8(%ebp),%eax
801084f0:	01 d0                	add    %edx,%eax
801084f2:	8b 00                	mov    (%eax),%eax
801084f4:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801084f9:	89 04 24             	mov    %eax,(%esp)
801084fc:	e8 6d f4 ff ff       	call   8010796e <p2v>
80108501:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
80108504:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108507:	89 04 24             	mov    %eax,(%esp)
8010850a:	e8 3a a5 ff ff       	call   80102a49 <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
8010850f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80108513:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
8010851a:	76 af                	jbe    801084cb <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
8010851c:	8b 45 08             	mov    0x8(%ebp),%eax
8010851f:	89 04 24             	mov    %eax,(%esp)
80108522:	e8 22 a5 ff ff       	call   80102a49 <kfree>
}
80108527:	c9                   	leave  
80108528:	c3                   	ret    

80108529 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80108529:	55                   	push   %ebp
8010852a:	89 e5                	mov    %esp,%ebp
8010852c:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
8010852f:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108536:	00 
80108537:	8b 45 0c             	mov    0xc(%ebp),%eax
8010853a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010853e:	8b 45 08             	mov    0x8(%ebp),%eax
80108541:	89 04 24             	mov    %eax,(%esp)
80108544:	e8 a8 f8 ff ff       	call   80107df1 <walkpgdir>
80108549:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
8010854c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80108550:	75 0c                	jne    8010855e <clearpteu+0x35>
    panic("clearpteu");
80108552:	c7 04 24 dc 8d 10 80 	movl   $0x80108ddc,(%esp)
80108559:	e8 dc 7f ff ff       	call   8010053a <panic>
  *pte &= ~PTE_U;
8010855e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108561:	8b 00                	mov    (%eax),%eax
80108563:	83 e0 fb             	and    $0xfffffffb,%eax
80108566:	89 c2                	mov    %eax,%edx
80108568:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010856b:	89 10                	mov    %edx,(%eax)
}
8010856d:	c9                   	leave  
8010856e:	c3                   	ret    

8010856f <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
8010856f:	55                   	push   %ebp
80108570:	89 e5                	mov    %esp,%ebp
80108572:	53                   	push   %ebx
80108573:	83 ec 44             	sub    $0x44,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80108576:	e8 b0 f9 ff ff       	call   80107f2b <setupkvm>
8010857b:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010857e:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108582:	75 0a                	jne    8010858e <copyuvm+0x1f>
    return 0;
80108584:	b8 00 00 00 00       	mov    $0x0,%eax
80108589:	e9 fd 00 00 00       	jmp    8010868b <copyuvm+0x11c>
  for(i = 0; i < sz; i += PGSIZE){
8010858e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80108595:	e9 d0 00 00 00       	jmp    8010866a <copyuvm+0xfb>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
8010859a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010859d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801085a4:	00 
801085a5:	89 44 24 04          	mov    %eax,0x4(%esp)
801085a9:	8b 45 08             	mov    0x8(%ebp),%eax
801085ac:	89 04 24             	mov    %eax,(%esp)
801085af:	e8 3d f8 ff ff       	call   80107df1 <walkpgdir>
801085b4:	89 45 ec             	mov    %eax,-0x14(%ebp)
801085b7:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801085bb:	75 0c                	jne    801085c9 <copyuvm+0x5a>
      panic("copyuvm: pte should exist");
801085bd:	c7 04 24 e6 8d 10 80 	movl   $0x80108de6,(%esp)
801085c4:	e8 71 7f ff ff       	call   8010053a <panic>
    if(!(*pte & PTE_P))
801085c9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085cc:	8b 00                	mov    (%eax),%eax
801085ce:	83 e0 01             	and    $0x1,%eax
801085d1:	85 c0                	test   %eax,%eax
801085d3:	75 0c                	jne    801085e1 <copyuvm+0x72>
      panic("copyuvm: page not present");
801085d5:	c7 04 24 00 8e 10 80 	movl   $0x80108e00,(%esp)
801085dc:	e8 59 7f ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
801085e1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085e4:	8b 00                	mov    (%eax),%eax
801085e6:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801085eb:	89 45 e8             	mov    %eax,-0x18(%ebp)
    flags = PTE_FLAGS(*pte);
801085ee:	8b 45 ec             	mov    -0x14(%ebp),%eax
801085f1:	8b 00                	mov    (%eax),%eax
801085f3:	25 ff 0f 00 00       	and    $0xfff,%eax
801085f8:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if((mem = kalloc()) == 0)
801085fb:	e8 e2 a4 ff ff       	call   80102ae2 <kalloc>
80108600:	89 45 e0             	mov    %eax,-0x20(%ebp)
80108603:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80108607:	75 02                	jne    8010860b <copyuvm+0x9c>
      goto bad;
80108609:	eb 70                	jmp    8010867b <copyuvm+0x10c>
    memmove(mem, (char*)p2v(pa), PGSIZE);
8010860b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010860e:	89 04 24             	mov    %eax,(%esp)
80108611:	e8 58 f3 ff ff       	call   8010796e <p2v>
80108616:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010861d:	00 
8010861e:	89 44 24 04          	mov    %eax,0x4(%esp)
80108622:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108625:	89 04 24             	mov    %eax,(%esp)
80108628:	e8 6d ca ff ff       	call   8010509a <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
8010862d:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80108630:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108633:	89 04 24             	mov    %eax,(%esp)
80108636:	e8 26 f3 ff ff       	call   80107961 <v2p>
8010863b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010863e:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80108642:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108646:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010864d:	00 
8010864e:	89 54 24 04          	mov    %edx,0x4(%esp)
80108652:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108655:	89 04 24             	mov    %eax,(%esp)
80108658:	e8 36 f8 ff ff       	call   80107e93 <mappages>
8010865d:	85 c0                	test   %eax,%eax
8010865f:	79 02                	jns    80108663 <copyuvm+0xf4>
      goto bad;
80108661:	eb 18                	jmp    8010867b <copyuvm+0x10c>
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
80108663:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
8010866a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010866d:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108670:	0f 82 24 ff ff ff    	jb     8010859a <copyuvm+0x2b>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
      goto bad;
  }
  return d;
80108676:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108679:	eb 10                	jmp    8010868b <copyuvm+0x11c>

bad:
  freevm(d);
8010867b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010867e:	89 04 24             	mov    %eax,(%esp)
80108681:	e8 09 fe ff ff       	call   8010848f <freevm>
  return 0;
80108686:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010868b:	83 c4 44             	add    $0x44,%esp
8010868e:	5b                   	pop    %ebx
8010868f:	5d                   	pop    %ebp
80108690:	c3                   	ret    

80108691 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80108691:	55                   	push   %ebp
80108692:	89 e5                	mov    %esp,%ebp
80108694:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80108697:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010869e:	00 
8010869f:	8b 45 0c             	mov    0xc(%ebp),%eax
801086a2:	89 44 24 04          	mov    %eax,0x4(%esp)
801086a6:	8b 45 08             	mov    0x8(%ebp),%eax
801086a9:	89 04 24             	mov    %eax,(%esp)
801086ac:	e8 40 f7 ff ff       	call   80107df1 <walkpgdir>
801086b1:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
801086b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086b7:	8b 00                	mov    (%eax),%eax
801086b9:	83 e0 01             	and    $0x1,%eax
801086bc:	85 c0                	test   %eax,%eax
801086be:	75 07                	jne    801086c7 <uva2ka+0x36>
    return 0;
801086c0:	b8 00 00 00 00       	mov    $0x0,%eax
801086c5:	eb 25                	jmp    801086ec <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
801086c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086ca:	8b 00                	mov    (%eax),%eax
801086cc:	83 e0 04             	and    $0x4,%eax
801086cf:	85 c0                	test   %eax,%eax
801086d1:	75 07                	jne    801086da <uva2ka+0x49>
    return 0;
801086d3:	b8 00 00 00 00       	mov    $0x0,%eax
801086d8:	eb 12                	jmp    801086ec <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
801086da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801086dd:	8b 00                	mov    (%eax),%eax
801086df:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801086e4:	89 04 24             	mov    %eax,(%esp)
801086e7:	e8 82 f2 ff ff       	call   8010796e <p2v>
}
801086ec:	c9                   	leave  
801086ed:	c3                   	ret    

801086ee <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
801086ee:	55                   	push   %ebp
801086ef:	89 e5                	mov    %esp,%ebp
801086f1:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
801086f4:	8b 45 10             	mov    0x10(%ebp),%eax
801086f7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
801086fa:	e9 87 00 00 00       	jmp    80108786 <copyout+0x98>
    va0 = (uint)PGROUNDDOWN(va);
801086ff:	8b 45 0c             	mov    0xc(%ebp),%eax
80108702:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108707:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
8010870a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010870d:	89 44 24 04          	mov    %eax,0x4(%esp)
80108711:	8b 45 08             	mov    0x8(%ebp),%eax
80108714:	89 04 24             	mov    %eax,(%esp)
80108717:	e8 75 ff ff ff       	call   80108691 <uva2ka>
8010871c:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
8010871f:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80108723:	75 07                	jne    8010872c <copyout+0x3e>
      return -1;
80108725:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010872a:	eb 69                	jmp    80108795 <copyout+0xa7>
    n = PGSIZE - (va - va0);
8010872c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010872f:	8b 55 ec             	mov    -0x14(%ebp),%edx
80108732:	29 c2                	sub    %eax,%edx
80108734:	89 d0                	mov    %edx,%eax
80108736:	05 00 10 00 00       	add    $0x1000,%eax
8010873b:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
8010873e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108741:	3b 45 14             	cmp    0x14(%ebp),%eax
80108744:	76 06                	jbe    8010874c <copyout+0x5e>
      n = len;
80108746:	8b 45 14             	mov    0x14(%ebp),%eax
80108749:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
8010874c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010874f:	8b 55 0c             	mov    0xc(%ebp),%edx
80108752:	29 c2                	sub    %eax,%edx
80108754:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108757:	01 c2                	add    %eax,%edx
80108759:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010875c:	89 44 24 08          	mov    %eax,0x8(%esp)
80108760:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108763:	89 44 24 04          	mov    %eax,0x4(%esp)
80108767:	89 14 24             	mov    %edx,(%esp)
8010876a:	e8 2b c9 ff ff       	call   8010509a <memmove>
    len -= n;
8010876f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108772:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
80108775:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108778:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
8010877b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010877e:	05 00 10 00 00       	add    $0x1000,%eax
80108783:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
80108786:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
8010878a:	0f 85 6f ff ff ff    	jne    801086ff <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
80108790:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108795:	c9                   	leave  
80108796:	c3                   	ret    
